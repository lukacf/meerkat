...
| Subcommand | Purpose |
|------------|---------|
| `spawn-helper <mob_id> <prompt> --agent-identity <id> [--profile] [--json]` | Spawn a short-lived helper, wait for completion, print the result |
| `fork-helper <mob_id> <source_member> <prompt> --agent-identity <id> [--profile] [--fork-context full-history\|last-messages] [--last-messages N] [--json]` | Fork from an existing member's context and run a helper |
