...
| `unit` (one lane per shard) | Rust-relevant change | `repo-cargo nextest run -p <changed packages> --lib --bins --profile ci-pr` (the fast lane minus the wall-clock latency bounds, which nightly and release keep) |
