...
CONSTANTS BooleanValues, BranchIdValues, CollectionPolicyKindValues, DependencyModeValues, FrameIdValues, LoopInstanceIdValues, MeerkatIdValues, NatValues, SetOfFrameIdValues, SetOfLoopInstanceIdValues, SetOfStepIdValues, StepIdValues, StepRunStatusValues, StringValues
...
PumpNodeScheduler ==
    ...
    /\ UNCHANGED << tracked_steps, ordered_steps, step_status, output_recorded, step_condition_results, step_has_conditions, step_dependencies, step_dependency_modes, step_branches, step_collection_policies, step_quorum_thresholds, step_target_counts, step_target_success_counts, step_target_terminal_failure_counts, target_retry_counts, failure_count, consecutive_failure_count, escalation_threshold, max_step_retries, pending_body_frame_loops, pending_body_frame_loop_membership, active_frame_count, max_active_nodes, max_active_frames, max_frame_depth, last_granted_loop >>
...
PumpFrameScheduler ==
    ...
    /\ UNCHANGED << tracked_steps, ordered_steps, step_status, output_recorded, step_condition_results, step_has_conditions, step_dependencies, step_dependency_modes, step_branches, step_collection_policies, step_quorum_thresholds, step_target_counts, step_target_success_counts, step_target_terminal_failure_counts, target_retry_counts, failure_count, consecutive_failure_count, escalation_threshold, max_step_retries, ready_frames, ready_frame_membership, active_node_count, max_active_nodes, max_active_frames, max_frame_depth, last_granted_frame >>
...
Next ==
    ...
    \/ \E step_id \in StepIdValues : \E target_id \in MeerkatIdValues : \E retry_key \in StringValues : RecordTargetFailure(step_id, target_id, retry_key)
    ...
    \/ \E frame_id \in FrameIdValues : FrameTerminated(frame_id)
    \/ TerminalizeCompleted
    \/ TerminalizeFailed
    \/ TerminalizeCanceled
    \/ TerminalStutter
...
CiStateConstraint == /\ model_step_count <= 0 /\ Cardinality(tracked_steps) <= 1 /\ Len(ordered_steps) <= 1 /\ Cardinality(DOMAIN step_status) <= 1 /\ Cardinality(DOMAIN output_recorded) <= 1 /\ Cardinality(DOMAIN step_condition_results) <= 1 /\ Cardinality(DOMAIN step_has_conditions) <= 1 /\ Cardinality(DOMAIN step_dependencies) <= 1 /\ Cardinality(DOMAIN step_dependency_modes) <= 1 /\ Cardinality(DOMAIN step_branches) <= 1 /\ Cardinality(DOMAIN step_collection_policies) <= 1 /\ Cardinality(DOMAIN step_quorum_thresholds) <= 1 /\ Cardinality(DOMAIN step_target_counts) <= 1 /\ Cardinality(DOMAIN step_target_success_counts) <= 1 /\ Cardinality(DOMAIN step_target_terminal_failure_counts) <= 1 /\ Cardinality(DOMAIN target_retry_counts) <= 1 /\ Len(ready_frames) <= 1 /\ Cardinality(ready_frame_membership) <= 1 /\ Len(pending_body_frame_loops) <= 1 /\ Cardinality(pending_body_frame_loop_membership) <= 1
