...
impl AgentMobToolSurface {
    ...
    fn ensure_create_authority(&self, tool_name: &str) -> Result<(), ToolError> {
        if self.authority_context.can_create_mobs() {
            return Ok(());
        }
        Err(ToolError::access_denied(tool_name))
    }

    fn ensure_mob_scope_authority(&self, tool_name: &str, mob_id: &MobId) -> Result<(), ToolError> {
        if self.authority_context.can_manage_mob(mob_id.as_str()) {
            return Ok(());
        }
        Err(ToolError::access_denied(tool_name))
    }
    ...
    async fn dispatch_delegate(
        ...
        self.ensure_create_authority(call.name)?;
    ...
    async fn dispatch_mob_destroy(
        ...
        self.ensure_mob_scope_authority(call.name, &mob_id)?;
    ...
    async fn dispatch_mob_list(
        ...
        let mob_list: Vec<serde_json::Value> = mobs
            .into_iter()
            .filter(|(id, _)| self.authority_context.can_manage_mob(id.as_str()))
            .map(|(id, status)| {
                json!({
                    "mob_id": id,
...
mod tests {
    ...
    #[tokio::test]
    async fn test_create_only_authority_allows_create_but_denies_existing_mob_control_until_reinjected()
     {
        let state = MobMcpState::new_in_memory();
        let session_id = SessionId::new();
        ...
        assert_eq!(
            listed["mobs"],
            json!([]),
            "create-only authority must not mutate in-process managed scope"
        );

        let destroy_args =
            serde_json::value::RawValue::from_string(json!({ "mob_id": mob_id }).to_string())
                .unwrap();
        let destroy_error = surface
            .dispatch(ToolCallView {
                id: "destroy-1",
                name: "mob_destroy",
                args: &destroy_args,
            ...
            .await
            .expect_err("existing-mob control should still require managed scope");
        assert!(matches!(destroy_error, ToolError::AccessDenied { .. }));
    }
