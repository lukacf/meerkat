...
The lower-level `meerkat_core::AgentBuilder` is not re-exported by the facade. Its standalone build path is an explicit unsafe core-level escape hatch for tests or embeddings that own every primitive directly.
