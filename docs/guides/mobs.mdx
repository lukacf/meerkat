...
forks in turn) without manage scope over the mob. The result is appended to the
forker's transcript as a durable System message and wakes the forker as a
background-job completion; on a one-shot `rkat run` without `--keep-alive` the
