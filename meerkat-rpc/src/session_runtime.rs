    ...
    async fn hot_swap_llm_client(
        ...
        if !self.runtime_adapter.contains_session(session_id).await {
            return Err(RpcError {
                code: error::INVALID_REQUEST,
                message: format!(
                    "runtime not ready for llm reconfiguration: live session {session_id} is not registered with the runtime adapter"
                ),
                data: None,
            });
        }
            ...
            pending_session
        {
            ...
            match self.service.create_session(req).await {
                Ok(result) => {
                    if let Err(err) = self
                        .runtime_adapter
                        .seed_live_session_llm_state(session_id)
                        .await
                    {
                        tracing::warn!(
                            session_id = %session_id,
                            error = %err,
                            "failed to seed machine-owned llm state after live session materialization"
                        );
                    }
                    if let Some((starting_system_context_state, current_system_context_state)) =
                        self.take_promoting_system_context_state(session_id).await
...
mod tests {
    ...
    async fn hot_swap_to_openai_hides_view_image() {
        ...
        let tool_scope = runtime
            .service
            .tool_scope_snapshot(&session_id)
            .await
            .expect("tool_scope_snapshot after hot-swap")
            .expect("live tool scope snapshot after hot-swap");
        assert!(
            !tool_scope
                .visible_names
                .iter()
                .any(|name| name == meerkat_core::VIEW_IMAGE_TOOL_NAME),
            "view_image should be absent from the committed visible tool set after hot-swap"
        );
    }
    ...
    async fn hot_swap_to_anthropic_clears_view_image_deny() {
        ...
        let tool_scope = runtime
            ...
            .tool_scope_snapshot(&session_id)
            .await
            .expect("tool_scope_snapshot after hot-swap back")
            .expect("live tool scope snapshot after hot-swap back");
        assert!(
            tool_scope
                .visible_names
                .iter()
                .any(|name| name == meerkat_core::VIEW_IMAGE_TOOL_NAME),
            "view_image should be visible again once the capability-owned hide is cleared"
        );
    }
    ...
    async fn materialized_hot_swap_updates_durable_identity_for_recovery() {
        ...
        let recovered_session = runtime
            ...
            .export_live_session(&session_id)
            .await
            .expect("recovered live session after re-materialization");
        let recovered_visibility = recovered_session
            .tool_visibility_state()
            .expect("recovered session should persist canonical visibility state");
        assert_eq!(
            recovered_visibility.capability_base_filter,
            meerkat_core::ToolFilter::Deny(["view_image".to_string()].into_iter().collect()),
            "recovered hot-swapped session should preserve the machine-owned capability filter"
        );
    }
