...
6. **Formal seam closure** -- async owner handoffs, wait barriers, and terminal classes are moving under formal machine/protocol ownership instead of shell-only glue
...
<Note>
**Formal seam closure:** Meerkat is extending machine authority beyond isolated reducers. The
current architecture work adds protocolized owner handoffs for semantically meaningful async seams
so runtime-backed and direct session paths cannot silently diverge on drain lifecycle, wait
barriers, or surfaced terminal truth.
</Note>
...
graph TD
    ...
    subgraph core["meerkat-core"]
        AGENT["Agent<br/>llm_client · tool_dispatcher<br/>session_store · session · budget<br/>retry_policy · state · compactor<br/>delegated-work coordinator"]
    end
...
| `WaitingForOps` | No LLM work, waiting for machine-owned barrier operations to complete |
...
2. `meerkat-core` depends on `meerkat-models` for catalog data and defaults
3. All other crates depend on `meerkat-core` for types and traits
4. `meerkat-tools` depends on `meerkat-mcp` for MCP routing
5. `meerkat` (facade) re-exports from all crates
6. `meerkat-cli` is the top-level binary crate
