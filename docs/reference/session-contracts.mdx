...
class (`durable` / `rebuildable_cache` / `scratch`) and its resolution; a
durable slot resolving to a non-persistent store without the realm manifest
declaring that domain ephemeral is a startup error — never a silent
