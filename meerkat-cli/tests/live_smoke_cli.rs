...
fn retryable_cli_generate_image_turn_failure(combined: &str) -> bool {
    ...
    lower.contains("llmfailure")
        || lower.contains("runtime loop failed")
        || lower.contains("empty response")
        || lower.contains("no tool call")
        || lower.contains("did not call")
        || lower.contains("model did not")
}
...
async fn inner_e2e_cli_generate_image_openai_default() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // 256: the model must emit a generate_image call plus blob_save_file.
    let rkat_dir = project_dir.join(".rkat");
    tokio::fs::create_dir_all(&rkat_dir).await?;
    let mut config = Config::default();
    ...
    let prompt = format!(
        r#"Use the generate_image tool exactly once. Pass request.target={{"target":"model","provider":"openai","model":"{image_model}"}}, request.intent="generate", request.prompt="A clean square PNG test card with CLI-IMAGE-73 written in large black letters", request.size="1024x1024", request.quality="low", request.format="png", request.count=1, and request.provider_params={{"background":"opaque","moderation":"low","action":"generate"}}. After generate_image returns, call blob_save_file exactly once with the returned blob_id, path "top-news-infographic.png", and overwrite=false. Then reply with CLI-IMAGE-73-DONE and no extra prose."#
    );
    let mut combined = String::new();
    for attempt in 1..=3 {
        let output = timeout(
