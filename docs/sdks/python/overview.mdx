...
The Python SDK does not auto-inject CLI-local `mob_*` tools. For current "mob as tool" behavior, use CLI `rkat run`/`rkat resume`, or use explicit `rkat mob ...` lifecycle commands.
