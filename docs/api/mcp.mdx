...
tool surface includes core session/config/history/blob/skills tools and schedule
tools. The previous realtime MCP proxy tools are retired.
</Note>
...
<Note>
**Realtime over MCP.** The previous MCP realtime proxy tools are retired while
the new live adapter API is rebuilt. MCP no longer exposes
`meerkat_realtime_open_info` or `meerkat_realtime_status`.
</Note>
