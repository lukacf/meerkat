...
mod tests {
    ...
    #[test]
    fn core_mob_comms_bridge_rejects_forged_token() {
        #[allow(improper_ctypes_definitions, unsafe_code)]
        unsafe extern "Rust" {
            #[link_name = concat!(
            ...
            fn core_mob_generated_comms_trust_authority_build(
        ...
        #[allow(unsafe_code)]
        let result = unsafe {
            core_mob_generated_comms_trust_authority_build(
        ...
        assert!(matches!(
            result,
            Err(message) if message.contains("canonical MobMachine bridge token")
        ));
    }
}
