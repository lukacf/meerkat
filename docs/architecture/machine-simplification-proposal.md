...
| Behavior-bearing runtime-control state should stay outside MeerkatMachine | rejected | `current_run_id` is now absorbed back into the checked-in Meerkat machine, the stale handwritten control booleans are gone, and live recycle no longer depends on a helper-only `Recovering` detour. The remaining work is to decide what to do with the still-dead recover/resume compatibility branch and the duplicated run-return bookkeeping, not to keep punting live shell state below the two-machine model. |
