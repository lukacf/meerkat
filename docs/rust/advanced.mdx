...
The lower-level `meerkat_core::AgentBuilder` is not re-exported by the facade. It is an explicit core-level escape hatch for tests or embeddings that opt into `meerkat-core/standalone-agent-builder` and own every primitive directly.
