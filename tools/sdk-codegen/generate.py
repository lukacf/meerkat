...
PY_GENERATED_HEADER = "# @generated - Generated by `python3 tools/sdk-codegen/generate.py`.\n"
TS_GENERATED_HEADER = "// @generated - Generated by `python3 tools/sdk-codegen/generate.py`.\n"
...
MOB_RPC_CONTRACT_TYPES = [
    ...
    "MobEventsParams",
    "MobEventsResult",
    "WireMobAgentRuntimeId",
    "WireMobEvent",
    "WireMobMemberSpawnedEvent",
    "MobMemberSendParams",
    "MobMemberSendResult",
    ...
    "MobFlowStatusResult",
    "WireMobRun",
    "WireMobStepLedgerEntry",
    "WireMobFailureLedgerEntry",
    "WireMobLoopIterationLedgerEntry",
    "MobFlowCancelParams",
    "MobFlowCancelResult",
...
def _promote_nested_schema_def(name: str) -> bool:
    return name.startswith("Realtime") or name in {
        ...
        "WireProvider",
        # Row 176: promote the typed transcript replacement mirror so
        # ForkSessionReplaceParams.replacement lands in generated SDK contracts
        # as a named union instead of a schema-local object widened to
        # `unknown` / `Any`.
        "WireTranscriptReplacement",
        # Row 141/165: schedule contracts are production-facing RPC/REST
        # shapes, so generated SDK types must keep the schedule/tool result
        # contracts concrete instead of widening them to dictionaries.
        "Schedule",
        "Occurrence",
        "ScheduleToolDescriptor",
        "ScheduleToolsResult",
...
def _one_of_discriminator_literals(
    ...
    variants = _one_of_typed_dict_variants(schema_root, schema)
    if variants is None:
        raise KeyError(f"{name} is not a generated discriminated union")
    discriminator_name, literal_variants = variants
    if discriminator_name != discriminator:
        raise KeyError(
            f"{name} uses discriminator {discriminator_name}, expected {discriminator}"
        )
    return [value for value, _ in literal_variants]
    ...
    *,
    ...
    for key in ("anyOf", "oneOf"):
        ...
        if isinstance(variants, list) and variants:
            ...
            variant_types: list[str] = []
            for variant in non_null:
                inner_type, _ = _typescript_type_from_schema(
                    root,
                    variant,
                    local_defs,
                    inline_objects=inline_objects,
...
def generate_python_types(schemas: dict, output_dir: Path, *, has_comms: bool = True, has_skills: bool = True) -> None:
    ...
    init_content = PY_GENERATED_HEADER
    ...
    types_content = PY_GENERATED_HEADER
    ...
    for name in MCP_LIVE_CONTRACT_TYPES:
        append_python_contract_dataclass(name)
    for name in AUTH_RPC_CONTRACT_TYPES:
        append_python_contract_dataclass(name)
    ...
    append_python_dataclass("CreateScheduleRequest", params_schema, "Request payload for schedule/create.")
    append_python_dataclass("ScheduleUpdateRequest", params_schema, "Request payload for REST schedule update.")
    ...
    append_python_alias("ScheduleToolCallParams", params_schema, "Request payload for schedule/call.")
    ...
    live_observation_tags = _one_of_discriminator_literals(
        wire_schema,
        "WireLiveAdapterObservation",
        "observation",
    )
    types_content += "\nWIRE_LIVE_ADAPTER_OBSERVATION_TAGS: tuple[str, ...] = (\n"
    types_content += "".join(f"    {value!r},\n" for value in live_observation_tags)
    types_content += ")\n"
    append_python_dataclass(
        "RuntimeAcceptResult",
        wire_schema,
    ...
    append_python_dataclass("Schedule", wire_schema, "Canonical schedule payload.")
    append_python_dataclass("Occurrence", wire_schema, "Canonical schedule occurrence payload.")
    append_python_dataclass("ScheduleToolDescriptor", wire_schema, "Generated schedule tool descriptor.")
    append_python_dataclass("ScheduleToolsResult", wire_schema, "Response payload for schedule/tools.")
    ...
    append_python_alias("ScheduleToolCallResult", wire_schema, "Response payload for schedule/call.")
    append_python_dataclass(
        "WireResolvedModelCapabilities",
        schemas.get("models", {}),
    ...
    append_python_alias("WireDeviceCompleteResult", wire_schema, "Response payload for auth/login/device_complete.")
    append_python_dataclass("WireProvisionApiKeyResult", wire_schema, "Response payload for auth/login/provision_api_key.")
    ...
    errors_content = PY_GENERATED_HEADER
...
def generate_typescript_types(schemas: dict, output_dir: Path, *, has_comms: bool = True, has_skills: bool = True) -> None:
    ...
    # Generate types
    types_content = TS_GENERATED_HEADER
    types_content += f"// Generated wire types for Meerkat SDK\n// Contract version: {contract_version}\n\n"
    ...
    def append_typescript_alias(name: str, root_schema: dict[str, Any]) -> None:
        ...
        if typed_dict_variants is not None:
            ...
            if variant_names:
                ...
                return
        alias_type, _ = _typescript_type_from_schema(
            schema_root,
            schema,
            local_defs,
            inline_objects=True,
    ...
    append_typescript_interface("CreateScheduleRequest", params_schema)
    append_typescript_interface("ScheduleUpdateRequest", params_schema)
    ...
    append_typescript_alias("ScheduleToolCallParams", params_schema)
    ...
    append_typescript_interface("Schedule", wire_schema)
    append_typescript_interface("Occurrence", wire_schema)
    append_typescript_interface("ScheduleToolDescriptor", wire_schema)
    append_typescript_interface("ScheduleToolsResult", wire_schema)
    ...
    append_typescript_alias("ScheduleToolCallResult", wire_schema)
    ...
    append_typescript_alias("WireDeviceCompleteResult", wire_schema)
    append_typescript_interface("WireProvisionApiKeyResult", wire_schema)
    ...
    # Generate errors
    errors_content = TS_GENERATED_HEADER
    errors_content += "// Generated error types for Meerkat SDK\n\n"
    ...
    # Index file
    index_content = TS_GENERATED_HEADER
...
def _web_events_ts_type(root: dict[str, Any], schema: Any) -> str:
    ...
    if "$ref" in schema:
        ...
        if ref_name:
            ...
            if sibling_schema:
                sibling_type = _web_events_ts_type(root, sibling_schema)
                if sibling_type != "unknown":
                    return f"{ref_name} & {sibling_type}"
            return ref_name
        return "unknown"
...
def generate_web_event_types(schemas: dict, output_dir: Path) -> None:
    ...
    stream_lagged_schema = events_schema.get("StreamLaggedEvent")
    ...
    if isinstance(stream_lagged_schema, dict):
        lagged_type = _web_events_ts_type(stream_lagged_schema, stream_lagged_schema)
        lines.append(f"export type StreamLaggedEvent = {lagged_type};")
        lagged_variants = _one_of_typed_dict_variants(
            stream_lagged_schema,
            stream_lagged_schema,
        )
        if lagged_variants is not None and len(lagged_variants[1]) == 1:
            lagged_tag = lagged_variants[1][0][0]
            lines.append(
                f"export const STREAM_LAGGED_EVENT_TYPE = {json.dumps(lagged_tag)} "
                'as const satisfies StreamLaggedEvent["type"];'
            )
        lines.append("")
...
def generate_web_mob_types(schemas: dict, output_dir: Path) -> None:
    output_dir.mkdir(parents=True, exist_ok=True)
    params_schema = schemas.get("params", {})
    wire_types_schema = schemas.get("wire-types", {})
    mob_schema = {
        **params_schema,
        **wire_types_schema,
        "$defs": {
            **params_schema.get("$defs", {}),
            **wire_types_schema.get("$defs", {}),
        },
    }
    wire_schema = _schema_root_with_nested_defs(mob_schema)
    emitted: set[str] = set()
    lines: list[str] = [
        TS_GENERATED_HEADER.rstrip(),
    ...
    def append_schema(name: str) -> None:
        schema = _lookup_named_schema(wire_schema, name)
        if not schema:
            raise KeyError(f"schema for generated web mob type {name} not found")
        if (
            ...
            and _one_of_typed_dict_variants(_schema_root_with_local_defs(wire_schema, schema), schema)
            is None
        ):
            append_interface(name)
            return
        append_alias(name)
