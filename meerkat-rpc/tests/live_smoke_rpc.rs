...
async fn e2e_scenario_16_kitchen_sink() {
    ...
    send_request(
        &mut writer,
        &serde_json::json!({
            "jsonrpc":"2.0","id":id,"method":"session/state",
    ...
    assert!(
        resp["error"].is_null(),
        "session/state failed for deferred session: {resp}"
    );
    ...
    send_request(
        &mut writer,
        &serde_json::json!({
            "jsonrpc":"2.0","id":id,"method":"session/accept",
            "params":{
    ...
    assert!(
        resp["error"].is_null(),
        "session/accept failed for deferred session: {resp}"
    );
    ...
    for _ in 0..120 {
        ...
        send_request(
            &mut writer,
            &serde_json::json!({
                "jsonrpc":"2.0","id":id,"method":"session/input",
        ...
        assert!(
            resp["error"].is_null(),
            "session/input failed for deferred session: {resp}"
        );
    ...
    assert!(
        consumed,
        "session/accept should fully consume the deferred-session input before continuing"
    );
