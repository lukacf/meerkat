---
title: "Python SDK"
description: "Python client for the Meerkat agent runtime, communicating over JSON-RPC with rkat."
icon: "python"
---
...
<Steps>
  <Step title="Install the SDK">
    ...
    The package is named `meerkat-sdk`. It has zero runtime dependencies -- only `pytest` and `pytest-asyncio` are needed for development.
  </Step>
...
<Tip>
If the `rkat` binary is not on `PATH`, you can pass its location explicitly:
...
<ParamField path="rkat_path" type="str" default="rkat">
  Path to the `rkat` binary. Resolved via `shutil.which()`.
</ParamField>
...
<Steps>
  <Step title="Initialize">
    Sends `initialize` and checks the server's `contract_version` against the SDK's `CONTRACT_VERSION` (`"0.1.0"`). Raises `MeerkatError` (code `"VERSION_MISMATCH"`) on incompatibility.
  </Step>
  <Step title="Fetch capabilities">
    Fetches capabilities via `capabilities/get` and caches them internally.
  </Step>
</Steps>
...
<ParamField path="model" type="Optional[str]">
  Model name (e.g. `"claude-sonnet-4-5"`, `"gpt-5.2"`).
</ParamField>

<ParamField path="provider" type="Optional[str]">
...
<ParamField path="output_schema" type="Optional[dict]">
  JSON schema for structured output extraction.
</ParamField>

<ParamField path="structured_output_retries" type="int" default="2">
  Retries for structured output validation.
</ParamField>

<ParamField path="hooks_override" type="Optional[dict]">
  Run-scoped hook overrides.
</ParamField>

<ParamField path="enable_builtins" type="bool" default="False">
  Enable built-in tools (task management, etc.).
</ParamField>

<ParamField path="enable_shell" type="bool" default="False">
  Enable shell tool (requires `enable_builtins`).
</ParamField>

<ParamField path="enable_subagents" type="bool" default="False">
  Enable sub-agent tools (fork, spawn).
</ParamField>

<ParamField path="enable_memory" type="bool" default="False">
  Enable semantic memory (`memory_search` tool).
</ParamField>

<ParamField path="host_mode" type="bool" default="False">
  Run in host mode for inter-agent comms.
</ParamField>

<ParamField path="comms_name" type="Optional[str]">
  Agent name for comms (required when `host_mode` is `True`).
</ParamField>

<ParamField path="provider_params" type="Optional[dict]">
...
<ParamField path="session_id" type="str" required>
  Session UUID from a prior `create_session()`.
</ParamField>
...
|--------|---------|---------|
| `get_config()` | `config/get` | `dict` |
| `set_config(config)` | `config/set` | `None` |
| `patch_config(patch)` | `config/patch` | `dict` (updated config) |
...
<ResponseField name="session_id" type="str" default='""'>
  Session UUID.
</ResponseField>

<ResponseField name="text" type="str" default='""'>
  Agent response text.
</ResponseField>

<ResponseField name="turns" type="int" default="0">
  Number of turns completed.
</ResponseField>

<ResponseField name="tool_calls" type="int" default="0">
  Number of tool calls made.
</ResponseField>

<ResponseField name="usage" type="Optional[WireUsage]">
  Token usage breakdown.
</ResponseField>

<ResponseField name="structured_output" type="Optional[Any]">
  Parsed structured output (when `output_schema` was provided).
</ResponseField>

<ResponseField name="schema_warnings" type="Optional[list]">
...
<ResponseField name="input_tokens" type="int" default="0">
  Input tokens consumed.
</ResponseField>

<ResponseField name="output_tokens" type="int" default="0">
  Output tokens generated.
</ResponseField>

<ResponseField name="total_tokens" type="int" default="0">
  Total tokens (input + output).
</ResponseField>

<ResponseField name="cache_creation_tokens" type="Optional[int]">
  Tokens used for cache creation (provider-specific).
</ResponseField>

<ResponseField name="cache_read_tokens" type="Optional[int]">
...
<ResponseField name="sequence" type="int" default="0">
  Monotonic sequence number.
</ResponseField>

<ResponseField name="event" type="Optional[dict]">
  The agent event payload.
</ResponseField>

<ResponseField name="contract_version" type="str" default='""'>
  Contract version of the server.
</ResponseField>
</Accordion>
...
`CapabilitiesResponse` is a dataclass returned by `get_capabilities()`:

<ResponseField name="contract_version" type="str" default='""'>
  Server contract version.
</ResponseField>

<ResponseField name="capabilities" type="list[CapabilityEntry]" default="[]">
  List of capability entries.
</ResponseField>

`CapabilityEntry` is a dataclass for a single capability:

<ResponseField name="id" type="str" default='""'>
  Capability identifier (e.g. `"sessions"`, `"shell"`).
</ResponseField>
...
<ResponseField name="status" type="str" default='"available"'>
  Status string: `"Available"`, `"DisabledByPolicy"`, `"NotCompiled"`, or `"NotSupportedByProtocol"`.
</ResponseField>
</Accordion>
...
<Tabs>
  <Tab title="Client-side codes">
    | Code | Description |
    |------|-------------|
    | `"BINARY_NOT_FOUND"` | `rkat` binary not found on PATH. |
    | `"NOT_CONNECTED"` | Operation attempted before `connect()`. |
    | `"CONNECTION_CLOSED"` | The `rkat rpc` process closed unexpectedly. |
    | `"VERSION_MISMATCH"` | Server contract version incompatible with SDK. |
    | `"CAPABILITY_UNAVAILABLE"` | Capability check failed via `require_capability()` or `SkillHelper.require_skills()`. |
  </Tab>
  <Tab title="Server-side codes">
...
- [TypeScript SDK](/sdks/typescript) - TypeScript SDK reference
