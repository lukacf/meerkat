...
| `pushEvent(sessionId, payload, source?)` | Push an external event into a session |
...
### pushEvent(sessionId, payload, source?)
...
Push an external event into a running session's inbox. Queued and processed at the next turn boundary.

```typescript
await client.pushEvent(
  sessionId,
  { alert: "build failed", repo: "main" },
  "ci",
);
```
