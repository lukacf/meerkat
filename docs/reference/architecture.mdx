...
- **RuntimeSessionAdapter**: bridges `SessionService` into the runtime's event-driven model and owns session runtime bindings / epoch recovery
...
`RuntimeSessionAdapter::prepare_bindings(session_id)` is the canonical runtime-backed entry seam.
...
- **Mob host control plane** (feature-gated): typed `meerkat_mob_*` tools such as `meerkat_mob_create`, `meerkat_mob_list`, `meerkat_mob_status`, `meerkat_mob_lifecycle`, `meerkat_mob_spawn`, `meerkat_mob_member_send`
