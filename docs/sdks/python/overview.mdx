...
[auth binding](/guides/auth) or the provider's environment key. This example
selects Claude. For a clean API-key setup, export your Anthropic key in the
shell that starts Python; the `rkat-rpc` child inherits it:
...
<Note>
Known event types without a handwritten parser class arrive as `UnknownEvent`
with their wire payload preserved in `event.data`. This includes server-tool
content, model-fallback, rewrite-receipt, and boundary-append events; they are
in the generated inventory. `RunStarted.input` preserves the typed content or
pending-tool-results input. The `Retrying` and `HookFailed` parsers still have
payload-shape gaps. See the [event compatibility
reference](/sdks/python/reference#event-compatibility-note) for exact behavior.
</Note>
