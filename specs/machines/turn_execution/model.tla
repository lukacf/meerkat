...
CONSTANTS AsyncOpRefValues, BooleanValues, ContentShapeValues, NatValues, RunIdValues
...
VARIABLES phase, model_step_count, active_run, primitive_kind, admitted_content_shape, vision_enabled, image_tool_results_enabled, tool_calls_pending, pending_op_refs, has_barrier_ops, barrier_satisfied, boundary_count, cancel_after_boundary, terminal_outcome, extraction_attempts, max_extraction_retries

vars == << phase, model_step_count, active_run, primitive_kind, admitted_content_shape, vision_enabled, image_tool_results_enabled, tool_calls_pending, pending_op_refs, has_barrier_ops, barrier_satisfied, boundary_count, cancel_after_boundary, terminal_outcome, extraction_attempts, max_extraction_retries >>
...
StartConversationRun(run_id) ==
    ...
    /\ extraction_attempts' = 0
    /\ max_extraction_retries' = 0
    /\ UNCHANGED << pending_op_refs, has_barrier_ops, barrier_satisfied >>
...
PrimitiveAppliedConversationTurn(run_id, arg_admitted_content_shape, arg_vision_enabled, arg_image_tool_results_enabled) ==
    ...
    /\ UNCHANGED << active_run, primitive_kind, tool_calls_pending, pending_op_refs, has_barrier_ops, barrier_satisfied, boundary_count, cancel_after_boundary, terminal_outcome, extraction_attempts, max_extraction_retries >>
...
PrimitiveAppliedImmediateAppend(run_id, arg_admitted_content_shape, arg_vision_enabled, arg_image_tool_results_enabled) ==
    ...
    /\ UNCHANGED << active_run, primitive_kind, tool_calls_pending, pending_op_refs, has_barrier_ops, barrier_satisfied, cancel_after_boundary, extraction_attempts, max_extraction_retries >>
...
PrimitiveAppliedImmediateAppendCancelsAfterBoundary(run_id, arg_admitted_content_shape, arg_vision_enabled, arg_image_tool_results_enabled) ==
    ...
    /\ UNCHANGED << active_run, primitive_kind, tool_calls_pending, pending_op_refs, has_barrier_ops, barrier_satisfied, extraction_attempts, max_extraction_retries >>
...
RegisterPendingOps(run_id, op_refs, arg_has_barrier_ops) ==
    /\ phase = "WaitingForOps"
    /\ (active_run = Some(run_id))
    /\ (tool_calls_pending > 0)
    /\ phase' = "WaitingForOps"
    /\ model_step_count' = model_step_count + 1
    /\ pending_op_refs' = Some(op_refs)
    /\ has_barrier_ops' = arg_has_barrier_ops
    /\ barrier_satisfied' = (IF arg_has_barrier_ops THEN FALSE ELSE TRUE)
...
OpsBarrierSatisfied(run_id) ==
    ...
    /\ model_step_count' = model_step_count + 1
    /\ barrier_satisfied' = TRUE
    /\ UNCHANGED << active_run, primitive_kind, admitted_content_shape, vision_enabled, image_tool_results_enabled, tool_calls_pending, pending_op_refs, has_barrier_ops, boundary_count, cancel_after_boundary, terminal_outcome, extraction_attempts, max_extraction_retries >>
...
LlmReturnedTerminal(run_id) ==
    ...
    /\ model_step_count' = model_step_count + 1
    /\ boundary_count' = (boundary_count) + 1
    /\ UNCHANGED << active_run, primitive_kind, admitted_content_shape, vision_enabled, image_tool_results_enabled, tool_calls_pending, pending_op_refs, has_barrier_ops, barrier_satisfied, cancel_after_boundary, terminal_outcome, extraction_attempts, max_extraction_retries >>
...
BoundaryContinue(run_id) ==
    ...
    /\ phase' = "CallingLlm"
    /\ model_step_count' = model_step_count + 1
    /\ UNCHANGED << active_run, primitive_kind, admitted_content_shape, vision_enabled, image_tool_results_enabled, tool_calls_pending, pending_op_refs, has_barrier_ops, barrier_satisfied, boundary_count, cancel_after_boundary, terminal_outcome, extraction_attempts, max_extraction_retries >>
...
BoundaryContinueCancelsAfterBoundary(run_id) ==
    ...
    /\ cancel_after_boundary' = FALSE
    /\ terminal_outcome' = "Cancelled"
    /\ UNCHANGED << active_run, primitive_kind, admitted_content_shape, vision_enabled, image_tool_results_enabled, tool_calls_pending, pending_op_refs, has_barrier_ops, barrier_satisfied, boundary_count, extraction_attempts, max_extraction_retries >>
...
BoundaryComplete(run_id) ==
    ...
    /\ model_step_count' = model_step_count + 1
    /\ terminal_outcome' = "Completed"
    /\ UNCHANGED << active_run, primitive_kind, admitted_content_shape, vision_enabled, image_tool_results_enabled, tool_calls_pending, pending_op_refs, has_barrier_ops, barrier_satisfied, boundary_count, cancel_after_boundary, extraction_attempts, max_extraction_retries >>
...
EnterExtraction(run_id, max_retries) ==
    ...
    /\ UNCHANGED << active_run, primitive_kind, admitted_content_shape, vision_enabled, image_tool_results_enabled, tool_calls_pending, pending_op_refs, has_barrier_ops, barrier_satisfied, boundary_count, cancel_after_boundary, terminal_outcome, extraction_attempts >>
...
ExtractionRetry(run_id) ==
    ...
    /\ UNCHANGED << active_run, primitive_kind, admitted_content_shape, vision_enabled, image_tool_results_enabled, tool_calls_pending, pending_op_refs, has_barrier_ops, barrier_satisfied, boundary_count, cancel_after_boundary, terminal_outcome, max_extraction_retries >>
...
CancelAfterBoundaryFromApplyingPrimitive(run_id) ==
    ...
    /\ model_step_count' = model_step_count + 1
    /\ cancel_after_boundary' = TRUE
    /\ UNCHANGED << active_run, primitive_kind, admitted_content_shape, vision_enabled, image_tool_results_enabled, tool_calls_pending, pending_op_refs, has_barrier_ops, barrier_satisfied, boundary_count, terminal_outcome, extraction_attempts, max_extraction_retries >>
...
TurnLimitReachedFromApplyingPrimitive(run_id) ==
    ...
    /\ boundary_count' = (boundary_count) + 1
    /\ terminal_outcome' = "Completed"
    /\ UNCHANGED << active_run, primitive_kind, admitted_content_shape, vision_enabled, image_tool_results_enabled, tool_calls_pending, pending_op_refs, has_barrier_ops, barrier_satisfied, cancel_after_boundary, extraction_attempts, max_extraction_retries >>
...
Next ==
    ...
    \/ \E run_id \in RunIdValues : \E op_refs \in SeqOfAsyncOpRefValues : \E arg_has_barrier_ops \in BOOLEAN : RegisterPendingOps(run_id, op_refs, arg_has_barrier_ops)
    \/ \E run_id \in RunIdValues : OpsBarrierSatisfied(run_id)
...
THEOREM Spec => []pending_op_refs_only_used_while_waiting
