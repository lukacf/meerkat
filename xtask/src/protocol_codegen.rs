...
pub fn run_protocol_codegen() -> Result<()> {
    ...
    if let Some(machine) = turn_machine {
        ...
        let code = generate_peer_ingress_authority(machine)?;
        let code = rustfmt_source(&code)?;
        let output_path = root.join("meerkat-core/src/generated/peer_ingress_authority.rs");
        if let Some(parent) = output_path.parent() {
            fs::create_dir_all(parent)
                .with_context(|| format!("create dir {}", parent.display()))?;
        }
...
/// Generate the core standalone peer-ingress authority adapter from the
/// MeerkatMachine peer-ingress classification / admission surface.
///
/// Public for the drift test. The rendered module is consumed by
/// `meerkat-comms` only when a session-owned `PeerCommsHandle` is absent, so
/// the compatibility path still delegates semantic peer-ingress facts to a
/// generated owner instead of a handwritten reducer.
pub fn render_peer_ingress_authority(machine: &MachineSchema) -> Result<String> {
    generate_peer_ingress_authority(machine)
}

fn generate_peer_ingress_authority(machine: &MachineSchema) -> Result<String> {
    validate_peer_ingress_authority_schema(machine)?;
    Ok(peer_ingress_authority_template().replace("__MACHINE__", machine.machine.as_str()))
}

fn validate_peer_ingress_authority_schema(machine: &MachineSchema) -> Result<()> {
    if machine.machine.as_str() != "MeerkatMachine" {
        bail!(
            "peer_ingress_authority can only be generated from MeerkatMachine, got {}",
            machine.machine
        );
    }

    for signal in ["ClassifyExternalEnvelope", "ClassifyPlainEvent"] {
        machine
            .signals
            .variant_named(signal)
            .with_context(|| format!("MeerkatMachine missing signal {signal}"))?;
    }
    for input in ["ResolvePeerIngressReceive", "ResolvePeerIngressDequeue"] {
        machine
            .inputs
            .variant_named(input)
            .with_context(|| format!("MeerkatMachine missing input {input}"))?;
    }
    for effect in [
        ...
        machine
            .effects
            .variant_named(effect)
            .with_context(|| format!("MeerkatMachine missing effect {effect}"))?;
    }
    Ok(())
}

fn peer_ingress_authority_template() -> &'static str {
    r#"// @generated — peer ingress authority adapter for `__MACHINE__`
        ...
        match &facts.kind {
            ...
            PeerIngressEnvelopeKind::Lifecycle { kind, params } => {
                ...
                PeerIngressAdmission {
                    ...
                    classification,
                    lifecycle_peer: Some(generated_lifecycle_subject(params, facts.from_peer.as_str())),
                    request_id: None,
                }
            }
...
fn generated_response_classification(
