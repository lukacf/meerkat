...
# Add scheduling
meerkat = { version = "0.5", default-features = false, features = ["anthropic", "session-store", "schedule"] }

# Kitchen sink
meerkat = { version = "0.5", features = ["all-providers", "session-store", "session-compaction", "comms", "mcp", "schedule"] }
```
