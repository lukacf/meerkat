...
fn running_worker_supersession_authorizes_cancellation_and_suppresses_late_terminal_result() {
    ...
    let before_feedback = authority.state().clone();
    apply(
        &mut authority,
        mm::MeerkatMachineInput::ResolveLiveDelegationCancellation {
            channel_id: CHANNEL.to_string(),
    ...
    .expect("late cancellation feedback remains accepted after terminal custody release");
    assert_eq!(
        authority.state(),
        &before_feedback,
        "late feedback cannot rewrite terminal truth"
    );
    assert!(
        apply(
            &mut authority,
            mm::MeerkatMachineInput::ResolveLiveDelegationCancellation {
                ...
                interaction_id: INTERACTION.to_string(),
                operation_id: operation_id(),
                worker_identity: "unrelated-member".to_string(),
                outcome: mm::LiveDelegationCancellationOutcome::Cancelled,
            },
        )
        .is_err(),
        "terminal feedback is still exact-worker fenced"
    );
