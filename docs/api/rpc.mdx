...
description: "Stateful JSON-RPC 2.0 interface for stdio and TCP."
icon: "terminal"
---

Meerkat exposes a JSON-RPC 2.0 surface for IDE integration, desktop apps, and automation tools. Stdio is the default, and the binary also supports TCP. Unlike REST and MCP, the RPC server keeps agents alive between turns for fast multi-turn conversations.
...
<Steps>
  <Step title="Start the server">
    ...
    TCP listeners are local-only by default. Binding
...
| `live/refresh` | Live | Apply mutable session config (instructions/tools/audio) to an open live channel; config-only, does not replay history; identity swaps (model/provider) require close + reopen |
