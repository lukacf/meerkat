...
<Note>
**Mob runtime contract vs backend selection:** CLI `run`/`resume` compose a CLI-local mob tool dispatcher into the agent tool set, while `rkat mob ...` is the explicit mob lifecycle surface. Backend selection remains realm-driven (`realm_manifest.json`) and independent from mob tool composition.
</Note>
...
Skill loading, resolution, and rendering:
