...
| `GET` | `/sessions/{id}/transcript-revisions/{revision}` | Fetch a retained transcript revision |
| `POST` | `/sessions/{id}/rewrite-transcript` | Commit a same-session transcript rewrite |
| `POST` | `/sessions/{id}/restore-transcript-revision` | Restore a retained transcript revision in place |
...
evidence, and close WorkGraph items through WorkGraph tools inside sessions;
goal and attention mutators stay on trusted in-process host authority.
</Note>
...
### GET /sessions/{id}/transcript-revisions/{revision}

Fetch a retained transcript revision for the same session. The response uses the same
message shape as `GET /sessions/{id}/history` and includes the requested revision id.

### POST /sessions/{id}/rewrite-transcript

Commit a same-session transcript rewrite against the current transcript head.
...
```json Request
{
  ...
  "reason": {
    "kind": "compaction",
    "note": "remove accidental secret from transcript"
  },
  "expected_parent_revision": "rev-current",
  "running_behavior": "reject"
}
```

Returns the new transcript revision metadata. The rewrite is same-session only:
it changes the durable session snapshot while preserving session identity.

### POST /sessions/{id}/restore-transcript-revision

Restore a retained transcript revision in place.

```json Request
{
  "revision": "rev-earlier",
  "reason": {
    "kind": "restore",
    "note": "restore before bad rewrite"
  },
  "expected_parent_revision": "rev-current",
  "running_behavior": "reject"
}
...
Returns the newly committed head revision that points back to the restored
transcript content.
