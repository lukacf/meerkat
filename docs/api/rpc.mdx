...
<Note>
Current RPC contract does not include `mob_*` methods and does not auto-compose CLI-local mob tools. Mob lifecycle operations are exposed today via CLI (`rkat mob ...`) and CLI tool composition (`rkat run`/`rkat resume`).
</Note>
