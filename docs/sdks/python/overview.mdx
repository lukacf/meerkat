...
`Session.turn(...)`, `Session.stream(...)`, and `DeferredSession.start_turn(...)`
use the same canonical `turn_metadata` carrier as session creation for runtime
metadata such as `model`, `provider`, `provider_params`,
`skill_references`, `additional_instructions`, and `keep_alive`.
