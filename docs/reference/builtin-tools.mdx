...
10K tokens), and stderr at half of that. Longer output keeps its first and last
halves with a marker between them that names the total size and suggests a
narrower command (`sed -n`, `head`, `tail`, `grep -n`) to page the middle.
