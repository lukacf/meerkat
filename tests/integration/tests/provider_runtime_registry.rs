...
//! in memory → `meerkat::default_provider_registry().resolve(...)` →
...
use meerkat::{ProviderAuthError, ResolverEnvironment};
use meerkat_core::{
...
fn unsupported_combination_surfaces_typed_error() {
    ...
    assert!(
        matches!(
            err,
            ProviderAuthError::Binding(meerkat::ProviderBindingError::ProviderMismatch)
        ),
