...
- `--wait-for-mcp` (block until all MCP servers finish connecting before the first turn)
- `--no-subagents`
- `-N` (short for `--no-subagents`)
- `--output-schema <SCHEMA>` (file path or inline JSON for structured output)
- `--output-schema-compat <lossy|strict>` (schema lowering mode)
- `--structured-output-retries <N>` (validation retry count, default: 2)
...
When `--enable-mob` (`-M`) is passed, `run` composes the consolidated mob tool surface (backed by `meerkat-mob-mcp`) into the agent tool list (`mob_create`, `mob_list`, `mob_lifecycle`, `mob_events`, `mob_run_flow`, `mob_flow_status`, `mob_cancel_flow`, `meerkat_spawn`, `meerkat_retire`, `meerkat_list`, `meerkat_wire`, `meerkat_message`, `mob_respawn`).
...
MCP servers connect in parallel in the background. Their tools become available as each server completes its handshake. The agent sees a `[MCP_PENDING]` system notice while servers are still connecting.

Use `--wait-for-mcp` when the first prompt requires MCP tools to be available immediately:

```bash
rkat run --wait-for-mcp -x "use the filesystem tool to list /tmp"
```

The wait timeout is derived from the configured servers: `max(connect_timeout_secs) + 5s`, capped at 60 seconds.
