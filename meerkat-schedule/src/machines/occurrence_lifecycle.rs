//! Occurrence lifecycle machine — DSL-generated canonical state.
//!
//! The generated `OccurrenceLifecycleMachineState` is the machine-owned
//! portion of `Occurrence`. It owns phase, tracking fields, and timing.
//! Non-machine metadata (trigger/target snapshots, policies, created_at)
//! lives in `OccurrenceContext`.
...
/// Simplified failure class for the machine state. The machine stores
/// failure class opaquely — it never branches on the specific variant.
/// Real `OccurrenceFailureClass` (8 variants) maps to these 3 categories.
