...
Live channels in Meerkat provide low-latency audio and text streaming through the `live/*` JSON-RPC methods. To use a live channel, create a session on a realtime-capable model (for example `gpt-realtime-2`) and call `live/open` to start the channel explicitly. The previous automatic attachment model (`session/realtime_attachment_status`, `realtime/open_info`, etc.) has been replaced by this caller-initiated surface.
...
<Note>
`ModelCapabilities.realtime` remains the capability bit that gates whether `live/open` succeeds. The `live/*` RPC methods replace the previous `realtime/*` bootstrap methods and `session/realtime_attachment_status`. The `--live-ws <addr>` flag on `rkat-rpc` enables the WebSocket listener required for audio transport.
</Note>
...
- **OpenAI**: any model whose name contains `realtime` (currently **`gpt-realtime-2`** as the canonical realtime model; `gpt-realtime` remains a legacy compatibility alias).
...
- **OpenAI Realtime API only.** The shipped provider integration is OpenAI's Realtime API (`gpt-realtime-2`; `gpt-realtime` kept as a compatibility alias). Other providers are not yet wired into the live transport layer.
