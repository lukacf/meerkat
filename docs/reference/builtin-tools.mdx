...
**Provider params:** Use top-level `size`, `quality`, `format`, and `intent` for normal requests; these are Meerkat fields, not raw provider params. OpenAI `provider_params` are advanced overrides for `background`, `output_compression`, `moderation`, and hosted-tool-only `action`. With the current `gpt-image-2` default, `background` is `auto` or `opaque` only, `output_compression` applies when `format` is `jpeg`/`webp`, `action` is usually omitted in favor of top-level `intent`, and `input_fidelity` is not accepted. Gemini accepts `aspect_ratio` and `image_size`.
