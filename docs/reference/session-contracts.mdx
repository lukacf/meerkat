...
| --- | --- | --- |
| Peer message or request, text-only prompt | request-only | A temporary injected-context row at the tail of exactly the next model request. Never Session state; the input is consumed at injection. |
| Prompt whose typed conversation appends are all `system_notice`, `user` or `injected_context` rows (for example a detached job's persisted background-job notice) | durable | The appends are written into the running turn's transcript after that boundary's tool results and before the next assistant message. The same request and every later request of the run see them, and they commit with the run like tool results. The input is consumed with the run terminal. |
| Prompt carrying any other typed append (a `system` row) | follow-up only | Never joins the running turn. |
...
extraction), the input takes the ordinary queued follow-up turn, which writes
the same transcript rows. Exactly one of the two paths writes them:
