...
- Supports keep-alive mode where enabled.
- Keep-alive mode is a lifecycle contract, not just a surface convenience: whichever
  owner path is active must keep inbox draining and turn-boundary suppression
  aligned with the canonical drain lifecycle truth.
...
- If `keep_alive = true`, the turn must preserve the session's keep-alive drain
  ownership semantics after the turn completes. Direct and runtime-backed
  service paths are not allowed to diverge on that contract.
