...
family. Where the host can deliver a later completion, the `council` agent tool
returns `status: "running"` with a `job_id`, and the structured outcome arrives
as that background job's completion; on a one-shot host (`rkat run` without
`--keep-alive`) the call blocks and returns it through the ordinary tool-result
channel. `fork_off` follows the same split. Cleanup debt is deliberately not a
