...
pub fn canonical_machine_coverage_manifests() -> Vec<MachineCoverageManifest> {
    vec![
        ...
        machine_manifest_from_schema(
            ...
            &[
                ...
                scenario(
                    ...
                    "RequestShutdownImmediateIdle, RequestShutdownImmediateAdmitted, RequestShutdownDeferredRunning, RequestShutdownDeferredCompleting, and RequestShutdownAlreadyShuttingDown establish the generated PendingAdmissionDrainRequested obligation; ResolvePendingAdmissionDrained closes it only in ShuttingDown, and AuthorizeSessionTeardown emits SessionTeardownAuthorized only after the shell has drained queued admission work, so every normal session-task exit consumes generated teardown authority",
                    CoverageClaims::none()
                        .transitions(&[
                            "RequestShutdownImmediateIdle",
                            "RequestShutdownImmediateAdmitted",
                        ...
                        .invariants(&["drain_obligation_only_while_shutting_down"]),
                ),
                scenario(
        ...
        machine_manifest_from_schema(
            ...
            &[
                scenario(
                    "temporary_council_request_identity_binding",
                    "One council id binds exactly one canonical request fingerprint: the exact request replays, a materially different request is a typed conflict that never rebinds the identity, and an empty fingerprint is refused outright",
                    CoverageClaims::none()
                        .transitions(&[
                            "OpenEmpty",
                            "OpenEmptyMalformed",
                ...
                scenario(
                    "temporary_council_advance_and_result_seal",
                    "A council advances Preparing -> Running -> Merging with idempotent replays, seals exactly one immutable result, and refuses a second seal under a different terminal class; a council that never reached a runnable discussion still enters merge so the explicit merge-back policy can produce its typed not-attempted outcome",
                    CoverageClaims::none()
                        .transitions(&[
                            "StartDiscussionPreparing",
                            "StartDiscussionReplay",
                ...
                scenario(
                    "temporary_council_single_executor_claim_and_fencing",
        ...
        machine_manifest_from_schema(
            ...
            &[
                scenario(
                    ...
                    "A reservation binds one request fingerprint and a positive reuse budget, exact reserve replay converges, a conflicting fingerprint is a typed reject, and a create-failure keeps the SAME request retryable without letting a different request steal the identity",
                    CoverageClaims::none()
                        .transitions(&[
                            "ReserveEmpty",
                ...
                scenario(
                    "forked_participant_bounded_single_holder_attachment",
                    "An active capability admits one attachment at a time and increments the use count exactly once, exact attach replay returns the original grant without a second increment, a concurrent attachment is typed busy, an invalid authentication observation changes nothing, and the bounded reuse budget terminalizes to Exhausted on the release that spends it",
                    CoverageClaims::none()
                        .transitions(&[
                            "AttachActiveGrant",
