...
input, and admits the child's one turn. On a host that outlives the call, such
as a long-lived server or `rkat run --keep-alive`, it then returns without
...
When the child's turn ends, the forker receives a background-job completion for
that `job_id`. Its detail is a JSON outcome naming the child (`agent_identity`,
`member_ref`) with a `status`:
...
There is no default deadline: the child runs for as long as its task needs.
`max_run_secs` is an opt-in autokill that cancels the child's run and retires
the child once it has run that long; omit it for no limit. `fork_off` rejects
arguments it does not define, so a misspelled or invented field is an argument
error instead of a silently ignored intent.

`council` follows the same pattern: on a host that outlives the call it returns
`status: "running"`, the `council_id`, and a `job_id`, and the sealed council
outcome arrives as that job's completion. See
[Temporary Councils](/guides/temporary-councils#agent-tool).

A one-shot host, such as `rkat run` without `--keep-alive`, may exit when the
turn ends and so cannot deliver a later completion. There both tools block and
return the outcome as their ordinary tool result: `fork_off` returns the
child's `bounded_result`, and `council` the sealed outcome. The agent loop's
default tool deadline does not apply to that wait; `max_run_secs` and the
council's `timeout_seconds` are the only bounds.
