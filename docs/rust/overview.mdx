...
match event {
    ...
    // Durable steer: typed appends written into the RUNNING turn at a model
    // boundary (reconcile with the run terminal; a discarded image redelivers)
    AgentEvent::BoundaryAppendApplied { run_id, input_id, content, append_count } => {}
