//! Fatal errors on the `rkat-mcp` binary are rendered for the operator.
//!
...
fn binary() -> Option<PathBuf> {
    let advertised = std::env::var_os("CARGO_BIN_EXE_rkat-mcp").map(PathBuf::from)?;
...
fn fatal_startup_error_renders_display_chain_and_exits_one() {
    let Some(binary) = binary() else {
        eprintln!(
            "skipping: {BINARY} binary not advertised (CARGO_BIN_EXE_rkat-mcp unset or missing)"
        );
        return;
    };
