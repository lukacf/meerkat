...
`connection_ref` inside `turn_metadata` to scope the swap to a different
realm or binding.
...
// RPC turn/start — hot-swap to openai binding in the same realm
{
  ...
  "params": {
    ...
    "turn_metadata": {
      ...
      "connection_ref": {
        "action": "set",
        "value": {"realm": "prod", "binding": "openai"}
      }
    }
  }
}
...
Pass `turn_metadata.connection_ref` on the host-side mob spawn surface to scope
a member to a specific binding:
...
`turn_metadata.connection_ref` override object only; the colon-joined
`realm:binding[:profile]` form is a CLI input convenience and is converted
before it crosses the wire boundary.
...
- [Hooks](/guides/hooks) — observe auth events or gate turns on
  credential state
- [Mobs](/guides/mobs) — per-member `turn_metadata.connection_ref` overrides
