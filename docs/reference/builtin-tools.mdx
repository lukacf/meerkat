...
`AgentFactory` is the only place tool categories are composed. The public `meerkat::AgentBuilder` delegates to the factory path with explicit client/tool/store overrides. The lower-level `meerkat_core::AgentBuilder` is not re-exported by the facade and is only for standalone core embeddings that opt into the standalone core feature. See [AgentBuilder vs AgentFactory](/rust/advanced#agentbuilder-vs-agentfactory).
