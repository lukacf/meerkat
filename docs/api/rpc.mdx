...
| `session/rewrite_transcript` | Session | Commit a same-session transcript rewrite |
| `session/transcript_revision` | Session | Read a transcript revision by id |
| `session/restore_transcript_revision` | Session | Restore an earlier transcript revision in place |
...
| `mob/wire_members_batch` | Mob | Wire multiple local mob member edges in one batch (mob feature) |
...
| `workgraph/goal/create` | WorkGraph | Create a session-scoped attention goal (workgraph feature) |
| `workgraph/goal/status` | WorkGraph | Read goal and attention status (workgraph feature) |
| `workgraph/goal/confirm` | WorkGraph | Attach completion confirmation evidence (workgraph feature) |
| `workgraph/goal/request_close` | WorkGraph | Request policy-gated goal closure (workgraph feature) |
| `workgraph/attention/list` | WorkGraph | List active attention bindings (workgraph feature) |
| `workgraph/attention/pause` | WorkGraph | Pause an attention binding (workgraph feature) |
| `workgraph/attention/resume` | WorkGraph | Resume an attention binding (workgraph feature) |
| `workgraph/attention/continue` | WorkGraph | Queue a hidden continuation for a session-bound attention binding (workgraph feature) |
...
host/session authority. Public JSON-RPC servers keep the methods visible for
contract stability, but fail closed for create, confirm, close, pause, resume,
and continuation injection.
