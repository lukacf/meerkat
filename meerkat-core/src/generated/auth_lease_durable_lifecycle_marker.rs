...
// Durable marker source fields: phase->new_state, expires_at->expires_at, generation->credential_generation, credential_published_at_millis->credential_published_at_millis, identity fields: credential_kind, realm, binding, profile, account
...
const FIELD_CREDENTIAL_KIND: &str = "credential_kind";
...
pub(crate) fn encode_marker_value(marker: DurableAuthLifecycleMarker) -> serde_json::Value {
    ...
    match marker.token_key.credential_identity() {
        crate::AuthCredentialIdentity::Binding(binding) => {
            map.insert(
                FIELD_CREDENTIAL_KIND.to_string(),
                serde_json::Value::String("binding".to_string()),
            );
            map.insert(
                FIELD_BINDING.to_string(),
                serde_json::Value::String(binding.binding.as_str().to_string()),
            );
            map.insert(
        ...
        crate::AuthCredentialIdentity::Account(account) => {
            ...
            map.insert(
                FIELD_ACCOUNT.to_string(),
                serde_json::Value::String(account.account.as_str().to_string()),
            );
        }
    }
    map.insert(
        ...
        match value.get(FIELD_CREDENTIAL_KIND)?.as_str()? {
            ...
            "account" => TokenKey::from_credential_identity(
                &crate::AuthCredentialIdentity::Account(crate::CredentialAccountRef {
                    realm,
                    account: crate::CredentialAccountId::parse(value.get(FIELD_ACCOUNT)?.as_str()?)
                        .ok()?,
                }),
            ),
            _ => return None,
