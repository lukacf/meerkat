...
pub fn render_machine_kernel_module(schema: &MachineSchema) -> String {
    ...
    if schema.rust.crate_name == "meerkat-mob" {
        match module_name.as_str() {
            "loop_iteration" => return render_loop_iteration_modeled_module(),
            "flow_frame" => return render_flow_frame_modeled_module(),
            "flow_run" => return render_flow_run_modeled_module(),
            _ => {}
        }
    }
    ...
    if schema.rust.crate_name == "meerkat-mob"
        || (schema.helpers.is_empty() && schema.derived.is_empty())
    {
        pushln!(&mut out, "    use super::*;");
        pushln!(
            &mut out,
...
fn maybe_remove<T: Ord + Clone + PartialEq>(seq: &mut Vec<T>, set: &mut std::collections::BTreeSet<T>, value: &T) {
    if set.remove(value) {
