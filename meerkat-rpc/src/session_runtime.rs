...
impl SessionService for RpcMobSessionService {
    ...
    async fn interrupt(&self, id: &SessionId) -> Result<(), SessionError> {
        self.service.interrupt(id).await
    }

    async fn cancel_after_boundary(&self, id: &SessionId) -> Result<(), SessionError> {
        self.service.cancel_after_boundary(id).await
    }
...
impl SessionRuntime {
    ...
    pub async fn interrupt(&self, session_id: &SessionId) -> Result<(), RpcError> {
        ...
        if staged_is_promoting {
            return match self.service.interrupt(session_id).await {
                Ok(()) => Ok(()),
            ...
            .await
        {
            ...
            }) if staged_is_promoting => match self.service.interrupt(session_id).await {
                Ok(()) => Ok(()),
    ...
    pub async fn list_sessions_rich(
        ...
        let mut result = Vec::new();
        let labels = query.labels.clone();
        let service_summaries = self.service.list(query).await.unwrap_or_default();
        let materialized_session_ids: std::collections::HashSet<_> = service_summaries
            .iter()
            .map(|summary| summary.session_id.clone())
            .collect();

        // Include pending sessions as synthetic entries.
        for (session_id, info) in self.staged_sessions.list(labels.as_ref()).await {
            if materialized_session_ids.contains(&session_id) {
                continue;
            }
...
mod tests {
    ...
    async fn archived_store_only_runtime_routed_turn_rejects_before_registration() {
        ...
        assert!(
            rejected
                .as_ref()
                .err()
                .is_some_and(|err| err.rpc_error.code == error::SESSION_NOT_FOUND),
    ...
    async fn runtime_routed_turn_capacity_full_rejects_before_input_accept() {
        ...
        assert!(
            rejected
                .as_ref()
                .err()
                .is_some_and(|err| err.rpc_error.message.contains("Max sessions")),
    ...
    async fn runtime_turn_keep_alive_override_on_persisted_only_unregisters_new_executor() {
        ...
        assert_eq!(err.rpc_error.code, error::INVALID_PARAMS);
        assert_eq!(
