...
- **Detached (hosts that outlive the call).** The call returns at once with
  `{"status": "running", "council_id": ..., "job_id": ...}`. The council runs
  as a background operation of the convener's session. When it ends, the
  convener receives a background-job completion for that `job_id` whose detail
  is the sealed outcome (`result`, `cleanup`, `replayed`). The council's own
  `timeout_seconds` bounds it; the agent loop's default tool deadline does not,
  so a council longer than that deadline still delivers its result.
- **Blocking (one-shot hosts).** A host that may exit when the turn ends, such
  as `rkat run` without `--keep-alive`, cannot deliver a later completion. There
  the call waits for the council and returns the sealed outcome as its ordinary
  tool result. No default tool deadline applies to the wait; `timeout_seconds`
  is the bound.
...
for a durable request on a process-bound store) arrives as a **failed**
background-job completion whose detail carries the typed error text. On a
