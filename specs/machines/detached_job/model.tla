...
CONSTANTS BooleanValues, DetachedJobRestartClassValues, DetachedJobTerminalKindValues, NatValues, StringValues
...
SubmitQueued(arg_job_id, arg_restart_class) ==
    ...
    /\ UNCHANGED << attempt_count, current_attempt_id, current_fence, current_worker_id, lease_expires_at_ms, heartbeat_at_ms, checkpoint_ref, runner_handle, progress_cursor, lease_expired, retry_due_at_ms, cancel_requested, terminal_kind, terminal_delivery_sequence, terminal_delivery_applied >>
...
ClaimQueued(attempt_id, worker_id, claimed_at_ms, arg_lease_expires_at_ms, arg_runner_handle) ==
    ...
    /\ retry_due_at_ms' = None
    /\ cancel_requested' = FALSE
    /\ UNCHANGED << job_id, restart_class, checkpoint_ref, progress_cursor, terminal_kind, terminal_delivery_sequence, terminal_delivery_applied >>
...
RenewRunningLease(attempt_id, fence, arg_heartbeat_at_ms, arg_lease_expires_at_ms) ==
    ...
    /\ UNCHANGED << job_id, restart_class, attempt_count, current_attempt_id, current_fence, current_worker_id, checkpoint_ref, runner_handle, progress_cursor, lease_expired, retry_due_at_ms, cancel_requested, terminal_kind, terminal_delivery_sequence, terminal_delivery_applied >>
...
ReportRunningProgress(attempt_id, fence, cursor, observed_at_ms) ==
    ...
    /\ UNCHANGED << job_id, restart_class, attempt_count, current_attempt_id, current_fence, current_worker_id, lease_expires_at_ms, heartbeat_at_ms, checkpoint_ref, runner_handle, lease_expired, retry_due_at_ms, cancel_requested, terminal_kind, terminal_delivery_sequence, terminal_delivery_applied >>
...
RecordRunningCheckpoint(attempt_id, fence, arg_checkpoint_ref, observed_at_ms) ==
    ...
    /\ UNCHANGED << job_id, restart_class, attempt_count, current_attempt_id, current_fence, current_worker_id, lease_expires_at_ms, heartbeat_at_ms, runner_handle, progress_cursor, lease_expired, retry_due_at_ms, cancel_requested, terminal_kind, terminal_delivery_sequence, terminal_delivery_applied >>
...
WaitExternalFromRunning(attempt_id, fence, observed_at_ms) ==
    ...
    /\ phase' = "WaitingExternal"
    /\ model_step_count' = model_step_count + 1
    /\ UNCHANGED << job_id, restart_class, attempt_count, current_attempt_id, current_fence, current_worker_id, lease_expires_at_ms, heartbeat_at_ms, checkpoint_ref, runner_handle, progress_cursor, lease_expired, retry_due_at_ms, cancel_requested, terminal_kind, terminal_delivery_sequence, terminal_delivery_applied >>
...
RequestCancelRunning ==
    ...
    /\ UNCHANGED << job_id, restart_class, attempt_count, current_attempt_id, current_fence, current_worker_id, lease_expires_at_ms, heartbeat_at_ms, checkpoint_ref, runner_handle, progress_cursor, lease_expired, retry_due_at_ms, terminal_kind, terminal_delivery_sequence, terminal_delivery_applied >>
...
RequestCancelQueued ==
    ...
    /\ terminal_delivery_sequence' = (terminal_delivery_sequence) + 1
    /\ UNCHANGED << job_id, restart_class, attempt_count, current_attempt_id, current_fence, current_worker_id, lease_expires_at_ms, heartbeat_at_ms, checkpoint_ref, runner_handle, progress_cursor, lease_expired, retry_due_at_ms, terminal_delivery_applied >>
...
LeaseExpiresRunning(attempt_id, fence, observed_at_ms) ==
    ...
    /\ model_step_count' = model_step_count + 1
    /\ lease_expired' = TRUE
    /\ UNCHANGED << job_id, restart_class, attempt_count, current_attempt_id, current_fence, current_worker_id, lease_expires_at_ms, heartbeat_at_ms, checkpoint_ref, runner_handle, progress_cursor, retry_due_at_ms, cancel_requested, terminal_kind, terminal_delivery_sequence, terminal_delivery_applied >>
...
ScheduleRetryAfterLoss(arg_retry_due_at_ms) ==
    ...
    /\ UNCHANGED << job_id, restart_class, attempt_count, current_attempt_id, current_fence, current_worker_id, lease_expires_at_ms, heartbeat_at_ms, checkpoint_ref, runner_handle, progress_cursor, lease_expired, cancel_requested, terminal_kind, terminal_delivery_sequence, terminal_delivery_applied >>
...
ClassifyNonResumableWorkerLoss(observed_at_ms) ==
    ...
    /\ terminal_delivery_sequence' = (terminal_delivery_sequence) + 1
    /\ UNCHANGED << job_id, restart_class, attempt_count, current_attempt_id, current_fence, current_worker_id, lease_expires_at_ms, heartbeat_at_ms, checkpoint_ref, runner_handle, progress_cursor, lease_expired, retry_due_at_ms, cancel_requested, terminal_delivery_applied >>
...
ApplySucceededDelivery(delivery_sequence) ==
    /\ phase = "Succeeded"
    /\ ((terminal_delivery_applied = FALSE) /\ (delivery_sequence = terminal_delivery_sequence))
    /\ phase' = "Succeeded"
    /\ model_step_count' = model_step_count + 1
    /\ terminal_delivery_applied' = TRUE
    /\ UNCHANGED << job_id, restart_class, attempt_count, current_attempt_id, current_fence, current_worker_id, lease_expires_at_ms, heartbeat_at_ms, checkpoint_ref, runner_handle, progress_cursor, lease_expired, retry_due_at_ms, cancel_requested, terminal_kind, terminal_delivery_sequence >>


ApplyFailedDelivery(delivery_sequence) ==
    /\ phase = "Failed"
    /\ ((terminal_delivery_applied = FALSE) /\ (delivery_sequence = terminal_delivery_sequence))
    /\ phase' = "Failed"
    /\ model_step_count' = model_step_count + 1
...
ApplyCancelledDelivery(delivery_sequence) ==
    /\ phase = "Cancelled"
    /\ ((terminal_delivery_applied = FALSE) /\ (delivery_sequence = terminal_delivery_sequence))
    /\ phase' = "Cancelled"
    /\ model_step_count' = model_step_count + 1
...
ApplyWorkerLostDelivery(delivery_sequence) ==
    /\ phase = "WorkerLost"
    /\ ((terminal_delivery_applied = FALSE) /\ (delivery_sequence = terminal_delivery_sequence))
    /\ phase' = "WorkerLost"
    /\ model_step_count' = model_step_count + 1
...
ApplyNeedsAttentionDelivery(delivery_sequence) ==
    /\ phase = "NeedsAttention"
    /\ ((terminal_delivery_applied = FALSE) /\ (delivery_sequence = terminal_delivery_sequence))
    /\ phase' = "NeedsAttention"
    /\ model_step_count' = model_step_count + 1
...
ObserveSucceededDeliveryAlreadyApplied(delivery_sequence) ==
    /\ phase = "Succeeded"
    /\ (terminal_delivery_applied /\ (delivery_sequence = terminal_delivery_sequence))
    /\ phase' = "Succeeded"
    /\ model_step_count' = model_step_count + 1
    /\ UNCHANGED << job_id, restart_class, attempt_count, current_attempt_id, current_fence, current_worker_id, lease_expires_at_ms, heartbeat_at_ms, checkpoint_ref, runner_handle, progress_cursor, lease_expired, retry_due_at_ms, cancel_requested, terminal_kind, terminal_delivery_sequence, terminal_delivery_applied >>


ObserveFailedDeliveryAlreadyApplied(delivery_sequence) ==
    /\ phase = "Failed"
    /\ (terminal_delivery_applied /\ (delivery_sequence = terminal_delivery_sequence))
    /\ phase' = "Failed"
    /\ model_step_count' = model_step_count + 1
...
ObserveCancelledDeliveryAlreadyApplied(delivery_sequence) ==
    /\ phase = "Cancelled"
    /\ (terminal_delivery_applied /\ (delivery_sequence = terminal_delivery_sequence))
    /\ phase' = "Cancelled"
    /\ model_step_count' = model_step_count + 1
...
ObserveWorkerLostDeliveryAlreadyApplied(delivery_sequence) ==
    /\ phase = "WorkerLost"
    /\ (terminal_delivery_applied /\ (delivery_sequence = terminal_delivery_sequence))
    /\ phase' = "WorkerLost"
    /\ model_step_count' = model_step_count + 1
...
ObserveNeedsAttentionDeliveryAlreadyApplied(delivery_sequence) ==
    /\ phase = "NeedsAttention"
    /\ (terminal_delivery_applied /\ (delivery_sequence = terminal_delivery_sequence))
    /\ phase' = "NeedsAttention"
    /\ model_step_count' = model_step_count + 1
...
Next ==
    ...
    \/ \E delivery_sequence \in 0..2 : ApplySucceededDelivery(delivery_sequence)
    \/ \E delivery_sequence \in 0..2 : ApplyFailedDelivery(delivery_sequence)
    \/ \E delivery_sequence \in 0..2 : ApplyCancelledDelivery(delivery_sequence)
    \/ \E delivery_sequence \in 0..2 : ApplyWorkerLostDelivery(delivery_sequence)
    \/ \E delivery_sequence \in 0..2 : ApplyNeedsAttentionDelivery(delivery_sequence)
    \/ \E delivery_sequence \in 0..2 : ObserveSucceededDeliveryAlreadyApplied(delivery_sequence)
    \/ \E delivery_sequence \in 0..2 : ObserveFailedDeliveryAlreadyApplied(delivery_sequence)
    \/ \E delivery_sequence \in 0..2 : ObserveCancelledDeliveryAlreadyApplied(delivery_sequence)
    \/ \E delivery_sequence \in 0..2 : ObserveWorkerLostDeliveryAlreadyApplied(delivery_sequence)
    \/ \E delivery_sequence \in 0..2 : ObserveNeedsAttentionDeliveryAlreadyApplied(delivery_sequence)
    \/ TerminalStutter
...
terminal_requires_delivery == (IF ((phase # "Succeeded") /\ (phase # "Failed") /\ (phase # "Cancelled") /\ (phase # "WorkerLost") /\ (phase # "NeedsAttention")) THEN TRUE ELSE ((terminal_kind # None) /\ (terminal_delivery_sequence > 0)))
