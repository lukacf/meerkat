...
Pass `turn_metadata.skill_references` in raw API requests to resolve and inject skills for a specific turn. SDKs expose `skill_refs` convenience options that serialize into this runtime metadata carrier:
...
See the [Python SDK](/sdks/python/reference#skills-parameters) and [TypeScript SDK](/sdks/typescript/reference#skills-parameters) for SDK wrappers. Session creation uses `turn_metadata.skill_references` (or `turnMetadata.skillReferences` in TypeScript); per-turn SDK helpers keep `skill_refs` / `skillRefs` convenience options that serialize into the canonical metadata carrier.
