...
The repository also contains a source-build-only `rkat-mini` profile, a smaller
static surface for run/session/config workflows. See
[Mini surfaces](/guides/mini-surfaces) for the exact contract and build command.
...
## Source-build `rkat-mini`

`rkat-mini` keeps the same task-first execution model but trims the command set down to:
