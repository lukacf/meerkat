...
Built-in tool categories: builtins (`enable_builtins`, including task tools plus utility helpers like `wait`, `datetime`, and `apply_patch`), shell (`enable_shell`), sub-agents (`enable_subagents`), semantic memory (`enable_memory`), and mob orchestration (`enable_mob`).
