...
| F2 | `meerkat-runtime/src/meerkat_machine/dsl.rs` (supervisor_bound_*) | Supervisor authorization has companion trust edge in router `TrustedPeers` | split ownership with step-lock | single owner | Already acknowledged by DSL comment (lines 114-134). Either: (a) move the trust edge into the DSL as well (add it to the AuthMachine trust surface), or (b) formalize the step-lock as a generated obligation pair so the shell cannot forget to mirror. Preferred direction: pull trust-edge mutations into AuthMachine authority, MeerkatMachine consumes via projection. | wave-c |
| F3 | `meerkat-machine-schema/src/catalog/dsl/meerkat_machine.rs:112` | `peer_ingress_mob_id` references MobId with no structural "mob-exists" invariant in MeerkatMachine | cross-machine id reference | foreign-key w/ cleanup discipline | Document explicit contract: MeerkatMachine tolerates dangling MobId iff a mob destruction propagates a `DetachIngress` input before destroying the mob. If that propagation is not already protocolized, add a `MobDestroyingSessionIngress` handoff obligation so mob-destroy → session-detach is compile-time paired. | wave-c |
