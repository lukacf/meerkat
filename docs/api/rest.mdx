...
| `POST` | `/workgraph/goal/request_close` | Request policy-gated goal closure |
...
| `POST` | `/workgraph/attention/continue` | Queue a hidden continuation for a session-bound attention binding |
...
host/session authority. Public REST servers keep the routes visible for
