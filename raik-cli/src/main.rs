//! raik-cli - Headless CLI for RAIK
...
use raik_client::AnthropicClient;
use raik_core::mcp_config::{McpScope, McpTransportKind};
use raik_core::SystemPromptConfig;
use raik_core::agent::AgentBuilder;
use raik_core::budget::BudgetLimits;
use raik_core::error::AgentError;
use raik_core::SessionId;
use raik_store::{JsonlStore, SessionFilter, SessionStore};
...
#[derive(Parser)]
#[command(name = "raik")]
#[command(about = "RAIK - Rust Agentic Interface Kit")]
struct Cli {
    #[command(subcommand)]
    command: Commands,
}
...
enum CliMcpScope {
    /// User-level config (~/.raik/mcp.toml)
    User,
    /// Project-level config (.raik/mcp.toml)
    Project,
...
async fn create_mcp_tools() -> anyhow::Result<Option<McpRouterAdapter>> {
    use raik_core::mcp_config::{McpConfig, McpScope};
    ...
    if !project_servers.is_empty() {
        ...
        for s in &project_servers {
            let target = match &s.server.transport {
                raik_core::mcp_config::McpTransportConfig::Stdio(stdio) => {
                ...
                raik_core::mcp_config::McpTransportConfig::Http(http) => http.url.clone(),
            };
