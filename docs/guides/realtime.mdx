...
The session's default model can be set in config (`~/.rkat/config.toml` or project-local) via `default_model`. Any session created without an explicit `model` parameter inherits the configured default — so setting `default_model = "gpt-realtime"` makes realtime the default for the whole install. See the [Configuration guide](/guides/configuration).
...
- [Configuration guide](/guides/configuration) — setting `default_model`
