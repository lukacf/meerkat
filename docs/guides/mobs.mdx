...
over MCP, and the browser runtime's `mob_retire` all cascade. Each retirement
runs through the ordinary retire path and every one is attempted; the first
failure is reported. Members spawned by other means are not affected. Embedded
Rust hosts choose: `MobHandle::retire` retires one member,
`MobHandle::retire_with_descendants` cascades.
