...
|---------|---------|---------|
| `init` | `rkat init` | Initialize project config |
...
| `capabilities` | `rkat capabilities` | Show runtime capabilities |
...
Creates `.rkat/config.toml` with sensible defaults for the current project.
...
<Accordion title="Inter-agent communication and events">
...
<ParamField path="--comms-listen-tcp" type="string">
  TCP address to listen on for signed comms (e.g., `0.0.0.0:4200`).
</ParamField>
...
<ParamField path="--host" type="boolean" default="false">
  Run in host mode: process the initial prompt, then stay alive listening for comms messages and external events.
</ParamField>

<ParamField path="--stdin" type="boolean" default="false">
  Read external events from stdin (newline-delimited JSON or plain text). Only meaningful with `--host`. Each line is injected into the agent's inbox.
</ParamField>
</Accordion>
...
Show runtime capabilities -- what features are compiled in and enabled by config.

```bash
rkat capabilities
```

Lists every known capability (sessions, structured output, hooks, builtins, shell, comms, sub-agents, memory, skills, MCP) with its current status: `Available`, `DisabledByPolicy`, or `NotCompiled`.
...
rkat config patch --file patch.json
```
