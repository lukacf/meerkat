...
owner; Windows binaries are cross-compiled from Linux with cargo-xwin
(clang-cl, lld-link, the Windows SDK) on a hosted Ubuntu runner, verified on a
windows-latest runner, and packaged there; registry publication remains
GitHub-hosted.
