fn main() {
    ...
    let Some(suffix) = generated_authority_bridge_symbol_suffix().or_else(docs_rs_unlinked_suffix)
        ...
        eprintln!("meerkat-runtime build could not locate the generated authority bridge suffix");
        std::process::exit(1);
    };
...
fn docs_rs_unlinked_suffix() -> Option<String> {
    ...
    println!(
        "cargo:warning=meerkat-runtime build could not locate the generated authority bridge suffix; DOCS_RS is set, so the meerkat-runtime documentation build continues with the unlinked placeholder suffix docsrs_unlinked"
    );
