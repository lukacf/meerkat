...
CiStateConstraint == /\ model_step_count <= 2 /\ Cardinality(tracked_steps) <= 1 /\ Len(ordered_steps) <= 1 /\ Cardinality(DOMAIN step_status) <= 1 /\ Cardinality(DOMAIN output_recorded) <= 1 /\ Cardinality(DOMAIN step_condition_results) <= 1 /\ Cardinality(DOMAIN step_has_conditions) <= 1 /\ Cardinality(DOMAIN step_dependencies) <= 1 /\ Cardinality(DOMAIN step_dependency_modes) <= 1 /\ Cardinality(DOMAIN step_branches) <= 1 /\ Cardinality(DOMAIN step_collection_policies) <= 1 /\ Cardinality(DOMAIN step_quorum_thresholds) <= 1 /\ Cardinality(DOMAIN step_target_counts) <= 1 /\ Cardinality(DOMAIN step_target_success_counts) <= 1 /\ Cardinality(DOMAIN step_target_terminal_failure_counts) <= 1 /\ Cardinality(DOMAIN target_retry_counts) <= 1 /\ Len(ready_frames) <= 1 /\ Cardinality(ready_frame_membership) <= 1 /\ Len(pending_body_frame_loops) <= 1 /\ Cardinality(pending_body_frame_loop_membership) <= 1
