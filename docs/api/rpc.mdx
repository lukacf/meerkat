...
<Steps>
  <Step title="Start the server">
    ...
    TCP and realtime WebSocket listeners are local-only by default. Binding
...
There is no caller-initiated attach/detach RPC.
</Note>
...
<ParamField body="turn_metadata" type="RuntimeTurnMetadata | null" default="null">
  Canonical first-turn runtime metadata. Use nested fields such as `model`, `provider`, `provider_params`, `skill_references`, `additional_instructions`, and `keep_alive` here.
</ParamField>
...
<ParamField body="comms_name" type="string | null" default="null">
  Agent name for inter-agent communication. Required when `turn_metadata.keep_alive` enables keep-alive.
</ParamField>
...
<ParamField body="turn_metadata" type="RuntimeTurnMetadata | null" default="null">
  Canonical per-turn metadata carrier. Put turn-scoped model/provider/provider_params, skill_references, flow_tool_overlay, additional_instructions, keep_alive, and render metadata inside this object; split top-level runtime metadata fields are rejected.
</ParamField>

<ParamField body="max_tokens" type="u32 | null" default="from session">
  Build-only deferred materialization override. Honored only when `turn/start` runs a deferred session's first turn; it is not runtime metadata.
</ParamField>

<ParamField body="system_prompt" type="string | null" default="from session">
  Build-only deferred materialization override. Honored only when `turn/start` runs a deferred session's first turn; it is not runtime metadata.
</ParamField>

<ParamField body="output_schema" type="object | null" default="from session">
  Build-only deferred materialization override. Honored only when `turn/start` runs a deferred session's first turn; it is not runtime metadata.
</ParamField>

<ParamField body="structured_output_retries" type="u32 | null" default="from session">
  Build-only deferred materialization override. Honored only when `turn/start` runs a deferred session's first turn; it is not runtime metadata.
</ParamField>
