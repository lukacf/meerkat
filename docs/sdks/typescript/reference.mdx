...
`accounting.presentedTokens` rather than per-call `inputTokens`. Every
...
`EventStream` is an `AsyncIterable<StreamEvent>` returned by
`createSessionStreaming()` and `session.stream()`. It yields events as the
agent runs, then makes the final `RunResult` available on `stream.result`.
`StreamEvent` includes `AgentEvent` and scoped wrappers. Use the exported type
guards for direct payloads because `UnknownEvent` keeps the event union
...
Event parsing fails closed: a present `type` outside the generated
`KNOWN_AGENT_EVENT_TYPES` inventory throws a `MeerkatError` with code
`UNKNOWN_EVENT_TYPE`. An inventory-known type without a handwritten parser
case arrives as `UnknownEvent`, preserving the wire type in `event.type` and
the complete payload as fields on the event. A missing event type or a malformed
payload for a handwritten parser case is preserved as `MalformedEvent`
...
`RunStartedEvent.input` preserves the typed `RunInput`: either
`{ kind: "content", content: ... }` or `{ kind: "pending_tool_results" }`.
Content may be text or content blocks; the continuation variant does not
fabricate a prompt. `RunFailedEvent` accepts the current `error_report` and
derives its `errorClass` and `error` display fields from that report.

The handwritten `RetryingEvent` and `HookFailedEvent` parsers still expect
legacy flat fields. Current wire events carry `retry` and `reason`,
respectively, so those payloads arrive as `MalformedEvent` with raw data
preserved. These are remaining SDK parser gaps, not alternate wire contracts.

### Session lifecycle events

```typescript
interface RunStartedEvent   { type: "run_started";   sessionId: string; input: RunInput; identity?: TranscriptMessageIdentity }
...
The inventory-known rows without handwritten parser cases listed above
arrive as `UnknownEvent`. Inspect `event.type` and the raw event fields using
the [RPC event reference](/api/rpc#notifications); generated wire types describe
their payloads even where no ergonomic event interface is exported.
...
const matches = (await client.listSkills()).filter(
  (entry) => entry.is_active && entry.key.skill_name === "extraction-email",
);
const [skill] = matches;
if (!skill || matches.length !== 1) {
  throw new Error("Expected one active extraction-email skill; select a source UUID if ambiguous.");
}
const skillRef = {
...
Discovery returns generated `key.source_uuid` / `key.skill_name` fields,
which must be mapped to `SkillRef`'s camelCase keys. The turn examples require
an installed active skill and reject ambiguous names; select the intended
source UUID explicitly when multiple sources provide that name.
...
console.log(CONTRACT_VERSION);  // "0.8.44"
```
