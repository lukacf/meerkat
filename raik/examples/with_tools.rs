//! Example demonstrating RAIK with custom tools
//!
...
impl AgentLlmClient for AnthropicLlmAdapter {
    async fn stream_response(
        ...
        tools: &[ToolDef],
        max_tokens: u32,
    ) -> Result<LlmStreamResult, raik::AgentError> {
        ...
        while let Some(result) = stream.next().await {
            match result {
                ...
                Err(e) => {
                    return Err(raik::AgentError::LlmError(e.to_string()));
                }
            }
        }
...
impl AgentSessionStore for MemoryStore {
    async fn save(&self, session: &Session) -> Result<(), raik::AgentError> {
        let mut sessions = self.sessions.lock().unwrap();
        sessions.insert(session.id().to_string(), session.clone());
        Ok(())
    }

    async fn load(&self, id: &str) -> Result<Option<Session>, raik::AgentError> {
