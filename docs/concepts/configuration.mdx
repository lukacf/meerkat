...
| `use_catalog_default_chain` under `[model_fallback]` (documented through 0.8.36) | ignored: without other settings in the table, no fallback policy, so it inherits the parent's explicit policy, off if none | `LegacyModelFallbackCatalogChain` |
