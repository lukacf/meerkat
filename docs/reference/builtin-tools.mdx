...
it. `MobMcpState::relink_restored_fork_children` runs the pass explicitly.
...
restart: its custody record carries the convener's job, and after the
post-restore recovery sweep the council's sealed result is delivered, or
`coordinator_interrupted` once the dead coordinator's lease has expired
(councils are never re-executed); `MobMcpState::relink_detached_councils` runs
this explicitly. Blocking, `council` returns the sealed outcome as its tool
...
it transitively spawned, deepest first, through the ordinary retire path; every
retirement is attempted and the first failure is reported. This applies to
`mob_retire_member`, `retire_member`, the `max_run_secs` autokill, the
retirement of a child whose turn failed, and host-side retirement (`mob/retire`
over JSON-RPC, `meerkat_mob_retire` over MCP). A forker retired through any of
these paths takes its descendants with it.
