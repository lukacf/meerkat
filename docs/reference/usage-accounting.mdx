...
summed cache counters that can exceed the input total. Every reporting surface
clamps them to the invariant above: the run result, run and compaction
rollback deltas, session read and list `billing.usage`, and mob run accounting.
...
callback tool results keeps one account when its staged results are applied
and it resumes through `run_pending`, so calls made before the suspension
appear in the resumed run's result. Any other run discards the suspended
