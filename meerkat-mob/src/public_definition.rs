...
#[error("{message}")]
pub struct PublicMobDefinitionDecodeError {
    message: String,
}

impl PublicMobDefinitionDecodeError {
...
type DecodeResult<T> = Result<T, PublicMobDefinitionDecodeError>;
...
/// explicit mechanical mapping into [`MobDefinition`].
pub fn decode_public_mob_definition(input: MobDefinitionInput) -> DecodeResult<MobDefinition> {
...
) -> DecodeResult<(ProfileName, ProfileBinding)> {
    match input {
...
fn decode_profile(input: MobProfileInput) -> DecodeResult<Profile> {
    Ok(Profile {
        model: input.model,
        skills: input.skills,
        tools: ToolConfig {
        ...
        output_schema: input
            ...
            .map(|schema| {
                serde_json::to_value(schema)
                    .map_err(|error| PublicMobDefinitionDecodeError::new(error.to_string()))
            })
            .transpose()?,
...
fn decode_flow_spec(flow_id: String, input: MobFlowSpecInput) -> DecodeResult<(FlowId, FlowSpec)> {
    Ok((
        FlowId::from(flow_id),
        FlowSpec {
            ...
            steps: input
                ...
                .collect::<DecodeResult<IndexMap<_, _>>>()?,
...
) -> DecodeResult<(StepId, FlowStepSpec)> {
    Ok((
        StepId::from(step_id),
        FlowStepSpec {
            role: ProfileName::from(input.role),
            message: ContentInput::try_from(input.message).map_err(|error| {
                PublicMobDefinitionDecodeError::new(format!("invalid flow step message: {error}"))
            })?,
...
fn decode_condition(input: MobConditionExprInput) -> DecodeResult<ConditionExpr> {
    Ok(match input {
...
fn decode_frame_spec(input: MobFrameSpecInput) -> DecodeResult<FrameSpec> {
    Ok(FrameSpec {
        nodes: input
            .nodes
            .into_iter()
...
) -> DecodeResult<(FlowNodeId, FlowNodeSpec)> {
    Ok((
...
fn decode_repeat_until(input: MobRepeatUntilInput) -> DecodeResult<RepeatUntilSpec> {
    Ok(RepeatUntilSpec {
...
mod tests {
    ...
    fn decode_public_mob_definition_maps_nested_flow_shapes() {
        ...
        assert_eq!(
            definition.session_cleanup_policy,
            SessionCleanupPolicy::Manual
        );
