...
## Routes
- `flow_step_dispatch_enters_runtime_admission`: `flow_run`.`AdmitStepWork` -> `runtime_control`.`SubmitCandidate` [Immediate]
- `mob_async_op_event_enters_runtime_admission`: `ops_lifecycle`.`SubmitOpEvent` -> `runtime_control`.`SubmitCandidate` [Immediate]
- `mob_peer_candidate_enters_runtime_admission`: `peer_comms`.`SubmitPeerInputCandidate` -> `runtime_control`.`SubmitCandidate` [Immediate]
...
- `mob-peer-orchestration` — member communication and orchestration stay inside the mob stack
