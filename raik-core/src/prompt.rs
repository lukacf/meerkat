...
//!
//! AGENTS.md discovery:
//! - Global: ~/.raik/AGENTS.md
//! - Project: ./AGENTS.md or ./.raik/AGENTS.md
//!
...
/// Default system prompt for RAIK agents
...
pub struct SystemPromptConfig {
    ...
    /// Custom path to global AGENTS.md (defaults to ~/.raik/AGENTS.md)
    pub global_agents_md_path: Option<PathBuf>,
    /// Custom path to project AGENTS.md (defaults to ./AGENTS.md or ./.raik/AGENTS.md)
...
/// Get the default global AGENTS.md path: ~/.raik/AGENTS.md
pub fn default_global_agents_md_path() -> Option<PathBuf> {
    std::env::var_os("HOME")
        .map(PathBuf::from)
        .map(|h| h.join(".raik/AGENTS.md"))
}

/// Find project AGENTS.md in current directory
/// Checks: ./AGENTS.md, ./.raik/AGENTS.md
...
/// Checks: <dir>/AGENTS.md, <dir>/.raik/AGENTS.md
pub fn find_project_agents_md_in(dir: &Path) -> Option<PathBuf> {
    ...
    // Check ./.raik/AGENTS.md
    let raik_path = dir.join(".raik/AGENTS.md");
    if raik_path.exists() {
        return Some(raik_path);
    }
...
mod tests {
    ...
    #[test]
    fn test_find_project_agents_md_in_raik_dir() {
        let temp = TempDir::new().unwrap();
        let raik_dir = temp.path().join(".raik");
        fs::create_dir_all(&raik_dir).unwrap();
        let agents_path = raik_dir.join("AGENTS.md");
        fs::write(&agents_path, "content").unwrap();
    ...
    fn test_find_project_agents_md_root_takes_precedence() {
        ...
        let raik_dir = temp.path().join(".raik");
        fs::create_dir_all(&raik_dir).unwrap();
        fs::write(raik_dir.join("AGENTS.md"), "raik content").unwrap();
