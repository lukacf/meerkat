...
The previous capability-driven realtime attachment model is retired. Mob live
audio will return through the new live adapter host, where targets are resolved
by `AgentIdentity` at the authority boundary instead of pinning stale session
ids.

## See also

- [Realtime guide](/guides/realtime) — status of the retired realtime surface
