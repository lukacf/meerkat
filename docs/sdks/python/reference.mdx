...
`peer_content_ingested`, `provider_cache_breakpoints_discarded`, and
`boundary_append_applied`. The handwritten parser has no typed case for these
nine rows, so they arrive as `UnknownEvent`.
