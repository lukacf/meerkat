...
Meerkat has one canonical ownership split:

- **`SessionService`** owns session lifecycle, deferred-first-turn state, archiving, and canonical session reads
- **runtime-backed surfaces** own `keep_alive`, Queue/Steer routing, comms drain lifecycle, external-event admission, callback-result admission, and request/turn commit semantics
...
The runtime-owned admission path is:

```text
Surface -> RuntimeSessionAdapter -> SessionService -> AgentFactory::build_agent()
```

The direct session lifecycle path is:

```text
Surface -> SessionService -> AgentFactory::build_agent()
```
...
- no surface-local transcript patching for callback/tool completion
