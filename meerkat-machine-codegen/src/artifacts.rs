...
const MACHINE_CODEGEN_HEADER_MD: &str =
    "<!-- @generated - Generated by `cargo xtask machine-codegen --all`. -->";
const MACHINE_CODEGEN_HEADER_TLA: &str =
    "\\* @generated - Generated by `cargo xtask machine-codegen --all`.";
const MACHINE_CODEGEN_HEADER_CFG: &str =
    "\\* @generated - Generated by `cargo xtask machine-codegen --all`.";
...
pub fn render_machine_contract_markdown(
    ...
    pushln!(&mut out, "{MACHINE_CODEGEN_HEADER_MD}");
    pushln!(&mut out);
    writeln!(
        &mut out,
...
pub fn render_machine_ci_cfg(schema: &MachineSchema, deep: bool) -> String {
    ...
    pushln!(&mut out, "{MACHINE_CODEGEN_HEADER_CFG}");
...
pub fn render_composition_driver(schema: &CompositionSchema) -> Option<String> {
    ...
    let mut fields_by_producer_effect: BTreeMap<&str, BTreeMap<&str, BTreeSet<&str>>> =
        BTreeMap::new();
    ...
    for route in &schema.routes {
        ...
        for binding in &route.bindings {
            ...
            if let RouteBindingSource::Field { from_field, .. } = &binding.source {
                ...
                fields_by_producer_effect
                    .entry(route.from_machine.as_str())
                    .or_default()
                    .entry(route.effect_variant.as_str())
                    .or_default()
                    .insert(from_field.as_str());
            }
        }
    }
    ...
    pushln!(&mut out, "/// Generated adapters for production seam code.");
    pushln!(
        &mut out,
    ...
    pushln!(&mut out, "pub mod adapters {{");
    pushln!(&mut out, "    use super::*;");
    pushln!(&mut out);
    pushln!(&mut out, "    #[derive(Debug, Clone, Copy, PartialEq, Eq)]");
    pushln!(&mut out, "    pub enum Field {{");
    for field in &fields {
        let variant = to_upper_camel_case_local(field);
        pushln!(&mut out, "        {variant},");
    }
    pushln!(&mut out, "    }}");
    pushln!(&mut out);
    pushln!(&mut out, "    impl Field {{");
    pushln!(&mut out, "        pub fn id(self) -> FieldId {{");
    pushln!(&mut out, "            match self {{");
    for field in &fields {
        let variant = to_upper_camel_case_local(field);
        let field_fn = to_snake_case_local(field);
        pushln!(
            &mut out,
            "                Self::{variant} => fields::{field_fn}(),"
        );
    }
    pushln!(&mut out, "            }}");
    pushln!(&mut out, "        }}");
    ...
    if !input_variants.is_empty() {
        ...
        for input_variant in &input_variants {
            let variant = to_upper_camel_case_local(input_variant);
            pushln!(&mut out, "        {variant},");
        }
        pushln!(&mut out, "    }}");
        pushln!(&mut out);
        ...
        for input_variant in &input_variants {
            let variant = to_upper_camel_case_local(input_variant);
            let input_fn = to_snake_case_local(input_variant);
            pushln!(
                &mut out,
                "            if id == &inputs::{input_fn}() {{ return Some(Self::{variant}); }}"
            );
        }
        pushln!(&mut out, "            None");
        pushln!(&mut out, "        }}");
    ...
    if !signal_variants.is_empty() {
        pushln!(&mut out, "    #[derive(Debug, Clone, Copy, PartialEq, Eq)]");
        pushln!(&mut out, "    pub enum Signal {{");
        for signal_variant in &signal_variants {
            let variant = to_upper_camel_case_local(signal_variant);
            pushln!(&mut out, "        {variant},");
        }
        pushln!(&mut out, "    }}");
        pushln!(&mut out);
        ...
        for signal_variant in &signal_variants {
            let variant = to_upper_camel_case_local(signal_variant);
            let signal_fn = to_snake_case_local(signal_variant);
            pushln!(
                &mut out,
                "            if id == &signals::{signal_fn}() {{ return Some(Self::{variant}); }}"
            );
        }
        pushln!(&mut out, "            None");
        pushln!(&mut out, "        }}");
        ...
        for signal_variant in &signal_variants {
            ...
            pushln!(
                &mut out,
                "                Self::{variant} => signals::{signal_fn}(),"
            );
        }
        pushln!(&mut out, "            }}");
        pushln!(&mut out, "        }}");
    ...
    for (instance_id, effect_variants) in &effect_variants_by_producer {
        let producer_mod = to_snake_case_local(instance_id);
        let effect_fields = fields_by_producer_effect
            .get(instance_id)
            .cloned()
            .unwrap_or_default();
        pushln!(&mut out, "    pub mod {producer_mod} {{");
        pushln!(&mut out, "        use super::super::*;");
        pushln!(&mut out, "        use super::Field;");
        pushln!(&mut out);
        pushln!(
            &mut out,
        ...
        for effect_variant in effect_variants {
            let variant = to_upper_camel_case_local(effect_variant);
            pushln!(&mut out, "            {variant},");
        }
        pushln!(&mut out, "        }}");
        pushln!(&mut out);
        pushln!(&mut out, "        impl Effect {{");
        pushln!(
            &mut out,
        ...
        for effect_variant in effect_variants {
            let variant = to_upper_camel_case_local(effect_variant);
            let effect_fn = to_snake_case_local(effect_variant);
            pushln!(
                &mut out,
                "                    Self::{variant} => effects::{producer_mod}::{effect_fn}(),"
            );
        }
        pushln!(&mut out, "                }}");
        pushln!(&mut out, "            }}");
        ...
        pushln!(
            &mut out,
            "                route_to_input(&producers::{producer_mod}_instance_id(), &self.variant_id())"
        );
        pushln!(&mut out, "            }}");
        pushln!(&mut out);
        pushln!(
        ...
        pushln!(
            &mut out,
            "                route_to_signal(&producers::{producer_mod}_instance_id(), &self.variant_id())"
        );
        pushln!(&mut out, "            }}");
        pushln!(&mut out);
        pushln!(
        ...
        for effect_variant in effect_variants {
            let variant = to_upper_camel_case_local(effect_variant);
            pushln!(&mut out, "                    Self::{variant} => {{");
            if let Some(fields_for_effect) = effect_fields.get(effect_variant) {
                for field in fields_for_effect {
                    let field_variant = to_upper_camel_case_local(field);
                    pushln!(
                        &mut out,
                        "                        if id == &Field::{field_variant}.id() {{ return Some(Field::{field_variant}); }}"
                    );
                }
            }
            pushln!(&mut out, "                        None");
...
fn to_upper_camel_case_local(value: &str) -> String {
    let mut out = String::new();
...
impl<'a> CompositionTlaCompiler<'a> {
    ...
    fn render(&self) -> std::result::Result<String, String> {
        ...
        pushln!(&mut out, "{MACHINE_CODEGEN_HEADER_TLA}");
        pushln!(&mut out);
        writeln!(
            &mut out,
