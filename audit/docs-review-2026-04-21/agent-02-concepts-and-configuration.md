...
2. `docs/concepts/providers.mdx:17-23` has a stale/misleading Anthropic model table. It lists an Opus fallback twice, including one row that reads like the top-tier/current recommendation, while the actual curated catalog marks `claude-opus-4-8` as the recommended Opus row with legacy Opus entries behind it (`meerkat-core/src/model_profile/capabilities/anthropic.rs:86-114`, `meerkat-core/src/model_profile/capabilities/anthropic.rs:127-155`). As written, the table can make readers think the same Opus line is both the highest-quality choice and a fallback at the same time.
