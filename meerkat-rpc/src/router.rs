...
"#,
    ...
    #[tokio::test]
    async fn retired_realtime_rpc_methods_are_not_publicly_callable() {
        let (router, _notif_rx) = test_router().await;
        ...
        for method in [
            ...
            let response = router
                .dispatch(make_request(method, serde_json::json!({})))
                .await
                .unwrap();
            assert_eq!(
                error_code(&response),
                error::METHOD_NOT_FOUND,
                "{method} must stay retired until the new live/* API lands"
            );
        }
    }
