...
The structured output appears in the response alongside the committed
main-turn text. The extraction JSON does not replace `text`.
...
Once provider schema compilation succeeds, event subscribers see
...
`turn_completed` events; their per-request usage rides the outcome event's
`request_usage`. When the final reply already validated, no extraction
request runs: `extraction_succeeded` then carries `"origin": "final_reply"` and
no `request_usage`. A schema-compilation failure emits
