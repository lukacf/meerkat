...
fn auth_lease_durable_marker_contract<'a>(
    compositions: &'a [CompositionSchema],
) -> Result<(
    &'a CompositionSchema,
    &'a EffectHandoffProtocol,
    &'a DurableMarkerProtocol,
)> {
...
const APPROVAL_LIFECYCLE_AUTHORITY_SOURCE: &str = r#"// @generated — approval lifecycle authority for `ApprovalLifecycleMachine`
...
impl ApprovalLifecycleMachineAuthority {
    ...
    fn insert_record(
        ...
        self.state.approval_statuses.insert(approval_id.clone(), status);
        self.state
        ...
        self.state.approval_has_expiry.insert(approval_id, has_expiry);
    }
        ...
        match input {
            ...
            ApprovalLifecycleInput::ObserveApprovalExpiry {
                ...
                let has_expiry = self
                    ...
                    .copied()
                    .unwrap_or(false);
                let resolved = if status == ApprovalLifecycleStatus::Pending
                    && has_expiry
                    && expired
                {
                    self.state
...
fn generate_approval_lifecycle_authority(machine: &MachineSchema) -> Result<String> {
    ...
    Ok(APPROVAL_LIFECYCLE_AUTHORITY_SOURCE.to_owned())
}
