...
`role=system` messages. An API with only a top-level instruction field collects
every System payload in authored System-message order. Provider projection
never changes, rejects, or rewrites the durable transcript based on where a
System row appears.
