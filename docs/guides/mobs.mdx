...
| CLI `rkat mob pack/deploy/web build` | Portable artifact workflows (`.mobpack`, browser bundles) | Distribution + reproducible deployment |
...
- `[profiles.<name>]`: profile model, tools, addressing behavior.
  - Optional `max_inline_peer_notifications`: controls inline peer lifecycle context (`None` default=50, `0` never inline, `-1` always inline, `>0` thresholded by peer count, `< -1` invalid).
...
- Provisioning runs in parallel across concurrent spawn requests; actor-side finalization remains serialized for deterministic event/roster/wiring updates.
- Duplicate `meerkat_id` requests are rejected even while an earlier spawn for the same id is still provisioning.
...
## Portable mob deployment (mobpack)

Mobpacks let you freeze a working mob as a single artifact, then deploy it consistently across environments.
...
### Deploy in strict trust mode

```bash
export RKAT_TRUST_POLICY=strict
rkat mob deploy ./dist/release-triage.mobpack "triage the top 5 open production regressions"
```

### Deploy as RPC service

```bash
rkat mob deploy ./dist/release-triage.mobpack "boot service" --surface rpc
```

## Browser deployment (WASM web build)

`rkat mob web build` compiles a browser-deployable bundle from `.mobpack`.
...
### Cool web pattern 1: In-browser incident war room

1. Build `ops-war-room.mobpack` with lead + triage + comms profiles.
2. Run `rkat mob web build`.
3. Host the output behind your internal auth gateway.
4. Open from a browser during incidents so responders get a zero-install multi-agent workspace.

### Cool web pattern 2: Embedded release assistant in internal dashboard

1. Build `release-assistant.mobpack`.
2. Compile web bundle.
3. Embed bundle route in your release dashboard app.
4. Use it as a context-aware co-pilot for changelog synthesis, rollout checklist generation, and regression triage in the same UI.
...
rkat mob run-flow <mob_id> --flow <flow_id> [--params '{"key":"value"}'] [--stream] [--stream-view <primary|mux|focus>] [--stream-focus <scope_id>]
rkat mob flow-status <mob_id> <run_id>
```

Flow definitions live under `[flows.<flow_id>]` in the mob definition.

Scoped streaming for flow runs:

- `--stream-view primary` (default): show only the run primary member scope.
- `--stream-view mux`: show all member and sub-agent scopes with `[<scope_id>]` prefixes.
- `--stream-view focus --stream-focus <scope_id>`: show one exact scope.
...
```bash
rkat mob run-flow team-mob --flow incident_response --stream --stream-view mux
rkat mob run-flow team-mob --flow incident_response --stream --stream-view focus --stream-focus mob:lead-1
```

Troubleshooting:

- Focus scope not seen: command exits non-zero and prints discovered scopes from the run.
- Too much interleaving in `mux`: narrow output with `focus` on a member or nested sub-agent scope.
