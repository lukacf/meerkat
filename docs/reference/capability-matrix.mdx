...
In `session_error_to_rpc`, `Agent` errors map per typed cause to the canonical
...
family. Where the host can deliver a later completion to the convener (a mob
member on any runtime-backed host, or any session on the JSON-RPC host, which
installs a `DetachedOwnerHost`), the `council` agent tool returns
`status: "running"` with a `job_id`; the structured outcome is recorded once
in the convener's transcript as a durable `BackgroundJob` system notice, which
joins a running convener's turn at its next model call or wakes an idle
convener for one turn. On a one-shot host (`rkat run` without
`--keep-alive`), or for a plain-session convener on a host without that hook
(`blocked_because: "no_owner_revival_host"`), the call blocks and returns it
through the ordinary tool-result channel. `fork_off` follows the same split. Cleanup debt is deliberately not a
tool failure: a sealed result whose cleanup retained debt carries
`cleanup.status = "debt"` or `"pending"`. Seating failures, wiring gaps,
exhausted budgets, and elapsed deadlines remain typed `exit_reason` values on
that result.
...
[dependencies]
meerkat = { version = "=0.8.44", default-features = false, features = ["anthropic"] }

# Add persistence
meerkat = { version = "=0.8.44", default-features = false, features = ["anthropic", "session-store"] }

# Add compaction
meerkat = { version = "=0.8.44", default-features = false, features = ["anthropic", "session-store", "session-compaction"] }

# Add facade-local predicate-schedule helper re-exports. The schedule service
# and WorkGraph substrate are already linked.
meerkat = { version = "=0.8.44", default-features = false, features = ["anthropic", "session-store", "schedule"] }

# Broad native facade with WebSocket live support
meerkat = { version = "=0.8.44", features = [
...
meerkat = { version = "=0.8.44", features = ["openai-realtime", "live-webrtc"] }
```
...
|------------|-------------|-----------|--------|--------|
| `vision` | Model can process image content in user messages | Yes | Yes for chat, Responses, and `gpt-realtime-2`; no for GPT Live rows | Yes |
| `image_tool_results` | Model can process image content in tool results | Yes | Yes for current chat and Responses rows; no for `gpt-realtime-2` or GPT Live rows | Yes |
...
| `realtime` | Model can back a caller-initiated live channel | No | `gpt-realtime-2` (Realtime), `gpt-live-1` (stable public Live), `gpt-live-1-codex` (experimental/private Live) | No current catalog row |
| `supports_web_search` | Model supports provider-native web search tools | Yes | Yes for chat catalog rows; not `gpt-realtime-2`, GPT Live, or Codex | Yes |

Catalog eligibility is not host availability. `gpt-realtime-2` uses the
...
| Anthropic Messages | Distinct top-level blocks for the leading prefix; later `role=system` messages on capable models | Model-gated placement (see below) |
...
- **Failure**: Ordinary recoverable failures preserve uncompacted history and
  normally emit `CompactionFailed`, allowing continuation. A typed provider
  `PolicyStop` or unresolved durable compaction-stage authority/cleanup error
  terminates the run fail-closed. Preserved history can still exceed provider
  capacity, so the next request is not guaranteed to succeed.
...
asynchronous event-audit projection. An audit append failure does not undo an
already-committed turn; unresolved compaction-stage authority must not be
