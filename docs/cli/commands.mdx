...
- `--no-stream`
- `--no-web-search`
- `--export-atif` (write an ATIF trajectory to the realm `trajectories/` directory after the turn, with one metered step per provider request: each tool-call turn, the answer, and each structured-output extraction request; default off)
- `--resume[=<SESSION>]`
...
With `--stream` or `--verbose`, `rkat run` prints one token line per provider
request (`N tokens (I in / O out, C cached, R reasoning)`, extraction requests
labelled `extraction:`) and closes with `total:`, the run's own `run_usage`.
Input counts are presented input, so cached input is included on every
provider. Before exiting, `rkat run` waits for the session event log
(`.rkat/sessions/<id>/events.jsonl` and the realm event store) to finish
writing and warns on stderr if it cannot.
...
The `Config` schema accepts template defaults under `[presentation.html]`:
...
Use the builtin template or a file read directly by the run command:
...
`--html-template-file` bypasses configured-template lookup. HTML output itself
remains available.
...
Interactive login therefore materializes `global` as the optional chain tail,
after which a single sign-in is inherited by workspace realms without a
per-realm re-login. Earlier `dev`-realm logins are
...
use environment-key auth. If `--secret` is omitted, scripted login reads one
line from stdin.
