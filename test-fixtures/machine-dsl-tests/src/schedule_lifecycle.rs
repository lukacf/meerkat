...
mod tests {
    ...
    fn schema_matches_hand_written() {
        let dsl_schema = ScheduleLifecycleMachineState::schema();
        let hand_schema = meerkat_machine_schema::catalog::schedule_lifecycle_machine();
        ...
        for hand_t in &hand_schema.transitions {
            let dsl_t = dsl_schema
                ...
                .unwrap_or_else(|| panic!("DSL missing transition `{}`", hand_t.name));
