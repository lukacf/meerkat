...
- `mob_member_lifecycle_anchor`: `MobMemberLifecycleAnchorMachine` @ actor `mob_member_lifecycle_anchor_actor`
- `mob_runtime_bridge_anchor`: `MobRuntimeBridgeAnchorMachine` @ actor `mob_runtime_bridge_anchor_actor`
- `mob_wiring_anchor`: `MobWiringAnchorMachine` @ actor `mob_wiring_anchor_actor`
- `mob_helper_result_anchor`: `MobHelperResultAnchorMachine` @ actor `mob_helper_result_anchor_actor`
...
- `mob_peer_candidate_tracks_wiring`: `peer_comms`.`SubmitPeerInputCandidate` -> `mob_wiring_anchor`.`PeerInputAdmitted` [Immediate]
- `mob_admitted_work_enters_ingress`: `runtime_control`.`SubmitAdmittedIngressEffect` -> `runtime_ingress`.`AdmitQueued` [Immediate]
- `mob_runtime_admission_tracks_wiring`: `runtime_control`.`SubmitAdmittedIngressEffect` -> `mob_wiring_anchor`.`RuntimeWorkAdmitted` [Immediate]
...
- `mob_runtime_start_tracks_bridge`: `runtime_control`.`SubmitRunPrimitive` -> `mob_runtime_bridge_anchor`.`RuntimeRunSubmitted` [Immediate]
...
- `mob_execution_completion_tracks_bridge`: `turn_execution`.`RunCompleted` -> `mob_runtime_bridge_anchor`.`RuntimeRunCompleted` [Immediate]
- `mob_execution_completion_anchors_helper_result`: `turn_execution`.`RunCompleted` -> `mob_helper_result_anchor`.`AnchorCompleted` [Immediate]
...
- `mob_execution_failure_tracks_bridge`: `turn_execution`.`RunFailed` -> `mob_runtime_bridge_anchor`.`RuntimeRunFailed` [Immediate]
- `mob_execution_failure_anchors_helper_result`: `turn_execution`.`RunFailed` -> `mob_helper_result_anchor`.`AnchorFailed` [Immediate]
...
- `mob_execution_cancel_tracks_bridge`: `turn_execution`.`RunCancelled` -> `mob_runtime_bridge_anchor`.`RuntimeRunCancelled` [Immediate]
- `mob_execution_cancel_anchors_helper_result`: `turn_execution`.`RunCancelled` -> `mob_helper_result_anchor`.`AnchorCancelled` [Immediate]
...
- `mob_member_force_cancel_tracks_bridge`: `mob_orchestrator`.`MemberForceCancelled` -> `mob_runtime_bridge_anchor`.`RuntimeStopRequested` [Immediate]
- `mob_member_force_cancel_anchors_helper_result`: `mob_orchestrator`.`MemberForceCancelled` -> `mob_helper_result_anchor`.`AnchorForceCancelled` [Immediate]
...
- `mob_ops_peer_ready_tracks_member_lifecycle`: `ops_lifecycle`.`ExposeOperationPeer` -> `mob_member_lifecycle_anchor`.`MemberPeerExposed` [Immediate]
...
- `mob_runtime_admission_is_observed_by_wiring_anchor` — mob runtime admission effects are mirrored into the wiring observation anchor
- `mob_runtime_start_is_observed_by_bridge_anchor` — mob run handoff into turn execution is mirrored into the runtime-bridge observation anchor
...
- `mob_execution_failure_is_handled` — mob turn-execution failure is handled by ingress/runtime control and mirrored into runtime-bridge and helper-result observation anchors
- `mob_execution_cancel_is_handled` — mob turn-execution cancellation is handled by ingress/runtime control and mirrored into runtime-bridge and helper-result observation anchors
- `mob_execution_completion_is_handled` — mob turn-execution completion is handled by ingress/runtime control and mirrored into runtime-bridge and helper-result observation anchors
...
- `mob_member_lifecycle_terminal_events_are_observed` — mob member lifecycle terminal events are mirrored from ops lifecycle into the member-lifecycle observation anchor
- `mob_wiring_anchor_tracks_peer_candidates` — mob peer candidate admission is mirrored into the wiring observation anchor through an explicit route
