...
Four tools are exposed to the LLM when comms is enabled. Tool definitions and dispatch are implemented in `meerkat-comms/src/mcp/tools.rs`.
...
| `meerkat-comms/src/mcp/tools.rs` | MCP tool schemas and dispatch |
...
| `meerkat-comms/src/agent/dispatcher.rs` | `CommsToolDispatcher`, `wrap_with_comms()` |
...
The comms crate registers a built-in skill `multi-agent-comms` via `inventory::submit!` (id: `"multi-agent-comms"`, requires: `["comms"]`). It also registers a capability `CapabilityId::Comms` (scope: `Universal`, requires feature: `"comms"`).
