...
Use this guide when you want to understand or operate:

- multi-agent runtime structure
- host-side mob control planes
- agent-side mob tool capabilities
- the difference between collaboration, orchestration, and work lanes
...
Each mob member runs in its own session, so live channels are per-member by construction. Point a profile at a realtime-capable model (for example `gpt-realtime-2`) and call `live/open` against the member's session to start a live channel. See the [Live Channels guide](/guides/realtime) for the full flow.

## See also

- [Live Channels guide](/guides/realtime) — per-member live audio channels
