    ...
    async fn ensure_supervisor_authorized(
        ...
        match reply {
            super::bridge_protocol::BridgeReply::Rejected { cause, reason } => {
                ...
                let error = Self::bridge_typed_rejection_error(cause, reason);
                let rollback = self
                    .supervisor_bridge
                    .rollback_recipient_trust(trust_install)
                    .await;
            ...
            other => {
                let rollback = self
                    ...
                    .await;
                Err(Self::bridge_trust_rollback_error(
                    "authorize supervisor unexpected response",
                    Self::unexpected_bridge_reply(other, "authorize supervisor response"),
                    rollback,
                ))
            }
        }
        ...
        match R::from_bridge_reply(reply) {
            ...
            Err(super::bridge_protocol::BridgeReply::Rejected { cause, reason }) => {
                ...
                Err(Self::bridge_trust_rollback_error(
                    ...
                    Self::bridge_typed_rejection_error(cause, reason),
                    rollback,
                ))
            }
            Err(other) => {
                let rollback = self
                    ...
                    .await;
                Err(Self::bridge_trust_rollback_error(
                    "bridge command unexpected response",
                    Self::unexpected_bridge_reply(other, "bridge command response"),
                    rollback,
                ))
            }
        }
                ...
                let authorize_error = match authorize_result {
                                ...
                                Ok(bind_response) => {
                                    ...
                                    match retry {
                                        Ok(super::bridge_protocol::BridgeReply::Rejected {
                                            ...
                                            let rollback = self
                                                .supervisor_bridge
                                                .rollback_recipient_trust(fallback_trust_install)
                                                .await;
                                            Some(Self::bridge_trust_rollback_error(
                                                "rotate supervisor authorize retry rejected",
                                                Self::bridge_typed_rejection_error(
                                                    retry_cause,
                                                    format!(
                                        ...
                                        Ok(other) => {
                                            let rollback = self
                                                ...
                                                .await;
                                            Some(Self::bridge_trust_rollback_error(
                                                "rotate supervisor authorize retry unexpected response",
                                                Self::unexpected_bridge_reply(
                                                    other,
                    ...
                    Ok(other) => {
                        let rollback = self
                            ...
                            .await;
                        Some(Self::bridge_trust_rollback_error(
                            "rotate supervisor authorize unexpected response",
                            Self::unexpected_bridge_reply(other, "rotate supervisor response"),
                            rollback,
                        ))
                    }
                    Err(error) => {
