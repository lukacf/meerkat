...
CONSTANTS AgentIdentityValues, AgentRuntimeIdValues, BooleanValues, FenceTokenValues, GenerationValues, InputIdValues, NatValues, RunIdValues, SessionIdValues, SetOfStringValues, StringValues, ToolFilterValues, ToolVisibilityWitnessValues, WorkIdValues
...
RECURSIVE meerkat_StagePersistentFilterIdle_ForEach1_filter_witnesses(_, _, _)
meerkat_StagePersistentFilterIdle_ForEach1_filter_witnesses(acc, remaining, outer_witnesses) == IF remaining = {} THEN acc ELSE LET item == CHOOSE x \in remaining : TRUE IN LET name == item IN LET next_acc == MapSet(acc, name, (IF name \in DOMAIN outer_witnesses THEN outer_witnesses[name] ELSE "None")) IN meerkat_StagePersistentFilterIdle_ForEach1_filter_witnesses(next_acc, remaining \ {item}, outer_witnesses)
...
RECURSIVE meerkat_RequestDeferredToolsAttached_ForEach3_staged_requested_deferred_names(_, _)
meerkat_RequestDeferredToolsAttached_ForEach3_staged_requested_deferred_names(acc, remaining) == IF remaining = {} THEN acc ELSE LET item == CHOOSE x \in remaining : TRUE IN LET name == item IN LET next_acc == (acc \cup {name}) IN meerkat_RequestDeferredToolsAttached_ForEach3_staged_requested_deferred_names(next_acc, remaining \ {item})

RECURSIVE meerkat_RequestDeferredToolsAttached_ForEach4_requested_witnesses(_, _, _)
meerkat_RequestDeferredToolsAttached_ForEach4_requested_witnesses(acc, remaining, outer_witnesses) == IF remaining = {} THEN acc ELSE LET item == CHOOSE x \in remaining : TRUE IN LET name == item IN LET next_acc == MapSet(acc, name, (IF name \in DOMAIN outer_witnesses THEN outer_witnesses[name] ELSE "None")) IN meerkat_RequestDeferredToolsAttached_ForEach4_requested_witnesses(next_acc, remaining \ {item}, outer_witnesses)
...
RECURSIVE meerkat_RequestDeferredToolsRunning_ForEach7_staged_requested_deferred_names(_, _)
meerkat_RequestDeferredToolsRunning_ForEach7_staged_requested_deferred_names(acc, remaining) == IF remaining = {} THEN acc ELSE LET item == CHOOSE x \in remaining : TRUE IN LET name == item IN LET next_acc == (acc \cup {name}) IN meerkat_RequestDeferredToolsRunning_ForEach7_staged_requested_deferred_names(next_acc, remaining \ {item})

RECURSIVE meerkat_RequestDeferredToolsRunning_ForEach8_requested_witnesses(_, _, _)
meerkat_RequestDeferredToolsRunning_ForEach8_requested_witnesses(acc, remaining, outer_witnesses) == IF remaining = {} THEN acc ELSE LET item == CHOOSE x \in remaining : TRUE IN LET name == item IN LET next_acc == MapSet(acc, name, (IF name \in DOMAIN outer_witnesses THEN outer_witnesses[name] ELSE "None")) IN meerkat_RequestDeferredToolsRunning_ForEach8_requested_witnesses(next_acc, remaining \ {item}, outer_witnesses)
...
meerkat_StagePersistentFilterAttached(arg_filter, arg_witnesses) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       ...
       /\ UNCHANGED << meerkat_session_id, meerkat_active_runtime_id, meerkat_active_fence_token, meerkat_active_generation, meerkat_active_work_id, meerkat_wake_pending, meerkat_process_pending, meerkat_peer_ingress_configured, meerkat_drain_running, meerkat_inherited_base_filter, meerkat_active_filter, meerkat_active_requested_deferred_names, meerkat_staged_requested_deferred_names, meerkat_requested_witnesses, meerkat_active_visibility_revision, meerkat_committed_visibility_revision, mob_phase, mob_active_identity, mob_active_runtime_id, mob_active_fence_token, mob_current_generation, mob_inflight_work_id, mob_active_member_count, mob_active_run_count, mob_pending_spawn_count, mob_retiring_member_count, mob_wiring_edge_count, mob_task_count, mob_event_subscription_count, mob_active_frame_count, mob_active_loop_count, mob_coordinator_bound, mob_kickoff_pending, witness_current_script_input, witness_remaining_script_inputs >>
       /\ pending_inputs' = SeqRemove(pending_inputs, packet)
       /\ observed_inputs' = observed_inputs
       /\ pending_routes' = pending_routes
       /\ delivered_routes' = delivered_routes
...
meerkat_StagePersistentFilterIdle(arg_filter, arg_witnesses) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       ...
       /\ meerkat_phase' = "Idle"
       /\ meerkat_staged_filter' = packet.payload.filter
       /\ meerkat_filter_witnesses' = meerkat_StagePersistentFilterIdle_ForEach1_filter_witnesses(meerkat_filter_witnesses, DOMAIN packet.payload.witnesses, packet.payload.witnesses)
       /\ meerkat_staged_visibility_revision' = (meerkat_staged_visibility_revision) + 1
       /\ UNCHANGED << meerkat_session_id, meerkat_active_runtime_id, meerkat_active_fence_token, meerkat_active_generation, meerkat_active_work_id, meerkat_wake_pending, meerkat_process_pending, meerkat_peer_ingress_configured, meerkat_drain_running, meerkat_inherited_base_filter, meerkat_active_filter, meerkat_active_requested_deferred_names, meerkat_staged_requested_deferred_names, meerkat_requested_witnesses, meerkat_active_visibility_revision, meerkat_committed_visibility_revision, mob_phase, mob_active_identity, mob_active_runtime_id, mob_active_fence_token, mob_current_generation, mob_inflight_work_id, mob_active_member_count, mob_active_run_count, mob_pending_spawn_count, mob_retiring_member_count, mob_wiring_edge_count, mob_task_count, mob_event_subscription_count, mob_active_frame_count, mob_active_loop_count, mob_coordinator_bound, mob_kickoff_pending, witness_current_script_input, witness_remaining_script_inputs >>
       /\ pending_inputs' = SeqRemove(pending_inputs, packet)
       /\ observed_inputs' = observed_inputs
...
meerkat_RequestDeferredToolsAttached(arg_names, arg_witnesses) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       ...
       /\ (meerkat_session_id # None)
       /\ meerkat_phase' = "Attached"
       /\ meerkat_staged_requested_deferred_names' = meerkat_RequestDeferredToolsAttached_ForEach3_staged_requested_deferred_names(meerkat_staged_requested_deferred_names, packet.payload.names)
       /\ meerkat_requested_witnesses' = meerkat_RequestDeferredToolsAttached_ForEach4_requested_witnesses(meerkat_requested_witnesses, DOMAIN packet.payload.witnesses, packet.payload.witnesses)
       /\ meerkat_staged_visibility_revision' = (meerkat_staged_visibility_revision) + 1
       /\ UNCHANGED << meerkat_session_id, meerkat_active_runtime_id, meerkat_active_fence_token, meerkat_active_generation, meerkat_active_work_id, meerkat_wake_pending, meerkat_process_pending, meerkat_peer_ingress_configured, meerkat_drain_running, meerkat_inherited_base_filter, meerkat_active_filter, meerkat_staged_filter, meerkat_active_requested_deferred_names, meerkat_filter_witnesses, meerkat_active_visibility_revision, meerkat_committed_visibility_revision, mob_phase, mob_active_identity, mob_active_runtime_id, mob_active_fence_token, mob_current_generation, mob_inflight_work_id, mob_active_member_count, mob_active_run_count, mob_pending_spawn_count, mob_retiring_member_count, mob_wiring_edge_count, mob_task_count, mob_event_subscription_count, mob_active_frame_count, mob_active_loop_count, mob_coordinator_bound, mob_kickoff_pending, witness_current_script_input, witness_remaining_script_inputs >>
       /\ pending_inputs' = SeqRemove(pending_inputs, packet)
       /\ observed_inputs' = observed_inputs
       /\ pending_routes' = pending_routes
       /\ delivered_routes' = delivered_routes
...
meerkat_RequestDeferredToolsRunning(arg_names, arg_witnesses) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       ...
       /\ (meerkat_session_id # None)
       /\ meerkat_phase' = "Running"
       /\ meerkat_staged_requested_deferred_names' = meerkat_RequestDeferredToolsRunning_ForEach7_staged_requested_deferred_names(meerkat_staged_requested_deferred_names, packet.payload.names)
       /\ meerkat_requested_witnesses' = meerkat_RequestDeferredToolsRunning_ForEach8_requested_witnesses(meerkat_requested_witnesses, DOMAIN packet.payload.witnesses, packet.payload.witnesses)
       /\ meerkat_staged_visibility_revision' = (meerkat_staged_visibility_revision) + 1
       /\ UNCHANGED << meerkat_session_id, meerkat_active_runtime_id, meerkat_active_fence_token, meerkat_active_generation, meerkat_active_work_id, meerkat_wake_pending, meerkat_process_pending, meerkat_peer_ingress_configured, meerkat_drain_running, meerkat_inherited_base_filter, meerkat_active_filter, meerkat_staged_filter, meerkat_active_requested_deferred_names, meerkat_filter_witnesses, meerkat_active_visibility_revision, meerkat_committed_visibility_revision, mob_phase, mob_active_identity, mob_active_runtime_id, mob_active_fence_token, mob_current_generation, mob_inflight_work_id, mob_active_member_count, mob_active_run_count, mob_pending_spawn_count, mob_retiring_member_count, mob_wiring_edge_count, mob_task_count, mob_event_subscription_count, mob_active_frame_count, mob_active_loop_count, mob_coordinator_bound, mob_kickoff_pending, witness_current_script_input, witness_remaining_script_inputs >>
       /\ pending_inputs' = SeqRemove(pending_inputs, packet)
       /\ observed_inputs' = observed_inputs
...
meerkat_PrepareBindingsIdle(arg_agent_runtime_id, arg_fence_token, arg_generation) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       ...
       /\ observed_transitions' = observed_transitions \cup {[machine |-> "meerkat", transition |-> "PrepareBindingsIdle", actor |-> "meerkat_kernel", step |-> (model_step_count + 1), from_phase |-> meerkat_phase, to_phase |-> "Idle"]}
       /\ model_step_count' = model_step_count + 1
...
meerkat_PrepareBindingsRecovering(arg_agent_runtime_id, arg_fence_token, arg_generation) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       /\ packet.machine = "meerkat"
       /\ packet.variant = "PrepareBindings"
       /\ packet.payload.agent_runtime_id = arg_agent_runtime_id
       ...
       /\ observed_inputs' = observed_inputs \cup {[machine |-> "mob", variant |-> "ObserveRuntimeReady", payload |-> [agent_runtime_id |-> (IF "value" \in DOMAIN Some(packet.payload.agent_runtime_id) THEN Some(packet.payload.agent_runtime_id)["value"] ELSE None), fence_token |-> (IF "value" \in DOMAIN Some(packet.payload.fence_token) THEN Some(packet.payload.fence_token)["value"] ELSE None)], source_kind |-> "route", source_route |-> "runtime_bound_reaches_mob", source_machine |-> "meerkat", source_effect |-> "RuntimeBound", effect_id |-> (model_step_count + 1)]}
       /\ pending_routes' = pending_routes
       /\ delivered_routes' = delivered_routes \cup { [route |-> "runtime_bound_reaches_mob", source_machine |-> "meerkat", effect |-> "RuntimeBound", target_machine |-> "mob", target_input |-> "ObserveRuntimeReady", payload |-> [agent_runtime_id |-> (IF "value" \in DOMAIN Some(packet.payload.agent_runtime_id) THEN Some(packet.payload.agent_runtime_id)["value"] ELSE None), fence_token |-> (IF "value" \in DOMAIN Some(packet.payload.fence_token) THEN Some(packet.payload.fence_token)["value"] ELSE None)], actor |-> "mob_kernel", effect_id |-> (model_step_count + 1), source_transition |-> "PrepareBindingsRecovering"] }
       /\ emitted_effects' = emitted_effects \cup { [machine |-> "meerkat", variant |-> "RuntimeBound", payload |-> [agent_runtime_id |-> (IF "value" \in DOMAIN Some(packet.payload.agent_runtime_id) THEN Some(packet.payload.agent_runtime_id)["value"] ELSE None), fence_token |-> (IF "value" \in DOMAIN Some(packet.payload.fence_token) THEN Some(packet.payload.fence_token)["value"] ELSE None), generation |-> (IF "value" \in DOMAIN Some(packet.payload.generation) THEN Some(packet.payload.generation)["value"] ELSE None)], effect_id |-> (model_step_count + 1), source_transition |-> "PrepareBindingsRecovering"] }
       /\ observed_transitions' = observed_transitions \cup {[machine |-> "meerkat", transition |-> "PrepareBindingsRecovering", actor |-> "meerkat_kernel", step |-> (model_step_count + 1), from_phase |-> meerkat_phase, to_phase |-> "Recovering"]}
       /\ model_step_count' = model_step_count + 1
...
meerkat_SetPeerIngressContextIdle(arg_keep_alive) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       ...
       /\ UNCHANGED << meerkat_session_id, meerkat_active_runtime_id, meerkat_active_fence_token, meerkat_active_generation, meerkat_active_work_id, meerkat_wake_pending, meerkat_process_pending, meerkat_inherited_base_filter, meerkat_active_filter, meerkat_staged_filter, meerkat_active_requested_deferred_names, meerkat_staged_requested_deferred_names, meerkat_requested_witnesses, meerkat_filter_witnesses, meerkat_active_visibility_revision, meerkat_staged_visibility_revision, meerkat_committed_visibility_revision, mob_phase, mob_active_identity, mob_active_runtime_id, mob_active_fence_token, mob_current_generation, mob_inflight_work_id, mob_active_member_count, mob_active_run_count, mob_pending_spawn_count, mob_retiring_member_count, mob_wiring_edge_count, mob_task_count, mob_event_subscription_count, mob_active_frame_count, mob_active_loop_count, mob_coordinator_bound, mob_kickoff_pending, witness_current_script_input, witness_remaining_script_inputs >>
       /\ pending_inputs' = SeqRemove(pending_inputs, packet)
       /\ observed_inputs' = observed_inputs
       /\ pending_routes' = pending_routes
       /\ delivered_routes' = delivered_routes
...
meerkat_SetPeerIngressContextRecovering(arg_keep_alive) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       /\ packet.machine = "meerkat"
       /\ packet.variant = "SetPeerIngressContext"
       /\ packet.payload.keep_alive = arg_keep_alive
       ...
       /\ delivered_routes' = delivered_routes
       /\ emitted_effects' = emitted_effects
       /\ observed_transitions' = observed_transitions \cup {[machine |-> "meerkat", transition |-> "SetPeerIngressContextRecovering", actor |-> "meerkat_kernel", step |-> (model_step_count + 1), from_phase |-> meerkat_phase, to_phase |-> "Recovering"]}
       /\ model_step_count' = model_step_count + 1
...
meerkat_NotifyDrainExitedRecovering(arg_reason) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       /\ packet.machine = "meerkat"
       /\ packet.variant = "NotifyDrainExited"
       /\ packet.payload.reason = arg_reason
       ...
       /\ pending_routes' = pending_routes
       /\ delivered_routes' = delivered_routes
       /\ emitted_effects' = emitted_effects \cup { [machine |-> "meerkat", variant |-> "RuntimeNotice", payload |-> [detail |-> "drain exited", kind |-> "drain"], effect_id |-> (model_step_count + 1), source_transition |-> "NotifyDrainExitedRecovering"] }
       /\ observed_transitions' = observed_transitions \cup {[machine |-> "meerkat", transition |-> "NotifyDrainExitedRecovering", actor |-> "meerkat_kernel", step |-> (model_step_count + 1), from_phase |-> meerkat_phase, to_phase |-> "Recovering"]}
       /\ model_step_count' = model_step_count + 1
...
meerkat_BoundaryAppliedPromote(arg_revision) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       ...
       /\ UNCHANGED << meerkat_session_id, meerkat_active_runtime_id, meerkat_active_fence_token, meerkat_active_generation, meerkat_active_work_id, meerkat_wake_pending, meerkat_process_pending, meerkat_peer_ingress_configured, meerkat_drain_running, meerkat_inherited_base_filter, meerkat_staged_filter, meerkat_staged_requested_deferred_names, meerkat_requested_witnesses, meerkat_filter_witnesses, meerkat_staged_visibility_revision, mob_phase, mob_active_identity, mob_active_runtime_id, mob_active_fence_token, mob_current_generation, mob_inflight_work_id, mob_active_member_count, mob_active_run_count, mob_pending_spawn_count, mob_retiring_member_count, mob_wiring_edge_count, mob_task_count, mob_event_subscription_count, mob_active_frame_count, mob_active_loop_count, mob_coordinator_bound, mob_kickoff_pending, witness_current_script_input, witness_remaining_script_inputs >>
       /\ pending_inputs' = SeqRemove(pending_inputs, packet)
       /\ observed_inputs' = observed_inputs
       /\ pending_routes' = pending_routes
       /\ delivered_routes' = delivered_routes
...
meerkat_PublishCommittedVisibleSetIdle(arg_revision) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       /\ packet.machine = "meerkat"
       ...
       /\ pending_routes' = pending_routes
       /\ delivered_routes' = delivered_routes
       /\ emitted_effects' = emitted_effects \cup { [machine |-> "meerkat", variant |-> "CommittedVisibleSetPublished", payload |-> [revision |-> packet.payload.revision], effect_id |-> (model_step_count + 1), source_transition |-> "PublishCommittedVisibleSetIdle"] }
...
meerkat_RecoverFromRecovering ==
    /\ \E packet \in SeqElements(pending_inputs) :
       ...
       /\ pending_routes' = pending_routes
       /\ delivered_routes' = delivered_routes
       /\ emitted_effects' = emitted_effects \cup { [machine |-> "meerkat", variant |-> "RuntimeNotice", payload |-> [detail |-> "runtime recovering", kind |-> "recover"], effect_id |-> (model_step_count + 1), source_transition |-> "RecoverFromRecovering"] }
       /\ observed_transitions' = observed_transitions \cup {[machine |-> "meerkat", transition |-> "RecoverFromRecovering", actor |-> "meerkat_kernel", step |-> (model_step_count + 1), from_phase |-> meerkat_phase, to_phase |-> "Recovering"]}
       /\ model_step_count' = model_step_count + 1
...
meerkat_Destroy ==
    /\ \E packet \in SeqElements(pending_inputs) :
       ...
       /\ meerkat_process_pending' = FALSE
       /\ meerkat_drain_running' = FALSE
       /\ UNCHANGED << meerkat_session_id, meerkat_active_runtime_id, meerkat_active_fence_token, meerkat_active_generation, meerkat_peer_ingress_configured, meerkat_inherited_base_filter, meerkat_active_filter, meerkat_staged_filter, meerkat_active_requested_deferred_names, meerkat_staged_requested_deferred_names, meerkat_requested_witnesses, meerkat_filter_witnesses, meerkat_active_visibility_revision, meerkat_staged_visibility_revision, meerkat_committed_visibility_revision, mob_phase, mob_active_identity, mob_active_runtime_id, mob_active_fence_token, mob_current_generation, mob_inflight_work_id, mob_active_member_count, mob_active_run_count, mob_pending_spawn_count, mob_retiring_member_count, mob_wiring_edge_count, mob_task_count, mob_event_subscription_count, mob_active_frame_count, mob_active_loop_count, mob_coordinator_bound, mob_kickoff_pending, witness_current_script_input, witness_remaining_script_inputs >>
...
meerkat_RecycleFromIdleOrRetired ==
    /\ \E packet \in SeqElements(pending_inputs) :
       ...
       /\ meerkat_peer_ingress_configured' = FALSE
       /\ meerkat_drain_running' = FALSE
       /\ UNCHANGED << meerkat_session_id, meerkat_inherited_base_filter, meerkat_active_filter, meerkat_staged_filter, meerkat_active_requested_deferred_names, meerkat_staged_requested_deferred_names, meerkat_requested_witnesses, meerkat_filter_witnesses, meerkat_active_visibility_revision, meerkat_staged_visibility_revision, meerkat_committed_visibility_revision, mob_phase, mob_active_identity, mob_active_runtime_id, mob_active_fence_token, mob_current_generation, mob_inflight_work_id, mob_active_member_count, mob_active_run_count, mob_pending_spawn_count, mob_retiring_member_count, mob_wiring_edge_count, mob_task_count, mob_event_subscription_count, mob_active_frame_count, mob_active_loop_count, mob_coordinator_bound, mob_kickoff_pending, witness_current_script_input, witness_remaining_script_inputs >>
       /\ pending_inputs' = SeqRemove(pending_inputs, packet)
       /\ observed_inputs' = observed_inputs
       /\ pending_routes' = pending_routes
       /\ delivered_routes' = delivered_routes
...
mob_SpawnCreating(arg_agent_identity, arg_agent_runtime_id, arg_fence_token, arg_generation) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       /\ packet.machine = "mob"
       ...
       /\ pending_routes' = pending_routes
       /\ delivered_routes' = delivered_routes \cup { [route |-> "binding_request_reaches_meerkat", source_machine |-> "mob", effect |-> "RequestRuntimeBinding", target_machine |-> "meerkat", target_input |-> "PrepareBindings", payload |-> [agent_runtime_id |-> (IF "value" \in DOMAIN Some(packet.payload.agent_runtime_id) THEN Some(packet.payload.agent_runtime_id)["value"] ELSE None), fence_token |-> (IF "value" \in DOMAIN Some(packet.payload.fence_token) THEN Some(packet.payload.fence_token)["value"] ELSE None), generation |-> (IF "value" \in DOMAIN Some(packet.payload.generation) THEN Some(packet.payload.generation)["value"] ELSE None)], actor |-> "meerkat_kernel", effect_id |-> (model_step_count + 1), source_transition |-> "SpawnCreating"] }
       /\ emitted_effects' = emitted_effects \cup { [machine |-> "mob", variant |-> "RequestRuntimeBinding", payload |-> [agent_identity |-> (IF "value" \in DOMAIN Some(packet.payload.agent_identity) THEN Some(packet.payload.agent_identity)["value"] ELSE None), agent_runtime_id |-> (IF "value" \in DOMAIN Some(packet.payload.agent_runtime_id) THEN Some(packet.payload.agent_runtime_id)["value"] ELSE None), fence_token |-> (IF "value" \in DOMAIN Some(packet.payload.fence_token) THEN Some(packet.payload.fence_token)["value"] ELSE None), generation |-> (IF "value" \in DOMAIN Some(packet.payload.generation) THEN Some(packet.payload.generation)["value"] ELSE None)], effect_id |-> (model_step_count + 1), source_transition |-> "SpawnCreating"], [machine |-> "mob", variant |-> "EmitMemberLifecycleNotice", payload |-> [agent_identity |-> (IF "value" \in DOMAIN Some(packet.payload.agent_identity) THEN Some(packet.payload.agent_identity)["value"] ELSE None), kind |-> "spawned"], effect_id |-> (model_step_count + 1), source_transition |-> "SpawnCreating"] }
       /\ observed_transitions' = observed_transitions \cup {[machine |-> "mob", transition |-> "SpawnCreating", actor |-> "mob_kernel", step |-> (model_step_count + 1), from_phase |-> mob_phase, to_phase |-> "Creating"]}
       /\ model_step_count' = model_step_count + 1
...
mob_SubmitWorkCreating(arg_agent_runtime_id, arg_fence_token, arg_work_id) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       /\ packet.machine = "mob"
       ...
       /\ delivered_routes' = delivered_routes
       /\ emitted_effects' = emitted_effects
       /\ observed_transitions' = observed_transitions \cup {[machine |-> "mob", transition |-> "SubmitWorkCreating", actor |-> "mob_kernel", step |-> (model_step_count + 1), from_phase |-> mob_phase, to_phase |-> "Creating"]}
       /\ model_step_count' = model_step_count + 1
...
mob_CancelWorkCreating(arg_work_id) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       /\ packet.machine = "mob"
       ...
       /\ pending_routes' = pending_routes
       /\ delivered_routes' = delivered_routes
       /\ emitted_effects' = emitted_effects \cup { [machine |-> "mob", variant |-> "FlowTerminalized", payload |-> [tag |-> "unit"], effect_id |-> (model_step_count + 1), source_transition |-> "CancelWorkCreating"] }
       /\ observed_transitions' = observed_transitions \cup {[machine |-> "mob", transition |-> "CancelWorkCreating", actor |-> "mob_kernel", step |-> (model_step_count + 1), from_phase |-> mob_phase, to_phase |-> "Creating"]}
       /\ model_step_count' = model_step_count + 1
...
CoreNext ==
    ...
    \/ \E arg_agent_runtime_id \in AgentRuntimeIdValues : \E arg_fence_token \in FenceTokenValues : \E arg_generation \in GenerationValues : meerkat_PrepareBindingsRecovering(arg_agent_runtime_id, arg_fence_token, arg_generation)
    ...
    \/ \E arg_keep_alive \in BOOLEAN : meerkat_SetPeerIngressContextRecovering(arg_keep_alive)
    ...
    \/ \E arg_reason \in StringValues : meerkat_NotifyDrainExitedRecovering(arg_reason)
    ...
    \/ \E arg_revision \in 0..2 : meerkat_PublishCommittedVisibleSetIdle(arg_revision)
    ...
    \/ meerkat_RecycleFromAttached
    \/ mob_Start
    \/ \E arg_agent_identity \in AgentIdentityValues : \E arg_agent_runtime_id \in AgentRuntimeIdValues : \E arg_fence_token \in FenceTokenValues : \E arg_generation \in GenerationValues : mob_SpawnCreating(arg_agent_identity, arg_agent_runtime_id, arg_fence_token, arg_generation)
    ...
    \/ \E arg_agent_runtime_id \in AgentRuntimeIdValues : \E arg_fence_token \in FenceTokenValues : \E arg_work_id \in WorkIdValues : mob_SubmitWorkCreating(arg_agent_runtime_id, arg_fence_token, arg_work_id)
    ...
    \/ \E arg_work_id \in WorkIdValues : mob_CancelWorkCreating(arg_work_id)
