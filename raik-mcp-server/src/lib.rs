//! raik-mcp-server - MCP server exposing RAIK as a tool
//!
//! This crate provides an MCP server that exposes RAIK agent capabilities
//! as MCP tools: raik_run and raik_resume.
...
/// Input schema for raik_run tool
...
/// Input schema for raik_resume tool
#[derive(Debug, Deserialize)]
...
pub fn tools_list() -> Vec<Value> {
    vec![
        json!({
            "name": "raik_run",
            "description": "Run a new RAIK agent with the given prompt. Returns the agent's response.",
            "inputSchema": {
                "type": "object",
                "properties": {
                    "prompt": {
        ...
        json!({
            "name": "raik_resume",
            "description": "Resume an existing RAIK session with a new prompt.",
            "inputSchema": {
                "type": "object",
                "properties": {
                    "session_id": {
...
pub async fn handle_tools_call(tool_name: &str, arguments: &Value) -> Result<Value, String> {
    match tool_name {
        "raik_run" => {
            ...
            handle_raik_run(input).await
        }
        "raik_resume" => {
            let input: RaikResumeInput = serde_json::from_value(arguments.clone())
                .map_err(|e| format!("Invalid arguments: {}", e))?;
            handle_raik_resume(input).await
        }
        _ => Err(format!("Unknown tool: {}", tool_name)),
    }
}

async fn handle_raik_run(input: RaikRunInput) -> Result<Value, String> {
...
async fn handle_raik_resume(input: RaikResumeInput) -> Result<Value, String> {
    // Get API key from environment
    let api_key = std::env::var("ANTHROPIC_API_KEY")
        .map_err(|_| "ANTHROPIC_API_KEY environment variable not set")?;
    ...
    let session_id = raik::SessionId::parse(&input.session_id)
...
use raik::{AgentLlmClient, AgentSessionStore, AgentToolDispatcher, LlmStreamResult, Message, ToolDef};
...
mod tests {
    ...
    #[test]
    fn test_raik_resume_input_parsing() {
        let input_json = json!({
