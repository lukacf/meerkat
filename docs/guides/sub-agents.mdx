...
<Accordion title="SubAgentConfig">
...
5. Communication uses in-process (`CommsBootstrap::for_child_inproc`) with the parent's realm namespace, so child/parent comms stay isolated from other realms in the same process.
...
- [Built-in tools reference](/reference/builtin-tools) - tool parameter details
- [Configuration: sub-agents](/concepts/configuration) - config file settings
- [Architecture](/reference/architecture) - how sub-agents fit into the agent loop
