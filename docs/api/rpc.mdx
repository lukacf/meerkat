...
The RPC surface uses the canonical split between session lifecycle and
runtime-owned admission:

- `session/create`, `turn/start`, `session/read`, `session/list`, and `session/archive` route through the shared `SessionService`
- `keep_alive`, `session/external_event`, and `runtime/*` methods use the runtime-backed ingress/control plane
- deferred first-turn rebuilds come from canonical persisted session state, not surface-local staging
