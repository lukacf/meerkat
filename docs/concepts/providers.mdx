...
- Anthropic, Gemini, the private ChatGPT Responses backend, and OpenAI
  Realtime expose top-level-only instruction channels. Their adapters preserve
  every instruction's exact bytes and transcript order, but cannot preserve
  its position relative to user turns. They use distinct provider blocks or
  parts where supported and `"\n\n"` as the stable separator for singular
  string fields.
...
Realtime reconnect and refresh use the same transcript-derived projection;
there is no provider-only `runtime_system_context` sidecar.
