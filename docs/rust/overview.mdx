...
<Steps>
  <Step title="Add the dependency">
    ```toml
    [dependencies]
    meerkat = { version = "0.6.0", features = ["sqlite-store", "session-store"] }
    meerkat-core = "0.6.0"
    meerkat-store = "0.6.0"
...
The lower-level example below uses a persistent `SessionService` directly. Hosts
that need Queue/Steer routing, live topology management, or comms drain should
wrap the same service with the runtime-backed surface.
