...
If a session was started on a non-realtime model and later needs realtime, send the next turn with `turn_metadata.model` and, when needed, `turn_metadata.provider` / `turn_metadata.provider_params`. JSON-RPC `turn/start`, REST continue, and host SDK turn helpers all route that canonical metadata through the runtime; the runtime then orchestrates the swap via `MeerkatMachine::reconfigure_live_topology` (see [Live-topology reconfigure](#live-topology-reconfigure) below).
...
- **No standalone reconfigure method.** Remote callers request a mid-session swap on the next turn through `turn_metadata.model` / `turn_metadata.provider`; host code can still call `reconfigure_live_topology` via `SessionLlmReconfigureHost` for internal orchestration.
