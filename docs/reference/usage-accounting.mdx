...
and add no row. Legacy sessions saved before 0.8.22 summed raw cache counters;
their reported totals are clamped so the subset invariant holds. On a fresh session
