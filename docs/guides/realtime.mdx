...
async with MeerkatClient() as client:
    ...
    while True:
        status = await client.session_realtime_attachment_status(session.id)
...
Every member spawned under that profile is initialized on `gpt-realtime-1.5` and its session gets a realtime binding automatically. Use `session/realtime_attachment_statuses` with the member session IDs to observe attachment state across the whole mob in one round trip.
...
- [JSON-RPC API](/api/rpc) — `session/create`, `session/realtime_attachment_status`, `session/realtime_attachment_statuses`, `realtime/*` bootstrap
