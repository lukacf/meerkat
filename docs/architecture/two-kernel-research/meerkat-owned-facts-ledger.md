...
| --- | --- | --- | --- | --- | --- | --- | --- |
| Control and Binding | Registered runtime binding | `RuntimeSessionAdapter.sessions`; `RuntimeSessionEntry.driver`; `RuntimeSessionEntry.completions`; `RuntimeSessionEntry.attachment` | Control and Binding | `binding.session_id`, `binding.runtime_id`, `binding.driver_present`, `binding.completions_present`, `binding.attachment_live` | session registration plus live entry publication in `RuntimeSessionAdapter` | service helper lookups, ready/not-ready views | split publication; recovery drift |
...
| Drain | Comms drain lifecycle | `CommsDrainLifecycleMachine.phase`; `CommsDrainLifecycleMachine.mode`; `RuntimeSessionAdapter.comms_drain_slots` | Drain and Keep-Alive | `drain.phase`, `drain.mode` | drain slot lifecycle authority | keep-alive status views | wake/drain race |
