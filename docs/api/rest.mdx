...
<ParamField body="max_tokens" type="u32 | null" default="from session">
  Build-only deferred materialization override. Honored only when this request runs a deferred session's first turn; it is not runtime metadata.
</ParamField>
...
<ParamField body="output_schema" type="OutputSchema | null" default="from session">
  Build-only deferred materialization override. Honored only when this request runs a deferred session's first turn; it is not runtime metadata.
</ParamField>
...
curl -X POST http://127.0.0.1:8080/sessions \
  -H 'content-type: application/json' \
  -d '{"prompt":"Ready for realtime interaction.","turn_metadata":{"model":"gpt-realtime-1.5","provider":"openai"}}'
```
...
<Note>
Keep-alive mode requires `turn_metadata.keep_alive` with `action: "set"` and a
`comms_name`. If keep-alive is requested but the binary was not compiled with
comms support, the server returns a `BAD_REQUEST` error.
</Note>
