...
| Anthropic Messages | All System rows as top-level system blocks | Authored System-row order |
| Gemini `generateContent` | All System rows as `systemInstruction.parts` | Authored System-row order |
| Private ChatGPT Responses | All System rows as one instruction string | Authored System-row order |
| OpenAI Realtime | All System rows as one session instruction string | Authored System-row order |

Top-level-only adapters collect every `System` payload without requiring a
leading prefix. They preserve exact bytes, authored System-message order,
empty and whitespace-only messages, and duplicates. Distinct provider blocks
or parts remain distinct; singular string channels use `"\n\n"` as their
stable separator. System placement is never a session-admission or
provider-rebind restriction.
...
instruction projection only from all canonical `System` rows in authored
System-message order. `SystemNotice` remains an explicit in-place history
event.
