...
- `meerkat-contracts/src/wire/runtime.rs` — no `RealtimeAttachmentStatuses` batch result. Grep for `realtime_attachment_statuses` returned zero. Wave-a deleted the batch on most paths; `meerkat-rpc/src/handlers/runtime.rs:3` only mentions it in a module comment. **Superseded by LUC-507:** the singular `session/realtime_attachment_status` surface is also retired; live transport status must not become Meerkat semantic status.
