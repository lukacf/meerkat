...
Temporary-council failures reuse the same canonical codes rather than minting
new ones, and each carries a typed `data` payload:

| Condition | `ErrorCode` | JSON-RPC | `data` |
|-----------|-------------|----------|--------|
| Request refused before any side effect | `InvalidParams` | -32602 | `{kind: "invalid_request", detail}` |
| Council id bound to a different request | `DuplicateInput` | -32004 | `{council_id, stored_fingerprint, presented_fingerprint}` |
| Another coordinator holds a live claim | `SessionBusy` | -32002 | `{council_id, current_claim_epoch}` |
| This executor's claim was superseded | `StaleFence` | -32028 | `{council_id, current_claim_epoch}` |
| `durable` declared against process-bound custody | `CapabilityUnavailable` | -32020 | `{council_id, required, available}` |
| Custody, lifecycle, mob, or coordinator-task fault | `InternalError` | -32603 | `{kind, detail}` |

Cleanup debt is deliberately not an error: a sealed result whose cleanup
retained debt is a success carrying `cleanup.status = "debt"` or `"pending"`.
Seating failures, wiring gaps, exhausted budgets, and elapsed deadlines are
typed `exit_reason` values on a sealed result for the same reason.
...
- **CLI**: General commands use 0 for success, 1 for error, and 2 for graceful budget exhaustion. Multi-host mob commands additionally preserve the typed operator failures: 45 scope denied, 46 host unavailable, 47 stale cursor, and 48 stale fence. Error details are printed to stderr. Temporary-council commands project their own typed failures through the same canonical `ErrorCode` mapping: 2 invalid request, 13 conflicting request, 11 contested claim, 40 durability unavailable, 48 fenced executor, and 1 for a coordinator-side fault.
...
| Temporary councils | Run, read, recover | Run, read, recover | Run, read, recover | Run and read only |
...
The temporary-council recovery sweep is owner-console only. It seals typed
terminals, revokes capabilities, and destroys temporary mobs for councils the
caller never started, so it is withheld from the LLM-reachable MCP tool surface
for the same reason host binding and grant administration are. Exact run
admission may recover only its requested council id, so no LLM-facing surface
needs realm-wide recovery authority to make progress. Agent-facing delegation
tools expose no council verb at all: council
participation mints capabilities over members of arbitrary mobs, which is a
principal-lane decision.
