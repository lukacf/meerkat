...
impl ScheduleService {
    ...
    pub async fn update(
        ...
        if mutator.revision_bumped {
            self.supersede_pending_future_occurrences(
                &mutator.schedule,
                store_now,
    ...
    pub async fn delete(&self, schedule_id: &ScheduleId) -> Result<Schedule, ScheduleDomainError> {
        ...
        self.supersede_pending_future_occurrences(
            &deleted,
            self.store.get_store_time_utc().await?,
            None,
        )
        .await?;
        Ok(deleted)
    ...
    async fn supersede_pending_future_occurrences(
        &self,
        schedule: &Schedule,
        store_now_utc: chrono::DateTime<Utc>,
        ...
        let pending = self
            ...
            .list_occurrences(OccurrenceFilter {
                ...
                include_terminal: false,
                phase: Some(OccurrencePhase::Pending),
                due_after_utc: Some(store_now_utc),
                ..OccurrenceFilter::default()
            })
            .await?;
