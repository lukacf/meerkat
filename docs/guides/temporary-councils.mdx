...
- **Detached (hosts that outlive the call).** The JSON-RPC, REST, and MCP
  ...
  with the same record, which wakes the convener. The job **fails** instead
  when the exit reason is a failure (`participant_seating_failed`,
  ...
  partial result. The System message survives later model calls, turns, and
  reloads and is readable through the convener's session history; the job
  notice is transient.
...
deadline still delivers its result.
