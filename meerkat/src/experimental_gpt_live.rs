...
pub enum PublicGptLivePlaybackPolicy {
    ...
    /// Retain assistant speech in the document observation component, not as
    /// canonical played assistant rows. Release each segment without waiting
    /// for a browser playback ACK; model projection labels its uncertainty.
            ...
            if matches!(exit, ExitKind::ProviderManaged) {
                ...
                for group in 0..4 {
                    ...
                    for segment in 1..=2 {
                        ...
                        assert!(receipt.observed_after_message_count.is_some());
                        assert_eq!(
                    ...
                    assert!(context.contains("UNMEASURED"));
                }
        ...
        async fn summarize(
            ...
            if self.require_unmeasured_history && call > 0 {
                let observation = snapshot
                    ...
                    .iter()
                    .find_map(|message| {
                        if let meerkat_core::Message::User(user) = message
    ...
    async fn run_context_and_result_recovery_with_history(
        ...
        if let Some(case @ (SummaryTestCase::Changed | SummaryTestCase::Failed)) = summary_case {
            ...
            match case {
                ...
                SummaryTestCase::Normal => unreachable!(),
            }
            assert!(
                runtime
