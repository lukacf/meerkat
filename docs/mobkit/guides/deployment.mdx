...
live WebSocket transport. `mobkit_gateway` serves its protected console
surfaces **open by default when top-level `auth_config` is absent**. Supplying
that init parameter installs the shared JWT/OIDC configuration and requires
app auth on those surfaces. `rpc_gateway` is fail-closed by default: it trusts
no signing key and refuses protected console requests until the host passes
`runtime_options.auth_config` or explicitly opts out with
`runtime_options.console_require_app_auth = false`. Without gateway auth,
loopback or an authenticating front proxy must provide the access boundary.
See [Authentication](/mobkit/guides/authentication) for the shared auth shape.
...
|-------|-------|------------|
| `http_base_url` | The same-host form, preserving the bound port and address family: `0.0.0.0:PORT` becomes `http://127.0.0.1:PORT`; `[::]:PORT` becomes `http://[::1]:PORT`. Concrete bound addresses are unchanged | The SDK's own SSE, multipart and console RPC calls; same-host proxies |
...
The shared gate refuses a non-loopback `http_listen` at init, before any runtime
is bootstrapped, unless one of two things is true in its admission-time state:
...
`rpc_gateway` applies its `runtime_options.auth_config` before this check.
`mobkit_gateway` accepts the same JWT/OIDC shape as top-level `auth_config`,
but its earlier exposure gate currently uses the open default decision state,
not the later auth overlay used by the HTTP router. On that binary a
non-loopback bind therefore still requires `allow_remote` (init param,
`--allow-remote`, or `MOBKIT_HTTP_ALLOW_REMOTE=1`), even when auth is configured.
On both binaries a refused bind answers `mobkit/init` with a `-32602` error on
the request id whose message names the address and every way out.
...
At serving startup, every admitted non-loopback listener logs one WARN line on
...
open decision state used by its earlier exposure gate. It cannot be silenced
below WARN by the default filter.

## Reading schedule watchdog diagnostics

Both gateways install a resident watchdog that probes every 60 seconds when
persistent schedule-service inputs are available. Ephemeral launches and
launches whose schedule store is unavailable do not install it. An immediate
...
## Building with `openai-live`: link the system Opus

The `openai-live` feature pulls Meerkat's WebRTC transport, whose `opus`
dependency builds bundled Opus with CMake when `pkg-config` finds no system
library. Install the system library in the build stage so the build links it
and never runs CMake (which also avoids the CMake 4 incompatibility of the
bundled sources):
...
# runtime stage (dynamic link)
RUN apt-get update \
 && apt-get install -y --no-install-recommends libopus0 \
 && rm -rf /var/lib/apt/lists/*
```

For a static link with no runtime package, set `OPUS_STATIC=1` in the build
stage alongside `libopus-dev`.
...
## Repairing a refused durable session

A member whose committed WholeBlob document fails meerkat's audited-endpoint
guard (`SessionError::WholeBlobAuditedEndpointDivergence`) is parked with a
typed `session_repair_required` hold instead of retrying: it shows as
`needs_repair` in the console and on `mobkit/member_health`, and the hold
carries the two exact `rkat` commands for the runtime store this gateway
opened. Both gateways derive that scope from their own storage composition,
never from a guess:

- the default persistent launch opens `<state_dir>/runtime.sqlite`, so the
  commands are `rkat session repair-wholeblob <session_id> --runtime-store
  <state_dir>/runtime.sqlite --json` (diagnose) and the same with
  `--apply --json` (repair; add `--accept-shorter` only when the diagnose
  report says the live transcript is shorter than the audited endpoint);
- a composite storage provider's meerkat-level realm renders as
  `rkat --state-root <state_dir> --realm mobkit session repair-wholeblob
  <session_id> ...`;
- an in-memory or declared-ephemeral runtime store has no document to repair,
  so no scope is derived and the commands keep explicit `<state-root>` /
  `<realm>` placeholders.

Stop nothing: the member is already parked. Run the diagnose command, then
the apply command, then `mobkit/reload_member` on the identity; the same
...
## WorkGraph realms for mob members

Meerkat builds every mob member in the realm `mob.<mob_id>` and rescopes the
WorkGraph service the gateway hands the mob runtime to that realm, so a member
resolves its attention bindings there and nowhere else. The gateway's own
WorkGraph service is scoped to the same realm at construction, which is why a
goal bound to a member from the console or the RPC surface lands where the
member reads it.

Two consequences matter to an operator:

- A goal, binding or reassignment whose target names a member of another mob
  (an explicit `owner` key of the form `mob/<other>/agent/<identity>`) is
  refused with JSON-RPC `-32602` and `data.kind =
  "attention_target_realm_mismatch"`. It is not stored anywhere. Create that
  work through the other mob's own runtime.
- Bindings written before this rule existed are moved at bootstrap. A child
  mob spawned by an agent tool used to share the parent runtime's service
  unscoped, so a goal bound to a child-mob member lived in the parent realm
  and the child member found it there; it no longer does. On every bootstrap
  the runtime scans its realm for active or paused bindings whose owner key
  names another mob, re-creates each in that mob's realm over the same store
  (title, description, mode, policies, priority, labels, timing fields,
  external and evidence refs, delegated authority, projection policy and the
  paused state carried; edges of the original item are not), and cancels the
  original goal, which stops its binding. A second bootstrap finds nothing to
  move. `mobkit/capabilities` reports the run under `workgraph_realm_migration`
  (`mode`, `scanned_bindings`, `skipped_terminal`, `migrated[]` with the old
  and new binding and item ids, and an `outcome` that records a failed scan or
  write instead of aborting the launch). Library hosts choose the mode with
  `MobBootstrapSpec::with_workgraph_realm_migration` (`apply`, `dry_run`,
  `off`).

## Blob ids are not meerkat content blob ids

A MobKit blob store applies one of two digest recipes depending on how an
object enters it. Images that arrive through meerkat's `BlobStore` face
(`Base64BlobStoreAdapter::put_image`, used by the gateway, the unified runtime
and the console voice host) and images uploaded through the console
(`mobkit/blob/upload`, multipart `image_upload` placeholders) are addressed
with meerkat's `content_blob_id`, `sha256(canonical_media_type || 0x00 ||
base64_text)` over the base64 form, so meerkat's read-back verification
(durable fork preflight, realtime image hydration) accepts them. Objects
written through MobKit's native `BinaryBlobStore::put_bytes` (non-image
`mobkit/blob/*` objects), and image blobs written by MobKit builds that
predate this rule, carry MobKit's own address, `sha256(media_type || 0x00 ||
raw_bytes)` over the decoded bytes (`meerkat-mobkit/src/blob_store.rs`,
`compute_blob_id`). Both render as `sha256:<64 hex digits>`, so the same PNG
under the two recipes carries two visually indistinguishable ids. The store
resolves both forms through both faces, so references written before the rule
keep hydrating; still, compare ids only within one recipe. The side by side
recipe, provenance rules and the planned unification are documented in the
meerkat storage operations runbook under "Blob digests in meerkat and MobKit".
