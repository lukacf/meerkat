...
A `pre_tool_execution` foreground hook that must approve each tool call before
...
Ordinary foreground hooks report lifecycle events in the session event stream.
These examples show possible outcomes for the foreground `safety-gate` hook,
...
// HookFailed -- emitted when a foreground hook errors or times out
