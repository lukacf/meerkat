...
[dependencies]
meerkat = { version = "=0.8.37", default-features = false, features = ["anthropic"] }

# Add persistence
meerkat = { version = "=0.8.37", default-features = false, features = ["anthropic", "session-store"] }

# Add compaction
meerkat = { version = "=0.8.37", default-features = false, features = ["anthropic", "session-store", "session-compaction"] }

# Add facade-local predicate-schedule helper re-exports. The schedule service
# and WorkGraph substrate are already linked.
meerkat = { version = "=0.8.37", default-features = false, features = ["anthropic", "session-store", "schedule"] }

# Broad native facade with WebSocket live support
meerkat = { version = "=0.8.37", features = [
...
meerkat = { version = "=0.8.37", features = ["openai-realtime", "live-webrtc"] }
```
...
| Gate | Requirement |
|------|-------------|
| Realtime model | The selected Rust `ModelProfile::is_realtime()` projection is `true` |
...
`ModelProfile` describes per-model capabilities used for feature gating and tool
visibility. Its Rust `interaction_kind` is the catalog-owned contract:
`Text`, `TurnBasedRealtime`, or `ContinuousLive`. `is_realtime()` derives the
coarse live capability from that value; it is not a separately stored boolean.
The public wire `realtime` field remains that derived capability projection.
Providers select a protocol from the typed interaction kind, not a model-name
prefix or the coarse boolean. A live-capable catalog declaration still requires
the corresponding provider factory and host transport.

The modality and feature projections include:
...
| `realtime` (wire), `is_realtime()` (Rust) | Model can back a caller-initiated live channel | No | `gpt-realtime-2` only | No current catalog row |
