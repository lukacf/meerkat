...
24. **`connection_ref` propagates everywhere a credential decision is made.** `SessionLlmIdentity.connection_ref` is persisted, hot-swap re-resolves through the same binding, and surfaces (RPC `session/create`, REST `POST /sessions`, MCP, SDKs, Web `runtime.createSession` / `mob.spawn`) must accept and forward it. Surface-level rejection of typed `connection_ref` variants is scope reduction, not pragmatism.
...
26. **Provider-native web search and tool capability is per-model.** `ModelProfile.tools.web_search` / `ModelProfile.tools.mcp` / etc. gate visibility — when a tool unexpectedly disappears under a particular model, the answer is almost always profile-driven gating, not a bug.
27. **`profile.tools.mcp` provenance filtering.** Profile-injected MCP servers are filtered by source provenance; review changes to MCP loading paths against the profile filter, not just the global `.rkat/mcp.toml`.
...
29. **No `session/realtime_attachment_statuses` plural.** Only the singular `session/realtime_attachment_status` exists; surfaces that need batch behavior fan out per session id. Stragglers in older docs/audit notes do not constitute a contract.
...
31. **Web SDK auth model.** Browser-hosted flows use `connectionRef` + `registerExternalAuthResolver` (resolves to typed `AuthCredential`s in the host page) — per-session `apiKey` fields were removed. New WASM-bridged auth wiring must go through the resolver path, not by re-introducing inline credentials.
