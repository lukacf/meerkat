...
/// Rule 8: a refusal must be a typed outcome that names what happened and what
/// the caller may trust. Through 0.8.23 a registration arriving inside the
/// drain window got a bare `GuardRejected`, which surfaces as an untyped input
/// validation failure - indistinguishable, from the caller's side, from "this
/// session may never be registered again". It is instead a RETRYABLE verdict
/// that names the still-open producer obligations.
#[test]
fn register_while_draining_is_a_typed_refusal_naming_open_obligations() {
    let mut authority = registered_authority();
    MeerkatMachineMutator::apply(&mut authority, begin_unregister_input())
        .expect("BeginUnregisterSession opens the drain window");
    // Close one obligation so the verdict has to distinguish open from closed
    ...
    .expect("runtime-loop drain closes its obligation");

    let refused = MeerkatMachineMutator::apply(
        &mut authority,
        MeerkatMachineInput::RegisterSession {
            session_id: mm_dsl::SessionId::from(TEST_SESSION),
            runtime_epoch_id: None,
    ...
    let effects = refused.into_effects();
    let named = effects
        .iter()
        .find_map(|effect| match effect {
            MeerkatMachineEffect::SessionRegistrationRejected {
                reason,
        ...
        .unwrap_or_else(|| {
            panic!("the draining refusal must be an explicit verdict, got {effects:?}")
        });
    assert_eq!(
        named,
        (
            mm_dsl::SessionRegistrationRejectReasonKind::UnregisterTeardownInProgress,
            false,
            true,
            true
        ),
        "the verdict must name the teardown reason and exactly which producer \
    ...
    assert_eq!(
        state.registration_phase,
        mm_dsl::RegistrationPhase::Draining,
        "a refused registration must not perturb the drain window it refused into"
    );
    assert!(!state.unregister_runtime_loop_drain_pending);
    assert!(state.unregister_comms_drain_exit_pending);
    assert!(state.unregister_completion_waiter_drain_pending);
