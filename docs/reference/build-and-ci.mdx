...
- `gcp-buildbuddy` calls `buildbuddy.yml` in `full-fresh` mode. It starts the
  ...
  the prebuild, static checks, the native lanes (all-features clippy as a
  Bazel aspect; unit and integration-fast as cargo-equivalent actions running
  the pre-push hook's nextest invocations, one process per test), and
  authority governance (seam inventory, RMAT audit, machine authority). SDK suites, WASM check, minimal-feature, feature-matrix,
...
- The top-level `CI gate` requires both component results to be `success`.
  It also enforces a 4200-second (70-minute) push-to-terminal budget measured
