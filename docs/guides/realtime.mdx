...
`turning_mode` distinguishes `provider_managed` (provider-owned turn detection),
`explicit_commit` (caller commits), and `continuous` (observations without
provider-defined user turns). Legacy realtime channels support only the first
two; omission preserves `provider_managed`. They reject `continuous` before
provider connection or attachment. The continuous value is reserved for a
separately activated public Live profile, not a way to switch an ordinary
session's model or grant execution permission.

The optional `profile_id` field selects a named public continuous profile on
the owning host. It must be a non-null profile identifier; it cannot be combined
with a private `execution_identity`, a legacy turning mode, or `seed_max_chars`.
An omitted selector preserves the legacy open path. Hosts without public-profile
control reject the request before provider I/O rather than treating the profile
as an executor model. Profile configuration alone never supplies activation.
...
### Retained member Live observations

`mob/member_live_observations` reads the independent retained Live ledger
through the member's current owner, including after channel close or owner
restart. It returns typed continuous observations, not finalized `Message`
rows. The response includes the original session/member identity, exact
captured snapshot prefix, coverage, and controlling-host versus remote-host
provenance.

Pass `mob_id`, `agent_identity`, optional `channel_id`, optional opaque
`cursor`, and a positive `limit` (default 64, maximum 256). Continue only with
the returned `next_cursor` while `has_more` is true. Later appends do not
rebase the captured prefix. Expired snapshots, mismatched cursors, unsupported
owners, and integrity faults are explicit failures, not empty-history
fallbacks. The complete compact bridge reply is bounded to 128 KiB.

Every page requires `ReadHistory` authorization; neither an active channel nor
a Live execution grant is required. Remote retained reads require bridge V7;
older peers reject them without falling back to ordinary transcript history.
Legacy member opens without a profile selector retain their V4 protocol, and
ordinary bridge authority defaults remain V6.

The same read is available as `rkat mob member-live-observations`, Python
`mob.member_live_observations(...)`, TypeScript
`mob.memberLiveObservations(...)`, REST
`GET /mob/{id}/members/{agent_identity}/live-observations`, and public MCP
`meerkat_mob_member_live_observations`. REST and public MCP still expose no
Live mutation/control family.
