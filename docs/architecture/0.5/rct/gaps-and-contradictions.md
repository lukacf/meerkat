...
| `GAP-004` | Canonical machine file layout differed across docs | schema-generated machines use the workflow-spec layout under `src/machines/<machine>/`; PureHand machines use the owner files/directories named in the implementation plan | `meerkat_machine_schema_workflow_spec.md`, `meerkat_0_5_implementation_plan.md` |
...
| `GAP-010` | PureHand harness ordering was ambiguous in the critical path | PureHand harness scaffolding lands before PureHand extraction and surface dependencies on those kernels | `rct/plan.yaml`, `rct/checklist.yaml` |
...
2. landed machine schemas or explicit Rust kernels become the long-term
