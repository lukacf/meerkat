...
- **Detached (hosts that outlive the call).** The JSON-RPC, REST, and MCP
  servers, `rkat run --keep-alive`, and `rkat mob deploy --surface rpc` can
  deliver a completion after the call returns. There the call returns at once
  with `{"status": "running", "council_id": ..., "job_id": ...}` and the
  council runs as a background operation of the convener's session. When it
  ends, the record `{"tool": "council", "job_id": ..., "outcome": {"result",
  "cleanup", "replayed"}}` is appended once to the convener's session as an
  ordinary durable System message ("Background council job `<job_id>`
  finished:" followed by the record), and the background job then completes
  with the same record, which wakes the convener. The System message survives
  later model calls, turns, and reloads and is readable through the convener's
  session history; the job notice is transient.
- **Blocking (one-shot hosts).** A host that may exit when the turn ends cannot
  ...
  returns the sealed outcome as its ordinary tool result.

In both forms the council's own `timeout_seconds` is the only deadline. The
agent loop's default tool deadline does not cut the call, so a council longer
than that deadline still delivers its result.
...
`{"error": ...}` as its outcome and the background job **fails**. On a
