...
| `GET` | `/sessions/{id}/status` | Read runtime state |
| `GET` | `/sessions/{id}/realtime-attachment-status` | Read the runtime-owned realtime attachment status for a session |
| `POST` | `/sessions/{id}/submit` | Admit runtime input explicitly |
| `POST` | `/sessions/{id}/retire` | Retire a runtime |
| `POST` | `/sessions/{id}/reset` | Reset a runtime |
| `GET` | `/sessions/{id}/submissions` | List runtime input state entries |
| `GET` | `/sessions/{session_id}/submissions/{submission_id}` | Read one runtime input state |
