...
| INV-007 | Embedded profile reduction is allowed to shrink the reachable state graph by excluding capabilities such as mobs, background task execution, or selected long-running tool classes, but it must do so by capability gating and backend absence rather than by introducing a second runtime state machine or alternate session semantics. | All phases | Embedded profile and runtime composition | Architecture review plus embedded-profile tests showing unsupported capabilities fail deterministically on the canonical runtime path | `SPECIFIED` | `UNEXECUTED` |
...
| CONTRACT-008 | Embedded profile exclusions such as no `meerkat-mob`, no background async task execution, or no selected long-running tool classes must be represented as explicit unavailable capabilities on the canonical runtime path, not by replacing `RuntimeSessionAdapter` or `PersistentSessionService` with a specialized embedded authority model. | Phase 2 and Phase 3 | Embedded profile and embedded surface runtime | Embedded profile tests, unsupported-capability smoke, and architecture review against runtime ownership | `SPECIFIED` | `UNEXECUTED` |
