...
| JSON-RPC `mob/*` | Host control plane for mob lifecycle, members, flows, tasks, and profiles. |
...
The task board is mob-owned state. Task creation and updates flow through the
mob runtime rather than through ad hoc per-agent notes.
