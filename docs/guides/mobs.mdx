...
- **`usage_total`** is the sum of the readable members only. When
  ...
  instead of a zero. Cache and reasoning counters are summed too: member usage
  is already normalized, so on every provider the cache counters are subsets
  of `input_tokens` and reasoning is a subset of `output_tokens`.
