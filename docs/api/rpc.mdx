...
than 1,000 items or 1,000 edges fails with `INVALID_PARAMS` and must be
narrowed rather than truncated. The result is `{"items": [...]}`.
...
edge to a terminal item unless `include_terminal` is true. The same 1,000
item, edge, and attention-binding atomic scan bounds apply as for
`workgraph/ready`.
