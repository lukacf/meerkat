...
| `session/status` | Runtime | Read runtime-backed ingress state |
...
| `session/submit` | Runtime | Admit external work directly into runtime ingress |
| `session/retire` | Runtime | Retire runtime-backed ingress |
| `session/reset` | Runtime | Reset runtime-backed ingress |
| `session/submission` | Runtime | Read runtime input lifecycle state |
| `session/submissions` | Runtime | List runtime input lifecycle records |
...
<Note>
`session/status`, `session/submit`, `session/retire`, `session/reset`, `session/submission`, `session/submissions` are advertised only when the server is running with the runtime-backed ingress/control-plane surface enabled.
</Note>
...
```json Response
{
  ...
  "result": {
    ...
    "methods": [
      ...
      "session/status", "session/realtime_attachment_status", "session/realtime_attachment_statuses", "session/submit", "session/retire", "session/reset",
      "session/submission", "session/submissions",
