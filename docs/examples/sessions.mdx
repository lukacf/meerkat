...
<Tabs>
  ...
  <Tab title="REST">
    ...
    curl -X POST http://localhost:8080/sessions \
      ...
      -d '{"prompt": "What is the capital of France?", "turn_metadata": {"model": "claude-sonnet-4-6"}}'
    ```
  </Tab>
  <Tab title="MCP">
    ```json
...
<Tabs>
  ...
  <Tab title="REST">
    ...
    curl -X POST http://localhost:8080/sessions \
      -H "Content-Type: application/json" \
      -d '{"prompt":"Explain monads","turn_metadata":{"model":"gpt-5.2","provider":"openai","provider_params":{"action":"set","value":{"temperature":0.7}}}}'
    ```
  </Tab>
  <Tab title="MCP">
    ```json
