...
CONSTANTS BooleanValues, BranchIdValues, DependencyModeValues, FlowNodeIdValues, FlowNodeKindValues, FrameIdValues, FrameScopeValues, LoopInstanceIdValues, NatValues, NodeRunStatusValues, SetOfBranchIdValues, SetOfFlowNodeIdValues
...
AnyDepCompleted(node_id) == (\E dep_id \in SeqElements((IF node_id \in DOMAIN node_dependencies THEN node_dependencies[node_id] ELSE <<>>)) : ((IF dep_id \in DOMAIN node_status THEN node_status[dep_id] ELSE "None") = "Completed"))
AllDepsCompleted(node_id) == (\A dep_id \in SeqElements((IF node_id \in DOMAIN node_dependencies THEN node_dependencies[node_id] ELSE <<>>)) : ((IF dep_id \in DOMAIN node_status THEN node_status[dep_id] ELSE "None") = "Completed"))
NodeAdmissionEligible(node_id) == (IF (Len((IF node_id \in DOMAIN node_dependencies THEN node_dependencies[node_id] ELSE <<>>)) = 0) THEN TRUE ELSE (IF ((IF node_id \in DOMAIN node_dependency_modes THEN node_dependency_modes[node_id] ELSE "None") = "All") THEN (\A dep_id \in SeqElements((IF node_id \in DOMAIN node_dependencies THEN node_dependencies[node_id] ELSE <<>>)) : (((IF dep_id \in DOMAIN node_status THEN node_status[dep_id] ELSE "None") = "Completed") \/ ((IF dep_id \in DOMAIN node_status THEN node_status[dep_id] ELSE "None") = "Failed") \/ ((IF dep_id \in DOMAIN node_status THEN node_status[dep_id] ELSE "None") = "Skipped") \/ ((IF dep_id \in DOMAIN node_status THEN node_status[dep_id] ELSE "None") = "Canceled"))) ELSE ((\E dep_id \in SeqElements((IF node_id \in DOMAIN node_dependencies THEN node_dependencies[node_id] ELSE <<>>)) : ((IF dep_id \in DOMAIN node_status THEN node_status[dep_id] ELSE "None") = "Completed")) \/ (\A dep_id \in SeqElements((IF node_id \in DOMAIN node_dependencies THEN node_dependencies[node_id] ELSE <<>>)) : (((IF dep_id \in DOMAIN node_status THEN node_status[dep_id] ELSE "None") = "Completed") \/ ((IF dep_id \in DOMAIN node_status THEN node_status[dep_id] ELSE "None") = "Failed") \/ ((IF dep_id \in DOMAIN node_status THEN node_status[dep_id] ELSE "None") = "Skipped") \/ ((IF dep_id \in DOMAIN node_status THEN node_status[dep_id] ELSE "None") = "Canceled"))))))
...
AdmitNextReadyNode_Fail ==
    /\ phase = "Running"
    /\ (Len(ready_queue) > 0)
    /\ (((IF Head(ready_queue) \in DOMAIN node_dependency_modes THEN node_dependency_modes[Head(ready_queue)] ELSE "None") = "Any") /\ (\A dep_id \in SeqElements((IF Head(ready_queue) \in DOMAIN node_dependencies THEN node_dependencies[Head(ready_queue)] ELSE <<>>)) : (((IF dep_id \in DOMAIN node_status THEN node_status[dep_id] ELSE "None") = "Completed") \/ ((IF dep_id \in DOMAIN node_status THEN node_status[dep_id] ELSE "None") = "Failed") \/ ((IF dep_id \in DOMAIN node_status THEN node_status[dep_id] ELSE "None") = "Skipped") \/ ((IF dep_id \in DOMAIN node_status THEN node_status[dep_id] ELSE "None") = "Canceled"))) /\ ~(AnyDepCompleted(Head(ready_queue))))
    /\ phase' = "Running"
    /\ model_step_count' = model_step_count + 1
    /\ last_admitted_node' = Head(ready_queue)
