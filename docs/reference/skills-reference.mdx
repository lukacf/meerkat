...
- **Per-turn injection**: raw RPC/REST requests use `turn_metadata.skill_references: SkillKey[]`; SDK turn helpers expose `skill_refs: SkillRef[]` convenience options that resolve into that runtime metadata carrier.
