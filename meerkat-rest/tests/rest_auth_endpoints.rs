...
//! /auth/bindings/{binding_id} -> POST /sessions with turn_metadata.connection_ref". The
...
/// `POST /sessions` accepts a canonical `turn_metadata.connection_ref`
/// field on the body. When the referenced realm doesn't exist, the
/// server returns a structured error — not 404 or a panic.
#[tokio::test]
async fn post_sessions_accepts_turn_metadata_connection_ref() {
    let app = build_app();
    ...
    let body = serde_json::json!({
        ...
        "turn_metadata": {
            "connection_ref": {
                "action": "set",
                "value": {"realm": "test-realm", "binding": "default"}
            }
        }
    });
    ...
    assert!(
        status.is_success() || status.is_client_error() || status.is_server_error(),
        "status must be valid HTTP code for turn_metadata.connection_ref body, got {status}"
    );
}
