...
- `turn_admission_shutdown_drain_and_teardown` — RequestShutdownImmediateIdle and RequestShutdownImmediateAdmitted establish the generated PendingAdmissionDrainRequested obligation immediately; RequestShutdownDeferredRunning and RequestShutdownDeferredCompleting record shutdown_pending so FinalizeTurnToShutdown establishes that obligation after the active turn resolves; RequestShutdownAlreadyShuttingDown preserves rather than remints it; ResolvePendingAdmissionDrained closes it only in ShuttingDown, and AuthorizeSessionTeardown emits SessionTeardownAuthorized exactly once after the shell has drained queued admission work, so every normal session-task exit consumes singular generated teardown authority
...
- `FinalizeTurnToShutdown`
  - anchors: `session_turn_admission_authority`
  - scenarios: `turn_admission_claim_run_finalize`, `turn_admission_shutdown_drain_and_teardown`
- `FinalizeTurnToIdle`
  - anchors: `session_turn_admission_authority`
  - scenarios: `turn_admission_claim_run_finalize`
