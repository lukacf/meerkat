...
### Delivery vs observation (Rust SDK)

When injecting events from application code, the public Rust SDK now keeps delivery and observation separate.

The service's `event_injector()` returns an `Arc<dyn EventInjector>`. Use `inject()` to push an external event into the session inbox:
...
If you need streaming output, subscribe at the scope you actually want:

- **session-scoped**: use the session's primary `event_tx`
- **agent-scoped**: use `MobHandle::subscribe_agent_events(...)`
- **mob-scoped**: use `MobHandle::subscribe_mob_events(...)`
...
- Any streaming surface must be configured separately from injection.
...
- [Examples: comms](/examples/comms) - host mode, messaging, sub-agents across surfaces
