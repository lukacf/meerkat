...
`GET /mob/{id}/members/{agent_identity}/live-observations` reads retained
Live transcript observations, not ordinary `Message` history. Optional query
parameters are `channel_id`, opaque `cursor`, and `limit` (default 64, range
1-256). Every page requires the normal `ReadHistory` grant, not a Live
activation grant. This read-only endpoint adds no Live mutation or control
route.
