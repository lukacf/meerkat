...
held fence; reports state-root adoption (report-only —
realms are used where they lie); and runs the machine-owned
legacy-checkpoint adoption in bulk (sqlite realms; jsonl realms heal
lazily on first authority touch and report the sweep as skipped). A realm
