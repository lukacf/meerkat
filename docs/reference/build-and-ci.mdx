...
| `unit` (one lane per shard) | Rust-relevant change | `repo-cargo nextest run -p <changed packages> --lib --bins --profile ci-pr` (the PR lane's named profile, identical to `fast`) |
