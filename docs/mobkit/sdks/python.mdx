...
Create `config/mob.toml` with a `worker` profile and configure credentials for
its model's provider:

```toml config/mob.toml
[mob]
id = "quickstart"
...
Profiles are templates, so the example explicitly ensures `agent-1` exists
...
| `.on_error(callback)` | Error hook. Receives every runtime `ErrorEvent` (`category`, `message`, `context`) the gateway forwards as the `mobkit/on_error` notification; sync or async, fire-and-forget. Without it the events reach only the gateway's stderr ERROR line (see [Error hook and logging](/mobkit/guides/unified-runtime#error-hook-and-logging)) |
