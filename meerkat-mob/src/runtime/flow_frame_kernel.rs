...
use meerkat_machine_kernels::{
    KernelEffect, KernelFields, KernelInput, KernelNamedVariant, KernelValue,
};
        ...
        match node_spec {
            FlowNodeSpec::Step(s) => {
                node_kind.insert(
                    ...
                    KernelValue::NamedVariant {
                        enum_name: KernelNamedVariant::new_static("FlowNodeKind", "Step").enum_name,
                        variant: KernelNamedVariant::new_static("FlowNodeKind", "Step").variant,
                    },
                );
                node_deps.insert(
                    k.clone(),
            ...
            FlowNodeSpec::RepeatUntil(l) => {
                node_kind.insert(
                    k.clone(),
                    KernelValue::NamedVariant {
                        enum_name: KernelNamedVariant::new_static("FlowNodeKind", "Loop").enum_name,
                        variant: KernelNamedVariant::new_static("FlowNodeKind", "Loop").variant,
                    },
                );
                node_deps.insert(
                    k.clone(),
...
fn dep_mode_kv(mode: &DependencyMode) -> KernelValue {
    ...
    KernelValue::NamedVariant {
        enum_name: KernelNamedVariant::new_static("DependencyMode", variant).enum_name,
        variant: KernelNamedVariant::new_static("DependencyMode", variant).variant,
    }
}
