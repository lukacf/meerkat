...
<Tabs>
  <Tab title="Inventory section">
    ...
    The inventory never names tools. When a session actually composes both
    `browse_skills` and `load_skill`, the factory appends one guidance line
    after the inventory telling the model to browse with `browse_skills` and
    activate with `load_skill`.
...
The skill inventory mentions `browse_skills` and `load_skill` only when both
are composed for the session; on a normal factory-built session, use public
