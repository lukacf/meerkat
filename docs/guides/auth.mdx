...
at `~/.rkat/config.toml`. Because `global` is the universal tail of every realm
chain, a single sign-in is inherited by every workspace realm (credential reads
inherit down the chain). For a binding owned by a specific realm such as
`prod:openai`, manage that realm's config entries directly; credential *writes*
are strict-owner and land only in the realm that defines the binding. See
...
binding in the owning realm's config; credential writes are strict-owner and
land only in the realm that defines the binding.
...
```rust
use meerkat::{AgentBuildConfig, RuntimeBuildMode, Session, SessionId};
