...
`[profiles.<name>]` table carries `system_prompt` now fails with an error that
names the profile, the key, and the mechanisms above. Any other key the profile
does not declare is reported as a warning diagnostic by definition validation
and logged when the definition loads; parsing continues, and the key is
ignored.
