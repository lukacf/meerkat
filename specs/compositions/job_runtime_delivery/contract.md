...
- `job_notification_enters_runtime_inbox`: `job`.`NotificationCommitted` -> `runtime_delivery`.`CommitDelivery` [Enqueue]
...
## Transaction Plans
- `job_result_and_outbox` via `commit_job_delivery` / `DetachedJobStore::compare_and_swap` — job delivery payload and pending outbox row commit atomically under generated job authority
...
- `job_notification_reaches_runtime_delivery_authority` — job notification outbox entries enter generated runtime delivery authority before any acknowledgement
