...
| `FailedWithData` | `SESSION_ERROR` | -32603 | 500 | tool error | exit 1 |
...
[dependencies]
meerkat = { version = "0.7.0-alpha.0", default-features = false, features = ["anthropic"] }

# Add persistence
meerkat = { version = "0.7.0-alpha.0", default-features = false, features = ["anthropic", "session-store"] }

# Add compaction
meerkat = { version = "0.7.0-alpha.0", default-features = false, features = ["anthropic", "session-store", "session-compaction"] }

# Add scheduling
meerkat = { version = "0.7.0-alpha.0", default-features = false, features = ["anthropic", "session-store", "schedule"] }

# Kitchen sink
meerkat = { version = "0.7.0-alpha.0", features = [
...
These fields are set in `meerkat-core/src/model_profile/**` and exposed via `ModelProfile`. They control:
