...
job_ClaimQueued(arg_attempt_id, arg_worker_id, arg_claimed_at_ms, arg_lease_expires_at_ms, arg_runner_handle) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       ...
       /\ job_retry_due_at_ms' = None
       /\ job_cancel_requested' = FALSE
       /\ UNCHANGED << job_job_id, job_restart_class, job_checkpoint_ref, job_progress_cursor, job_terminal_kind, job_terminal_delivery_sequence, job_terminal_delivery_applied, runtime_delivery_phase, runtime_delivery_delivery_ids, runtime_delivery_delivery_sequences, runtime_delivery_delivery_source_sequences, runtime_delivery_committed_sequences, runtime_delivery_next_sequence, runtime_delivery_applied_cursor, witness_current_script_input, witness_remaining_script_inputs >>
       /\ pending_inputs' = SeqRemove(pending_inputs, packet)
       /\ observed_inputs' = observed_inputs
       /\ pending_routes' = pending_routes
       /\ delivered_routes' = delivered_routes
...
job_RequestCancelRunning ==
    /\ \E packet \in SeqElements(pending_inputs) :
       ...
       /\ UNCHANGED << job_job_id, job_restart_class, job_attempt_count, job_current_attempt_id, job_current_fence, job_current_worker_id, job_lease_expires_at_ms, job_heartbeat_at_ms, job_checkpoint_ref, job_runner_handle, job_progress_cursor, job_lease_expired, job_retry_due_at_ms, job_terminal_kind, job_terminal_delivery_sequence, job_terminal_delivery_applied, runtime_delivery_phase, runtime_delivery_delivery_ids, runtime_delivery_delivery_sequences, runtime_delivery_delivery_source_sequences, runtime_delivery_committed_sequences, runtime_delivery_next_sequence, runtime_delivery_applied_cursor, witness_current_script_input, witness_remaining_script_inputs >>
       /\ pending_inputs' = SeqRemove(pending_inputs, packet)
       /\ observed_inputs' = observed_inputs
       /\ pending_routes' = pending_routes
       /\ delivered_routes' = delivered_routes
...
job_RequestCancelQueued ==
    /\ \E packet \in SeqElements(pending_inputs) :
       ...
       /\ job_terminal_delivery_sequence' = (job_terminal_delivery_sequence) + 1
       /\ UNCHANGED << job_job_id, job_restart_class, job_attempt_count, job_current_attempt_id, job_current_fence, job_current_worker_id, job_lease_expires_at_ms, job_heartbeat_at_ms, job_checkpoint_ref, job_runner_handle, job_progress_cursor, job_lease_expired, job_retry_due_at_ms, job_terminal_delivery_applied, runtime_delivery_phase, runtime_delivery_delivery_ids, runtime_delivery_delivery_sequences, runtime_delivery_delivery_source_sequences, runtime_delivery_committed_sequences, runtime_delivery_next_sequence, runtime_delivery_applied_cursor, witness_current_script_input, witness_remaining_script_inputs >>
       /\ pending_inputs' = SeqRemove(pending_inputs, packet)
       /\ observed_inputs' = observed_inputs
       /\ pending_routes' = AppendIfMissing(pending_routes, [route |-> "job_terminal_enters_runtime_inbox", source_machine |-> "job", effect |-> "TerminalCommitted", target_machine |-> "runtime_delivery", target_input |-> "CommitDelivery", payload |-> [delivery_id |-> job_job_id, source_sequence |-> (job_terminal_delivery_sequence) + 1], actor |-> "runtime_delivery_authority", effect_id |-> (model_step_count + 1), source_transition |-> "RequestCancelQueued"])
       /\ delivered_routes' = delivered_routes
       /\ emitted_effects' = emitted_effects \cup { [machine |-> "job", variant |-> "TerminalCommitted", payload |-> [delivery_sequence |-> (job_terminal_delivery_sequence) + 1, job_id |-> job_job_id, terminal_kind |-> "Cancelled"], effect_id |-> (model_step_count + 1), source_transition |-> "RequestCancelQueued"] }
...
job_RequestCancelRetryScheduled ==
    /\ \E packet \in SeqElements(pending_inputs) :
       ...
       /\ delivered_routes' = delivered_routes
       /\ emitted_effects' = emitted_effects \cup { [machine |-> "job", variant |-> "TerminalCommitted", payload |-> [delivery_sequence |-> (job_terminal_delivery_sequence) + 1, job_id |-> job_job_id, terminal_kind |-> "Cancelled"], effect_id |-> (model_step_count + 1), source_transition |-> "RequestCancelRetryScheduled"] }
...
job_RequestCancelLossObserved ==
    /\ \E packet \in SeqElements(pending_inputs) :
       ...
       /\ delivered_routes' = delivered_routes
       /\ emitted_effects' = emitted_effects \cup { [machine |-> "job", variant |-> "TerminalCommitted", payload |-> [delivery_sequence |-> (job_terminal_delivery_sequence) + 1, job_id |-> job_job_id, terminal_kind |-> "Cancelled"], effect_id |-> (model_step_count + 1), source_transition |-> "RequestCancelLossObserved"] }
...
job_ScheduleRetryAfterLoss(arg_retry_due_at_ms) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       ...
       /\ UNCHANGED << job_job_id, job_restart_class, job_attempt_count, job_current_attempt_id, job_current_fence, job_current_worker_id, job_lease_expires_at_ms, job_heartbeat_at_ms, job_checkpoint_ref, job_runner_handle, job_progress_cursor, job_lease_expired, job_cancel_requested, job_terminal_kind, job_terminal_delivery_sequence, job_terminal_delivery_applied, runtime_delivery_phase, runtime_delivery_delivery_ids, runtime_delivery_delivery_sequences, runtime_delivery_delivery_source_sequences, runtime_delivery_committed_sequences, runtime_delivery_next_sequence, runtime_delivery_applied_cursor, witness_current_script_input, witness_remaining_script_inputs >>
       /\ pending_inputs' = SeqRemove(pending_inputs, packet)
       /\ observed_inputs' = observed_inputs
       /\ pending_routes' = pending_routes
       /\ delivered_routes' = delivered_routes
...
job_ClassifyNonResumableWorkerLoss(arg_observed_at_ms) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       ...
       /\ delivered_routes' = delivered_routes
       /\ emitted_effects' = emitted_effects \cup { [machine |-> "job", variant |-> "TerminalCommitted", payload |-> [delivery_sequence |-> (job_terminal_delivery_sequence) + 1, job_id |-> job_job_id, terminal_kind |-> "WorkerLost"], effect_id |-> (model_step_count + 1), source_transition |-> "ClassifyNonResumableWorkerLoss"] }
...
job_CompleteRunningAttempt(arg_attempt_id, arg_fence, arg_completed_at_ms) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       ...
       /\ delivered_routes' = delivered_routes
       /\ emitted_effects' = emitted_effects \cup { [machine |-> "job", variant |-> "TerminalCommitted", payload |-> [delivery_sequence |-> (job_terminal_delivery_sequence) + 1, job_id |-> job_job_id, terminal_kind |-> "Succeeded"], effect_id |-> (model_step_count + 1), source_transition |-> "CompleteRunningAttempt"] }
...
job_CompleteWaitingExternalAttempt(arg_attempt_id, arg_fence, arg_completed_at_ms) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       ...
       /\ delivered_routes' = delivered_routes
       /\ emitted_effects' = emitted_effects \cup { [machine |-> "job", variant |-> "TerminalCommitted", payload |-> [delivery_sequence |-> (job_terminal_delivery_sequence) + 1, job_id |-> job_job_id, terminal_kind |-> "Succeeded"], effect_id |-> (model_step_count + 1), source_transition |-> "CompleteWaitingExternalAttempt"] }
...
job_FailRunningAttempt(arg_attempt_id, arg_fence, arg_failed_at_ms) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       ...
       /\ delivered_routes' = delivered_routes
       /\ emitted_effects' = emitted_effects \cup { [machine |-> "job", variant |-> "TerminalCommitted", payload |-> [delivery_sequence |-> (job_terminal_delivery_sequence) + 1, job_id |-> job_job_id, terminal_kind |-> "Failed"], effect_id |-> (model_step_count + 1), source_transition |-> "FailRunningAttempt"] }
...
job_FailWaitingExternalAttempt(arg_attempt_id, arg_fence, arg_failed_at_ms) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       ...
       /\ delivered_routes' = delivered_routes
       /\ emitted_effects' = emitted_effects \cup { [machine |-> "job", variant |-> "TerminalCommitted", payload |-> [delivery_sequence |-> (job_terminal_delivery_sequence) + 1, job_id |-> job_job_id, terminal_kind |-> "Failed"], effect_id |-> (model_step_count + 1), source_transition |-> "FailWaitingExternalAttempt"] }
...
job_AcknowledgeRunningCancel(arg_attempt_id, arg_fence, arg_acknowledged_at_ms) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       ...
       /\ delivered_routes' = delivered_routes
       /\ emitted_effects' = emitted_effects \cup { [machine |-> "job", variant |-> "TerminalCommitted", payload |-> [delivery_sequence |-> (job_terminal_delivery_sequence) + 1, job_id |-> job_job_id, terminal_kind |-> "Cancelled"], effect_id |-> (model_step_count + 1), source_transition |-> "AcknowledgeRunningCancel"] }
...
job_AcknowledgeWaitingExternalCancel(arg_attempt_id, arg_fence, arg_acknowledged_at_ms) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       ...
       /\ delivered_routes' = delivered_routes
       /\ emitted_effects' = emitted_effects \cup { [machine |-> "job", variant |-> "TerminalCommitted", payload |-> [delivery_sequence |-> (job_terminal_delivery_sequence) + 1, job_id |-> job_job_id, terminal_kind |-> "Cancelled"], effect_id |-> (model_step_count + 1), source_transition |-> "AcknowledgeWaitingExternalCancel"] }
...
job_MarkQueuedNeedsAttention(arg_observed_at_ms) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       ...
       /\ delivered_routes' = delivered_routes
       /\ emitted_effects' = emitted_effects \cup { [machine |-> "job", variant |-> "TerminalCommitted", payload |-> [delivery_sequence |-> (job_terminal_delivery_sequence) + 1, job_id |-> job_job_id, terminal_kind |-> "NeedsAttention"], effect_id |-> (model_step_count + 1), source_transition |-> "MarkQueuedNeedsAttention"] }
...
job_MarkRunningNeedsAttention(arg_observed_at_ms) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       ...
       /\ delivered_routes' = delivered_routes
       /\ emitted_effects' = emitted_effects \cup { [machine |-> "job", variant |-> "TerminalCommitted", payload |-> [delivery_sequence |-> (job_terminal_delivery_sequence) + 1, job_id |-> job_job_id, terminal_kind |-> "NeedsAttention"], effect_id |-> (model_step_count + 1), source_transition |-> "MarkRunningNeedsAttention"] }
...
job_MarkWaitingExternalNeedsAttention(arg_observed_at_ms) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       ...
       /\ delivered_routes' = delivered_routes
       /\ emitted_effects' = emitted_effects \cup { [machine |-> "job", variant |-> "TerminalCommitted", payload |-> [delivery_sequence |-> (job_terminal_delivery_sequence) + 1, job_id |-> job_job_id, terminal_kind |-> "NeedsAttention"], effect_id |-> (model_step_count + 1), source_transition |-> "MarkWaitingExternalNeedsAttention"] }
...
job_MarkLossObservedNeedsAttention(arg_observed_at_ms) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       ...
       /\ delivered_routes' = delivered_routes
       /\ emitted_effects' = emitted_effects \cup { [machine |-> "job", variant |-> "TerminalCommitted", payload |-> [delivery_sequence |-> (job_terminal_delivery_sequence) + 1, job_id |-> job_job_id, terminal_kind |-> "NeedsAttention"], effect_id |-> (model_step_count + 1), source_transition |-> "MarkLossObservedNeedsAttention"] }
...
job_MarkRetryScheduledNeedsAttention(arg_observed_at_ms) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       ...
       /\ delivered_routes' = delivered_routes
       /\ emitted_effects' = emitted_effects \cup { [machine |-> "job", variant |-> "TerminalCommitted", payload |-> [delivery_sequence |-> (job_terminal_delivery_sequence) + 1, job_id |-> job_job_id, terminal_kind |-> "NeedsAttention"], effect_id |-> (model_step_count + 1), source_transition |-> "MarkRetryScheduledNeedsAttention"] }
...
job_ApplySucceededDelivery(arg_delivery_sequence) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       /\ packet.machine = "job"
       ...
       /\ ~HigherPriorityReady("job_authority")
       /\ job_phase = "Succeeded"
       /\ ((job_terminal_delivery_applied = FALSE) /\ (packet.payload.delivery_sequence = job_terminal_delivery_sequence))
       /\ job_phase' = "Succeeded"
       ...
       /\ pending_routes' = pending_routes
       /\ delivered_routes' = delivered_routes
       /\ emitted_effects' = emitted_effects \cup { [machine |-> "job", variant |-> "DeliveryApplied", payload |-> [delivery_sequence |-> packet.payload.delivery_sequence], effect_id |-> (model_step_count + 1), source_transition |-> "ApplySucceededDelivery"] }
...
job_ApplyFailedDelivery(arg_delivery_sequence) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       /\ packet.machine = "job"
       /\ packet.variant = "MarkDeliveryApplied"
       /\ packet.payload.delivery_sequence = arg_delivery_sequence
       ...
       /\ pending_routes' = pending_routes
       /\ delivered_routes' = delivered_routes
       /\ emitted_effects' = emitted_effects \cup { [machine |-> "job", variant |-> "DeliveryApplied", payload |-> [delivery_sequence |-> packet.payload.delivery_sequence], effect_id |-> (model_step_count + 1), source_transition |-> "ApplyFailedDelivery"] }
...
job_ApplyCancelledDelivery(arg_delivery_sequence) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       /\ packet.machine = "job"
       /\ packet.variant = "MarkDeliveryApplied"
       /\ packet.payload.delivery_sequence = arg_delivery_sequence
       ...
       /\ pending_routes' = pending_routes
       /\ delivered_routes' = delivered_routes
       /\ emitted_effects' = emitted_effects \cup { [machine |-> "job", variant |-> "DeliveryApplied", payload |-> [delivery_sequence |-> packet.payload.delivery_sequence], effect_id |-> (model_step_count + 1), source_transition |-> "ApplyCancelledDelivery"] }
...
job_ApplyWorkerLostDelivery(arg_delivery_sequence) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       /\ packet.machine = "job"
       /\ packet.variant = "MarkDeliveryApplied"
       /\ packet.payload.delivery_sequence = arg_delivery_sequence
       ...
       /\ pending_routes' = pending_routes
       /\ delivered_routes' = delivered_routes
       /\ emitted_effects' = emitted_effects \cup { [machine |-> "job", variant |-> "DeliveryApplied", payload |-> [delivery_sequence |-> packet.payload.delivery_sequence], effect_id |-> (model_step_count + 1), source_transition |-> "ApplyWorkerLostDelivery"] }
...
job_ApplyNeedsAttentionDelivery(arg_delivery_sequence) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       /\ packet.machine = "job"
       /\ packet.variant = "MarkDeliveryApplied"
       /\ packet.payload.delivery_sequence = arg_delivery_sequence
       ...
       /\ pending_routes' = pending_routes
       /\ delivered_routes' = delivered_routes
       /\ emitted_effects' = emitted_effects \cup { [machine |-> "job", variant |-> "DeliveryApplied", payload |-> [delivery_sequence |-> packet.payload.delivery_sequence], effect_id |-> (model_step_count + 1), source_transition |-> "ApplyNeedsAttentionDelivery"] }
...
job_ObserveSucceededDeliveryAlreadyApplied(arg_delivery_sequence) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       /\ packet.machine = "job"
       /\ packet.variant = "MarkDeliveryApplied"
       /\ packet.payload.delivery_sequence = arg_delivery_sequence
       /\ ~HigherPriorityReady("job_authority")
       /\ job_phase = "Succeeded"
       /\ (job_terminal_delivery_applied /\ (packet.payload.delivery_sequence = job_terminal_delivery_sequence))
       /\ job_phase' = "Succeeded"
       /\ UNCHANGED << job_job_id, job_restart_class, job_attempt_count, job_current_attempt_id, job_current_fence, job_current_worker_id, job_lease_expires_at_ms, job_heartbeat_at_ms, job_checkpoint_ref, job_runner_handle, job_progress_cursor, job_lease_expired, job_retry_due_at_ms, job_cancel_requested, job_terminal_kind, job_terminal_delivery_sequence, job_terminal_delivery_applied, runtime_delivery_phase, runtime_delivery_delivery_ids, runtime_delivery_delivery_sequences, runtime_delivery_delivery_source_sequences, runtime_delivery_committed_sequences, runtime_delivery_next_sequence, runtime_delivery_applied_cursor, witness_current_script_input, witness_remaining_script_inputs >>
       /\ pending_inputs' = SeqRemove(pending_inputs, packet)
       /\ observed_inputs' = observed_inputs
       /\ pending_routes' = pending_routes
       /\ delivered_routes' = delivered_routes
       /\ emitted_effects' = emitted_effects \cup { [machine |-> "job", variant |-> "DeliveryApplied", payload |-> [delivery_sequence |-> packet.payload.delivery_sequence], effect_id |-> (model_step_count + 1), source_transition |-> "ObserveSucceededDeliveryAlreadyApplied"] }
...
job_ObserveFailedDeliveryAlreadyApplied(arg_delivery_sequence) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       /\ packet.machine = "job"
       /\ packet.variant = "MarkDeliveryApplied"
       /\ packet.payload.delivery_sequence = arg_delivery_sequence
       ...
       /\ pending_routes' = pending_routes
       /\ delivered_routes' = delivered_routes
       /\ emitted_effects' = emitted_effects \cup { [machine |-> "job", variant |-> "DeliveryApplied", payload |-> [delivery_sequence |-> packet.payload.delivery_sequence], effect_id |-> (model_step_count + 1), source_transition |-> "ObserveFailedDeliveryAlreadyApplied"] }
...
job_ObserveCancelledDeliveryAlreadyApplied(arg_delivery_sequence) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       /\ packet.machine = "job"
       /\ packet.variant = "MarkDeliveryApplied"
       /\ packet.payload.delivery_sequence = arg_delivery_sequence
       ...
       /\ pending_routes' = pending_routes
       /\ delivered_routes' = delivered_routes
       /\ emitted_effects' = emitted_effects \cup { [machine |-> "job", variant |-> "DeliveryApplied", payload |-> [delivery_sequence |-> packet.payload.delivery_sequence], effect_id |-> (model_step_count + 1), source_transition |-> "ObserveCancelledDeliveryAlreadyApplied"] }
...
job_ObserveWorkerLostDeliveryAlreadyApplied(arg_delivery_sequence) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       /\ packet.machine = "job"
       /\ packet.variant = "MarkDeliveryApplied"
       /\ packet.payload.delivery_sequence = arg_delivery_sequence
       ...
       /\ pending_routes' = pending_routes
       /\ delivered_routes' = delivered_routes
       /\ emitted_effects' = emitted_effects \cup { [machine |-> "job", variant |-> "DeliveryApplied", payload |-> [delivery_sequence |-> packet.payload.delivery_sequence], effect_id |-> (model_step_count + 1), source_transition |-> "ObserveWorkerLostDeliveryAlreadyApplied"] }
...
job_ObserveNeedsAttentionDeliveryAlreadyApplied(arg_delivery_sequence) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       /\ packet.machine = "job"
       /\ packet.variant = "MarkDeliveryApplied"
       /\ packet.payload.delivery_sequence = arg_delivery_sequence
       ...
       /\ pending_routes' = pending_routes
       /\ delivered_routes' = delivered_routes
       /\ emitted_effects' = emitted_effects \cup { [machine |-> "job", variant |-> "DeliveryApplied", payload |-> [delivery_sequence |-> packet.payload.delivery_sequence], effect_id |-> (model_step_count + 1), source_transition |-> "ObserveNeedsAttentionDeliveryAlreadyApplied"] }
...
job_terminal_requires_delivery == (IF ((job_phase # "Succeeded") /\ (job_phase # "Failed") /\ (job_phase # "Cancelled") /\ (job_phase # "WorkerLost") /\ (job_phase # "NeedsAttention")) THEN TRUE ELSE ((job_terminal_kind # None) /\ (job_terminal_delivery_sequence > 0)))
...
runtime_delivery_CommitNewDelivery(arg_delivery_id, arg_source_sequence) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       ...
       /\ pending_inputs' = SeqRemove(pending_inputs, packet)
       /\ observed_inputs' = observed_inputs
       /\ pending_routes' = AppendIfMissing(pending_routes, [route |-> "runtime_delivery_commit_acknowledges_job_outbox", source_machine |-> "runtime_delivery", effect |-> "DeliveryCommitted", target_machine |-> "job", target_input |-> "MarkDeliveryApplied", payload |-> [delivery_sequence |-> packet.payload.source_sequence], actor |-> "job_authority", effect_id |-> (model_step_count + 1), source_transition |-> "CommitNewDelivery"])
       /\ delivered_routes' = delivered_routes
...
runtime_delivery_ReuseCommittedDelivery(arg_delivery_id, arg_source_sequence) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       ...
       /\ pending_inputs' = SeqRemove(pending_inputs, packet)
       /\ observed_inputs' = observed_inputs
       /\ pending_routes' = AppendIfMissing(pending_routes, [route |-> "runtime_delivery_reuse_acknowledges_job_outbox", source_machine |-> "runtime_delivery", effect |-> "DeliveryReused", target_machine |-> "job", target_input |-> "MarkDeliveryApplied", payload |-> [delivery_sequence |-> packet.payload.source_sequence], actor |-> "job_authority", effect_id |-> (model_step_count + 1), source_transition |-> "ReuseCommittedDelivery"])
       /\ delivered_routes' = delivered_routes
...
EntryPacketAdmissible_job(packet) ==
    ...
    \/ /\ (packet.variant = "MarkDeliveryApplied") /\ (job_phase = "Succeeded") /\ (((job_terminal_delivery_applied = FALSE) /\ (packet.payload.delivery_sequence = job_terminal_delivery_sequence)))
    \/ /\ (packet.variant = "MarkDeliveryApplied") /\ (job_phase = "Failed") /\ (((job_terminal_delivery_applied = FALSE) /\ (packet.payload.delivery_sequence = job_terminal_delivery_sequence)))
    \/ /\ (packet.variant = "MarkDeliveryApplied") /\ (job_phase = "Cancelled") /\ (((job_terminal_delivery_applied = FALSE) /\ (packet.payload.delivery_sequence = job_terminal_delivery_sequence)))
    \/ /\ (packet.variant = "MarkDeliveryApplied") /\ (job_phase = "WorkerLost") /\ (((job_terminal_delivery_applied = FALSE) /\ (packet.payload.delivery_sequence = job_terminal_delivery_sequence)))
    \/ /\ (packet.variant = "MarkDeliveryApplied") /\ (job_phase = "NeedsAttention") /\ (((job_terminal_delivery_applied = FALSE) /\ (packet.payload.delivery_sequence = job_terminal_delivery_sequence)))
    \/ /\ (packet.variant = "MarkDeliveryApplied") /\ (job_phase = "Succeeded") /\ ((job_terminal_delivery_applied /\ (packet.payload.delivery_sequence = job_terminal_delivery_sequence)))
    \/ /\ (packet.variant = "MarkDeliveryApplied") /\ (job_phase = "Failed") /\ ((job_terminal_delivery_applied /\ (packet.payload.delivery_sequence = job_terminal_delivery_sequence)))
    \/ /\ (packet.variant = "MarkDeliveryApplied") /\ (job_phase = "Cancelled") /\ ((job_terminal_delivery_applied /\ (packet.payload.delivery_sequence = job_terminal_delivery_sequence)))
    \/ /\ (packet.variant = "MarkDeliveryApplied") /\ (job_phase = "WorkerLost") /\ ((job_terminal_delivery_applied /\ (packet.payload.delivery_sequence = job_terminal_delivery_sequence)))
    \/ /\ (packet.variant = "MarkDeliveryApplied") /\ (job_phase = "NeedsAttention") /\ ((job_terminal_delivery_applied /\ (packet.payload.delivery_sequence = job_terminal_delivery_sequence)))
...
CoreNext ==
    ...
    \/ \E arg_delivery_sequence \in 0..2 : job_ApplySucceededDelivery(arg_delivery_sequence)
    \/ \E arg_delivery_sequence \in 0..2 : job_ApplyFailedDelivery(arg_delivery_sequence)
    \/ \E arg_delivery_sequence \in 0..2 : job_ApplyCancelledDelivery(arg_delivery_sequence)
    \/ \E arg_delivery_sequence \in 0..2 : job_ApplyWorkerLostDelivery(arg_delivery_sequence)
    \/ \E arg_delivery_sequence \in 0..2 : job_ApplyNeedsAttentionDelivery(arg_delivery_sequence)
    \/ \E arg_delivery_sequence \in 0..2 : job_ObserveSucceededDeliveryAlreadyApplied(arg_delivery_sequence)
    \/ \E arg_delivery_sequence \in 0..2 : job_ObserveFailedDeliveryAlreadyApplied(arg_delivery_sequence)
    \/ \E arg_delivery_sequence \in 0..2 : job_ObserveCancelledDeliveryAlreadyApplied(arg_delivery_sequence)
    \/ \E arg_delivery_sequence \in 0..2 : job_ObserveWorkerLostDeliveryAlreadyApplied(arg_delivery_sequence)
    \/ \E arg_delivery_sequence \in 0..2 : job_ObserveNeedsAttentionDeliveryAlreadyApplied(arg_delivery_sequence)
...
CoverageInstrumentation == RouteCoverage_job_terminal_enters_runtime_inbox /\ RouteCoverage_runtime_delivery_commit_acknowledges_job_outbox /\ RouteCoverage_runtime_delivery_reuse_acknowledges_job_outbox

CiStateConstraint == /\ model_step_count <= 8 /\ Len(pending_inputs) <= 8 /\ Cardinality(observed_inputs) <= 10 /\ Len(pending_routes) <= 8 /\ Cardinality(delivered_routes) <= 0 /\ Cardinality(emitted_effects) <= 0 /\ Cardinality(observed_transitions) <= 8 /\ Cardinality(runtime_delivery_delivery_ids) <= 0 /\ Cardinality(DOMAIN runtime_delivery_delivery_sequences) <= 0 /\ Cardinality(DOMAIN runtime_delivery_delivery_source_sequences) <= 0 /\ Cardinality(runtime_delivery_committed_sequences) <= 0
DeepStateConstraint == /\ model_step_count <= 6 /\ Len(pending_inputs) <= 2 /\ Cardinality(observed_inputs) <= 6 /\ Len(pending_routes) <= 2 /\ Cardinality(delivered_routes) <= 2 /\ Cardinality(emitted_effects) <= 2 /\ Cardinality(observed_transitions) <= 6 /\ Cardinality(runtime_delivery_delivery_ids) <= 2 /\ Cardinality(DOMAIN runtime_delivery_delivery_sequences) <= 2 /\ Cardinality(DOMAIN runtime_delivery_delivery_source_sequences) <= 2 /\ Cardinality(runtime_delivery_committed_sequences) <= 2
WitnessStateConstraint_runtime_delivery_first_commit == /\ model_step_count <= 12 /\ Len(pending_inputs) <= 8 /\ Cardinality(observed_inputs) <= 13 /\ Len(pending_routes) <= 4 /\ Cardinality(delivered_routes) <= 3 /\ Cardinality(emitted_effects) <= 6 /\ Cardinality(observed_transitions) <= 12 /\ Cardinality(runtime_delivery_delivery_ids) <= 2 /\ Cardinality(DOMAIN runtime_delivery_delivery_sequences) <= 2 /\ Cardinality(DOMAIN runtime_delivery_delivery_source_sequences) <= 2 /\ Cardinality(runtime_delivery_committed_sequences) <= 2
