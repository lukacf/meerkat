...
- Open PRs that must merge before 0.8.34, in this order:
  1. **#1110** `fix/sdk-child-stderr-and-close` @ `136bc2037` (draft). Fixes #1103 (HomeCore SDK findings). Stacked on the #1109 tree, so it is mergeable as is. Its pre-push gate was RUNNING in worktree `/home/luka/src/wt/meerkat-dsdk` when this handover was written (log `/tmp/rb/push-dsdk.log`); if that gate finished green the exact-tree evidence is cached and a re-push is instant; otherwise run the gate (section 3). Author-verified: fmt, clippy on mcp/rpc/rest, Python 542 tests, 6 binary tests.
...
1. **#1110.** If `/tmp/rb/push-dsdk.log` ends with `exit=0`, the gate passed and the branch tree is already on origin; otherwise from `/home/luka/src/wt/meerkat-dsdk`: `git push --force-with-lease origin fix/sdk-child-stderr-and-close` (the gate runs; hosted CI starts on completion). Mark ready (`gh pr ready 1110`), wait for all checks (18-19 total, none pending), squash-merge, `git fetch`, confirm `git rev-parse origin/main^{tree}` equals the branch tree.
