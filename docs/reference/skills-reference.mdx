...
- `turn_metadata.skill_references` carries plain `SkillKey` objects for first-turn/session creation and per-turn injection:
...
Legacy tagged `SkillRef` wrappers, top-level `preload_skills`, top-level `skill_refs`, and string refs are rejected on wire/API surfaces.
