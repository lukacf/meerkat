...
| `roster().await` / `list_meerkats().await` | Membership inspection |
| `spawn_member_ref(...).await` | Spawn member |
| `spawn_many_member_refs(Vec<SpawnMemberSpec>).await` | Batch spawn members in parallel (results preserve input order) |
| `spawn_many(Vec<SpawnMemberSpec>).await` | Batch spawn compatibility API returning session IDs |
| `retire(...).await` | Retire member |
| `wire(...).await` / `unwire(...).await` | Peer graph mutation |
| `external_turn(...).await` / `internal_turn(...).await` | Turn dispatch |
...
async fn orchestrate(
    ...
    handle
        ...
        .await?;
    handle
        .external_turn(MeerkatId::from("lead-1"), "Coordinate work.".to_string())
        .await?;
