...
- **First-turn or per-turn injection**: raw RPC/REST requests use `turn_metadata.skill_references: SkillKey[]`; SDKs may expose `skill_refs: SkillRef[]` convenience options that resolve into that runtime metadata carrier.
