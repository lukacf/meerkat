...
> behavior is represented by `meerkat-live`, `crates/meerkat-core/src/live_adapter.rs`,
...
**Sample sites.**
- `crates/meerkat-session/src/ephemeral.rs:2548-2572` — top-level `select!` races `run_fut`
...
- `crates/meerkat-mob/src/runtime/actor.rs:3320` — `spawn_fut.catch_unwind().await` converts
  panics to `MobError::Internal`, but outer actor cancellation mid-spawn orphans
  `spawn_ticket` with no `SpawnProvisioned` reply.
- `crates/meerkat-mob/src/runtime/actor.rs:6102` — flow-stream `select!`; on actor shutdown
  the completion signal is lost.
- `crates/meerkat-rpc/src/realtime_ws.rs:2891` — realtime WS `select!` over DSL + audio +
...
- `crates/meerkat-anthropic/src/client.rs:966` — streaming `while let Some(chunk) = stream.next().await`;
...
**Sample sites.**
- `crates/meerkat-cli/src/main.rs:140-142` — `TuiPipeline` holds three `Option<JoinHandle<_>>`;
  only one `.abort()` at 5452; other handles may leak.
- `crates/meerkat-rest/src/lib.rs:2169-2195` — `spawn_event_forwarder` → `drain_event_forwarder`
  hand-off window; panic in between leaks the task.
- `crates/meerkat-store/src/realm.rs:114-214` — realm lease heartbeat; abort only in optional
...
- `crates/meerkat-comms/src/inproc.rs:108+` — global `InprocRegistry` never unregisters
...
**Sample sites.**
- `crates/meerkat-anthropic/src/client.rs:19` — `DEFAULT_REQUEST_TIMEOUT = 300s` hardcoded,
  not in `Config`.
- `crates/meerkat-anthropic/src/client.rs:1483-1484` — stub tests hardcode 5s/120s; divergent
  from production.
- `crates/meerkat-store/src/realm.rs:89-91` — 30s stale + 20ms retry + 5s timeout; three magic
  numbers, no relational test.
- `crates/meerkat-store/src/realm.rs:612,952,1043` — three copy-pasted 2s/10ms busy-wait loops,
  no shared budget.
- `crates/meerkat-session/src/ephemeral.rs:45` — `EVENT_CHANNEL_CAPACITY = 256` for both mpsc
...
**Sample sites.**
- `crates/meerkat-session/src/event_store.rs:25` — `EVENT_SCHEMA_VERSION: u32 = 1`; never
  bumped; no v0→v1 test. A silent bump breaks every `.rkat/` session.
- `crates/meerkat-mob/src/event.rs:218,529` — mob event v6 with `bail!` on mismatch; no
  migration. v5 unrecoverable on upgrade.
- `crates/meerkat-mob/src/event.rs:921` — test writes `version+1` to assert rejection. Policy
  is "refuse unknown," not surfaced to users.
- `crates/meerkat-contracts/src/wire/session.rs:362` — `unwrap_or(Value::Null)` silently
  downgrades malformed tool args to null — indistinguishable from legit "no args."
- `crates/meerkat-rpc/src/handlers/session.rs:61-102` — 15 consecutive `#[serde(default)]`;
...
**Sample sites.**
- `crates/meerkat-rest/src/lib.rs:2362,5771` + `crates/meerkat-rpc/src/session_runtime.rs:3741` —
  three `mpsc::unbounded_channel()` in production paths.
- `crates/meerkat-session/src/ephemeral.rs:1487` — broadcast(256); `Lagged` logged + continued
  at 2568, subscriber events dropped silently.
- `crates/meerkat-session/src/ephemeral.rs:2316,2343` — unbounded `blocks.extend`; compaction
  is only backpressure and it's optional.
- `crates/meerkat/src/service_factory.rs:778-1233` — 13 `mpsc::channel(8)` with immediate
...
**Sample sites.**
- `crates/meerkat-anthropic/src/client.rs:17` — pool idle 90s; `reqwest` doesn't notify the
  machine when it recycles connection; next request silently reconnects.
- `crates/meerkat-core/src/error.rs:327-332,433` — `retry_after` parsed but no explicit
  `WaitingForRetryAfter` state; delay hidden in retry layer.
- `crates/meerkat-store/src/realm.rs:89` — 30s stale; a paused remote loses lease without
  local reflection until refresh.
- `crates/meerkat-openai/src/live.rs:2069` — `conversation_id: None` — future OpenAI changes
...
**Sample sites.**
- `crates/meerkat-comms/src/io_task.rs:39,63` — `require_peer_auth: bool` is a parameter;
  caller must remember `true`. No compile-time guarantee prod path sets it.
- `crates/meerkat-comms/src/inbox.rs:483 vs 505` — TOCTOU: 483 reads `trusted_peers` under
  queue lock; 505 re-reads separately. Trust revoked in between ⇒ enqueue-accept /
  dequeue-reject split.
- `crates/meerkat-comms/src/inproc.rs:108+` — 17 `pub fn` on global `InprocRegistry`; any
  caller can `register_with_meta_in_namespace` impersonating another pubkey.
- `crates/meerkat-contracts/src/wire/session.rs:362` — `unwrap_or(Value::Null)` turns
...
**Sample sites.**
- `crates/meerkat-session/src/ephemeral.rs` — `blocks.extend(...)` accepts arbitrary
  block-count per turn; runaway provider bloats memory.
- `crates/meerkat-contracts/src/wire/session.rs:815+` — `WireContentBlock` round-trips
  arbitrary `Value`; no size limit on tool-arg raw values.
- `crates/meerkat-comms/src/io_task.rs` — envelope size only transport-bounded; 1 GB signed
...
**Sample sites.**
- `crates/meerkat-mob/src/runtime/actor.rs:3320` — `spawn.catch_unwind()` → `MobError::Internal`,
  but partial mutation inside spawn closure is preserved. No rollback.
- `crates/meerkat-mob/src/runtime/actor_turn_executor.rs:269` — `reconcile.catch_unwind()`;
  same; reconcile may have half-applied to session.
- `crates/meerkat-core/src/model_profile/capabilities.rs:222` +
...
**Sample sites.**
- `crates/meerkat-mob/src/runtime/flow_frame_engine.rs:314-337` — `0..=max_retries` then
  `bail!`; no spec for post-bail cascade.
- `crates/meerkat-rpc/src/session_runtime.rs:3741` — unbounded lifecycle-channel drop
  triggers nothing; downstream assumes liveness.
- `crates/meerkat-session/src/ephemeral.rs:2568` — `event_stream_open = false` on sink drop;
...
**Sample sites.**
- `crates/meerkat-anthropic/src/client.rs:1483-1484` — test timeouts 5s/120s vs prod 30s/300s;
  tests pass, prod fails at 5s.
- `crates/meerkat/src/service_factory.rs:778-1233` — 13 `_rx` drops; harness exercises a path
  production won't.
...
1. **`crates/meerkat-session/src/ephemeral.rs:2548-2572`** — model the three-way `select!`
...
2. **`crates/meerkat-mob/src/runtime/actor.rs:3320` + `actor_turn_executor.rs:269`** — pair
   `catch_unwind` with `TransitionGuard::snapshot_and_restore`; teach the mob authority
   the "partial-transition" state.
3. **`crates/meerkat-anthropic/src/client.rs:19` + the 300s/5s/120s cluster** — surface
   `TimingProfile` through config; TLC annotates `WaitingForLlm` with deadline.
4. **`crates/meerkat-rest/src/lib.rs:2362,5771` + `crates/meerkat-rpc/src/session_runtime.rs:3741`** —
   kill the 3 `unbounded_channel` sites; pick `Capacity` constants and document
   overflow policy (drop-oldest / reject / block).
5. **`crates/meerkat-comms/src/io_task.rs:39,63`** — promote `require_peer_auth` from
