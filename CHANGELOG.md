...
- Behaviour-only: `Usage::add` and `CumulativeUsage::add_turn` now aggregate
  ...
  clearing them. Cache reads are clamped to the input total and cache writes to
  what reads leave, so reads plus writes never exceed input; reasoning is
  clamped to output. `Usage::cumulative_delta_since` is new.
...
- A run that suspends for callback results continues its usage account only
  when that run's staged callback results are applied and the run resumes
  through `run_pending`. Any other run discards the suspended account instead
  of absorbing its calls. The carry-over is in memory, so an agent rebuilt from
  storage resumes with a fresh account.
- Pre-0.8.22 sessions report normalized usage everywhere: session read and list
  ...
  compaction on such a session no longer moves the reported total.
...
- The pre-push dispatcher now bounds its own cache footprint. Every source
  ...
  removed, even after the source worktree was deleted. After a passed gate,
  and only then, `scripts/pre-push-prune-lanes.sh` keeps the
  ...
  hook-cache and `targets/<schema>/<toolchain>/` roots are candidates. A lane
  is never removed while it is the current lane, its dispatcher lock is held
  by a live process, any live process names it in its command line,
  environment, or working directory, or any file beneath it changed within
