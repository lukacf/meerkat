...
No surface constructs agents directly. `AgentFactory` consolidates provider resolution, tool dispatch wiring, comms setup, hook resolution, and skill injection into a single construction pipeline.
...
That ownership is expressed through the explicit runtime-binding contract:

- runtime-backed surfaces obtain `SessionRuntimeBindings` from `MeerkatMachine::prepare_bindings(session_id)`
- those bindings flow through `SessionBuildOptions.runtime_build_mode = RuntimeBuildMode::SessionOwned(bindings)`
- standalone/testing/embedded paths opt into `RuntimeBuildMode::StandaloneEphemeral`
...
`SessionService` defines the substrate lifecycle. Production surfaces (CLI, REST, RPC, MCP) use `PersistentSessionService` under a `MeerkatMachine` that owns `keep_alive`, Queue/Steer routing, comms drain, and ingress admission. `EphemeralSessionService` is the in-memory substrate for testing and embedded use — it supports Queue-only turns and rejects runtime-owned semantics (Steer, render_metadata). The `.rkat/sessions/` files are derived projections materialized by `SessionProjector` — delete them and replay from the event store to get identical content.
...
The named test lanes exist because the architecture makes them possible. Core has no I/O, so unit tests need no mocks. Every trait has a test implementation. The repo separates `unit`, `integration-fast`, deterministic `e2e-fast`, real-local-resource `e2e-system`, targeted live-provider `e2e-live`, and compound live-provider `e2e-smoke` coverage, with Cargo aliases and hooks aligned to those lanes.
