...
| `make test-public-live-contracts` | Native public Live representation, retained-ledger, scope, and loopback protocol contracts. |
| `make test-public-live-consumers` | Build/test independent WS and RTC consumer workspaces; inspect their locked Cargo graphs for public-only feature closure and transport separation. |
| `make check-public-live-feature-isolation` | Independent consumer checks plus separate Cargo feature checks without cross-row feature unification. |
| `make test-public-live-browser-contract` | Compile and execute the WASM browser contract in headless Chrome through `wasm-pack`. |
...
The public Live contract targets use Cargo directly; they do not select the
BuildBuddy backend. These checks require no provider credentials and are not
real-provider or public-profile activation evidence. The browser target runs
the actual Rust/WASM browser suite; a Web SDK TypeScript test run alone does
not replace it. It requires `wasm-pack` and a working Chrome/WebDriver setup.
Put a `chromedriver` compatible with the installed Chrome version on `PATH`;
the driver automatically downloaded by `wasm-pack` can target a newer browser.
