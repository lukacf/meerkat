...
Startup distinguishes **Starting voice** from **Connecting audio**. Once audio
is active, the strip separately shows **Reading context**, **Preparing context**,
or **Sending context** while the initial summary is prepared and supplied.
You can speak, mute, and send text during preparation. **Context supplied** means
the provider acknowledged it, not that the model has recalled or spoken it.
Context failures stay visible without disabling the connected audio; a failed
status read is shown separately and retried.

Context preparation is a separate, tool-free summary request using the background
agent's model and credentials; it does not change that agent's model or
reasoning settings. The summary targets compact notes rather than replaying
logs, and uses low or disabled OpenAI reasoning where the model catalog supports
it. Meerkat orders the captured history and newer conversation updates, and
cancels or fences context work when the call closes or changes. A context job
from an old call cannot update a replacement call.
...
<Steps>
  ...
  <Step title="Send message">
    ...
    UUID per send. Reusing a key with different content is rejected with HTTP
  ...
  <Step title="Receive response frames">
    ...
    in the [JSON-RPC reference](/mobkit/api/rpc)).
...
`/healthz` remain public. See [authentication](/mobkit/guides/authentication) for the
