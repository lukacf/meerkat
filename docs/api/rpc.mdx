...
| `runtime/realtime_attachment_status` | Runtime | Read runtime-backed realtime attachment status |
...
| `mob/realtime_attach` | Mob | Attach realtime attachment intent to a member (mob feature) |
| `mob/realtime_detach` | Mob | Detach realtime attachment intent from a member (mob feature) |
...
<Note>
Public documentation uses `runtime/realtime_attachment_status`,
`mob/realtime_attach`, and `mob/realtime_detach` for the realtime attachment capability.
</Note>
...
```json Response
{
  ...
  "result": {
    ...
    "methods": [
      ...
      "runtime/state", "runtime/realtime_attachment_status", "runtime/accept", "runtime/retire", "runtime/reset",
      ...
      "mob/spawn_helper", "mob/fork_helper", "mob/force_cancel", "mob/realtime_attach", "mob/realtime_detach", "mob/member_status",
      "mob/wait_kickoff", "mob/turn_start",
...
Attach, detach, and inspect the runtime-owned realtime (audio) channel bound to a session or mob member. See the [Realtime voice guide](/guides/realtime) for the attachment state machine, the `RealtimeAttachmentSignalAuthority` authority-epoch model, and the `reconfigure_live_topology` flow for swapping provider/model on a realtime-attached session.

### mob/realtime_attach

Stage durable realtime intent for a mob member. Intent is keyed on `AgentIdentity` and survives member respawn; the underlying binding epoch rotates per-runtime.
...
```json Response
{"jsonrpc": "2.0", "id": 20, "result": {"attached": true}}
```
</CodeGroup>

<ParamField body="mob_id" type="string" required>
  Mob ID.
</ParamField>

<ParamField body="agent_identity" type="string" required>
  Stable per-member identity. Must reference an existing mob member.
</ParamField>

Typed wire contract: `MobRealtimeAttachParams` / `MobRealtimeAttachResult`.

### mob/realtime_detach

Clear realtime intent for a mob member. Idempotent.
...
```json Response
{"jsonrpc": "2.0", "id": 21, "result": {"detached": true}}
```
</CodeGroup>
...
Typed wire contract: `MobRealtimeDetachParams` / `MobRealtimeDetachResult`.
...
`mob/member_status` responses include the same value under the `realtime_attachment_status` field for session IDs bound to mob members.

### Realtime bootstrap methods (`realtime/*`)

In addition to the attachment lifecycle above, three product-layer methods handle opening and inspecting realtime WebSocket channels:

| Method | Purpose |
|--------|---------|
| `realtime/open_info` | Issue a single-use bootstrap token for opening a realtime channel |
| `realtime/status` | Read product-layer realtime channel status for a session or mob member |
| `realtime/capabilities` | Read product-layer realtime capabilities (supported input/output kinds, interrupt/transcript support) |

These are emitted by `rkat-rpc`'s realtime WebSocket listener. The lifecycle methods above (`mob/realtime_attach`, `mob/realtime_detach`, `runtime/realtime_attachment_status`) are the authoritative attachment surface — the `realtime/*` methods are the public product layer built on top.
