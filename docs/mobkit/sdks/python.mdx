...
<Note>
  ...
  [source-build instructions](/mobkit/quickstart#install). Pass that binary's path to
  `.gateway(...)`.
</Note>

## Quick start

In a fresh project directory, create `config/mob.toml`:
...
Set `OPENAI_API_KEY` in your environment for the model above, or choose a
supported model and its matching provider credentials. Run the following
script from that project directory, replacing `/path/to/rpc_gateway` with
the binary you installed. Profiles are templates, not pre-created members,
so the example explicitly creates an ephemeral worker before sending.
...
<Warning>
  ...
  network. See [Authentication](/mobkit/guides/authentication) and
  [Deployment](/mobkit/guides/deployment) for authenticated hosting. The Python
  subscription helpers below do not forward bearer tokens; configuring
  `.auth(...)` alone does not authenticate those SSE requests.
</Warning>
...
# Wait for an identity completion after the pre-delivery cursor baseline,
# even if consecutive answers have identical text. This is not request
# correlation: concurrent/in-flight work can satisfy the same barrier.
...
completion cursor advances; the preview is not request-correlated. Interpret
it as this send's answer only if you exclude all other completion-producing
work for the identity throughout the wait, including already-running kickoff
turns and peer or background work.

### DurableAgentSpec fields

The roster provider returns one `DurableAgentSpec` per identity. The SDK
serializes the shared wire shape that Rust deserializes: `identity`, `profile`,
...
including unset `runtime_mode_override` and `initial_message`.
...
| `runtime_mode_override` | `str`, optional | `autonomous_host` or `turn_driven`. `None` defers to the profile's `runtime_mode`, whose meerkat default is `autonomous_host`. Ambient per-turn memory requires `turn_driven` (see the [configuration reference](/mobkit/reference/configuration)) |
...
| `previous_identities` | `list[str]` | Call-site-dependent prior-identity context, not an authoritative membership snapshot on every callback. Topology queries and edge reconciliation supply currently registered identities. Bootstrap, full roster refresh/reconcile, and reset-time spec adoption currently pass an empty list; an empty list does not imply that no identities are registered. |
...
| `.on_error(callback)` | Error hook. Receives every runtime `ErrorEvent` (`category`, `message`, `context`) the gateway forwards as the `mobkit/on_error` notification; sync or async, fire-and-forget. Without it the events reach only the gateway's normal log sink: ERROR for failures, INFO for `actor_loop_recovered`, subject to configured tracing filters and stderr handling (see [Error hook and logging](/mobkit/guides/unified-runtime#error-hook-and-logging)) |
...
[roster and member lifecycle](/mobkit/concepts/roster#role-migration).
...
refused at init. See [Deployment](/mobkit/guides/deployment).
...
registered with the public `.agent_customizer(customizer)`. It runs when an
identity is actually built or re-materialized, including fresh creation and
...
handlers. The hook receives the durable spec (so `spec.profile` is the profile
name), and registers tools on the draft:
...
caching is one such knob. With no profile or request override, the pinned Meerkat
