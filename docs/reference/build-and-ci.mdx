...
pushes to `main`, `ci/**`, `feat/**`, and `feature/**`, and manual
dispatches, entirely with Cargo on GitHub-hosted `ubuntu-latest` runners
(4 vCPU), and it is sized to a 20-minute push-to-terminal budget:

| Job | Runs | Command |
| --- | --- | --- |
| `changes` | always | `scripts/ci-cargo-lanes.mjs`: classifies the changed paths into directly changed packages, bounded shards, the reverse-dependency closure, and conditional-lane flags |
| `fmt-governance` | always | `make fmt-check docs-check semver-breaks-selftest verify-version-parity verify-lock-consistency verify-bazel-module-lock-inputs check-rust-release-config ci-lanes-selftest` plus the release doctor contract test |
| `ratchets` | always (steps conditional) | schema, SDK codegen, event inventory, RPC/REST surface, and SDK wrapper freshness when generated-contract paths changed; `machine-check-drift` and `protocol-check-drift` when machine-authority paths changed |
| `clippy` (one lane per shard) | Rust-relevant change | `repo-cargo clippy -p <changed packages> --no-deps --all-targets --all-features -- -D warnings` |
| `unit` (one lane per shard) | Rust-relevant change | `repo-cargo nextest run -p <changed packages> --lib --bins --profile fast` |
| `closure-check` | Rust-relevant change | `repo-cargo check -p <reverse-dependency closure> --all-targets --all-features` (catches downstream compile breaks the direct lanes miss) |
| `wasm-check` | web-runtime, contracts, or workspace-wide change | `make wasm-check` |
| `sdk-host` | `sdks/python`, `sdks/typescript`, or SDK tooling change | `make test-sdk-python test-sdk-typescript` |
| `gate` (`CI gate`) | `!cancelled()` | fail-closed aggregate, 1200-second budget from run creation, schema-4 attestation on `main` pushes |
...
path escalates to the whole workspace (packed into at most six shards); a
...
Pull request CI uses one concurrency group per PR (`pr-<number>`, or
`pr-<ref>` for pushes) with `cancel-in-progress: true`. Starting another
PR-associated attempt in that group automatically cancels the older
in-progress attempt, even when nobody issues an explicit cancel command. The
