...
use meerkat_mob::ids::{FlowId, MobId, ProfileName, StepId};
use meerkat_mob::profile::{Profile, ProfileBinding};
use meerkat_mob::{MobDefinition, MobHandle, MobRun, RunId, SpawnMemberSpec};
...
impl AdaptiveDriver {
    ...
    pub async fn initialize_run(
        ...
        started_at_ms: u64,
    ) -> Result<meerkat_mob::AdaptiveDriverCapability, AdaptiveError> {
        let limits = adaptive_run_limits_from_policy(policy, started_at_ms)?;
        Ok(self
            .control_mob
            .initialize_adaptive_run(meerkat_mob::InitializeAdaptiveRunRequest {
    ...
    pub async fn record_planning_decision(
        &self,
        capability: &meerkat_mob::AdaptiveDriverCapability,
        decision: &LayerDecision,
    ) -> Result<(), AdaptiveError> {
        let kind = match decision {
            LayerDecision::RunLayer { .. } => meerkat_mob::AdaptivePlanningDecisionKind::RunLayer,
            LayerDecision::Finish { .. } => meerkat_mob::AdaptivePlanningDecisionKind::Finish,
        };
        Ok(self
            .control_mob
    ...
    pub async fn resolve_layer_admission(
        ...
        observed_at_ms: u64,
    ) -> Result<meerkat_mob::AdaptiveLayerAdmission, AdaptiveError> {
        Ok(self
            .control_mob
            .resolve_adaptive_layer_admission(
                capability,
                meerkat_mob::AdaptiveLayerAdmissionRequest {
    ...
    pub async fn record_layer_provisioned(
        ...
        Ok(self
            ...
            .record_adaptive_layer_provisioned(
                capability,
                meerkat_mob::AdaptiveLayerAttempt {
                    layer_id: layer_id.as_str().to_string(),
                    attempt,
                },
            )
    ...
    pub async fn record_layer_run_started(
        ...
        layer_id: &LayerId,
        attempt: u64,
        child_run_id: meerkat_mob::RunId,
    ) -> Result<(), AdaptiveError> {
        Ok(self
            .control_mob
            .record_adaptive_layer_run_started(
                capability,
                meerkat_mob::AdaptiveLayerRunStart {
                    layer_id: layer_id.as_str().to_string(),
                    attempt,
                    child_run_id,
                },
    ...
    pub async fn record_layer_result_validated(
        ...
        Ok(self
            ...
            .record_adaptive_layer_result_validated(
                capability,
                meerkat_mob::AdaptiveLayerResultDigest {
                    layer_id: layer_id.as_str().to_string(),
                    attempt,
    ...
    pub async fn record_layer_setup_fault(
        ...
        layer_id: &LayerId,
        attempt: u64,
        fault: meerkat_mob::AdaptiveLayerSetupFault,
        spawned_members: u64,
        ...
        Ok(self
            ...
            .record_adaptive_layer_setup_fault(
                capability,
                meerkat_mob::AdaptiveLayerSetupFaultObservation {
                    layer_id: layer_id.as_str().to_string(),
                    attempt,
                    fault,
                    spawned_members,
    ...
    pub async fn record_layer_mob_retained(
        ...
        layer_id: &LayerId,
        attempt: u64,
        disposition: meerkat_mob::AdaptiveLayerDisposition,
    ) -> Result<(), AdaptiveError> {
        Ok(self
            .control_mob
            .record_adaptive_layer_mob_retained(
                capability,
                meerkat_mob::AdaptiveLayerRetention {
                    layer_id: layer_id.as_str().to_string(),
                    attempt,
                    disposition,
                },
    ...
    pub async fn snapshot(
        &self,
        capability: &meerkat_mob::AdaptiveDriverCapability,
    ) -> Result<meerkat_mob::AdaptiveRunSnapshot, AdaptiveError> {
...
pub enum AdaptiveLayerCleanup {
    Destroyed,
    Retained(meerkat_mob::AdaptiveLayerDisposition),
}
...
impl AdaptiveKernel for AdaptiveDriver {
    type Capability = meerkat_mob::AdaptiveDriverCapability;
        ...
        match decision {
            ...
            LayerDecision::RunLayer { plan, .. } => {
                ...
                let admission = kernel
                    ...
                    .await?;
                if !matches!(admission, meerkat_mob::AdaptiveLayerAdmission::Allowed) {
                    continue;
                }
...
pub fn adaptive_run_limits_from_policy(
    policy: &AdaptivePolicy,
    started_at_ms: u64,
) -> Result<meerkat_mob::AdaptiveRunLimits, AdaptiveError> {
    policy.limits.validate_complete("composed")?;
    Ok(meerkat_mob::AdaptiveRunLimits {
...
mod tests {
    ...
    impl AdaptiveKernel for FakeKernel {
        ...
        async fn resolve_layer_admission(
            ...
            Ok(meerkat_mob::AdaptiveLayerAdmission::Allowed)
        }
        ...
        async fn record_layer_setup_fault(
            ...
            layer_id: &LayerId,
            _attempt: u64,
            _fault: meerkat_mob::AdaptiveLayerSetupFault,
        ...
        async fn record_layer_mob_retained(
            ...
            layer_id: &LayerId,
            _attempt: u64,
            _disposition: meerkat_mob::AdaptiveLayerDisposition,
        ) -> Result<(), AdaptiveError> {
