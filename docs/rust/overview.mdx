...
Before running the model turn below, make credentials available for its
Claude model through a compatible configured [auth binding](/guides/auth) or
an Anthropic environment key. For a clean API-key setup:

```bash
export ANTHROPIC_API_KEY="<your-anthropic-api-key>"
```

A compatible existing binding can be used instead; another provider's key
does not authenticate this Claude request. Merely constructing the
...
match event {
    ...
    AgentEvent::ExtractionSucceeded { session_id, structured_output, schema_warnings, origin, .. } => {}
    ...
    // Durable steer: mid-turn appends and exact discarded applications
...
Reconcile `BoundaryAppendApplied` with `BoundaryAppendsDiscarded` for the same
session, run, and input IDs, and with canonical transcript history. The discard
payload carries `session_id`, `run_id`, and `input_ids`; the applied event can
include canonical `notices` and the application-time `transcript_start` offset.
A run terminal alone does not prove discard. A discard does not terminalize
the input or invalidate its later application in another run; retained
