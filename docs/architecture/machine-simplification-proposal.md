...
| Behavior-bearing runtime-control state should stay outside MeerkatMachine | rejected | `current_run_id` is now absorbed back into the checked-in Meerkat machine, the stale handwritten control booleans are gone, and the live wake/process carrier currently runs through ingress + `post_admission_signal` rather than through a separate handwritten control state vector. The remaining work is to decide which of the still-live internal control/recovery semantics should be lifted further into the two-machine model, not to keep punting them to handwritten shell state. |
