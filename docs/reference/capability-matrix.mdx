...
family. Where the host can deliver a later completion (the JSON-RPC, REST and
MCP servers, `rkat run --keep-alive`), the `council` agent tool returns
`status: "running"` with a `job_id`; the structured outcome is appended to the
convener's session as a durable System message and delivered as that
background job's completion. On a one-shot host (`rkat run` without
`--keep-alive`) the call blocks and returns it through the ordinary
tool-result channel. `fork_off` follows the same split. Cleanup debt is deliberately not a
