...
/// Exact document-owned observation source incorporated by this attempt.
/// Canonical message revisions alone do not cover this component.
...
impl CompactionObservationSource {
    ...
    fn from_observations(
        observations: &[crate::LiveUnmeasuredAssistantObservation<'_>],
...
mod tests {
    ...
    fn unmeasured_voice(message: &Message, expected: &str) -> bool {
        matches!(message, Message::User(user)
            if user.transcript_role.is_injected_context()
                && user.text_content().contains(expected)
    ...
    fn session_with_unmeasured_voice() -> Session {
        ...
        assert_eq!(session.live_unmeasured_assistant_observations().count(), 1);
        session
    }
    ...
    fn retain_voice_observation(session: &mut Session, item: &str, text: &str) {
        ...
        session.append_realtime_transcript_event(
            crate::RealtimeTranscriptEvent::AssistantPlaybackTerminalSettled {
                channel_id: channel.to_string(),
                interaction_id: interaction,
                response_id: item.into(),
                item_id: item.into(),
                content_index: 0,
                settlement: Box::new(crate::LiveAssistantPlaybackSettlement {
    ...
    fn ordinary_text_context_includes_unmeasured_voice_after_reload() {
        ...
        assert_eq!(
            ...
            2,
            "observation data is not a fabricated canonical assistant turn"
        );
        ...
        assert_eq!(projected[2], reloaded.messages()[1]);
    ...
    #[tokio::test]
    async fn stale_compaction_cannot_consume_later_observation_only_changes() {
        ...
        assert_eq!(session.transcript_revision().unwrap(), revision);
        ...
        assert_eq!(session.live_unmeasured_assistant_observations().count(), 2);
    }
    ...
    async fn compaction_integrates_voice_observations_before_rebase_and_reload() {
        ...
        assert!(
            ...
            "synthetic observation rows cannot masquerade as canonical discard offsets"
        );
        session
            .replace_messages_for_compaction_internal(
                outcome.new_messages,
        ...
        assert_eq!(
            session.live_unmeasured_assistant_observations().count(),
            0,
