...
async fn create_session_inner(
        ...
        .await
    {
        Ok(admission) => admission,
        Err(err) => {
            return failed_terminal(req_ctx.as_ref(), ApiError::Conflict(err.to_string()));
        }
    };
    ...
    match result {
        Ok(run_result) => committed_terminal(
            req_ctx.as_ref(),
            Ok(Json(run_result_to_response(run_result, &state.realm))),
        ),
        Err(err) => {
            ...
            let message = match err {
                ...
                other => return committed_terminal(req_ctx.as_ref(), Err(other)),
            };
            committed_terminal(
                req_ctx.as_ref(),
                Err(ApiError::InternalWithData {
...
async fn continue_session_inner(
    ...
    #[cfg(feature = "mcp")]
    let mut mcp_boundary_committed = false;
    #[cfg(not(feature = "mcp"))]
...
async fn apply_mcp_boundary_to_turn_prompt(
    ...
    let mut mcp_text = String::new();
    apply_mcp_boundary(state, session_id, event_tx, &mut mcp_text, None).await?;
    if !mcp_text.is_empty() {
...
mod tests {
    ...
    fn assert_respond_without_publish_error<'a>(
        terminal: &'a RequestTerminal<Result<Json<SessionResponse>, ApiError>>,
    ) -> &'a ApiError {
        assert!(
            terminal.is_respond_without_publish(),
            "expected respond-without-publish terminal, got {terminal:?}"
        );
        terminal
            .payload()
            .as_ref()
            .expect_err("expected error terminal payload")
    }
    ...
    async fn rest_create_post_prepare_validation_failure_cleans_unpublished_state_without_request_context()
     {
        ...
        let ApiError::BadRequest(message) = assert_respond_without_publish_error(&outcome) else {
            panic!("expected bad request validation failure, got {outcome:?}");
        };
        assert!(
    ...
    async fn rest_continue_capacity_full_rejects_before_input_accept() {
        ...
        let ApiError::Conflict(message) = assert_respond_without_publish_error(&outcome) else {
            panic!("expected capacity conflict before runtime accept, got {outcome:?}");
        };
        assert!(
    ...
    async fn rest_continue_rebuild_accept_failure_discards_live_capacity() {
        ...
        let ApiError::Internal(message) = assert_respond_without_publish_error(&outcome) else {
            panic!("expected stopped-runtime accept failure, got {outcome:?}");
        };
        assert!(
    ...
    async fn rest_continue_rebuild_unpublished_cleanup_retries_after_active_completion() {
        ...
        let ctx = executor
            .try_begin_request(
                "rest-rebuild-unpublished-cleanup",
                meerkat::surface::SurfaceRequestKind::SessionTurn,
                noop_request_action(),
            )
        ...
        assert!(matches!(
            ...
            Ok(meerkat::surface::CompleteOutcome::Completed)
        ));
        assert!(
            state
                .runtime_adapter
    ...
    async fn rest_continue_capacity_full_rejects_before_keep_alive_persist() {
        ...
        let ApiError::Conflict(message) = assert_respond_without_publish_error(&outcome) else {
            panic!("expected capacity conflict before keep_alive persist, got {outcome:?}");
        };
        assert!(
            message.contains("Max sessions"),
            "capacity rejection should mention max sessions: {message}"
    ...
    async fn rest_continue_capacity_full_rejects_before_mcp_boundary_apply() {
        ...
        let ApiError::Conflict(message) = assert_respond_without_publish_error(&outcome) else {
            panic!("expected capacity conflict before MCP boundary, got {outcome:?}");
        };
        assert!(
            message.contains("Max sessions"),
            "capacity rejection should mention max sessions: {message}"
    ...
    fn test_continue_session_binds_lifecycle_before_mcp_boundary() {
        ...
        let rebuild_start = body
            .find("let final_result = if requires_rebuild")
            .expect("continue_session_inner should have a rebuild path");
        let live_start = body
            .find("    } else {\n        let runtime_registration_lock")
            .expect("continue_session_inner should have a live continue path");
        let rebuild_body = &body[rebuild_start..live_start];
        let live_body = &body[live_start..];
        let rebuild_apply_mcp = rebuild_body
            .find("apply_mcp_boundary")
            .expect("rebuild continue should apply MCP turn boundary");
        let live_apply_mcp = live_body
            .find("apply_mcp_boundary")
            .expect("live continue should apply MCP turn boundary");
        let bind_rebuild = rebuild_body
            .find(".bind_runtime_session")
            .expect("rebuild continue path should bind runtime session lifecycle");
        let bind_live = live_body
            .find(".bind_runtime_session")
            .expect("live continue path should bind runtime session lifecycle");
        let install_cancel_rebuild = rebuild_body
            .find(".install_cancel_action_or_cancelled")
            .expect("rebuild continue path should install cancel action");
        let install_cancel_live = live_body
            .find(".install_cancel_action_or_cancelled")
            .expect("live continue path should install cancel action");

        assert!(
            bind_rebuild < rebuild_apply_mcp,
            "rebuild continue must bind request lifecycle before MCP staged side effects"
        );
        assert!(
            bind_live < live_apply_mcp,
            "live continue must bind request lifecycle before MCP staged side effects"
        );
        assert!(
            install_cancel_rebuild < rebuild_apply_mcp,
            "rebuild continue must install cancel action before MCP staged side effects"
        );
        assert!(
            install_cancel_live < live_apply_mcp,
            "live continue must install cancel action before MCP staged side effects"
        );
    }
    ...
    fn test_rebuild_continue_cleans_prepared_runtime_when_cancel_wins_before_cleanup() {
        ...
        let cleanup = body[cancel_branch..]
            .find("unregister_rest_runtime_if_new_idle_locked(")
    ...
    async fn test_continue_session_keep_alive_live_missing_failure_unregisters_new_runtime() {
        ...
        let ApiError::BadRequest(message) = assert_respond_without_publish_error(&outcome) else {
            panic!("expected live-missing keep_alive rejection, got {outcome:?}");
        };
        assert!(
