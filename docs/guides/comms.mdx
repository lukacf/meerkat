...
- **agent-scoped**: use `MobHandle::subscribe_agent_events(...)` or `mob/stream_open` (RPC) / `Mob.subscribe(meerkatId)` (Web SDK) / `mob.subscribe_member_events(meerkat_id)` (Python/TypeScript SDK)
- **mob-scoped**: use `MobHandle::subscribe_mob_events(...)` or `mob/stream_open` (RPC) / `Mob.subscribeAll()` (Web SDK) / `mob.subscribe_events()` (Python/TypeScript SDK)
