...
| --- | --- | --- | --- | --- | --- | --- |
| Durable member identity | MobKit `AgentIdentity` layer outside the current repo; `meerkat-mob::ids::MeerkatId`; `RosterEntry.meerkat_id` | `MobMachine` | Identity registry inside `MobMachine` | Compatibility alias from `MeerkatId` to `AgentIdentity` | Yes | split-owner truth; cross-surface field loss |
...
| Active incarnation binding (`AgentIdentity -> AgentRuntimeId -> hidden session runtime`) | `MobActor.roster`; `RosterEntry.member_ref`; `RosterEntry.session_id()` | `MobMachine` | Runtime-binding substate inside `MobMachine` | Compatibility view exposing `MemberRef` or `SessionId`; supporting capability indices such as `SessionBackend.runtime_sessions` and `PendingSpawnLineage.tasks` | Yes | split-owner truth; recovery/resume drift |
...
| Session-scoped runtime control phase | `RuntimeControlMachine`; `RuntimeSessionAdapter.sessions`; attachment publication and control-plane methods | `MeerkatMachine` | Top-level control region inside `MeerkatMachine` | Surface views exposed through service helpers | No | wake/drain race; recovery/resume drift |
...
| Tool-surface visibility and pending mutation truth | `ExternalToolSurfaceMachine`; `McpRouter.servers`; `RouterProjectionSnapshot`; `ToolScope` staging | `MeerkatMachine` | Tool-surface region inside `MeerkatMachine` | Visible tool snapshots; MCP notices | No | split-owner truth; cross-surface field loss |
...
| Comms drain and keep-alive lifecycle | `CommsDrainLifecycleMachine`; `RuntimeSessionAdapter.comms_drain_slots`; keep-alive drain task controls | `MeerkatMachine` | Drain-control region inside `MeerkatMachine` | Host keep-alive status views | No | wake/drain race |
| Session-scoped runtime recovery snapshot | persistent session store; runtime recovery and registration flow; `RuntimeSessionAdapter.sessions` | `MeerkatMachine` | Recovery region inside `MeerkatMachine` | Ready/recovered lifecycle receipts back to `MobMachine` | Indirectly, through lifecycle results | recovery/resume drift |
...
| Peer directory reachability and last send failure | `PeerDirectoryReachabilityMachine`; comms directory/transport layer | Perimeter transport layer | Perimeter | Peer health and reachability views | No | transport edge cases |
