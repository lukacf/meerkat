...
- **Forked session branches**: lightweight history branching without a separate sub-agent owner
...
- **Inbox**: bounded message queue drained at turn boundaries
...
In `0.5`, the standalone sub-agent subsystem is gone. Delegated work now routes through mob orchestration plus the canonical runtime/control-plane path, while `Session::fork()` remains as the lightweight history-branching primitive.
