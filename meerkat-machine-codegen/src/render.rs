...
pub fn render_machine_kernel_module(schema: &MachineSchema) -> String {
    ...
    let schema_fn_path = format!("meerkat_machine_schema::catalog::dsl::dsl_{module_name}_machine");
    render_machine_kernel_module_with_paths(schema, &schema_fn_path, "crate::runtime")
}

#[cfg(not(test))]
pub fn render_machine_kernel_module_with_schema_path(
    schema: &MachineSchema,
    schema_fn_path: &str,
) -> String {
    render_machine_kernel_module_with_paths(schema, schema_fn_path, "crate::runtime")
...
#[cfg(not(test))]
pub fn render_runtime_local_machine_kernel_module(
    schema: &MachineSchema,
    schema_fn_path: &str,
) -> String {
    render_machine_kernel_module_with_paths(schema, schema_fn_path, "meerkat_machine_kernels")
}

#[cfg(not(test))]
fn render_machine_kernel_module_with_paths(
    schema: &MachineSchema,
    schema_fn_path: &str,
    kernel_import_root: &str,
) -> String {
    let mut out = String::new();
    pushln!(
        &mut out,
    ...
    pushln!(&mut out, "use {kernel_import_root}::{{");
    pushln!(
        &mut out,
