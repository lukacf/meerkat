...
[dependencies]
meerkat = { version = "=0.8.35", default-features = false, features = ["anthropic"] }

# Add persistence
meerkat = { version = "=0.8.35", default-features = false, features = ["anthropic", "session-store"] }

# Add compaction
meerkat = { version = "=0.8.35", default-features = false, features = ["anthropic", "session-store", "session-compaction"] }

# Add facade-local predicate-schedule helper re-exports. The schedule service
# and WorkGraph substrate are already linked.
meerkat = { version = "=0.8.35", default-features = false, features = ["anthropic", "session-store", "schedule"] }

# Broad native facade with WebSocket live support
meerkat = { version = "=0.8.35", features = [
...
meerkat = { version = "=0.8.35", features = ["openai-realtime", "live-webrtc"] }
```
