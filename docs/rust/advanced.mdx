...
Public comms injection is now a pure delivery API. `event_injector()` gives you an `EventInjector`, and `inject()` queues an external event for the session's next host-mode drain cycle.
...
2. The host loop drains the inbox and processes the event during the next turn boundary.
