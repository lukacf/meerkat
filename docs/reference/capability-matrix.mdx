...
[dependencies]
meerkat = { version = "=0.8.40", default-features = false, features = ["anthropic"] }

# Add persistence
meerkat = { version = "=0.8.40", default-features = false, features = ["anthropic", "session-store"] }

# Add compaction
meerkat = { version = "=0.8.40", default-features = false, features = ["anthropic", "session-store", "session-compaction"] }

# Add facade-local predicate-schedule helper re-exports. The schedule service
# and WorkGraph substrate are already linked.
meerkat = { version = "=0.8.40", default-features = false, features = ["anthropic", "session-store", "schedule"] }

# Broad native facade with WebSocket live support
meerkat = { version = "=0.8.40", features = [
...
meerkat = { version = "=0.8.40", features = ["openai-realtime", "live-webrtc"] }
```
...
| `decision` | Decision crate plus `tools.decision_enabled` | Batched semantic decision service (`decide` tool and shared evaluation) |
...
policy and registration, `meerkat-decision` owns Decision policy and the
`decide` tool, and `meerkat-skills` owns Skills policy.
