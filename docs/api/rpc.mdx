...
| `mob/adaptive_start` | Mob | `MobAdaptiveStartParams` | `MobAdaptiveStartResult` | Start an AdaptiveRun on an adaptive mobpack control mob (mob feature) |
| `mob/adaptive_status` | Mob | `MobAdaptiveRunParams` | `MobAdaptiveStatusResult` | Read AdaptiveRun status from kernel state (mob feature) |
| `mob/adaptive_layers` | Mob | `MobAdaptiveRunParams` | `MobAdaptiveLayersResult` | List AdaptiveRun layer ledger entries (mob feature) |
| `mob/adaptive_events` | Mob | `MobAdaptiveRunParams` | `MobAdaptiveEventsResult` | Read AdaptiveRun event projection entries (mob feature) |
| `mob/adaptive_result` | Mob | `MobAdaptiveRunParams` | `MobAdaptiveResultResult` | Read the final AdaptiveRun result body (mob feature) |
| `mob/adaptive_cancel` | Mob | `MobAdaptiveRunParams` | `MobAdaptiveCancelResult` | Request host cancellation for an active AdaptiveRun (mob feature) |
| `mob/adaptive_retry_layer` | Mob | `MobAdaptiveRetryLayerParams` | `MobAdaptiveRetryLayerResult` | Request a retry for a failed AdaptiveRun layer (mob feature) |
...
```json Response
{
  ...
  "id": 8,
  "result": {
    "contract_version": {"major": 0, "minor": 7, "patch": 0},
    "capabilities": [
      {
        "id": "sessions",
