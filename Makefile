...
# Minimal build - library crates only, no optional features
test-minimal:
	@echo "$(GREEN)Running minimal feature checks...$(NC)"
...
test-feature-matrix-surface:
	...
	cargo check -p meerkat-rpc --features comms,mcp
	cargo check -p meerkat-rest --no-default-features
	cargo check -p meerkat-rest --features comms
	cargo check -p meerkat-mcp-server --no-default-features
	cargo check -p meerkat-mcp-server --features comms
	cargo check -p meerkat-cli --no-default-features
	cargo check -p meerkat-cli --features comms,mcp
