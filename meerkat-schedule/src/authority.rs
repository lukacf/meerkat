...
use crate::types::{
    CreateScheduleRequest, DeliveryReceipt, MisfirePolicy, MissingTargetPolicy, Occurrence,
    OccurrenceFailureClass, OccurrenceOrdinal, OccurrencePhase, OverlapPolicy, Schedule,
    SchedulePhase, ScheduleRevision, TriggerSpec, UpdateScheduleRequest,
};
...
fn write_back_occurrence(
    ...
    // Failure class — preserve the ORIGINAL from the input, not the DSL's
    // lossy 3-variant projection. The DSL tracks whether failure_class is
    // Some or None (for invariant checking), but the actual value comes
    // from the input.
    occ.failure_class = match input {
        ...
        _ => {
            // Other transitions don't touch failure_class — if DSL has Some,
            // preserve the existing value.
            if dsl.failure_class.is_some() {
                occ.failure_class
            } else {
                None
            }
...
fn to_dsl_failure_class(fc: OccurrenceFailureClass) -> occ_dsl::OccurrenceFailureClass {
    match fc {
        OccurrenceFailureClass::TargetMaterializationFailed
        | OccurrenceFailureClass::TargetMissing
        | OccurrenceFailureClass::TargetBusy => occ_dsl::OccurrenceFailureClass::TargetUnavailable,
        OccurrenceFailureClass::RuntimeRejected
        | OccurrenceFailureClass::MobRejected
        | OccurrenceFailureClass::LeaseLost
        | OccurrenceFailureClass::TransportError
        | OccurrenceFailureClass::InternalError => occ_dsl::OccurrenceFailureClass::InternalError,
    }
}
...
fn to_dsl_misfire_policy(policy: &MisfirePolicy) -> sched_dsl::MisfirePolicy {
    match policy {
        ...
        MisfirePolicy::CatchUpWithin { .. } => sched_dsl::MisfirePolicy::Execute,
    }
}

fn to_dsl_overlap_policy(policy: &OverlapPolicy) -> sched_dsl::OverlapPolicy {
    match policy {
        OverlapPolicy::AllowConcurrent => sched_dsl::OverlapPolicy::Queue,
        OverlapPolicy::SkipIfRunning => sched_dsl::OverlapPolicy::SkipIfRunning,
    }
}

fn to_dsl_missing_target_policy(policy: &MissingTargetPolicy) -> sched_dsl::MissingTargetPolicy {
    match policy {
