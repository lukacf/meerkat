...
def generate_web_auth_types(schemas: dict, output_dir: Path) -> None:
    ...
    lines.extend(
        [
            ...
            "  value: unknown,",
            "  path: string,",
            "  parse: (entry: unknown, entryPath: string) => unknown,",
            "): void {",
            ...
            "  expectRecordMap(record.backends, `${path}.backends`, parseWireBackendProfile);",
            "  expectRecordMap(record.auth_profiles, `${path}.auth_profiles`, parseWireAuthProfile);",
            "  expectRecordMap(record.bindings, `${path}.bindings`, parseWireProviderBinding);",
            ...
            "  record.auth_profiles.forEach((entry, index) => parseWireAuthProfile(entry, `${path}.auth_profiles[${index}]`));",
            "  if (!Array.isArray(record.backend_profiles)) fail(`${path}.backend_profiles`, 'array');",
            "  record.backend_profiles.forEach((entry, index) => parseWireBackendProfile(entry, `${path}.backend_profiles[${index}]`));",
            "  if (!Array.isArray(record.bindings)) fail(`${path}.bindings`, 'array');",
            "  record.bindings.forEach((entry, index) => parseWireProviderBinding(entry, `${path}.bindings[${index}]`));",
