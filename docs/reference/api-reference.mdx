...
streams, schedule, skills, runtime, mob, MCP, comms, auth, realm, and
approval methods enabled. Live audio/text adapter surfaces (`live/*`)
appear when `rkat-rpc` is started with `--live-ws` and the runtime has a
provider-backed live adapter wired in.
...
| `SessionLlmReconfigureRequest` | `meerkat_runtime` | Request to swap provider/model on a session (`model`, `provider`, `provider_params`) |
| `ModelCapabilities` | `meerkat_models` | Per-model capability set; `realtime: bool` is consulted by the live-adapter precheck before opening a `live/*` channel |
