...
| RPC | JSON-RPC | `event/push` method | None (implicit) |
...
Push events to a running session via JSON-RPC.
...
- The agent must be running in **host mode** with events (`run_host_mode_with_events`).
