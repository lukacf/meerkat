...
refresh replaces (`background_job` and `auth_reauth_required` notices, and
`mcp_pending` notices whose blocks are not `persisted`); written into a running
turn it would vanish from later requests, so it takes the follow-up turn.
