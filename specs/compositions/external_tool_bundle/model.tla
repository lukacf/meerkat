...
CONSTANTS AdmissionEffectValues, AsyncOpRefValues, BooleanValues, ContentShapeValues, HandlingModeValues, NatValues, OperationIdValues, RequestIdValues, ReservationKeyValues, RunIdValues, StringValues, SurfaceIdValues, TurnNumberValues, WorkIdValues
...
external_tool_surface_ApplyBoundaryAdd(arg_surface_id, arg_applied_at_turn) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       ...
       /\ obligation_surface_completion' = obligation_surface_completion \cup {[surface_id |-> packet.payload.surface_id, operation |-> "Add", applied_at_turn |-> "unknown_applied_at_turn"]}
       /\ model_step_count' = model_step_count + 1
...
external_tool_surface_ApplyBoundaryReload(arg_surface_id, arg_applied_at_turn) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       ...
       /\ obligation_surface_completion' = obligation_surface_completion \cup {[surface_id |-> packet.payload.surface_id, operation |-> "Reload", applied_at_turn |-> "unknown_applied_at_turn"]}
       /\ model_step_count' = model_step_count + 1
...
CiStateConstraint == /\ model_step_count <= 6 /\ Len(pending_inputs) <= 1 /\ Cardinality(observed_inputs) <= 4 /\ Len(pending_routes) <= 1 /\ Cardinality(delivered_routes) <= 1 /\ Cardinality(emitted_effects) <= 1 /\ Cardinality(observed_transitions) <= 6 /\ Cardinality(external_tool_surface_known_surfaces) <= 1 /\ Cardinality(external_tool_surface_visible_surfaces) <= 1 /\ Cardinality(DOMAIN external_tool_surface_base_state) <= 1 /\ Cardinality(DOMAIN external_tool_surface_pending_op) <= 1 /\ Cardinality(DOMAIN external_tool_surface_staged_op) <= 1 /\ Cardinality(DOMAIN external_tool_surface_inflight_calls) <= 1 /\ Cardinality(DOMAIN external_tool_surface_last_delta_operation) <= 1 /\ Cardinality(DOMAIN external_tool_surface_last_delta_phase) <= 1 /\ Len(turn_execution_barrier_operation_ids) <= 1
DeepStateConstraint == /\ model_step_count <= 6 /\ Len(pending_inputs) <= 2 /\ Cardinality(observed_inputs) <= 6 /\ Len(pending_routes) <= 2 /\ Cardinality(delivered_routes) <= 2 /\ Cardinality(emitted_effects) <= 2 /\ Cardinality(observed_transitions) <= 6 /\ Cardinality(external_tool_surface_known_surfaces) <= 2 /\ Cardinality(external_tool_surface_visible_surfaces) <= 2 /\ Cardinality(DOMAIN external_tool_surface_base_state) <= 2 /\ Cardinality(DOMAIN external_tool_surface_pending_op) <= 2 /\ Cardinality(DOMAIN external_tool_surface_staged_op) <= 2 /\ Cardinality(DOMAIN external_tool_surface_inflight_calls) <= 2 /\ Cardinality(DOMAIN external_tool_surface_last_delta_operation) <= 2 /\ Cardinality(DOMAIN external_tool_surface_last_delta_phase) <= 2 /\ Len(turn_execution_barrier_operation_ids) <= 2
WitnessStateConstraint_surface_add_notifies_control == /\ model_step_count <= 14 /\ Len(pending_inputs) <= 6 /\ Cardinality(observed_inputs) <= 12 /\ Len(pending_routes) <= 2 /\ Cardinality(delivered_routes) <= 4 /\ Cardinality(emitted_effects) <= 8 /\ Cardinality(observed_transitions) <= 14 /\ Cardinality(external_tool_surface_known_surfaces) <= 6 /\ Cardinality(external_tool_surface_visible_surfaces) <= 6 /\ Cardinality(DOMAIN external_tool_surface_base_state) <= 3 /\ Cardinality(DOMAIN external_tool_surface_pending_op) <= 3 /\ Cardinality(DOMAIN external_tool_surface_staged_op) <= 3 /\ Cardinality(DOMAIN external_tool_surface_inflight_calls) <= 3 /\ Cardinality(DOMAIN external_tool_surface_last_delta_operation) <= 3 /\ Cardinality(DOMAIN external_tool_surface_last_delta_phase) <= 3 /\ Len(turn_execution_barrier_operation_ids) <= 6
...
WitnessStateConstraint_surface_completion_feedback == /\ model_step_count <= 16 /\ Len(pending_inputs) <= 8 /\ Cardinality(observed_inputs) <= 14 /\ Len(pending_routes) <= 2 /\ Cardinality(delivered_routes) <= 4 /\ Cardinality(emitted_effects) <= 9 /\ Cardinality(observed_transitions) <= 16 /\ Cardinality(external_tool_surface_known_surfaces) <= 6 /\ Cardinality(external_tool_surface_visible_surfaces) <= 6 /\ Cardinality(DOMAIN external_tool_surface_base_state) <= 3 /\ Cardinality(DOMAIN external_tool_surface_pending_op) <= 3 /\ Cardinality(DOMAIN external_tool_surface_staged_op) <= 3 /\ Cardinality(DOMAIN external_tool_surface_inflight_calls) <= 3 /\ Cardinality(DOMAIN external_tool_surface_last_delta_operation) <= 3 /\ Cardinality(DOMAIN external_tool_surface_last_delta_phase) <= 3 /\ Len(turn_execution_barrier_operation_ids) <= 6
...
WitnessTransitionOrder_surface_completion_feedback_1 == <> (\E earlier \in observed_transitions, later \in observed_transitions : /\ earlier.machine = "turn_execution" /\ earlier.transition = "LlmReturnedTerminal" /\ later.machine = "external_tool_surface" /\ later.transition = "ApplyBoundaryAdd" /\ earlier.step < later.step)
WitnessTransitionOrder_surface_completion_feedback_2 == <> (\E earlier \in observed_transitions, later \in observed_transitions : /\ earlier.machine = "external_tool_surface" /\ earlier.transition = "ApplyBoundaryAdd" /\ later.machine = "external_tool_surface" /\ later.transition = "PendingSucceededAdd" /\ earlier.step < later.step)
