...
| REQ-002 | Terminal item, paused/stopped/superseded binding, missing item, stale revision, or unresolved target fails closed. | Negative tests for each ineligibility reason. | PARTIAL; trusted runtime continuation and service projection fail closed |
...
| 2026-05-26T13:12:00Z | `./scripts/repo-cargo test -p meerkat-contracts --features schema workgraph_rpc` | Superseded evidence: this earlier broad RPC catalog exposure was narrowed before merge. Current public RPC exposes WorkGraph observability plus narrow goal status/list controls; trusted mutation/continuation paths are not advertised as public RPC. |
| 2026-05-26T13:12:00Z | `./scripts/repo-cargo check -p meerkat-rpc -p meerkat-contracts` | Superseded evidence: earlier public RPC handlers for goal creation and attention pause/resume were removed or moved behind trusted/in-process paths before merge. |
...
| 2026-05-26T11:21:30Z | `./scripts/repo-cargo test -p meerkat-rest test_workgraph_rest_routes_expose_observability_and_narrow_goal_control` | Superseded evidence: an earlier REST iteration exposed close/confirmation behavior publicly. Final REST coverage asserts mutating routes are absent and mutation remains trusted/in-process. |
| 2026-05-26T11:21:30Z | `./scripts/repo-cargo test -p meerkat-contracts --features schema workgraph_rpc` | Superseded evidence: `workgraph/goal/confirm` and `workgraph/goal/request_close` were advertised during an earlier public-mutation iteration, then removed from the public catalog before merge. |
| 2026-05-26T11:21:30Z | `./scripts/repo-cargo check -p meerkat-rpc -p meerkat-rest -p meerkat-workgraph -p meerkat-contracts` | Superseded evidence: package check passed during the public-mutation iteration; final RPC/REST surfaces expose observability and narrow goal status/list controls only. |
| 2026-05-26T11:21:30Z | `./scripts/repo-cargo run -p meerkat-contracts --features schema --bin emit-schemas && python3 tools/sdk-codegen/generate.py` | Superseded evidence: schema artifacts and SDK generated types were later narrowed to the final public WorkGraph observability and goal-status surfaces. |
...
| 2026-05-26T11:44:55Z | `./scripts/repo-cargo check -p rkat -p meerkat-rpc -p meerkat-rest -p meerkat-contracts -p meerkat-workgraph` | Superseded evidence: package check passed during a broader public-control iteration. The final design keeps explicit attention continuation as a trusted runtime operation, not a public RPC/REST handoff. |
...
| 2026-05-26T11:44:55Z | `./scripts/repo-cargo test -p meerkat-contracts --features schema workgraph_rpc_surface_exposes_observability_and_narrow_goal_control` | Superseded evidence: the final catalog test asserts public observability and narrow goal controls only; `workgraph/attention/continue` is intentionally not public. |
| 2026-05-26T11:44:55Z | `./scripts/repo-cargo test -p meerkat-rest test_workgraph_rest_routes_expose_observability_and_narrow_goal_control` | Superseded evidence: final REST coverage asserts public route narrowing; paused/stale attention fails closed through trusted runtime continuation validation rather than a public continuation endpoint. |
| 2026-05-26T11:44:55Z | `./scripts/repo-cargo run -p meerkat-contracts --features schema --bin emit-schemas && python3 tools/sdk-codegen/generate.py` | Superseded evidence: schema/SDK regeneration was later narrowed to the final public WorkGraph observability and goal-status surfaces. |
