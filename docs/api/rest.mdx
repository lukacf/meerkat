...
| `POST` | `/mob/{id}/members/{agent_identity}/realtime/attach` | Attach realtime (audio) channel intent to a mob member (mob feature) |
| `POST` | `/mob/{id}/members/{agent_identity}/realtime/detach` | Detach realtime channel intent from a mob member (mob feature) |
...
<ParamField body="handling_mode" type="string | null">
  Handling mode for `peer_message` and `peer_request`. Local callers must provide `"queue"` or `"steer"`.
</ParamField>

<Note>
`peer_message` is the default collaboration primitive. `peer_request` is only a structured ask with correlated replies. The old peer-side reservation stream option was removed; requests no longer accept a `stream` field.
</Note>
...
Attach or detach a realtime (audio) channel to a specific mob member, or read the runtime-owned attachment status for a session. See the [Realtime voice guide](/guides/realtime) for the attachment state machine, authority-epoch model, and live-topology reconfigure flow.

### POST /mob/\{id\}/members/\{agent_identity\}/realtime/attach

Stage realtime (audio) intent for a mob member. Durable: intent survives member respawn. Requires the `mob` feature.

```bash Request
curl -X POST http://127.0.0.1:8080/mob/release-triage/members/lead-1/realtime/attach
```
...
### POST /mob/\{id\}/members/\{agent_identity\}/realtime/detach

Clear realtime intent for a mob member. Idempotent. Requires the `mob` feature.

```bash Request
curl -X POST http://127.0.0.1:8080/mob/release-triage/members/lead-1/realtime/detach
```
