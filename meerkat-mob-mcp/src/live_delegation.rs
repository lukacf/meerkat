    ...
    async fn supersede_previous_delegation(
        ...
        if !previous.task.is_finished() {
                ...
                directive
            {
                ...
                if outcome == LiveDelegationCancellationOutcome::Failed {
                    self.active
                        .lock()
                        .await
                        .insert(channel_key.clone(), previous);
                    return Err("exact previous delegation cancellation failed; source transcript was not mutated".to_string());
                }
            }
        }
        previous
