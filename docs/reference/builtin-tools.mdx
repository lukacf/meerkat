...
| `max_run_elapsed` | The opt-in `max_run_secs` elapsed. The run was cancelled and the child retired; `retirement_error` reports a failed retirement. |
...
`BackgroundJob` system notice: "Background fork_off job `<job_id>` finished
(`completed` or `failed`):" followed by the outcome JSON. Its typed block,
`SystemNoticeBlock::BackgroundJob`, carries the `job_id`, the tool name, the
status, the outcome JSON as `detail`, and `persisted: true`, which marks it as
a durable completion record rather than a progress notice that is refreshed at
each model call (`SystemNoticeMessage::persisted_background_job_id` finds it).
