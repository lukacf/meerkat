...
See the [Python SDK](/sdks/python/reference#skills-parameters) and [TypeScript SDK](/sdks/typescript/reference#skills-parameters) for SDK wrappers. CLI `--skill` and SDK `preload_skills` / `preloadSkills` are convenience options that serialize into the canonical metadata carrier.
