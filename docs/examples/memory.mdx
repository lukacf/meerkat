...
<Tabs>
  ...
  <Tab title="Rust">
    ...
    let agent = factory
        .build_agent(AgentBuildConfig::new("gpt-5.6-sol"), &config)
        .await?;
    ```
  </Tab>
</Tabs>
...
If the built-in summarization request fails with a recognized over-context or
request-too-large error, Meerkat can use a deterministic mechanical handoff
summary to shed old rows. This is allowed only when current history has no
protected, unsummarized assistant transcript observations with source
`SpokenUnmeasured`. With those observations, the compaction attempt emits
`compaction_failed` and preserves the original transcript, as it does for
ordinary LLM errors. A configured curator never falls back to the mechanical
or LLM path.
...
`max_request_bytes` is a host cap, not an estimate of the current request.
Compaction triggers at four-fifths of the effective cap. When the provider also
supplies an exact request-body cap, the stricter cap wins.

## Supply a compaction curator

Use a curator when a host application has its own deterministic or supervised
handoff-summary path. The runtime still decides when compaction is required and
validates the resulting rewrite.

```rust
use async_trait::async_trait;
use meerkat_core::{
    CompactionCurator, CompactionCuratorError, CompactionWindow,
    CuratedCompactionSummary,
};
use std::sync::Arc;
...
#[async_trait]
impl CompactionCurator for HostCurator {
    async fn curate_summary(
        &self,
        window: CompactionWindow<'_>,
    ) -> Result<CuratedCompactionSummary, CompactionCuratorError> {
        let summary = host_summary_service(window.messages).await
            .map_err(|error| CompactionCuratorError::Failed(error.to_string()))?;
        CuratedCompactionSummary::new(summary)
    }
}

let mut build = AgentBuildConfig::new("gpt-5.6-sol");
build.compaction_curator_override = Some(Arc::new(HostCurator));
```

There is no automatic LLM fallback. A curator error emits
`compaction_failed`, records no transcript rewrite, and leaves the original
...
<Tabs>
  <Tab title="CLI">
    ...
    The `limits.budget` setting above supplies the aggregate token limit
    corresponding to `budget_limits.max_tokens` in the API examples.
  </Tab>
  <Tab title="JSON-RPC">
    ```json
    {
