...
ResolveRuntimeSnapshotReadSourceCommittedHead(session_id, relation, store_provenance, session_is_live) ==
    /\ phase = "Ready"
    /\ ((relation = "VerifiedStrictDescendant") /\ (store_provenance = "Committed") /\ (session_is_live = FALSE))
    /\ phase' = "Ready"
    /\ model_step_count' = model_step_count + 1
    /\ UNCHANGED << session_first_turn_phase, session_pending_initial_prompt_present, session_pending_tool_results_count, session_lifecycle_terminal >>


ResolveRuntimeSnapshotReadSourceRecoveryRequired(session_id, relation, store_provenance, session_is_live) ==
    /\ phase = "Ready"
    /\ ((relation = "VerifiedStrictDescendant") /\ (store_provenance # "Committed") /\ (session_is_live = FALSE))
    /\ phase' = "Ready"
    /\ model_step_count' = model_step_count + 1
...
ResolveRuntimeSnapshotReadSourceQuarantine(session_id, relation, store_provenance, session_is_live) ==
    /\ phase = "Ready"
    /\ (IF ((relation = "Diverged") /\ (store_provenance # "IntraTurn")) THEN TRUE ELSE (relation = "Unverifiable"))
    /\ phase' = "Ready"
    /\ model_step_count' = model_step_count + 1
    /\ UNCHANGED << session_first_turn_phase, session_pending_initial_prompt_present, session_pending_tool_results_count, session_lifecycle_terminal >>


ResolveRuntimeSnapshotReadSourceSnapshot(session_id, relation, store_provenance, session_is_live) ==
    /\ phase = "Ready"
    /\ ((relation # "Unverifiable") /\ (IF (relation # "Diverged") THEN TRUE ELSE (store_provenance = "IntraTurn")) /\ (IF (relation # "VerifiedStrictDescendant") THEN TRUE ELSE (session_is_live = TRUE)))
    /\ phase' = "Ready"
    /\ model_step_count' = model_step_count + 1
    /\ UNCHANGED << session_first_turn_phase, session_pending_initial_prompt_present, session_pending_tool_results_count, session_lifecycle_terminal >>
...
ClassifyDurableTailRepairable(session_id, candidate_id, relation, run_id_cardinality, terminal_stop_reason, dangling_tool_use_count, orphan_tool_result_count, messages_after_terminal) ==
    /\ phase = "Ready"
    /\ ((relation = "VerifiedStrictDescendant") /\ (run_id_cardinality = "SingleRunId") /\ (orphan_tool_result_count = 0) /\ (messages_after_terminal = FALSE) /\ (IF (terminal_stop_reason = "ToolUse") THEN TRUE ELSE (terminal_stop_reason = "Absent")))
    /\ phase' = "Ready"
    /\ model_step_count' = model_step_count + 1
    /\ UNCHANGED << session_first_turn_phase, session_pending_initial_prompt_present, session_pending_tool_results_count, session_lifecycle_terminal >>
...
ClassifyDurableTailAmbiguous(session_id, candidate_id, relation, run_id_cardinality, terminal_stop_reason, dangling_tool_use_count, orphan_tool_result_count, messages_after_terminal) ==
    /\ phase = "Ready"
    /\ (IF (relation # "VerifiedStrictDescendant") THEN TRUE ELSE (IF (run_id_cardinality # "SingleRunId") THEN TRUE ELSE (IF (orphan_tool_result_count # 0) THEN TRUE ELSE (IF (messages_after_terminal = TRUE) THEN TRUE ELSE (IF (terminal_stop_reason = "Other") THEN TRUE ELSE ((terminal_stop_reason = "EndTurn") /\ (dangling_tool_use_count # 0)))))))
    /\ phase' = "Ready"
    /\ model_step_count' = model_step_count + 1
    /\ UNCHANGED << session_first_turn_phase, session_pending_initial_prompt_present, session_pending_tool_results_count, session_lifecycle_terminal >>


ResolveRuntimeProjectionConflictRetain(session_id, relation, authority_supersedes_row) ==
    /\ phase = "Ready"
...
ResolveRuntimeProjectionConflictConverge(session_id, relation, authority_supersedes_row) ==
    /\ phase = "Ready"
    /\ ((relation # "VerifiedStrictDescendant") /\ (authority_supersedes_row = TRUE))
    /\ phase' = "Ready"
    /\ model_step_count' = model_step_count + 1
    /\ UNCHANGED << session_first_turn_phase, session_pending_initial_prompt_present, session_pending_tool_results_count, session_lifecycle_terminal >>


ResolveRuntimeProjectionConflictReject(session_id, relation, authority_supersedes_row) ==
    /\ phase = "Ready"
    /\ ((relation # "VerifiedStrictDescendant") /\ (authority_supersedes_row = FALSE))
    /\ phase' = "Ready"
    /\ model_step_count' = model_step_count + 1
    /\ UNCHANGED << session_first_turn_phase, session_pending_initial_prompt_present, session_pending_tool_results_count, session_lifecycle_terminal >>
...
Next ==
    ...
    \/ \E session_id \in SessionIdValues : \E relation \in DurableHeadRelationValues : \E store_provenance \in CheckpointProvenanceClassValues : ResolveRuntimeSnapshotReadSourceCommittedHead(session_id, relation, store_provenance, FALSE)
    \/ \E session_id \in SessionIdValues : \E relation \in DurableHeadRelationValues : \E store_provenance \in CheckpointProvenanceClassValues : ResolveRuntimeSnapshotReadSourceRecoveryRequired(session_id, relation, store_provenance, FALSE)
    \/ \E session_id \in SessionIdValues : \E relation \in DurableHeadRelationValues : \E store_provenance \in CheckpointProvenanceClassValues : \E session_is_live \in BOOLEAN : ResolveRuntimeSnapshotReadSourceQuarantine(session_id, relation, store_provenance, session_is_live)
    \/ \E session_id \in SessionIdValues : \E relation \in DurableHeadRelationValues : \E store_provenance \in CheckpointProvenanceClassValues : \E session_is_live \in BOOLEAN : ResolveRuntimeSnapshotReadSourceSnapshot(session_id, relation, store_provenance, session_is_live)
    ...
    \/ \E session_id \in SessionIdValues : \E relation \in DurableHeadRelationValues : \E authority_supersedes_row \in BOOLEAN : ResolveRuntimeProjectionConflictRetain(session_id, relation, authority_supersedes_row)
    \/ \E session_id \in SessionIdValues : \E relation \in DurableHeadRelationValues : ResolveRuntimeProjectionConflictConverge(session_id, relation, TRUE)
    \/ \E session_id \in SessionIdValues : \E relation \in DurableHeadRelationValues : ResolveRuntimeProjectionConflictReject(session_id, relation, FALSE)
