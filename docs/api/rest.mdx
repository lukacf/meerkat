...
The previous REST realtime routes are retired. `/realtime/open_info`,
`/realtime/status`, `/realtime/capabilities`, and
`/sessions/{id}/realtime-attachment-status` are no longer part of the public
REST API. A replacement live API will be documented after the live adapter host
is exposed.
