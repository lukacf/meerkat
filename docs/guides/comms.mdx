...
| `--host` | Run in keep-alive mode (stay alive for comms messages and events after initial prompt) |
...
<Steps>
  ...
  <Step title="Enter wait loop">
    After the first run completes, the session enters keep-alive mode and the active owner path keeps the inbox consumer alive.
  </Step>
  <Step title="Handle incoming messages and events">
    When comms messages or external events arrive, they flow into the same inbox. The currently active keep-alive owner path drains that inbox and turns those items into new session turns. Runtime-backed and direct session-service paths use different owner mechanics, but they are expected to follow the same drain-lifecycle truth.
  </Step>
