...
| `GET` | `/sessions/{id}/state` | Read runtime state |
| `GET` | `/sessions/{id}/realtime-attachment-status` | Read the runtime-owned realtime attachment status for a session |
| `POST` | `/sessions/{id}/accept` | Admit runtime input explicitly |
| `POST` | `/sessions/{id}/retire` | Retire a runtime |
| `POST` | `/sessions/{id}/reset` | Reset a runtime |
| `GET` | `/sessions/{id}/inputs` | List runtime input state entries |
| `GET` | `/sessions/{session_id}/inputs/{input_id}` | Read one runtime input state |
| `GET` | `/auth/profiles` | List auth profiles |
| `POST` | `/auth/profiles` | Persist credentials for a binding |
| `GET` | `/auth/profiles/{id}` | Read an auth profile |
| `DELETE` | `/auth/profiles/{id}` | Delete persisted credentials for a binding |
...
| `GET` | `/auth/status/{id}` | Read auth status for a binding |
| `POST` | `/auth/logout/{id}` | Log out a binding |
