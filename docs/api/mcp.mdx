...
still agent-side and comes from composing `MobMcpDispatcher` into
`SessionBuildOptions.external_tools`, which provides `mob_*` tools to the
model. Public host MCP surfaces should not re-export that raw dispatcher.
</Note>

<Note>
**Realtime attachment over MCP.** The public MCP tool set exposes only realtime
bootstrap and status surfaces (`meerkat_realtime_open_info`,
`meerkat_realtime_status`, `meerkat_realtime_capabilities`). Direct
attach/detach of a realtime channel to a mob member is not exposed as a public
MCP tool — use the JSON-RPC `mob/realtime_attach` / `mob/realtime_detach`
surface for lifecycle control. See the [Realtime voice guide](/guides/realtime)
for the full attachment state model and authority-epoch contract.
</Note>
