...
This page is a quick-lookup index. For settled 0.5 session/runtime semantics, see [Session contracts](/reference/session-contracts). For detailed usage with examples, see the [Rust SDK reference](/rust/overview).
...
| `RealtimeAttachmentStatus` | `meerkat_runtime` | Runtime-owned realtime attachment state (`Unattached`, `IntentPresentUnbound`, `BindingNotReady`, `BindingReady`, `ReplacementPending`, `ReattachRequired`) |
| `RealtimeAttachmentSignalAuthority` | `meerkat_runtime` | Authority-epoch token (`session_id` + `authority_epoch`) required on every provider callback |
| `SessionLlmReconfigureRequest` | `meerkat_runtime` | Request to swap provider/model on a session; capability-driven realtime transport branch attaches/detaches the binding based on the target model's `ModelCapabilities.realtime` (`model`, `provider`, `provider_params`) |
| `ModelCapabilities` | `meerkat_models` | Per-model capability set; `realtime: bool` drives automatic realtime transport attach/detach |
...
| `ContractVersion` | Semver version (`0.5.1` currently) |
