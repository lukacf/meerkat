...
- enable realtime-capable models
- understand attachment state
- bootstrap a realtime channel
- reason about realtime in the context of the normal session model
...
Set the model in `turn_metadata` on `session/create` (RPC) or the equivalent
REST / SDK call:
...
curl -X POST http://127.0.0.1:8080/sessions \
  ...
  -d '{"prompt":"Let'"'"'s talk.","turn_metadata":{"model":"gpt-realtime-1.5","provider":"openai"}}'
```
...
async with MeerkatClient() as client:
    ...
    session = await client.create_session(
        prompt="Let's talk.",
        turn_metadata={"model": "gpt-realtime-1.5", "provider": "openai"},
    )
```
</CodeGroup>

The runtime resolves the model, sees `ModelCapabilities.realtime == true`, and brings up the realtime transport automatically. Poll `session/realtime_attachment_status` (below) to confirm the binding has reached `BindingReady`.

### Configuration defaults

The session's default model can be set in config (`~/.rkat/config.toml` or project-local) via `default_model`. Any session created without an explicit `turn_metadata.model` inherits the configured default, so setting `default_model = "gpt-realtime-1.5"` makes realtime the default for the whole install. See the [Configuration guide](/concepts/configuration).

### Mid-session: swap to a realtime-capable model

If a session was started on one model and later needs another, send the next turn with `turn_metadata.model` and, when needed, `turn_metadata.provider` / `turn_metadata.provider_params`. JSON-RPC `turn/start`, REST continue, and host SDK turn helpers all route that canonical metadata through the runtime so the live LLM identity is updated for the turn. Realtime transport attachment is still host-orchestrated through `MeerkatMachine::reconfigure_live_topology` when a host needs to bring up or tear down a realtime binding (see [Live-topology reconfigure](#live-topology-reconfigure) below).

When a host performs that live-topology reconfigure for a realtime-capable model, the runtime mints a fresh authority epoch and brings the binding up. When the host reconfigures to a non-realtime model, the existing binding is torn down and the response surfaces `ValidationFailed` so the caller knows the binding is gone.

## Observing attachment status

The runtime-owned `RealtimeAttachmentStatus` is surfaced by `session/realtime_attachment_status`:
...
```bash REST
curl http://127.0.0.1:8080/sessions/01936f8a-.../realtime-attachment-status
# => {"session_id": "...", "status": "binding_ready"}
```
</CodeGroup>
...
2. Poll `session/realtime_attachment_status` until it reports `binding_ready`.
...
Step 2 of the flow drives any in-flight turn through `cancel_after_boundary_inner`, which sends the runtime loop a boundary-cancel effect and uses the live boundary handle when the executor exposes one. This is idempotent and gated on runtime attachment presence — calling it on an idle session is a no-op. It ensures the turn reaches a safe boundary before the guard on `MarkLiveTopologyDetached` allows the binding to drop.
...
Every member spawned under that profile is initialized on `gpt-realtime-1.5` and its session gets a realtime binding automatically. Use `session/realtime_attachment_status` with each member session ID to observe attachment state.
...
- **No standalone reconfigure method.** Remote callers request a mid-session model/provider identity swap on the next turn through `turn_metadata.model` / `turn_metadata.provider`; realtime transport attach/detach remains host-controlled via `SessionLlmReconfigureHost`.
...
- [Identity, Generation, and Hidden Runtime Epoch](/architecture/identity-generation-and-runtime-epoch) — architecture rationale for the authority/epoch model
