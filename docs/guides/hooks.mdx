...
<Tabs>
  ...
  <Tab title="HTTP">
    ...
    The `method` defaults to `"POST"` if not specified. Response body size is
    capped at `payload_max_bytes`. Header values are literal configuration;
    the hook runtime does not expand environment-variable references.
  </Tab>
</Tabs>

Foreground hook runtime failures fail closed through typed `HookEngineError` values; they
are never converted into warning-only success or a hook-local `Deny` decision.
Every background hook is required to be `Observe`, because asynchronous work
cannot safely publish a guardrail decision after loop progression.
...
engine-denial outcomes. Engine timeouts and execution failures remain typed
`HookEngineError` values, mapped to `HookFailureReason::Timeout` or
...
For ordinary agent-loop hook points, the session event stream can include:

| Event | When |
|-------|------|
| `HookStarted` | A foreground hook, or an actually launched ordinary background hook, starts execution |
| `HookCompleted` | A foreground hook returns without an execution failure; a denial is reported separately |
| `HookFailed` | A foreground hook fails or times out, including the hard engine-error path |
| `HookDenied` | A foreground hook returns a `Deny` decision |

Ordinary background work does not promise a terminal `HookCompleted` or
`HookFailed` session event. Later failures and capacity skips are observable
