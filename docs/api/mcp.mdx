...
| `meerkat_mob_temporary_council_run` | Run one bounded [temporary council](/guides/temporary-councils) of source-owned forked participants (mob feature) |
| `meerkat_mob_temporary_council_get` | Read one sealed temporary-council record projection (mob feature) |
...
<Note>
There is deliberately no `meerkat_mob_temporary_council_recover` tool. A
recovery sweep seals terminals, revokes capabilities, and destroys temporary
mobs for councils the caller never started — the same realm-wide administrative
class that keeps host binding and grant administration off this surface.
Recovery stays on the owner consoles (`mob/temporary_council_recover`,
`POST /mob/temporary-councils/recover`, `rkat mob council-recover`). Exact run
admission may recover only its requested council id.
</Note>
