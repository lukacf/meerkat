...
mod tests {
    ...
    fn flow_run_state_from_raw(mut state: KernelState) -> flow_run::State {
        let mut seeded =
        ...
        state = seeded;
        let compat_state =
            meerkat_machine_kernels::test_oracle::compat_generated::flow_run::from_test_oracle_state(state)
                .expect("typed flow_run state");
        compat_to_local(compat_state)
    }

    fn flow_run_state_to_raw(state: &flow_run::State) -> KernelState {
    ...
    fn flow_frame_state_from_raw(mut state: KernelState) -> flow_frame::State {
        let mut seeded =
        ...
        state = seeded;
        let compat_state =
            meerkat_machine_kernels::test_oracle::compat_generated::flow_frame::from_test_oracle_state(state)
                .expect("typed flow_frame state");
        compat_to_local(compat_state)
    }

    fn loop_iteration_state_from_raw(mut state: KernelState) -> loop_iteration::State {
        let mut seeded =
