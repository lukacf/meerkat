...
pub fn run_protocol_codegen() -> Result<()> {
    ...
    let mob_coordination_lifecycle_machine =
        dsl::dsl_mob_coordination_lifecycle_authority_machine_production_schema();
    let code = generate_mob_coordination_lifecycle_authority(&mob_coordination_lifecycle_machine)?;
    let code = rustfmt_source(&code)?;
    let output_path =
        root.join("meerkat-mob/src/generated/mob_coordination_lifecycle_authority.rs");
    if let Some(parent) = output_path.parent() {
        fs::create_dir_all(parent).with_context(|| format!("create dir {}", parent.display()))?;
    }
    fs::write(&output_path, &code).with_context(|| format!("write {}", output_path.display()))?;
    ...
    let session_deferred_machine =
        dsl::dsl_session_deferred_turn_authority_machine_production_schema();
    let code = generate_session_deferred_turn_authority(&session_deferred_machine)?;
    let code = rustfmt_source(&code)?;
    let output_path = root.join("meerkat-core/src/generated/session_deferred_turn_authority.rs");
    if let Some(parent) = output_path.parent() {
        fs::create_dir_all(parent).with_context(|| format!("create dir {}", parent.display()))?;
    }
    fs::write(&output_path, &code).with_context(|| format!("write {}", output_path.display()))?;
...
#[derive(Debug, Clone)]
struct MobCoordinationStatusArm {
    variant: String,
    requires_unexpired: bool,
}

#[derive(Debug, Clone)]
struct MobCoordinationLifecyclePlan {
    work_intent_variants: Vec<String>,
    resource_claim_variants: Vec<String>,
    resource_claim_expired_variant: String,
    initial_record_revision: u64,
    initial_event_sequence: u64,
    work_intent_update: Vec<MobCoordinationStatusArm>,
    resource_claim_update: Vec<MobCoordinationStatusArm>,
}

fn mob_coordination_named_enum_variants(
    machine: &MachineSchema,
    enum_name: &str,
) -> Result<Vec<String>> {
    let binding = machine
        .named_types
        .iter()
        .find(|binding| binding.name.as_str() == enum_name)
        .with_context(|| {
            format!(
                "{} missing named type `{enum_name}`",
                machine.machine.as_str()
            )
        })?;
    let RustTypeAtom::StringEnum { variants } = &binding.rust else {
        bail!("{enum_name} must be a string enum");
    };
    Ok(variants
        .iter()
...
fn mob_coordination_transitions_for_input<'a>(
    machine: &'a MachineSchema,
    input: &str,
) -> Vec<&'a TransitionSchema> {
    machine
...
fn mob_coordination_single_effect<'a>(
    transition: &'a TransitionSchema,
    effect: &str,
) -> Result<&'a EffectEmit> {
    if transition.emit.len() != 1 {
...
fn mob_coordination_single_transition_for_input<'a>(
    machine: &'a MachineSchema,
    input: &str,
) -> Result<&'a TransitionSchema> {
    let transitions = mob_coordination_transitions_for_input(machine, input);
    match transitions.as_slice() {
        [transition] => Ok(*transition),
        _ => bail!(
            "{} expected exactly one `{input}` transition, found {}",
            machine.machine.as_str(),
            transitions.len()
        ),
    }
...
fn mob_coordination_init_u64(machine: &MachineSchema, field: &str) -> Result<u64> {
    let init = machine
        .state
        .init
        .fields
...
fn mob_coordination_effect_expr<'a>(effect: &'a EffectEmit, field: &str) -> Result<&'a Expr> {
    effect
        .fields
        .iter()
        .find_map(|(candidate, expr)| (candidate.as_str() == field).then_some(expr))
...
fn mob_coordination_expr_is_binding(expr: &Expr, binding: &str) -> bool {
    matches!(expr, Expr::Binding(candidate) if candidate == binding)
}

fn mob_coordination_expr_is_field(expr: &Expr, field: &str) -> bool {
    matches!(expr, Expr::Field(candidate) if candidate.as_str() == field)
}

fn mob_coordination_expr_is_binding_plus_one(expr: &Expr, binding: &str) -> bool {
    matches!(
        expr,
        Expr::Add(left, right)
            if mob_coordination_expr_is_binding(left, binding)
                && matches!(&**right, Expr::U64(1))
    )
}

fn mob_coordination_guard_has_binding_eq_binding(
    expr: &Expr,
    left_binding: &str,
    right_binding: &str,
) -> bool {
    match expr {
        Expr::And(items) => items.iter().any(|item| {
            mob_coordination_guard_has_binding_eq_binding(item, left_binding, right_binding)
        }),
        Expr::Eq(left, right) => {
            (mob_coordination_expr_is_binding(left, left_binding)
                && mob_coordination_expr_is_binding(right, right_binding))
                || (mob_coordination_expr_is_binding(left, right_binding)
                    && mob_coordination_expr_is_binding(right, left_binding))
        }
        _ => false,
    }
}

fn mob_coordination_guard_has_binding_gt_u64(expr: &Expr, binding: &str, value: u64) -> bool {
    match expr {
        Expr::And(items) => items
            .iter()
            .any(|item| mob_coordination_guard_has_binding_gt_u64(item, binding, value)),
        Expr::Gt(left, right) => {
            mob_coordination_expr_is_binding(left, binding)
                && matches!(&**right, Expr::U64(candidate) if *candidate == value)
        }
        _ => false,
    }
}

fn mob_coordination_guard_has_binding_eq_bool(expr: &Expr, binding: &str, value: bool) -> bool {
    match expr {
        Expr::And(items) => items
            .iter()
            .any(|item| mob_coordination_guard_has_binding_eq_bool(item, binding, value)),
        Expr::Eq(left, right) => {
            (mob_coordination_expr_is_binding(left, binding)
                && matches!(&**right, Expr::Bool(candidate) if *candidate == value))
                || (mob_coordination_expr_is_binding(right, binding)
                    && matches!(&**left, Expr::Bool(candidate) if *candidate == value))
        }
        _ => false,
    }
}

fn mob_coordination_guard_has_len_gt_u64(expr: &Expr, binding: &str, value: u64) -> bool {
    match expr {
        Expr::And(items) => items
            .iter()
            .any(|item| mob_coordination_guard_has_len_gt_u64(item, binding, value)),
        Expr::Gt(left, right) => {
            matches!(
                &**left,
                Expr::Len(inner) if mob_coordination_expr_is_binding(inner, binding)
            ) && matches!(&**right, Expr::U64(candidate) if *candidate == value)
        }
        _ => false,
    }
...
fn mob_coordination_expr_has_binding_neq_none(expr: &Expr, binding: &str) -> bool {
    match expr {
        Expr::And(items) | Expr::Or(items) => items
            .iter()
            .any(|item| mob_coordination_expr_has_binding_neq_none(item, binding)),
        Expr::Neq(left, right) => {
            (mob_coordination_expr_is_binding(left, binding) && matches!(&**right, Expr::None))
                || (mob_coordination_expr_is_binding(right, binding)
                    && matches!(&**left, Expr::None))
        }
        _ => false,
    }
}

fn mob_coordination_guard_has_owner_presence(
    expr: &Expr,
    ...
    match expr {
        Expr::And(items) => items.iter().any(|item| {
            mob_coordination_guard_has_owner_presence(item, principal_binding, agent_binding)
        }),
        Expr::Or(items) => {
            items
                .iter()
                .any(|item| mob_coordination_expr_has_binding_neq_none(item, principal_binding))
                && items
                    .iter()
                    .any(|item| mob_coordination_expr_has_binding_neq_none(item, agent_binding))
        }
        _ => false,
    }
}

fn mob_coordination_transition_has_binding_eq_binding(
    transition: &TransitionSchema,
    left_binding: &str,
    right_binding: &str,
) -> bool {
    transition.guards.iter().any(|guard| {
        mob_coordination_guard_has_binding_eq_binding(&guard.expr, left_binding, right_binding)
    })
}

fn mob_coordination_transition_has_binding_gt_u64(
    transition: &TransitionSchema,
    binding: &str,
    value: u64,
) -> bool {
    transition
        .guards
        .iter()
        .any(|guard| mob_coordination_guard_has_binding_gt_u64(&guard.expr, binding, value))
}

fn mob_coordination_effect_status_variant(effect: &EffectEmit, enum_name: &str) -> Result<String> {
    let expr = effect
        .fields
        .iter()
        .find_map(|(field, expr)| (field.as_str() == "status").then_some(expr))
        .with_context(|| format!("{} effect missing `status` field", effect.variant.as_str()))?;
    let Expr::NamedVariant {
        enum_name: emitted_enum,
        variant,
    } = expr
    else {
        bail!(
            "{} effect status must be a `{enum_name}` variant",
            effect.variant.as_str()
        );
    };
    if emitted_enum.as_str() != enum_name {
        bail!(
            "{} effect status used `{}`, expected `{enum_name}`",
            effect.variant.as_str(),
            emitted_enum.as_str()
        );
    }
...
fn mob_coordination_collect_requested_status<'a>(
    expr: &'a Expr,
    enum_name: &str,
    out: &mut Vec<&'a str>,
) {
    match expr {
        Expr::And(items) => {
            for item in items {
                mob_coordination_collect_requested_status(item, enum_name, out);
            }
        }
        Expr::Eq(left, right) => {
            if let (
                ...
                && binding == "requested_status"
                && candidate_enum.as_str() == enum_name
            {
                out.push(variant.as_str());
            }
            if let (
                Expr::NamedVariant {
                    enum_name: candidate_enum,
...
fn mob_coordination_requested_status_guard(
    transition: &TransitionSchema,
    enum_name: &str,
) -> Result<String> {
    let mut variants = Vec::new();
    for guard in &transition.guards {
        mob_coordination_collect_requested_status(&guard.expr, enum_name, &mut variants);
    }
    match variants.as_slice() {
        [variant] => Ok((*variant).to_owned()),
        [] => bail!(
            "{} must guard requested_status against `{enum_name}`",
            transition.name.as_str()
        ),
        _ => bail!(
            "{} has multiple requested_status guards: {:?}",
            transition.name.as_str(),
            variants
        ),
    }
...
fn mob_coordination_guard_requires_unexpired(expr: &Expr) -> bool {
    match expr {
        Expr::And(items) => items.iter().any(mob_coordination_guard_requires_unexpired),
        Expr::Eq(left, right) => {
            matches!(
                (&**left, &**right),
                (Expr::Binding(binding), Expr::Bool(false)) if binding == "is_expired"
            ) || matches!(
                (&**left, &**right),
                (Expr::Bool(false), Expr::Binding(binding)) if binding == "is_expired"
            )
        }
        _ => false,
    }
...
fn mob_coordination_transition_requires_unexpired(transition: &TransitionSchema) -> bool {
    transition
        .guards
        .iter()
        .any(|guard| mob_coordination_guard_requires_unexpired(&guard.expr))
}

fn mob_coordination_derive_status_arms(
    machine: &MachineSchema,
    input: &str,
    effect: &str,
    enum_name: &str,
) -> Result<Vec<MobCoordinationStatusArm>> {
    let transitions = mob_coordination_transitions_for_input(machine, input);
    if transitions.is_empty() {
        bail!("{} missing `{input}` transitions", machine.machine.as_str());
    }
    let mut arms = Vec::new();
    let mut seen = std::collections::BTreeSet::new();
    for transition in transitions {
        let requested = mob_coordination_requested_status_guard(transition, enum_name)?;
        let emitted = mob_coordination_effect_status_variant(
            mob_coordination_single_effect(transition, effect)?,
            enum_name,
        )?;
        if requested != emitted {
            bail!(
                "{} authorizes `{}` but emits `{}`",
                transition.name.as_str(),
                requested,
                emitted
            );
        ...
        if !seen.insert(requested.clone()) {
            bail!(
                "{} has duplicate `{}` transition for `{requested}`",
                machine.machine.as_str(),
                input
            );
        }
        arms.push(MobCoordinationStatusArm {
            variant: requested,
            requires_unexpired: mob_coordination_transition_requires_unexpired(transition),
        });
    }
    Ok(arms)
}

fn mob_coordination_ensure_record_covers_all(
    label: &str,
    variants: &[String],
    arms: &[MobCoordinationStatusArm],
) -> Result<()> {
    let actual: std::collections::BTreeSet<_> =
        arms.iter().map(|arm| arm.variant.as_str()).collect();
    let expected: std::collections::BTreeSet<_> = variants.iter().map(String::as_str).collect();
    if actual != expected {
        bail!("{label} record transitions cover {actual:?}, expected {expected:?}");
    }
    if arms.iter().any(|arm| arm.requires_unexpired) {
        bail!("{label} record transitions must not depend on expiry");
    }
    Ok(())
}

fn mob_coordination_ensure_update_covers_all(
    label: &str,
    variants: &[String],
    arms: &[MobCoordinationStatusArm],
) -> Result<()> {
    ...
    let expected: std::collections::BTreeSet<_> = variants.iter().map(String::as_str).collect();
    if actual != expected {
        bail!("{label} update transitions cover {actual:?}, expected {expected:?}");
    }
    Ok(())
}

fn mob_coordination_derive_initial_u64_authorization(
    machine: &MachineSchema,
    input: &str,
    effect_name: &str,
    effect_field: &str,
    ...
    let value = mob_coordination_init_u64(machine, state_field)?;
    let transition = mob_coordination_single_transition_for_input(machine, input)?;
    let effect = mob_coordination_single_effect(transition, effect_name)?;
    let expr = mob_coordination_effect_expr(effect, effect_field)?;
    if !mob_coordination_expr_is_field(expr, state_field) {
        bail!(
            "{} must emit `{effect_field}` from state field `{state_field}`",
            transition.name.as_str()
        );
    }
    Ok(value)
...
fn mob_coordination_validate_revision_advance(machine: &MachineSchema) -> Result<()> {
    let transition =
        mob_coordination_single_transition_for_input(machine, "AdvanceRecordRevision")?;
    if !mob_coordination_transition_has_binding_eq_binding(
        transition,
        "current_revision",
        "expected_revision",
    ) {
        bail!(
            "{} must require current_revision == expected_revision",
            transition.name.as_str()
        );
    }
    if !mob_coordination_transition_has_binding_gt_u64(transition, "current_revision", 0) {
        bail!(
            "{} must require positive current_revision",
            transition.name.as_str()
        );
    }
    let effect = mob_coordination_single_effect(transition, "RecordRevisionAuthorized")?;
    let expr = mob_coordination_effect_expr(effect, "revision")?;
    if !mob_coordination_expr_is_binding_plus_one(expr, "current_revision") {
        bail!(
            "{} must authorize revision as current_revision + 1",
            transition.name.as_str()
        );
    }
    Ok(())
...
fn mob_coordination_expr_is_named_variant(expr: &Expr, enum_name: &str, variant: &str) -> bool {
    matches!(
        expr,
        Expr::NamedVariant {
            enum_name: candidate_enum,
            variant: candidate_variant,
        } if candidate_enum.as_str() == enum_name && candidate_variant.as_str() == variant
    )
}

fn mob_coordination_validate_record_authorization(
    machine: &MachineSchema,
    input: &str,
    effect_name: &str,
    id_field: &str,
    record_value_fields: &[(&str, &str)],
    required_true_fields: &[&str],
) -> Result<()> {
    let transition = mob_coordination_single_transition_for_input(machine, input)?;
    let effect = mob_coordination_single_effect(transition, effect_name)?;
    mob_coordination_validate_effect_binding(transition, effect, id_field, id_field)?;
    for (effect_field, binding) in record_value_fields {
        mob_coordination_validate_effect_binding(transition, effect, effect_field, binding)?;
    }
    if !mob_coordination_expr_is_field(
        mob_coordination_effect_expr(effect, "revision")?,
        "initial_record_revision",
    ) {
        bail!(
            "{} must authorize record revision from initial_record_revision",
            transition.name.as_str()
        );
    }
    for field in [
        ...
        mob_coordination_validate_effect_binding(transition, effect, field, field)?;
    }
    for field in required_true_fields {
        mob_coordination_validate_effect_binding(transition, effect, field, field)?;
    }
    if !mob_coordination_transition_has_binding_eq_bool(transition, "already_exists", false) {
        bail!(
            "{} must reject duplicate records through the generated guard",
            transition.name.as_str()
        );
    }
    for field in required_true_fields {
        if !mob_coordination_transition_has_binding_eq_bool(transition, field, true) {
            bail!(
                "{} must require `{field}` in the generated guard",
                transition.name.as_str()
            );
        }
    }
    for field in ["metadata_public", "owning_mob_ref_matches"] {
        if !mob_coordination_transition_has_binding_eq_bool(transition, field, true) {
            bail!(
                "{} must require `{field}` in the generated guard",
                transition.name.as_str()
    ...
    if !mob_coordination_transition_has_len_gt_u64(transition, "resource_tokens", 0) {
        bail!(
            "{} must require non-empty resource_tokens in the generated guard",
            transition.name.as_str()
        );
    }
    if !mob_coordination_transition_has_owner_presence(
        transition,
        "owner_principal_key",
        "owner_agent_identity",
    ) {
        bail!(
            "{} must require principal or agent owner in the generated guard",
            transition.name.as_str()
        );
    }
    Ok(())
...
fn mob_coordination_validate_effect_binding(
    transition: &TransitionSchema,
    effect: &EffectEmit,
    effect_field: &str,
    binding: &str,
) -> Result<()> {
    if !mob_coordination_expr_is_binding(
        mob_coordination_effect_expr(effect, effect_field)?,
        binding,
    ) {
        bail!(
            "{} must authorize `{effect_field}` from input binding `{binding}`",
            transition.name.as_str()
        );
    }
    Ok(())
...
fn mob_coordination_validate_event_sequence_publication(
    machine: &MachineSchema,
    input: &str,
    effect_name: &str,
    event_variant: &str,
    ...
    let transition = mob_coordination_single_transition_for_input(machine, input)?;
    if !mob_coordination_transition_has_binding_gt_u64(transition, "current_next_sequence", 0) {
        bail!(
            "{} must require positive current_next_sequence",
            transition.name.as_str()
        );
    }
    let effect = mob_coordination_single_effect(transition, effect_name)?;
    let event_kind = mob_coordination_effect_expr(effect, "event_kind")?;
    if !mob_coordination_expr_is_named_variant(
        event_kind,
        "MobCoordinationEventKind",
        event_variant,
    ) {
        bail!(
            "{} must authorize event kind `{event_variant}`",
            transition.name.as_str()
        );
    }
    for (effect_field, binding) in payload_fields {
        mob_coordination_validate_effect_binding(transition, effect, effect_field, binding)?;
    }
    let sequence = mob_coordination_effect_expr(effect, "sequence")?;
    if !mob_coordination_expr_is_binding(sequence, "current_next_sequence") {
        bail!(
            "{} must authorize event sequence from current_next_sequence",
            transition.name.as_str()
        );
    }
    let next_sequence = mob_coordination_effect_expr(effect, "next_sequence")?;
    if !mob_coordination_expr_is_binding_plus_one(next_sequence, "current_next_sequence") {
        bail!(
            "{} must authorize next event sequence as current_next_sequence + 1",
            transition.name.as_str()
        );
    }
    Ok(())
...
fn validate_mob_coordination_lifecycle_authority_schema(
    machine: &MachineSchema,
) -> Result<MobCoordinationLifecyclePlan> {
    machine
        .validate()
        .context("validate MobCoordinationLifecycleAuthorityMachine schema")?;
    if machine.machine.as_str() != "MobCoordinationLifecycleAuthorityMachine" {
        bail!(
            "expected MobCoordinationLifecycleAuthorityMachine, got {}",
            machine.machine.as_str()
        );
    }
    let work_intent_variants =
        mob_coordination_named_enum_variants(machine, "MobCoordinationWorkIntentStatus")?;
    let resource_claim_variants =
        mob_coordination_named_enum_variants(machine, "MobCoordinationResourceClaimStatus")?;
    let work_intent_record = mob_coordination_derive_status_arms(
        machine,
        "RecordWorkIntentStatus",
        "WorkIntentStatusAuthorized",
        "MobCoordinationWorkIntentStatus",
    )?;
    let work_intent_update = mob_coordination_derive_status_arms(
        machine,
        "UpdateWorkIntentStatus",
        "WorkIntentStatusAuthorized",
        "MobCoordinationWorkIntentStatus",
    )?;
    let resource_claim_record = mob_coordination_derive_status_arms(
        machine,
        "RecordResourceClaimStatus",
        "ResourceClaimStatusAuthorized",
        "MobCoordinationResourceClaimStatus",
    )?;
    let resource_claim_update = mob_coordination_derive_status_arms(
        machine,
        "UpdateResourceClaimStatus",
        "ResourceClaimStatusAuthorized",
        "MobCoordinationResourceClaimStatus",
    )?;
    mob_coordination_ensure_record_covers_all(
        "work intent",
        &work_intent_variants,
        &work_intent_record,
    )?;
    mob_coordination_ensure_record_covers_all(
        "resource claim",
        &resource_claim_variants,
        &resource_claim_record,
    )?;
    mob_coordination_ensure_update_covers_all(
        "work intent",
        &work_intent_variants,
        &work_intent_update,
    )?;
    mob_coordination_ensure_update_covers_all(
        "resource claim",
        &resource_claim_variants,
        &resource_claim_update,
    )?;
    let resource_claim_expired_variant = resource_claim_variants
        .iter()
        .find(|variant| variant.as_str() == "Expired")
        .cloned()
        .context("MobCoordinationResourceClaimStatus missing Expired variant")?;
    if !resource_claim_update
        .iter()
        .any(|arm| arm.variant == resource_claim_expired_variant && !arm.requires_unexpired)
    {
        bail!("resource claim Expired status must be terminal in update transitions");
    }
    let initial_record_revision = mob_coordination_derive_initial_u64_authorization(
        machine,
        "AuthorizeInitialRecordRevision",
        "RecordRevisionAuthorized",
        "revision",
        "initial_record_revision",
    )?;
    mob_coordination_validate_record_authorization(
        machine,
        "RecordWorkIntent",
    ...
    mob_coordination_validate_record_authorization(
        ...
        "claim_id",
        &[("kind", "requested_kind"), ("status", "requested_status")],
        &[],
    )?;
    mob_coordination_validate_revision_advance(machine)?;
    let initial_event_sequence = mob_coordination_derive_initial_u64_authorization(
        machine,
        "AuthorizeInitialEventSequence",
        "EventCursorAuthorized",
        "next_sequence",
        "initial_event_sequence",
    )?;
    mob_coordination_validate_event_sequence_publication(
        machine,
        "PublishWorkIntentRecordedEvent",
    ...
    mob_coordination_validate_event_sequence_publication(
        ...
        &[("claim_id", "claim_id"), ("overlap_ids", "overlap_ids")],
    )?;
    Ok(MobCoordinationLifecyclePlan {
        work_intent_variants,
        resource_claim_variants,
        resource_claim_expired_variant,
        initial_record_revision,
        initial_event_sequence,
        work_intent_update,
        resource_claim_update,
...
fn mob_coordination_transition_has_binding_eq_bool(
    transition: &TransitionSchema,
    binding: &str,
    value: bool,
) -> bool {
    transition
        .guards
        .iter()
        .any(|guard| mob_coordination_guard_has_binding_eq_bool(&guard.expr, binding, value))
}

fn mob_coordination_transition_has_len_gt_u64(
    transition: &TransitionSchema,
    binding: &str,
    value: u64,
) -> bool {
    transition
        .guards
        .iter()
        .any(|guard| mob_coordination_guard_has_len_gt_u64(&guard.expr, binding, value))
}

fn mob_coordination_transition_has_owner_presence(
    transition: &TransitionSchema,
    principal_binding: &str,
    agent_binding: &str,
) -> bool {
    transition.guards.iter().any(|guard| {
        mob_coordination_guard_has_owner_presence(&guard.expr, principal_binding, agent_binding)
    })
}

fn mob_coordination_emit_record_return(out: &mut String) -> Result<()> {
    writeln!(out, "    requested")?;
    Ok(())
}

fn mob_coordination_emit_update_match(
    out: &mut String,
    rust_type: &str,
    arms: &[MobCoordinationStatusArm],
) -> Result<()> {
    for arm in arms {
        if arm.requires_unexpired {
            writeln!(
                out,
                "        {rust_type}::{} if !is_expired => Ok({rust_type}::{}),",
                arm.variant, arm.variant
            )?;
        } else {
            writeln!(
                out,
                "        {rust_type}::{} => Ok({rust_type}::{}),",
                arm.variant, arm.variant
            )?;
        }
    }
...
fn mob_coordination_emit_terminal_match(
    out: &mut String,
    rust_type: &str,
    variants: &[String],
    update_arms: &[MobCoordinationStatusArm],
) -> Result<()> {
    let live_variants: std::collections::BTreeSet<_> = update_arms
        .iter()
        .filter(|arm| arm.requires_unexpired)
        .map(|arm| arm.variant.as_str())
        .collect();
    for variant in variants {
        let terminal = !live_variants.contains(variant.as_str());
        writeln!(out, "        {rust_type}::{variant} => {terminal},")?;
    }
    Ok(())
}

fn generate_mob_coordination_lifecycle_authority(machine: &MachineSchema) -> Result<String> {
    let plan = validate_mob_coordination_lifecycle_authority_schema(machine)?;
    let mut out = String::new();
    writeln!(
        &mut out,
        "// @generated - mob coordination lifecycle authority for `{}`",
        machine.machine.as_str()
    )?;
    writeln!(
        &mut out,
    ...
    writeln!(&mut out, "use std::fmt;")?;
    writeln!(&mut out)?;
    writeln!(&mut out, "use chrono::{{DateTime, Utc}};")?;
    writeln!(&mut out)?;
    writeln!(&mut out, "use meerkat_core::SurfaceMetadataError;")?;
    writeln!(&mut out)?;
    writeln!(
        &mut out,
        "use crate::coordination::{{CoordinationOwner, CoordinationRecordRefs, CoordinationResourceRef, MobCoordinationEvent, MobCoordinationEventKind, NewResourceClaim, NewWorkIntent, ResourceClaim, ResourceClaimId, ResourceClaimKind, ResourceClaimStatus, WorkIntent, WorkIntentId, WorkIntentStatus}};"
    )?;
    writeln!(&mut out, "use crate::ids::MobId;")?;
    writeln!(&mut out)?;
    writeln!(
        &mut out,
        "pub const INITIAL_RECORD_REVISION: u64 = {};",
        plan.initial_record_revision
    )?;
    writeln!(
        &mut out,
        "const INITIAL_EVENT_SEQUENCE: u64 = {};",
        plan.initial_event_sequence
    )?;
    writeln!(&mut out)?;
    ...
    writeln!(
        &mut out,
        "    WorkIntentAlreadyExists {{ id: WorkIntentId }},"
    )?;
    writeln!(
        &mut out,
        "    ResourceClaimAlreadyExists {{ id: ResourceClaimId }},"
    )?;
    writeln!(&mut out, "    EmptyWorkIntentSummary,")?;
    writeln!(&mut out, "    MissingOwner,")?;
    writeln!(&mut out, "    EmptyOwnerAgentIdentity,")?;
    writeln!(&mut out, "    EmptyResources,")?;
    writeln!(
        &mut out,
        "    MismatchedMobRef {{ expected: MobId, actual: MobId }},"
    )?;
    writeln!(&mut out, "    InvalidExpiryTimestamp,")?;
    writeln!(&mut out, "    InvalidMetadata(SurfaceMetadataError),")?;
    writeln!(
        &mut out,
        "    RecordRevisionRejected {{ current: u64, expected: u64 }},"
    )?;
    writeln!(
        &mut out,
        "    EventSequenceRejected {{ next_sequence: u64 }},"
    )?;
    writeln!(
        &mut out,
        "    WorkIntentStatusRejected {{ requested: WorkIntentStatus, is_expired: bool }},"
    )?;
    writeln!(
        &mut out,
        "    ResourceClaimStatusRejected {{ requested: ResourceClaimStatus, is_expired: bool }},"
    )?;
    writeln!(&mut out, "}}")?;
    writeln!(&mut out)?;
    writeln!(
    ...
    writeln!(
        &mut out,
        "            Self::WorkIntentAlreadyExists {{ id }} => write!(f, \"generated mob coordination lifecycle authority rejected duplicate work intent '{{id}}'\"),"
    )?;
    writeln!(
        &mut out,
        "            Self::ResourceClaimAlreadyExists {{ id }} => write!(f, \"generated mob coordination lifecycle authority rejected duplicate resource claim '{{id}}'\"),"
    )?;
    writeln!(
        &mut out,
        "            Self::EmptyWorkIntentSummary => write!(f, \"generated mob coordination lifecycle authority rejected empty work intent summary\"),"
    )?;
    writeln!(
        &mut out,
        "            Self::MissingOwner => write!(f, \"generated mob coordination lifecycle authority rejected record without owner\"),"
    )?;
    writeln!(
        &mut out,
        "            Self::EmptyOwnerAgentIdentity => write!(f, \"generated mob coordination lifecycle authority rejected empty owner agent identity\"),"
    )?;
    writeln!(
        &mut out,
        "            Self::EmptyResources => write!(f, \"generated mob coordination lifecycle authority rejected record without resources\"),"
    )?;
    writeln!(
        &mut out,
        "            Self::MismatchedMobRef {{ expected, actual }} => write!(f, \"generated mob coordination lifecycle authority rejected mob ref mismatch: expected '{{expected}}', actual '{{actual}}'\"),"
    )?;
    writeln!(
        &mut out,
        "            Self::InvalidExpiryTimestamp => write!(f, \"generated mob coordination lifecycle authority rejected expiry timestamp outside u64 millis range\"),"
    )?;
    writeln!(
        &mut out,
        "            Self::InvalidMetadata(error) => write!(f, \"generated mob coordination lifecycle authority rejected metadata: {{error}}\"),"
    )?;
    writeln!(
        &mut out,
        "            Self::RecordRevisionRejected {{ current, expected }} => write!(f, \"generated mob coordination lifecycle authority rejected record revision: expected {{expected}}, current {{current}}\"),"
    )?;
    writeln!(
        &mut out,
        "            Self::EventSequenceRejected {{ next_sequence }} => write!(f, \"generated mob coordination lifecycle authority rejected event sequence {{next_sequence}}\"),"
    )?;
    writeln!(
        &mut out,
        "            Self::WorkIntentStatusRejected {{ requested, is_expired }} => write!(f, \"generated mob coordination lifecycle authority rejected work intent status {{requested:?}} with expired={{is_expired}}\"),"
    )?;
    writeln!(
        &mut out,
        "            Self::ResourceClaimStatusRejected {{ requested, is_expired }} => write!(f, \"generated mob coordination lifecycle authority rejected resource claim status {{requested:?}} with expired={{is_expired}}\"),"
    )?;
    writeln!(&mut out, "        }}")?;
    writeln!(&mut out, "    }}")?;
    writeln!(&mut out, "}}")?;
...
fn expires_at_utc_ms(
    ...
    expires_at
        .map(|expires_at| {
            u64::try_from(expires_at.timestamp_millis()).map_err(|_| {
                MobCoordinationLifecycleAuthorityError::InvalidExpiryTimestamp
            })
        })
        .transpose()
}
...
pub fn authorize_work_intent_record(
    ...
    if already_exists {
        return Err(MobCoordinationLifecycleAuthorityError::WorkIntentAlreadyExists {
            id: draft.id,
        });
    }
...
fn authorize_event_publication(
    ...
    let next_event_sequence = next_sequence.checked_add(1).ok_or(
        MobCoordinationLifecycleAuthorityError::EventSequenceRejected { next_sequence },
    )?;
...
"#,
    ...
    mob_coordination_emit_record_return(&mut out)?;
    writeln!(&mut out, "}}")?;
    writeln!(&mut out)?;
    writeln!(
        &mut out,
        "pub fn authorize_resource_claim_record_status(requested: ResourceClaimStatus) -> ResourceClaimStatus {{"
    )?;
    mob_coordination_emit_record_return(&mut out)?;
    writeln!(&mut out, "}}")?;
    writeln!(&mut out)?;
    writeln!(
        &mut out,
        "pub fn authorize_work_intent_update_status(requested: WorkIntentStatus, is_expired: bool) -> Result<WorkIntentStatus, MobCoordinationLifecycleAuthorityError> {{"
    )?;
    writeln!(&mut out, "    match requested {{")?;
    mob_coordination_emit_update_match(&mut out, "WorkIntentStatus", &plan.work_intent_update)?;
    writeln!(
        &mut out,
        "        other => Err(MobCoordinationLifecycleAuthorityError::WorkIntentStatusRejected {{ requested: other, is_expired }}),"
    )?;
    writeln!(&mut out, "    }}")?;
    writeln!(&mut out, "}}")?;
    writeln!(&mut out)?;
    writeln!(
        &mut out,
        "pub fn authorize_resource_claim_update_status(requested: ResourceClaimStatus, is_expired: bool) -> Result<ResourceClaimStatus, MobCoordinationLifecycleAuthorityError> {{"
    )?;
    writeln!(&mut out, "    match requested {{")?;
    mob_coordination_emit_update_match(
        &mut out,
        "ResourceClaimStatus",
        &plan.resource_claim_update,
    )?;
    writeln!(
        &mut out,
        "        other => Err(MobCoordinationLifecycleAuthorityError::ResourceClaimStatusRejected {{ requested: other, is_expired }}),"
    )?;
    writeln!(&mut out, "    }}")?;
    writeln!(&mut out, "}}")?;
    writeln!(&mut out)?;
    ...
    writeln!(&mut out, "    match status {{")?;
    mob_coordination_emit_terminal_match(
        &mut out,
        "WorkIntentStatus",
        &plan.work_intent_variants,
        &plan.work_intent_update,
    )?;
    writeln!(&mut out, "    }}")?;
    writeln!(&mut out, "}}")?;
    writeln!(&mut out)?;
    ...
    writeln!(
        &mut out,
        "    matches!(status, ResourceClaimStatus::{})",
        plan.resource_claim_expired_variant
    )?;
    writeln!(&mut out, "}}")?;
    Ok(out)
}

pub fn render_mob_coordination_lifecycle_authority(machine: &MachineSchema) -> Result<String> {
    generate_mob_coordination_lifecycle_authority(machine)
}
...
pub fn render_session_deferred_turn_authority(machine: &MachineSchema) -> Result<String> {
    generate_session_deferred_turn_authority(machine)
}
...
fn generate_session_deferred_turn_authority(machine: &MachineSchema) -> Result<String> {
    validate_session_deferred_turn_authority_schema(machine)?;
    let mut out = String::new();
    writeln!(
        &mut out,
        "// @generated - session deferred-turn authority for `{}`",
        machine.machine
    )?;
    writeln!(
        &mut out,
    ...
    writeln!(&mut out, "        SessionDeferredTurnState,")?;
    writeln!(&mut out, "    }},")?;
    writeln!(&mut out, "    time_compat::SystemTime,")?;
    writeln!(&mut out, "    types::{{ContentInput, ToolResult}},")?;
    writeln!(&mut out, "}};")?;
    writeln!(&mut out)?;

    let cfg = SessionAuthorityRenderConfig {
        schema_name: "SessionDeferredTurnAuthorityMachine",
        input_enum: "SessionDeferredTurnAuthorityInput",
        effect_enum: "SessionDeferredTurnAuthorityEffect",
        phase_enum: "SessionDeferredTurnAuthorityPhase",
        state_struct: "SessionDeferredTurnAuthorityMachineState",
        authority_struct: "SessionDeferredTurnAuthorityMachineAuthority",
        error_struct: "SessionDeferredTurnAuthorityError",
        error_message: "generated session deferred-turn authority rejected",
    };
    emit_session_named_string_enum(
        &mut out,
        machine,
    ...
    emit_session_named_string_enum(
        &mut out,
        machine,
        "InitialPromptStageDecision",
        "Clear",
        cfg.schema_name,
    )?;
    emit_session_authority_core(&mut out, machine, cfg)?;
    emit_session_deferred_turn_domain_helpers(&mut out)?;
    Ok(out)
}
...
fn emit_session_deferred_turn_domain_helpers(out: &mut String) -> Result<()> {
    let source = r#"
fn deferred_turn_authority_effect(input: SessionDeferredTurnAuthorityInput, expected: &'static str) -> Result<Vec<SessionDeferredTurnAuthorityEffect>, SessionDeferredTurnAuthorityError> {
    let mut authority = SessionDeferredTurnAuthorityMachineAuthority::new();
    let effects = authority.apply_input(input)?;
    if effects.iter().any(|effect| matches!(
        (expected, effect),
        ("FirstTurnPhaseResolved", SessionDeferredTurnAuthorityEffect::FirstTurnPhaseResolved { .. })
            | ("InitialTurnOverridesResolved", SessionDeferredTurnAuthorityEffect::InitialTurnOverridesResolved { .. })
            | ("InitialPromptStageResolved", SessionDeferredTurnAuthorityEffect::InitialPromptStageResolved { .. })
            | ("ToolResultsStageResolved", SessionDeferredTurnAuthorityEffect::ToolResultsStageResolved { .. })
            | ("ConsumedInputsRestoreResolved", SessionDeferredTurnAuthorityEffect::ConsumedInputsRestoreResolved { .. })
            | ("SnapshotRestoreAuthorized", SessionDeferredTurnAuthorityEffect::SnapshotRestoreAuthorized)
    )) {
        Ok(effects)
    } else {
...
pub fn resolve_tool_results_stage(
    ...
    for effect in effects {
        if let SessionDeferredTurnAuthorityEffect::ToolResultsStageResolved { accepted_count } = effect {
...
fn first_turn_phase_effect(
    ...
    for effect in effects {
        if let SessionDeferredTurnAuthorityEffect::FirstTurnPhaseResolved { phase, was_pending } = effect {
...
fn validate_session_deferred_turn_authority_schema(machine: &MachineSchema) -> Result<()> {
    machine
        .validate()
        .context("validate SessionDeferredTurnAuthorityMachine schema")?;
    if machine.machine.as_str() != "SessionDeferredTurnAuthorityMachine" {
        bail!(
            "session deferred-turn generator received unexpected machine `{}`",
            machine.machine
        );
    }
    for required in [
        ...
        machine.inputs.variant_named(required).with_context(|| {
            format!("SessionDeferredTurnAuthorityMachine missing input `{required}`")
        })?;
    }
    for required in [
        "FirstTurnPhaseResolved",
        ...
        machine.effects.variant_named(required).with_context(|| {
            format!("SessionDeferredTurnAuthorityMachine missing effect `{required}`")
        })?;
    }
    for required in [
        "MarkInitialTurnPendingInactiveOrPending",
        "MarkInitialTurnPendingConsumed",
        "StartInitialTurnPending",
        "StartInitialTurnInactive",
        "StartInitialTurnConsumed",
        "RestoreInitialTurnPending",
        "ResolveInitialTurnOverridesAllowed",
        "ResolveInitialTurnOverridesDenied",
        "ResolveInitialPromptStore",
        "ResolveInitialPromptClear",
        "ResolveToolResultsStage",
        "ResolveConsumedInputsRestore",
        "AuthorizeRestoreDeferredTurnState",
    ] {
        if !machine
            .transitions
            .iter()
            .any(|transition| transition.name.as_str() == required)
        {
            bail!("SessionDeferredTurnAuthorityMachine missing transition `{required}`");
        }
    }
    for required in ["DeferredTurnFirstTurnPhase", "InitialPromptStageDecision"] {
        let binding = machine
            .named_types
            .iter()
            .find(|binding| binding.name.as_str() == required)
            .with_context(|| {
                format!("SessionDeferredTurnAuthorityMachine missing named type `{required}`")
            })?;
        if !matches!(binding.rust, RustTypeAtom::StringEnum { .. }) {
            bail!(
                "SessionDeferredTurnAuthorityMachine named type `{required}` must be a string enum"
            );
        }
    }
    Ok(())
