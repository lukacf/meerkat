...
```typescript
interface PeerContentIngestedEvent {
  type: "peer_content_ingested";
  kind: CommsNoticeKind;                       // message | request | response_* | lifecycle
  peer?: SystemNoticePeer | null;              // canonical peer id + display name, when resolved
  request_id?: string | null;                  // correlation id for request/response conventions
  sender_taint?: SenderContentTaint | null;    // "clean" | "tainted"; absent = no declaration
}
```

Emitted when inbound peer content is committed into the session's context
(both queued deliveries and steered mid-turn context). `sender_taint` is the
...
console.log(CONTRACT_VERSION);  // "0.8.20"
```
