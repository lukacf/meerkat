...
- **Detached (hosts that outlive the call).** A host with a runtime adapter
  ...
  returns, provided the convener can be made live again to receive it: it is
  a mob member, or the host installs a `DetachedOwnerHost` (the JSON-RPC host
  does). There the call returns at once with
  ...
  idempotency key `council:<job_id>`: a convener in the middle of a turn sees
  it at that turn's next model call, saved once and with no second turn (one
  follow-up turn only if the turn ends before another model call), and an
  idle convener runs one wake turn that sees it. A
  ...
  host's `DetachedOwnerHost` when it is a plain session. The status is
...
- **Blocking.** The call waits for the council and returns the sealed outcome
  as its ordinary tool result, with `blocked_because`:
  `host_declared_unavailable` on a host that may exit when the turn ends (the
  `rkat` CLI unless it stays alive, such as `rkat run` without
  `--keep-alive`); `no_runtime_adapter` for a host that declares delivery
  without a runtime; or `no_owner_revival_host` for a convener that is a plain
  session on a host without a `DetachedOwnerHost` (a top-level REST,
  MCP-server, or `rkat run --keep-alive` session). No detached job is owed,
