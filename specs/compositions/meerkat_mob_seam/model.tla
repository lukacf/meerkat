...
mob_SubmitWorkRunning(arg_agent_runtime_id, arg_fence_token, arg_work_id) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       ...
       /\ mob_phase' = "Running"
       /\ UNCHANGED << meerkat_phase, meerkat_session_id, meerkat_active_runtime_id, meerkat_active_fence_token, meerkat_active_generation, meerkat_attachment_live, meerkat_current_run_id, meerkat_pre_run_phase, meerkat_peer_ingress_configured, meerkat_drain_running, meerkat_silent_intent_overrides, meerkat_active_requested_deferred_names, meerkat_staged_requested_deferred_names, meerkat_active_visibility_revision, meerkat_staged_visibility_revision, mob_live_runtime_ids, mob_runtime_fence_tokens, mob_active_member_count, mob_active_run_count, mob_pending_spawn_count, mob_wiring_edge_count, mob_coordinator_bound, witness_current_script_input, witness_remaining_script_inputs >>
       /\ pending_inputs' = AppendIfMissing(SeqRemove(pending_inputs, packet), [machine |-> "meerkat", variant |-> "Ingest", payload |-> [runtime_id |-> packet.payload.agent_runtime_id], source_kind |-> "route", source_route |-> "work_request_reaches_meerkat", source_machine |-> "mob", source_effect |-> "RequestRuntimeIngress", effect_id |-> (model_step_count + 1)])
       /\ observed_inputs' = observed_inputs \cup {[machine |-> "meerkat", variant |-> "Ingest", payload |-> [runtime_id |-> packet.payload.agent_runtime_id], source_kind |-> "route", source_route |-> "work_request_reaches_meerkat", source_machine |-> "mob", source_effect |-> "RequestRuntimeIngress", effect_id |-> (model_step_count + 1)]}
       /\ pending_routes' = pending_routes
       /\ delivered_routes' = delivered_routes \cup { [route |-> "work_request_reaches_meerkat", source_machine |-> "mob", effect |-> "RequestRuntimeIngress", target_machine |-> "meerkat", target_input |-> "Ingest", payload |-> [runtime_id |-> packet.payload.agent_runtime_id], actor |-> "meerkat_kernel", effect_id |-> (model_step_count + 1), source_transition |-> "SubmitWorkRunning"] }
       /\ emitted_effects' = emitted_effects \cup { [machine |-> "mob", variant |-> "RequestRuntimeIngress", payload |-> [agent_runtime_id |-> packet.payload.agent_runtime_id, fence_token |-> packet.payload.fence_token, work_id |-> packet.payload.work_id], effect_id |-> (model_step_count + 1), source_transition |-> "SubmitWorkRunning"] }
...
CoreNext ==
    ...
    \/ meerkat_EnsureDrainRunningAttached
    \/ meerkat_EnsureDrainRunningRunning
    \/ \E arg_runtime_id \in AgentRuntimeIdValues : meerkat_IngestIdle(arg_runtime_id)
    \/ \E arg_runtime_id \in AgentRuntimeIdValues : meerkat_IngestAttached(arg_runtime_id)
    \/ \E arg_runtime_id \in AgentRuntimeIdValues : meerkat_IngestRunning(arg_runtime_id)
...
WitnessStateConstraint_basic_round_trip == /\ model_step_count <= 8 /\ Len(pending_inputs) <= 8 /\ Cardinality(observed_inputs) <= 14 /\ Len(pending_routes) <= 8 /\ Cardinality(delivered_routes) <= 4 /\ Cardinality(emitted_effects) <= 0 /\ Cardinality(observed_transitions) <= 8 /\ Cardinality(meerkat_silent_intent_overrides) <= 0 /\ Cardinality(meerkat_active_requested_deferred_names) <= 0 /\ Cardinality(meerkat_staged_requested_deferred_names) <= 0 /\ Cardinality(mob_live_runtime_ids) <= 0 /\ Cardinality(DOMAIN mob_runtime_fence_tokens) <= 0
