...
[dependencies]
meerkat = { version = "0.8.20", default-features = false, features = ["anthropic"] }

# Add persistence
meerkat = { version = "0.8.20", default-features = false, features = ["anthropic", "session-store"] }

# Add compaction
meerkat = { version = "0.8.20", default-features = false, features = ["anthropic", "session-store", "session-compaction"] }

# Add scheduling
meerkat = { version = "0.8.20", default-features = false, features = ["anthropic", "session-store", "schedule"] }

# Kitchen sink
meerkat = { version = "0.8.20", features = [
...
- **Guards**: Never on first turn. Ordinary compactions wait at least 3 turns;
  ...
  cost guard so an already-oversized request can recover immediately.
