...
- Crash during an in-flight turn loses at most the work the intra-turn
  checkpointer had not yet projected; any durable tail it did write is
...
The intra-turn checkpointer writes best-effort projections to the canonical
session store outside the boundary transaction, so a crash or shutdown race
can leave a durable transcript tail — up to and including a fully completed
turn — whose runtime boundary commit never landed. Durable is not the same as
runtime-committed, and neither copy is silently preferred: a cold read drives
a typed, machine-owned read-source decision (serve the committed runtime
snapshot, serve the committed store head, recovery required, or quarantine)
instead of a shell comparison.

The recovery rule is never-discard: every verified durable descendant of the
committed authority is preserved.
...
- **`load_canonical_head`** returns the persisted head row ONLY when
  ...
  `load_head` (which may synthesize a deterministic head from a legacy blob —
  an O(document) parse), it answers `None` for absent AND blob-only sessions
  and never reads the blob. `Some` promises the row is the persisted head
  itself and that `load_messages` over `head.strand` page-serves exactly the
  rows the head covers.
...
**The service read path is unchanged in this release**: `read_history`,
`read_transcript_revision`, and `list_transcript_revisions` still perform the
full authoritative load (with its arbitration, stamp verification, and
rewrite-log replay/audit backfill) on every read. These verbs are the store
substrate for the head-trusted range-read fast paths scheduled for the next
release; nothing consumes them on the read path yet.
