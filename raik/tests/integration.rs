//! Integration tests for RAIK (Gate 2: Choke Point Tests)
//!
...
mod mcp_protocol {
    ...
    #[tokio::test]
    async fn test_raik_mcp_server_tools_list() {
        // Test with raik-mcp-server if available
        let manifest_dir = std::env::var("CARGO_MANIFEST_DIR").unwrap_or_default();
        let workspace_root = std::path::Path::new(&manifest_dir).parent().unwrap_or(std::path::Path::new("."));
        let server_path = workspace_root.join("target/debug/raik-mcp-server");

        if !server_path.exists() {
            eprintln!("Skipping: raik-mcp-server not built (run cargo build -p raik-mcp-server)");
            return;
        }
        ...
        let connection = McpConnection::connect(&config)
            .await
            .expect("Should connect to raik-mcp-server");

        // List tools
        let tools = connection.list_tools().await.expect("Should list tools");

        // Verify raik_run tool exists
        let raik_run = tools
            .iter()
            .find(|t| t.name == "raik_run")
            .expect("raik-mcp-server should have raik_run tool");
        assert_eq!(raik_run.name, "raik_run");

        // Verify raik_resume tool exists
        let raik_resume = tools
            .iter()
            .find(|t| t.name == "raik_resume")
            .expect("raik-mcp-server should have raik_resume tool");
        assert_eq!(raik_resume.name, "raik_resume");
