...
Meerkat is a library-first agent runtime. The same execution substrate powers the CLI, REST, JSON-RPC, MCP, Rust embedding, Python/TypeScript SDKs layered on top of `rkat-rpc`, and the browser/WASM runtime. Reduced CLI/RPC profiles remain available as source builds for custom distributions.
...
| `rkat-mini` | Runtime-backed source-build reduced profile | Smaller CLI command set for custom embeddable/product distributions |
| `rkat-rpc` | Runtime-backed | JSON-RPC 2.0 over stdio/TCP, SDK backend, optional `live/*` audio/text WebSocket adapter (`--live-ws`) |
| `rkat-rpc-mini` | Runtime-backed source-build reduced profile | Core session/config/catalog/capability RPC subset for custom distributions |
...
| Live audio/text adapter | Optional `meerkat-live` adapter exposed by `rkat-rpc --live-ws`; provider session lifecycle is owned by the live adapter, not by `MeerkatMachine`. `live/open` requires a runtime model whose `ModelCapabilities.realtime == true` and a provider with a wired live adapter |
...
- The `live/*` adapter surface is capability-gated: opening a live channel for a session requires the session's resolved model to have `ModelCapabilities.realtime == true` and the provider to have a registered live adapter factory.
...
code and records that live reducer transitions are gated behind
...
| `meerkat-runtime` | `MeerkatMachine`, runtime control plane, session binding/recovery, live-adapter host orchestration |
