...
async fn wait_for_input_state(
    ...
    for _ in 0..40 {
        let (status, payload) = request_json(
            app,
            Method::GET,
            format!("/sessions/{session_id}/submissions/{input_id}"),
            None,
        )
        .await;
...
async fn e2e_scenario_21_rest_runtime_accept_input_roundtrip() {
    ...
    // REST session/status lazily attaches an executor, so the initial state
    ...
    let (status, listed) = request_json(
        &app,
        Method::GET,
        format!("/sessions/{session_id}/submissions"),
        None,
    )
    .await;
    assert_eq!(status, StatusCode::OK);
...
async fn e2e_scenario_22_rest_runtime_reset_and_retire_drain_staged_inputs() {
...

    {
        ...
        let (status, accepted) = request_json(
            &app,
            Method::POST,
            format!("/sessions/{reset_session_id}/submit"),
        ...
        let (status, reset_payload) = request_json(
            &app,
            Method::POST,
            format!("/sessions/{reset_session_id}/reset"),
            None,
        )
        .await;
        assert_eq!(status, StatusCode::OK);
        ...
        let (status, runtime_state) = request_json(
            &app,
            Method::GET,
            format!("/sessions/{reset_session_id}/status"),
            None,
        )
        .await;
        assert_eq!(status, StatusCode::OK);
    ...
    let (status, accepted) = request_json(
        &app,
        Method::POST,
        format!("/sessions/{retire_session_id}/submit"),
        Some(durable_prompt_input(
    ...
    let (status, retire_payload) = request_json(
        &app,
        Method::POST,
        format!("/sessions/{retire_session_id}/retire"),
        None,
    )
    .await;
    assert_eq!(status, StatusCode::OK);
    ...
    let (status, runtime_state) = request_json(
        &app,
        Method::GET,
        format!("/sessions/{retire_session_id}/status"),
        None,
    )
    .await;
    assert_eq!(status, StatusCode::OK);
    ...
    let (status, reset_payload) = request_json(
        &app,
        Method::POST,
        format!("/sessions/{retire_session_id}/reset"),
        None,
    )
    .await;
    assert_eq!(status, StatusCode::OK);
