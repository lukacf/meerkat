...
The two durable resume holds are not service faults: the session exists, its
...
structured payload (`tail_held_for_recovery` / `evidence_quarantined`, plus
`session_error_code`, `session_id`, and `content_retained: true`) so callers
distinguish the hold class without parsing messages.
...
[dependencies]
meerkat = { version = "0.8.10", default-features = false, features = ["anthropic"] }

# Add persistence
meerkat = { version = "0.8.10", default-features = false, features = ["anthropic", "session-store"] }

# Add compaction
meerkat = { version = "0.8.10", default-features = false, features = ["anthropic", "session-store", "session-compaction"] }

# Add scheduling
meerkat = { version = "0.8.10", default-features = false, features = ["anthropic", "session-store", "schedule"] }

# Kitchen sink
meerkat = { version = "0.8.10", features = [
