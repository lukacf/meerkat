...
- 📌 Remaining: A5 (DSL-schema migration), C5 — all tracked, all require regression coverage per the "extensive regression tests" standard. Priority order:
    - P0 (dogma-critical): A5-DSL (full schema migration from `MeerkatId` → `AgentIdentity` inside `MobMachineCommand` variants).
    - P2 (surface completeness): C5 (blocked on work-tracking ledger per `handle.rs:1010`).
