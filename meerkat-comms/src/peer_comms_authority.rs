...
//!
//! The live runtime is long-lived, so the authority cannot be a one-shot batch
//! reducer. `Dropped` and `Delivered` therefore act as reusable steady states:
//! future trusted ingress can move back to `Received`, and trust mutations can
//! happen without resetting queued work.
//!
//! - 4 states: Absent, Received, Dropped, Delivered
//! - 3 inputs: TrustPeer, ReceivePeerEnvelope, SubmitTypedPeerInput
//! - reusable transitions with guards (peer_is_trusted, peer_is_not_trusted,
...
pub(crate) enum PeerCommsInput {
    ...
    /// Remove a peer from the current trusted set.
    RemoveTrustedPeer { peer_id: PeerId },
...
fn class_for(raw_kind: &RawPeerKind) -> PeerInputClass {
    match raw_kind {
        ...
        RawPeerKind::PeerLifecycleAdded => PeerInputClass::PeerLifecycleAdded,
        RawPeerKind::PeerLifecycleRetired => PeerInputClass::PeerLifecycleRetired,
        ...
        RawPeerKind::Ack => PeerInputClass::Ack,
...
struct PeerCommsFields {
    ...
    lifecycle_peer: BTreeMap<RawItemId, Option<String>>,
...
impl PeerCommsAuthority {
    ...
    pub(crate) fn new() -> Self {
        Self::new_with_auth_required(true)
    }

    /// Create an authority in the initial `Absent` state with explicit
    /// peer-auth policy.
    pub(crate) fn new_with_auth_required(auth_required: bool) -> Self {
        Self {
            phase: PeerIngressState::Absent,
            fields: PeerCommsFields::new(auth_required),
        }
    }
    ...
    pub(crate) fn with_phase(phase: PeerIngressState) -> Self {
        Self {
            phase,
            fields: PeerCommsFields::new(true),
        }
    }
    ...
    /// Whether a raw item currently has a classification entry.
    pub(crate) fn has_classified_item(&self, raw_item_id: &RawItemId) -> bool {
        self.fields.classified_as.contains_key(raw_item_id)
    }

    /// Recorded trust decision for a raw item at ingress time.
    pub(crate) fn trusted_snapshot_for(&self, raw_item_id: &RawItemId) -> Option<bool> {
        self.fields.trusted_snapshot.get(raw_item_id).copied()
    }
    ...
    fn evaluate(
        ...
        let next_phase = match input {
            // -----------------------------------------------------------------
            // TrustPeer: self-loop from any phase
            ...
            // -----------------------------------------------------------------
            // RemoveTrustedPeer: self-loop from any phase
            // Updates: set_remove trusted_peers
            // -----------------------------------------------------------------
            PeerCommsInput::RemoveTrustedPeer { peer_id } => {
                fields.trusted_peers.remove(peer_id);
                phase
            }

            // -----------------------------------------------------------------
            // ReceivePeerEnvelope (admitted): any phase → Received
            // Guard: peer_is_trusted or auth_is_open
            // -----------------------------------------------------------------
            // ReceivePeerEnvelope (untrusted): if queue empty → Dropped
            //                                else → Received
            ...
            PeerCommsInput::ReceivePeerEnvelope {
                ...
                let admitted = trusted || !fields.auth_required;
                let had_queued_work = !fields.submission_queue.is_empty();
                ...
                fields
                    .lifecycle_peer
                    .insert(raw_item_id.clone(), lifecycle_peer.clone());

                if admitted {
                    // ReceiveAdmittedPeerEnvelope transition
                    fields
                        .classified_as
                        .insert(raw_item_id.clone(), class_for(raw_kind));
                    fields.trusted_snapshot.insert(raw_item_id.clone(), trusted);
                    fields.submission_queue.push_back(raw_item_id.clone());
                    PeerIngressState::Received
                } else {
                    // DropUntrustedPeerEnvelope transition
                    fields.trusted_snapshot.insert(raw_item_id.clone(), false);
                    if had_queued_work {
                        PeerIngressState::Received
                    } else {
            ...
            PeerCommsInput::SubmitTypedPeerInput { raw_item_id } => {
                ...
                effects.push(PeerCommsEffect::SubmitPeerInputCandidate(
                    SubmitPeerInputCandidate {
                        ...
                        lifecycle_peer: lifecycle_peer_val,
                    },
                ));

                // Remove from queue
                fields.submission_queue.retain(|id| id != raw_item_id);
...
mod tests {
    ...
    #[test]
    fn remove_trusted_peer_from_received_keeps_phase() {
        let mut auth = make_authority();
        auth.apply(PeerCommsInput::TrustPeer {
            peer_id: peer("alice"),
        })
        ...
        let t = auth
            ...
            .expect("remove trust from received");
        assert_eq!(t.next_phase, PeerIngressState::Received);
        assert!(!auth.is_peer_trusted(&peer("alice")));
    ...
    fn receive_from_dropped_can_resume_to_received_when_peer_is_trusted() {
        ...
        let t = auth
            ...
            .expect("receive from dropped");
        assert_eq!(t.next_phase, PeerIngressState::Received);
        assert_eq!(auth.queue_len(), 1);
    }

    #[test]
    fn receive_from_delivered_can_start_new_batch() {
        let mut auth = PeerCommsAuthority::with_phase(PeerIngressState::Delivered);
        auth.apply(PeerCommsInput::TrustPeer {
            peer_id: peer("alice"),
        })
    ...
    #[test]
    fn receive_untrusted_when_auth_disabled_transitions_to_received() {
        let mut auth = PeerCommsAuthority::new_with_auth_required(false);
        let t = auth
            .apply(receive_envelope(
                item("item-1"),
                peer("unknown"),
            ...
            .expect("receive untrusted with auth disabled");
        assert_eq!(t.next_phase, PeerIngressState::Received);
        assert_eq!(auth.queue_len(), 1);
        assert_eq!(
            auth.fields.classified_as.get(&item("item-1")),
    ...
    #[test]
    fn receive_from_dropped_can_drop_again() {
        let mut auth = make_authority();
        auth.apply(receive_envelope(
            item("item-1"),
            peer("untrusted"),
        ...
        let t = auth
            ...
            .expect("receive after dropped");
        assert_eq!(t.next_phase, PeerIngressState::Dropped);
        assert_eq!(auth.queue_len(), 0);
    }

    #[test]
    fn receive_untrusted_while_queue_has_work_does_not_poison_received_phase() {
        let mut auth = make_authority();
        auth.apply(PeerCommsInput::TrustPeer {
            peer_id: peer("alice"),
        })
        ...
        let t = auth
            ...
            .expect("untrusted receive while queue has work");

        assert_eq!(t.next_phase, PeerIngressState::Received);
        assert_eq!(auth.queue_len(), 1);
        assert_eq!(
            auth.fields.trusted_snapshot.get(&item("untrusted-1")),
            Some(&false)
        );
        assert!(
            !auth.fields.classified_as.contains_key(&item("untrusted-1")),
            "untrusted ingress must still not classify work"
        );
    }

    #[test]
    fn remove_trust_revokes_future_receive_but_not_existing_queue() {
        let mut auth = make_authority();
        auth.apply(PeerCommsInput::TrustPeer {
            peer_id: peer("alice"),
        })
        ...
        .unwrap();
        auth.apply(PeerCommsInput::RemoveTrustedPeer {
            peer_id: peer("alice"),
        })
        .unwrap();
        ...
        let t = auth
            ...
            .expect("receive after trust removal");

        assert_eq!(t.next_phase, PeerIngressState::Received);
        assert_eq!(auth.queue_len(), 1);
        assert!(
            !auth.fields.classified_as.contains_key(&item("trusted-2")),
            "future ingress after trust removal must not be classified when auth is required"
        );
        assert_eq!(
            auth.fields.trusted_snapshot.get(&item("trusted-2")),
            Some(&false)
        );
    }
    ...
    #[test]
    fn delivered_batch_can_accept_future_work() {
        let mut auth = make_authority();
        auth.apply(PeerCommsInput::TrustPeer {
            peer_id: peer("alice"),
        })
        ...
        assert_eq!(auth.phase(), PeerIngressState::Delivered);
        ...
        let t = auth
            ...
            .expect("receive new batch after delivered");
        assert_eq!(t.next_phase, PeerIngressState::Received);
        assert_eq!(auth.queue_len(), 1);
    }
    ...
    fn class_for_response_terminal() {
        assert_eq!(
            class_for(&RawPeerKind::ResponseTerminal),
    ...
    fn class_for_response_progress() {
        assert_eq!(
            class_for(&RawPeerKind::ResponseProgress),
            PeerInputClass::Response
        );
    }
    ...
    fn class_for_plain_event() {
        assert_eq!(
            class_for(&RawPeerKind::PlainEvent),
    ...
    #[test]
    fn class_for_ack() {
        assert_eq!(class_for(&RawPeerKind::Ack), PeerInputClass::Ack);
    }

    #[test]
    fn class_for_peer_lifecycle_added() {
        assert_eq!(
            class_for(&RawPeerKind::PeerLifecycleAdded),
            PeerInputClass::PeerLifecycleAdded
        );
    }

    #[test]
    fn class_for_peer_lifecycle_retired() {
        assert_eq!(
            class_for(&RawPeerKind::PeerLifecycleRetired),
    ...
    fn class_for_silent_request() {
        assert_eq!(
            class_for(&RawPeerKind::SilentRequest),
    ...
    fn submit_effect_carries_correlation_fields() {
        ...
        match &t.effects[0] {
            PeerCommsEffect::SubmitPeerInputCandidate(effect) => {
                ...
                assert_eq!(effect.reservation_key, Some(ReservationKey("rsv-7".into())));
                assert_eq!(effect.lifecycle_peer, None);
            }
        }
    }

    #[test]
    fn submit_effect_carries_lifecycle_peer() {
        let mut auth = make_authority();
        auth.apply(PeerCommsInput::TrustPeer {
            peer_id: peer("alice"),
        })
        ...
        auth.apply(PeerCommsInput::ReceivePeerEnvelope {
            raw_item_id: item("item-1"),
            peer_id: peer("alice"),
            raw_kind: RawPeerKind::PeerLifecycleAdded,
            text_projection: "peer added".to_string(),
            content_shape: ContentShape::Text,
            request_id: None,
            reservation_key: None,
            lifecycle_peer: Some("new-agent".to_string()),
        })
        .unwrap();
