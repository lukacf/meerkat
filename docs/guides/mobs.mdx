...
- `[profiles.<name>]`: profile model, tools, addressing behavior.
  - Optional `max_inline_peer_notifications`: controls inline peer lifecycle context (`None` default=50, `0` never inline, `-1` always inline, `>0` thresholded by peer count).
