...
description: "Comprehensive API reference for using Meerkat as a Rust library."
icon: "code"
---

The Meerkat Rust SDK gives you direct access to the agent loop, session lifecycle, tool dispatch, and LLM providers as library code. Use it when you need full control or are embedding Meerkat in a Rust application.
...
| **Types** | `Message`, `ToolCall`, `ToolResult` | Core conversation types |
| | `RunResult` | Result of a successful agent run |
| | `AgentEvent` | Streaming event variants |
| **Providers** | `AnthropicClient`, `OpenAiClient`, `GeminiClient` | Built-in LLM clients |
| **Storage** | `JsonlStore`, `MemoryStore` | Session persistence backends |
| **MCP** | `McpRouter` | Route tool calls to MCP servers |
...
<Steps>
  ...
  <Step title="Choose feature flags">
    ...
    <CodeGroup>
    ```toml Default (Anthropic + JSONL storage)
    [dependencies]
    meerkat = "0.1"
    ```

    ```toml All LLM providers
    [dependencies]
    meerkat = { version = "0.1", features = ["all-providers"] }
    ```

    ```toml Specific providers
    [dependencies]
    meerkat = { version = "0.1", features = ["anthropic", "openai", "gemini"] }
    ```

    ```toml Storage backends
    [dependencies]
...
<Accordion title="With per-request configuration (AgentBuildConfig)">
...
<Accordion title="With custom tools">
...
`SessionService` is the canonical lifecycle API for all session management. All surfaces (CLI, REST, MCP Server, JSON-RPC) route through it.
...
[Sessions](#sessions) above). `AgentBuilder` is used internally by
...
<Accordion title="Budget management details">
```rust
...
A complete example using `SessionService` for session lifecycle:
...
<Tabs>
  <Tab title="Python">
    ...
    - **Docs:** [Python SDK reference](/sdks/python)
  ...
  <Tab title="TypeScript">
    ...
    - **Node.js:** >= 18
    - **Docs:** [TypeScript SDK reference](/sdks/typescript)
...
- [Session contracts](/reference/session-contracts) - Session lifecycle operational contracts
