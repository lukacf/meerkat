...
use meerkat::surface::{
    ...
    build_recovered_session, install_prepared_runtime_interrupt_handle, prepare_surface_session,
    request_action,
};
use meerkat::{
...
async fn cleanup_prepared_mcp_runtime(
    service: &Arc<PersistentSessionService<FactoryAgentBuilder>>,
    runtime_adapter: &Arc<meerkat_runtime::MeerkatMachine>,
    ingress: &runtime_ingress::McpRuntimeIngressContext,
    session_id: &meerkat::SessionId,
...
async fn handle_meerkat_run(
    ...
    if let Some(context) = request_context.as_ref() {
        ...
        context.set_unpublished_cleanup(request_action(move || {
            ...
            async move {
                cleanup_prepared_mcp_runtime(&service, &runtime_adapter, &ingress, &session_id)
                    .await;
            }
        }));
...
mod tests {
    ...
    fn test_meerkat_tool_inputs_reject_unknown_stale_metadata_fields() {
        ...
        assert!(
            run_message.contains("turn_metadata") || run_message.contains("unknown field"),
            "unexpected run error: {run_message}"
        );

        let resume_err = serde_json::from_value::<MeerkatResumeInput>(json!({
            "session_id": "01234567-89ab-cdef-0123-456789abcdef",
            "prompt": "Continue",
            "clear_provider_params": true
        }))
        .expect_err("meerkat_resume must reject unknown stale metadata fields");
        let resume_message = resume_err.to_string();
        assert!(
            resume_message.contains("clear_provider_params")
                || resume_message.contains("unknown field"),
            "unexpected resume error: {resume_message}"
        );
    }
    ...
    #[tokio::test]
    async fn mcp_resume_live_materialization_rebuild_replaces_live_session() {
        let store: Arc<dyn SessionStore> = Arc::new(meerkat::MemoryStore::new());
        let state = MeerkatMcpState::new_with_store(store).await;
        ...
        let input: MeerkatResumeInput = serde_json::from_value(json!({
            ...
            "prompt": "resume with model override",
            "turn_metadata": {
                "model": "gpt-5.4-mini"
            }
        }))
        ...
        assert!(
            result.is_ok(),
            "live MCP materialization resume must replace, not duplicate, the live session: {result:?}"
        );
        assert!(
            state
                .runtime_adapter
