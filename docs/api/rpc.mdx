...
```json Response
{
  ...
  "id": 17,
  "result": {
    "contract_version": {"major": 0, "minor": 8, "patch": 44},
    "providers": [
      {
        "provider": "anthropic",
...
| `boundary_append_applied` | A durable steer's typed appends were written into the running turn at a model boundary; carries `run_id`, `input_id`, model-projected `content`, `append_count`, optional canonical `notices`, and optional application-time `transcript_start` offset |
| `boundary_appends_discarded` | Exact discarded application facts (`session_id`, `run_id`, `input_ids`), published after any required requeue persistence succeeds |
...
Reconcile `boundary_append_applied` against `boundary_appends_discarded` for
the same session, run, and input, and against canonical transcript history.
A run terminal alone does not prove that an append was discarded. A discard
does not terminalize the input or invalidate its later application in another
run; canonical retained history remains authoritative.
