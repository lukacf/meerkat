...
Request `provider_params` are provider-specific and do not replace Meerkat's universal image fields. Use top-level `size`, `quality`, `format`, and `intent`; the OpenAI adapter lowers `format` to provider-side `output_format`. OpenAI `provider_params` are advanced overrides for `background`, `output_compression`, `moderation`, and hosted-tool-only `action`; for the current `gpt-image-2` default, use `background: "auto"` or `"opaque"` (not `"transparent"`), use `output_compression` only with `format: "jpeg"` or `"webp"`, usually omit `action`, and omit `input_fidelity`. Gemini accepts `aspect_ratio` and `image_size`. See [Image generation](/guides/image-generation) for the exact request shape and troubleshooting.
...
```toml All providers
meerkat = { version = "0.6.5", features = ["all-providers", "jsonl-store"] }
```
