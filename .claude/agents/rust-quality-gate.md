...
Keep provider clients and general feature backends in their owning crates, not in core. Review provider implementations there and agent-client adaptation in `crates/meerkat-llm-core/src/adapter.rs`. Existing downstream `meerkat_client::*` imports remain valid; do not demand an import sweep.
