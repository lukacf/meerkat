...
pub fn protocol_emission_set() -> Result<Vec<GeneratedProtocolFile>> {
    ...
    // lives in the consumer crate that owns the production `<Prefix>InputVariant`
    // type, replacing the hand-maintained `<Prefix>MachineCatalogInput` enum.
    // The set of consumers is fixed (mob + meerkat) so emission is gated to
    ...
    // in the consumer macro, not the catalog), so deriving from inputs.variants
    // would over-generate 271 variants. Deferred until meerkat's runtime-internal
...
///
/// Derived from `MobMachine`'s `schema.inputs.variants`; each generated variant
/// projects to the production `MobMachineInputVariant` of the same name (the
/// DSL-generated input alphabet that the mob machine consumes). For `MobMachine`
/// the catalog command surface is the full input alphabet, so the mirror is a
/// 1:1 image of `schema.inputs.variants`.
fn generate_mob_machine_catalog_input(schema: &MachineSchema) -> Result<String> {
    generate_machine_catalog_input(
        schema,
...
/// Shared body for the per-machine `CatalogInput` mirrors.
///
/// `enum_name` is the generated mirror enum (e.g. `MobMachineCatalogInput`) and
/// `input_variant_path` is the fully-qualified production `<Prefix>InputVariant`
/// type the mirror projects onto. The fully-qualified path is used (rather than a
/// crate-local alias) so the generated module needs no `use` declarations and is
/// valid in any consumer crate that depends on `meerkat-machine-schema`.
fn generate_machine_catalog_input(
    schema: &MachineSchema,
    enum_name: &str,
    input_variant_path: &str,
) -> Result<String> {
    ...
    writeln!(
        &mut out,
        "// {enum_name} mirror derived from `{}`'s `schema.inputs.variants`.",
        schema.machine
    )?;
    writeln!(&mut out, "#![allow(dead_code)]")?;
    writeln!(&mut out)?;

    // Mirror enum: one unit variant per machine input, named exactly as the
    // input variant (already PascalCase). Derives match the hand enum.
    writeln!(&mut out, "#[derive(Debug, Clone, Copy, PartialEq, Eq)]")?;
    writeln!(&mut out, "pub enum {enum_name} {{")?;
    ...
    writeln!(&mut out, "impl {enum_name} {{")?;

    // `ALL`: every mirror variant in declaration order.
    writeln!(&mut out, "    pub const ALL: &'static [Self] = &[")?;
    for variant in &variants {
        writeln!(&mut out, "        Self::{variant},")?;
    }
    writeln!(&mut out, "    ];")?;
    writeln!(&mut out)?;

    // `input_variant`: project each mirror variant onto the production
    // `<Prefix>InputVariant` of the same name.
    writeln!(&mut out, "    #[must_use]")?;
    writeln!(
        &mut out,
        "    pub const fn input_variant(self) -> {input_variant_path} {{"
    )?;
    writeln!(&mut out, "        match self {{")?;
    for variant in &variants {
        writeln!(
            &mut out,
            "            Self::{variant} => {input_variant_path}::{variant},"
        )?;
    }
    writeln!(&mut out, "        }}")?;
    writeln!(&mut out, "    }}")?;
    ...
    // `as_str`: the variant's PascalCase name, matching the hand enum.
    writeln!(&mut out, "    #[must_use]")?;
    writeln!(&mut out, "    pub const fn as_str(self) -> &'static str {{")?;
    writeln!(&mut out, "        match self {{")?;
    for variant in &variants {
        writeln!(&mut out, "            Self::{variant} => \"{variant}\",")?;
    }
    writeln!(&mut out, "        }}")?;
    writeln!(&mut out, "    }}")?;
...
///
/// These are the field-agnostic *naming* facts (the public Rust
/// constant/function identifiers, the input/effect-variant names the
/// schema-walk keys off). They are scaffolding — the analogue of the
...
struct SessionPersistenceVersionFieldSpec {
    ...
    /// Input variant whose restore transitions accept this field's version.
...
struct SessionPersistenceVersionFieldPlan {
    ...
    /// Current version, read from the init field the restore transitions
    /// restore to.
    current_version: u32,
    ...
    /// fail-closed version encoder this is exactly `{current_version}` — any
    /// other accepted version (legacy default, missing-version fold) fails the
    /// walk loudly.
...
fn session_persistence_derive_field_plan(
    ...
    let mut current_state_field: Option<String> = None;
    ...
    for transition in restore_transitions {
        ...
        match &current_state_field {
            None => current_state_field = Some(restored_state_field),
            Some(existing) if *existing == restored_state_field => {}
            Some(existing) => bail!(
                "{} restores `{}` but a sibling restore transition restores `{}`",
                transition.name.as_str(),
    ...
    let Some(current_state_field) = current_state_field else {
        bail!(
            "{} has no restore transition for `{}`",
            machine.machine.as_str(),
            spec.restore_input
        );
    };
...
/// The emitter-side surface descriptors. Naming only (public identifiers);
/// every version/equality DECISION is derived from the schema.
...
fn validate_session_persistence_version_authority_schema(
    ...
    // Fail-closed witness shape: the only emitted decision is the
    // equality-checked restore verdict. A re-grown stamp/legacy effect must
    // fail the walk and force an explicit emitter update.
    let effect_names: Vec<&str> = machine
        .effects
        .variants
        .iter()
        .map(|variant| variant.name.as_str())
        .collect();
    if effect_names != ["VersionRestoreAuthorized"] {
        bail!(
            "{} must emit exactly [VersionRestoreAuthorized], got {effect_names:?}",
            machine.machine.as_str()
        );
    }
    let binding = machine
...
fn generate_session_persistence_version_authority(machine: &MachineSchema) -> Result<String> {
    ...
    // (1) Current version constants, one per field, every value read from the
    // machine's init block via the field plan.
