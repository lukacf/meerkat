...
use meerkat_core::{
    ...
    HookReasonCode, LlmStreamResult, Session, SessionDeferredTurnState, WaitInterrupt,
    WaitInterruptReceiver, WaitInterruptSender,
};
...
struct InterruptibleYieldAgent {
    session: Session,
    wait_interrupt_tx: WaitInterruptSender,
    wait_interrupt_rx: WaitInterruptReceiver,
    cancel_called: Arc<AtomicBool>,
    system_context_state: Arc<std::sync::Mutex<meerkat_core::SessionSystemContextState>>,
}

#[async_trait]
impl SessionAgent for InterruptibleYieldAgent {
    async fn run_with_events(
        &mut self,
        _prompt: meerkat_core::types::ContentInput,
        _event_tx: mpsc::Sender<AgentEvent>,
    ) -> Result<RunResult, meerkat_core::error::AgentError> {
        let mut rx = self.wait_interrupt_rx.clone();
        rx.borrow_and_update();
        let sleep_fut = tokio::time::sleep(tokio::time::Duration::from_secs(5));
        tokio::pin!(sleep_fut);
        ...
        match wake_reason {
            ...
            WakeReason::Interrupt(still_open) => {
                let interrupt = if still_open {
                    rx.borrow().clone()
                } else {
                    None
                };
                if let Some(interrupt) = interrupt.as_ref() {
                    return Ok(RunResult {
                        text: format!("yield interrupted: {}", interrupt.reason),
                        session_id: self.session.id().clone(),
                        usage: Usage::default(),
                        turns: 1,
                        tool_calls: 0,
                ...
                Err(meerkat_core::error::AgentError::InternalError(
                    "wait interrupt channel closed".to_string(),
                ))
            }
        }
    }
    ...
    fn wait_interrupt_sender(&self) -> Option<WaitInterruptSender> {
        Some(self.wait_interrupt_tx.clone())
    }
}

struct InterruptibleYieldAgentBuilder {
    cancel_called: Arc<AtomicBool>,
}

#[async_trait]
impl SessionAgentBuilder for InterruptibleYieldAgentBuilder {
    type Agent = InterruptibleYieldAgent;

    async fn build_agent(
        &self,
        _req: &CreateSessionRequest,
        _event_tx: mpsc::Sender<AgentEvent>,
    ) -> Result<InterruptibleYieldAgent, SessionError> {
        let (wait_interrupt_tx, wait_interrupt_rx) =
            tokio::sync::watch::channel(None::<WaitInterrupt>);
        Ok(InterruptibleYieldAgent {
            session: Session::new(),
            wait_interrupt_tx,
            wait_interrupt_rx,
...
#[tokio::test]
async fn test_interrupt_yielding_interrupts_cooperative_wait_without_cancel() {
    let cancel_called = Arc::new(AtomicBool::new(false));
    let service = Arc::new(EphemeralSessionService::new(
        InterruptibleYieldAgentBuilder {
    ...
    let _ = service
        .create_session(create_req_deferred("wait"))
        .await
        .expect("create deferred session");
    ...
    let sid_clone = session_id.clone();
    let turn =
        tokio::spawn(async move { service_clone.start_turn(&sid_clone, turn_req("wait")).await });
    ...
    service
        ...
        .await
        .expect("cooperative interrupt should succeed");

    let result = tokio::time::timeout(tokio::time::Duration::from_secs(1), turn)
        .await
        .expect("turn should complete promptly after cooperative interrupt")
        .expect("join should succeed")
        .expect("turn should succeed after cooperative interrupt");
    assert!(
        result.text.starts_with("yield interrupted:"),
        "unexpected turn result: {:?}",
        result.text
    );
    assert!(
        !cancel_called.load(Ordering::Acquire),
        "cooperative interrupt must not hard-cancel the turn"
    );
}
