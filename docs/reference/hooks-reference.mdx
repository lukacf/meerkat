...
`Observe` expresses observational intent, but is not by itself an enforced deny
barrier at ordinary foreground hook points: the current `DefaultHookEngine`
honors a foreground `Deny` even when the hook's capability is `Observe`.
Use `Guardrail` to declare intended `Allow` / typed
`Deny { hook_id, reason_code, message, payload }` behavior. Foreground hooks run by
