...
Neither form has a default deadline, and the agent loop's default tool
deadline does not cut a blocking call: the child runs for as long as its task
needs. `max_run_secs` is an opt-in autokill, honored in both forms, that
...
returns the sealed outcome as its tool result. Its own `timeout_seconds` is the
only deadline in both forms. See
...
A child created by `fork_off` belongs to the member that forked it, and so does
every member that child forks in turn. Ownership is recorded durably when the
child is seated and is read from the caller's own session binding, never from
tool arguments. Without manage scope over the mob, the forker can still:

- read any of its descendants with `mob_check_member`;
- retire any of its descendants with `mob_retire_member`, which also retires
  the members that descendant forked;
...
all of these tools behave as before.
