...
pub fn protocol_emission_set() -> Result<Vec<GeneratedProtocolFile>> {
    ...
    let live_request_machine = dsl::dsl_live_request_machine_production_schema();
    let code = generate_live_state_codec(&live_request_machine)?;
    push(
        root.join("meerkat-runtime/src/generated/live_request_state.rs"),
        &code,
    )?;

    let live_transcript_machine = dsl::dsl_live_transcript_machine_production_schema();
    let code = generate_live_state_codec(&live_transcript_machine)?;
    push(
        root.join("meerkat-runtime/src/generated/live_transcript_state.rs"),
        &code,
    )?;
...
fn generate_live_state_codec(machine: &MachineSchema) -> Result<String> {
    let state_type = format!("{}State", machine.machine);
    let mut out = format!(
        "// @generated -- {} state codec from the canonical machine schema.\n\
         // Generated by `xtask protocol-codegen`.\n\
         use crate::{}::*;\n\
         use serde::{{Deserialize, Serialize}};\n",
        machine.machine, machine.rust.module,
    );
    let phase = &machine.state.phase.name;
    writeln!(
        out,
        "#[derive(Serialize, Deserialize)]\nenum StoredPhase {{"
    )?;
    for variant in &machine.state.phase.variants {
        if !variant.fields.is_empty() {
            bail!("Live machine phase payloads need an explicit state codec");
        }
    ...
    for borrowed in [true, false] {
        ...
        let derive = if borrowed { "Serialize" } else { "Deserialize" };
        writeln!(
            out,
        ...
        for field in &machine.state.fields {
            ...
            let borrow = if borrowed { "&'a " } else { "" };
            writeln!(out, "{}: {borrow}{ty},", field.name)?;
        }
        writeln!(out, "}}")?;
    }
    writeln!(
        out,
        "pub(crate) fn encode(state: &{state_type}) -> Result<Vec<u8>, serde_json::Error> {{\n\
    ...
    writeln!(out, "}},")?;
    for field in &machine.state.fields {
        writeln!(out, "{}: &state.{},", field.name, field.name)?;
    }
    writeln!(out, "}})\n}}")?;
    writeln!(
        out,
        "pub(crate) fn decode(bytes: &[u8]) -> Result<{state_type}, serde_json::Error> {{\n\
         let snapshot: Snapshot = serde_json::from_slice(bytes)?;\n\
         if snapshot.format != {} {{\n\
         return Err(serde::de::Error::custom(\"unsupported Live machine snapshot version\"));\n\
    ...
    writeln!(out, "}},")?;
    for field in &machine.state.fields {
        writeln!(out, "{}: snapshot.{},", field.name, field.name)?;
    }
    writeln!(out, "}})\n}}")?;
    Ok(out)
}
...
fn render_session_document_expr(
    ...
    match expr {
        ...
        Expr::Mul(left, right) => Ok(format!(
            "({}) * ({})",
            render_session_document_expr(left, binding_types, machine)?,
            render_session_document_expr(right, binding_types, machine)?
        )),
        Expr::Div(left, right) => Ok(format!(
            "({}) / ({})",
            render_session_document_expr(left, binding_types, machine)?,
            render_session_document_expr(right, binding_types, machine)?
        )),
