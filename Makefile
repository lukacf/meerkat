...
# Exact high-blast Mob scalability case. The dedicated profile selects no
# neighboring tests and opts the Linux-hosted case back in explicitly.
...
# Local-only GitHub device-credential checks. Deliberately excluded from RBE.
e2e-copilot-live:
	@echo "$(YELLOW)Running local Copilot account live suite...$(NC)"
	@MEERKAT_STRICT_E2E_PREREQS=1 ./scripts/repo-cargo test -p meerkat-integration-tests --features integration-real-tests --test copilot_live_e2e -- --ignored --nocapture
...
e2e-auth:
	@echo "$(YELLOW)Running e2e-auth lane (ignored by default)...$(NC)"
	@scripts/run-build-backend-lane e2e-auth
...
buildbuddy-e2e-smoke-turbo-s:
	@echo "$(YELLOW)Running BuildBuddy e2e-smoke Turbo S shards...$(NC)"
	@scripts/buildbuddy-dev e2e-smoke-turbo-s $(BUILDBUDDY_ARGS)

buildbuddy-e2e-auth:
	@echo "$(YELLOW)Running BuildBuddy e2e-auth lane...$(NC)"
	@scripts/buildbuddy-dev e2e-auth $(BUILDBUDDY_ARGS)
...
# Dogma doctrine mirror gate: the canonical dogma docs under docs/architecture
# are mirrored into the meerkat-dogma-inquisition skill package. This is a
# cmp-based drift gate (exits 1 if the mirror diverges from the canonical docs),
...
# Regenerate all protocol-codegen artifacts (handoff protocol helpers,
# terminal surface mapping, generated authority contracts).
protocol-codegen: xtask-build
	@echo "$(GREEN)Running protocol-codegen...$(NC)"
	$(XTASK_BIN) protocol-codegen

# Check generated protocol-codegen artifacts for drift. Non-mutating, so it is
# safe on a dirty developer tree: it re-renders the canonical emission set in
# memory and compares it with what is committed. Covers every artifact
# protocol-codegen emits, by construction, because both read the same set.
protocol-check-drift: xtask-build
	@echo "$(GREEN)Running protocol-check-drift...$(NC)"
	$(XTASK_BIN) protocol-check-drift

machine-authority-docs-gate:
	@echo "$(GREEN)Running machine-authority docs alignment gate...$(NC)"
	$(CARGO) test -p meerkat-machine-schema --test docs_machine_authority_alignment -- --quiet

# The integration test embeds active Mobpack docs at compile time. Keep the
# feature explicit so the gate cannot silently pass with zero tests if rkat's
# default feature set changes.
mobpack-docs-contract-gate:
	@echo "$(GREEN)Running Mobpack docs contract gate...$(NC)"
	$(CARGO) test -p rkat --features mob --test help_mobpack_contract -- --quiet

# Docs are load-bearing inputs to validation scripts and compiled integration
# tests. Code changes cover these through the ratchet and integration lanes;
# docs-only CI invokes this exact complement from the governance job.
docs-only-contract-gate: docs-check verify-rpc-surface-alignment mobpack-docs-contract-gate

runtime-authority-bypass:
	@echo "$(GREEN)Running runtime-authority-bypass audit...$(NC)"
	$(CARGO) run -p xtask -- runtime-authority-bypass

storage-ambient-gate:
	@echo "$(GREEN)Running storage-ambient gate...$(NC)"
	$(CARGO) run -p xtask -- storage-ambient-gate
...
seam-inventory:
	...
	bash -lc 'ulimit -s unlimited 2>/dev/null || ulimit -s 65520; exec "$$@"' _ $(CARGO) run -p xtask -- seam-inventory --strict
...
# Verify generated SDK artifacts match the schema/code generator
verify-sdk-codegen-freshness:
	@scripts/verify-sdk-codegen-freshness

# Verify every schema-known event type has a generated inventory entry in each
# SDK (drives the fail-closed event parsers). Fails closed when a new wire event
# type is added without regenerating the SDK inventories.
verify-sdk-event-inventory:
	@python3 scripts/verify_sdk_event_inventory.py
...
# Verify REST OpenAPI catalog and the production axum router stay aligned
verify-rest-surface-alignment:
	@scripts/verify-rest-surface-alignment.sh
...
# Verify the pre-ledger 0.7.x corpus generator drops the transient capture
# artifacts `.gitignore` also drops. Repairing that in the minted manifest
# instead of in the generator (0.8.23) left the next `--capture` free to bind a
# path no clean checkout can have.
verify-fixture-mint-generator:
	@echo "$(GREEN)Checking pre-ledger corpus generator exclusions...$(NC)"
	@$(PYTHON) scripts/test_mint_pre_ledger_fixture.py
...
check-rust-release-packaging-contract:
	@scripts/test-release-packaging-isolation.sh

check-published-facade-link:
	@scripts/check-published-facade-link.sh
...
regen-schemas:
	...
	@echo "$(GREEN)Syncing Bazel package-version metadata...$(NC)"
	@scripts/sync-bazel-package-version.sh
...
# clean release does.
# Release codegen colours stack slots, so this measures the property the
# shipped rkat-rpc actually has: the router harness on 1 MiB worker stacks.
# Debug bloat is caught earlier and cheaper by the in-tree canary
# `rpc_dispatch_path_fits_debug_worker_stack_budget` (4 MiB at opt-level 0).
# Nightly only: a release build of meerkat-rpc's tests is minutes of runner
# time that must not land on the release path.
stack-budget-release:
	@echo "$(GREEN)Measuring the RPC dispatch path on 1 MiB release worker stacks...$(NC)"
	$(CARGO) nextest run --release -p meerkat-rpc --all-features --no-run
	@# RUST_MIN_STACK sizes every spawned thread of every process it reaches,
	@# rustc's included (it segfaults at 1 MiB), so build first and only run
	@# the test binaries under the budget.
	RUST_MIN_STACK=1048576 $(CARGO) nextest run --release -p meerkat-rpc --all-features --no-fail-fast -E 'test(/router::tests::/)'
...
help:
	...
	@echo "  $(GREEN)e2e-copilot-live$(NC)- Run local GitHub Copilot account suite (requires login)"
	...
	@echo "  $(GREEN)e2e-auth$(NC)      - Run release-grade auth smoke lane (ignored)"
	...
	@echo "  $(GREEN)buildbuddy-e2e-smoke-turbo-s$(NC)- Run sharded BuildBuddy e2e-smoke lane"
	@echo "  $(GREEN)buildbuddy-e2e-auth$(NC)- Run BuildBuddy e2e-auth lane"
