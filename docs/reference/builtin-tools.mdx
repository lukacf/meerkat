...
| `generate_image` | Utility | Yes | runtime image-generation wiring | Generate or edit assistant-owned images and return blob-backed image refs |
...
- **Builtins** (`CapabilityId::Builtins`): `task_list`, `task_create`, `task_get`, `task_update`, `datetime`, `apply_patch`, `view_image`, and runtime-registered `generate_image`. Controlled by `config.tools.builtins_enabled`.
