...
//!
//! The parent binds `meerkat_mob` explicitly to the local lib-test crate or
//! the external library. The CLI entry point is `probe.rs`; it keeps this
//! comms-only half independent of the controlling-mob fixture dependencies.
...
/// Function-item identity includes the defining crate, unlike a function
/// pointer's signature. This pins the raw probe's bridge owner in each host.
pub fn mob_bridge_function_type_id() -> std::any::TypeId {
