...
| `fork_off` | Mob | Yes | `mob` + generated authority | Run a bounded task in a durable transcript-forked member |
...
| `mob_retire_member` | Mob | Yes | `mob` + mob scope | Retire a member and archive its session |
| `mob_check_member` | Mob | Yes | `mob` + mob scope | Read member execution status and output |
| `mob_list_members` | Mob | Yes | `mob` + mob scope | List a mob's members |
