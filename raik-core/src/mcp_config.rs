...
//! - `project`: `.raik/mcp.toml` (searched upward from cwd) - local, shared in repo
//! - `user`: `~/.raik/mcp.toml` - global, personal
//!
...
pub enum McpScope {
    ...
    User,
    /// Project-level config: .raik/mcp.toml (searched upward)
    Project,
}
...
pub enum McpConfigError {
    ...
    #[error("Server '{0}' already exists. Remove it first with: raik mcp remove {0}")]
    ServerExists(String),
...
/// Get the user-level MCP config path: ~/.raik/mcp.toml
pub fn user_mcp_path() -> Option<PathBuf> {
    home_dir().map(|h| h.join(".raik/mcp.toml"))
}

/// Get the user-level MCP config directory: ~/.raik/
pub fn user_mcp_dir() -> Option<PathBuf> {
    home_dir().map(|h| h.join(".raik"))
}

/// Find project-level MCP config in cwd only: ./.raik/mcp.toml
...
pub fn find_project_mcp_in(dir: &Path) -> Option<PathBuf> {
    let candidate = dir.join(".raik/mcp.toml");
    if candidate.exists() {
        Some(candidate)
    } else {
        None
...
"#).unwrap();
    ...
    fn test_find_project_mcp_does_not_walk_up_tree() {
        ...
        // Create parent/.raik/mcp.toml (should NOT be found)
        let parent_raik = temp.path().join(".raik");
        fs::create_dir_all(&parent_raik).unwrap();
        fs::write(parent_raik.join("mcp.toml"), r#"
[[servers]]
name = "parent-server"
command = "should-not-load"
"#).unwrap();

        // Create parent/child/ directory (no .raik here)
        let child_dir = temp.path().join("child");
        fs::create_dir_all(&child_dir).unwrap();

        // Looking from child/ should NOT find parent/.raik/mcp.toml
    ...
    fn test_find_project_mcp_finds_config_in_current_dir() {
        ...
        // Create .raik/mcp.toml in the directory
        let raik_dir = temp.path().join(".raik");
        fs::create_dir_all(&raik_dir).unwrap();
        let config_path = raik_dir.join("mcp.toml");
...
"#;
    ...
    fn test_env_expansion_in_config() {
        ...
        unsafe {
            std::env::set_var("RAIK_TEST_API_KEY", "secret");
        }
        fs::write(
            &config_path,
            r#"
...
name = "remote"
url = "https://mcp.example.com/mcp"
headers = { Authorization = "Bearer ${RAIK_TEST_API_KEY}" }
"#,
        )
        .unwrap();
        ...
        unsafe {
            std::env::remove_var("RAIK_TEST_API_KEY");
        }
    }
}
