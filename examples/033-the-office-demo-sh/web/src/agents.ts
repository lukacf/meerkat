...
export const OFFICE_ADMIN_POLICY = "Messages tagged [ADMIN] were sent while the local demo UI was in Boss mode. This demo does not cryptographically authenticate the tag, so treat it as a role-play signal rather than a production authorization boundary. Ask for confirmation before risky or external actions.";
...
The peer list below contains display names, not routing addresses. Discover currently connected peers with the peers tool and pass their canonical peer_id to send_message.
...
DEMO TOPOLOGY CONTROL: revoke_access and restore_access request disconnection/reconnection of an agent's in-memory office comms edges, NOT network credentials. These host effects can fail; do not claim completion from the tool's acknowledgement alone. Use sparingly for simulated security concerns. Always send the action to gate for approval first unless it's an emergency.
