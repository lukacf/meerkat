...
| `SessionLlmReconfigureRequest` | `meerkat_runtime` | Request to swap provider/model while realtime-attached (`model`, `provider`, `provider_params`) |
| `MobRealtimeAttachParams` / `MobRealtimeAttachResult` | `meerkat_contracts::wire` | Wire types for `mob/realtime_attach` |
| `MobRealtimeDetachParams` / `MobRealtimeDetachResult` | `meerkat_contracts::wire` | Wire types for `mob/realtime_detach` |
