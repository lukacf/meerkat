...
| Anthropic Messages | Ordered top-level system blocks | Lossy: top-level-only |
| Gemini `generateContent` | Ordered `systemInstruction.parts` | Lossy: top-level-only |
| Private ChatGPT Responses | One ordered top-level instruction string | Lossy: top-level-only |
| OpenAI Realtime | One ordered session instruction string | Lossy: top-level-only |

Top-level-only adapters preserve every instruction's exact bytes and transcript
order, including empty, whitespace-only, and duplicate messages. They use
distinct blocks or parts when the provider supports them; singular string
channels use `"\n\n"` as their stable separator. They never trim,
deduplicate, replace, or silently drop instructions. A backend that cannot
carry all instruction bytes and order fails locally instead.
...
instruction projection only from canonical transcript `System` and
`SystemNotice` messages.
