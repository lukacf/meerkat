...
- **`SessionDocumentMachine` classifies and retains.**
  ...
  transitions are total and disjoint over the observation. The
  `RecoveryCandidateId` binds the exact evidence (session, authority stamp,
  store-head digest, CAS token, observed run identity), so classifying one
  head can never authorize mutating a later head.
- **`MeerkatMachine` authorizes.** `AuthorizeDurableTailRecovery` consumes the
  exact classified candidate and compares its run identity against the
  machine's own run facts. It emits `DurableTailRecoveryAuthorized` with a
  `DurableTailRecoveryDisposition`: `CommitCompleted`,
  `RepairAndCommitInterrupted`, `HoldIntact`, or `RefuseRecovery`. Recovery is
  admissible only from a quiescent phase (`Idle` or `Retired`) with no current
  run and no recorded terminal for the candidate's run; every other phase, and
  any conflicting run fact, refuses. All decision transitions are self-loops —
  authorization never mutates lifecycle or run state.
- **`RuntimeStore` realizes.** Only the authorizing effect may drive the
  recovery commit, and it lands through `RuntimeStore::atomic_apply` as one
  boundary: recovered session snapshot, boundary receipt, input lifecycle
  transitions, terminal outcome, and outbox rows commit together. No shell
  promotes or discards the tail.
...
closes an interrupted tail as interrupted (content preserved, provider-invalid
structure repaired with synthetic interrupted tool results and a typed
recovery notice, the original run terminalized — never requeued), or holds
ambiguous evidence intact with autonomous execution blocked. The
...
remains forbidden as an authority base.
