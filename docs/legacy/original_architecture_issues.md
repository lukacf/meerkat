@sdk-reviewer:
  ...
  ├─────────────────┼──────────────────────────┤
  │ MCP Server      │ claude-opus-4-5          │
  ├─────────────────┼──────────────────────────┤
  │ REST            │ claude-opus-4-5          │
  └─────────────────┴──────────────────────────┘
  Problem: Inconsistent defaults make behavior unpredictable. CLAUDE.md recommends gpt-5.2, gemini-3-flash-preview which none use.
  ...
  MCP resume has no model/max_tokens inputs and hard-codes `claude-opus-4-5` + 4096; REST resume uses server defaults instead of session metadata.
  ...
  pub const DEFAULT_MODEL_OPENAI: &str = "gpt-5.2";
  pub const DEFAULT_MODEL_GEMINI: &str = "gemini-3-flash-preview";
