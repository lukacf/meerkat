...
Prerequisites: a prebuilt wasm32 runtime artifact (`runtime_bg.wasm`) produced
by the `meerkat-web-runtime` build pipeline (e.g. `wasm-pack build
meerkat-web-runtime`). The CLI does not compile wasm32 itself and fails closed
without the artifact — it never emits a placeholder bundle.

Build:

```bash
rkat mob web build ./dist/release-triage.mobpack -o ./dist/release-triage-web --wasm ./path/to/runtime_bg.wasm
```

Output includes:

- `runtime.js` — bundle loader module pointing at the wasm and mobpack assets
- `runtime_bg.wasm` — the supplied compiled meerkat agent stack
