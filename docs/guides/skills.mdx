...
- understand how `turn_metadata.skill_references` carries skill injection
...
Legacy slash-delimited strings are no longer accepted by `turn_metadata.skill_references`. Some local CLI paths still accept a bare skill slug as a convenience, but API and tool surfaces should use structured keys.
...
### 2. Programmatic per-turn skill injection

Pass `turn_metadata.skill_references` in raw API requests and Python SDK calls, or `turnMetadata.skillReferences` in TypeScript, to resolve and inject skills for a specific turn:
...
Legacy top-level per-turn `skill_refs` and `skill_references` strings are retired and rejected; use typed keys in `turn_metadata.skill_references`.

### 3. Skill injection at session creation

Pass typed keys in `turn_metadata.skill_references` to inject skills at session
creation:
...
See the [Python SDK](/sdks/python/reference#skills-parameters) and [TypeScript SDK](/sdks/typescript/reference#skills-parameters) for SDK wrappers. Session creation and per-turn helpers use `turn_metadata.skill_references` (or `turnMetadata.skillReferences` in TypeScript).
