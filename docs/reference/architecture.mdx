...
graph TD
    ...
    STORE["meerkat-store<br/>SQLite / JSONL / Memory"]
    ...
    RUNTIME["meerkat-runtime<br/>RuntimeControlPlane / RuntimeDriver /<br/>MeerkatMachine"]
...
comms drain, and commit/cancel semantics.
</Note>
...
<Note>
**Mob runtime contract vs backend selection:** Agent-side mob tools are exposed through `meerkat-mob-mcp` dispatcher composition (`SessionBuildOptions.external_tools`). CLI `run`/`resume` pre-compose this path, while `rkat mob ...` is the explicit direct lifecycle surface. Public host APIs use typed `mob/*` or `meerkat_mob_*` control planes instead of re-exporting the raw dispatcher. Backend selection remains realm-driven (`realm_manifest.json`) and independent from mob tool composition.
</Note>
...
graph TD
    ...
    subgraph core["meerkat-core"]
        AGENT["Agent<br/>llm_client · tool_dispatcher<br/>session_store · session · budget<br/>retry_policy · state · compactor"]
    end

    subgraph authority["Machine authority layer"]
        SCHEMA2["meerkat-machine-schema<br/>2 kernel + perimeter machine definitions"]
    ...
    RUNT["meerkat-runtime<br/>RuntimeControlPlane / RuntimeDriver<br/>MeerkatMachine"]
    CLIENT["meerkat-client<br/>Anthropic / OpenAI / Gemini"]
...
- **MeerkatMachine**: bridges `SessionService` into the runtime's event-driven model and owns session runtime bindings / epoch recovery
...
This crate sits between surfaces and `SessionService`, providing the stateful runtime that owns
keep-alive, ingress admission, and live routing semantics for the product surfaces.

`MeerkatMachine::prepare_bindings(session_id)` is the canonical runtime-backed entry seam.
It returns `SessionRuntimeBindings`, which bundle the session-scoped ops lifecycle registry,
runtime epoch identity, and shared cursor state needed for recovery-safe completion handling.
</Accordion>
...
- One kernel per canonical machine (`MeerkatMachineKernel`, `MobMachineKernel`)
- Each kernel implements a sealed mutator trait that is the **only** code path allowed to mutate canonical state
- Authority modules in satellite crates (e.g., `MeerkatMachine` in `meerkat-runtime`, `MobMachine` in `meerkat-mob`) wrap the generated kernels with effect execution
...
Runtime-backed surfaces own keep-alive, Queue/Steer routing, comms drain lifecycle, and
external-event admission. `SessionService` remains the substrate lifecycle layer underneath those
semantics.
...
- **HnswMemoryStore**: production memory store using hnsw_rs + SQLite (feature: `memory-store-session`)
...
- **Mob host control plane** (feature-gated): typed `mob_*` tools such as `mob_create`, `mob_list`, `mob_status`, `mob_lifecycle`, `mob_spawn_member`, `mob_member_send` — all identity-keyed via `AgentIdentity`
- **Mob streaming** (feature-gated): `meerkat_mob_event_stream_open`, `meerkat_mob_event_stream_read`, `meerkat_mob_event_stream_close`
...
- **Contract version**: semver versioning for API compatibility (`0.5.1`)
...
- **MobStorage**: event/run/spec storage bundle (SQLite/WAL persistent or in-memory)
- **Flow engine**: `run_flow` / `flow_status` / `cancel_flow` for multi-step orchestration
- **FlowFrameEngine**: frame-based flow execution with `repeat_until` loops (v2 flows)
...
Prefabs are gone; all mob creation uses `MobDefinition` only.
...
Mob integration seam that serves two distinct roles:

- **MobMcpState**: shared state handle wrapping the `MobSessionService`
- **MobMcpDispatcher**: implements `AgentToolDispatcher`, routing agent-side `mob_*` tool calls into the mob runtime
- **Public MCP helpers**: `public_tools_list()` / `handle_public_tools_call()` expose typed `meerkat_mob_*` host control-plane tools for mob-only MCP hosts

Composed via `SessionBuildOptions.external_tools` when a surface wants to grant
agent-side `mob_*` capability. Public host surfaces use the typed helpers or
their own canonical typed transport contracts instead of re-exporting the raw
dispatcher.
- **AgentMobToolSurface**: agent-facing delegation tools (`delegate`, `mob_create`, `mob_destroy`, `mob_spawn_member`, `mob_retire_member`, `mob_check_member`, `mob_list_members`, `mob_list`)

Composed via `SessionBuildOptions.external_tools` so CLI, REST, RPC, and MCP server surfaces all get mob tool capability through the same path. Agent-facing delegation tools are late-bound via `MobToolsFactory`.
</Accordion>

<Accordion title="meerkat-schedule">
Durable scheduler for automated agent task execution:

- **ScheduleService**: CRUD operations + occurrence planning (projection from trigger specs within a planning horizon)
- **ScheduleDriver**: tick loop that claims due occurrences, dispatches delivery, and monitors completion
- **ScheduleLifecycleAuthority**: authority-backed schedule state transitions (Active, Paused, Completed, Cancelled)
- **OccurrenceLifecycleAuthority**: authority-backed occurrence state transitions (Pending, Claimed, Delivering, Completed, Skipped, Failed)
- **ScheduleStore**: persistence trait with Memory and SQLite implementations
- **Schedule tools**: agent-facing tools (`schedule_create`, `schedule_list`, `schedule_read`, `schedule_update`, `schedule_pause`, `schedule_resume`, `schedule_delete`)
- **Triggers**: `Cron(CalendarTriggerSpec)` and `Interval(IntervalTriggerSpec)` with `next_due_after()` computation
- **Policies**: `MisfirePolicy`, `OverlapPolicy`, `MissingTargetPolicy` for configurable delivery behavior
- **Targets**: `SessionTargetBinding` (single agent session) or `MobTargetBinding` (mob orchestration)

Stores are per-realm (not per-instance). `atomic_plan_mutation()` provides store-level atomicity for multi-step planning changes.
</Accordion>
...
- **SessionBuildOptions**: in-band request build options used by all surfaces, including `runtime_build_mode`
- **RuntimeBuildMode / SessionRuntimeBindings**: explicit runtime ownership contract for runtime-backed vs standalone builds
...
<Steps>
  ...
  <Step title="Delegate to factory">
    `FactoryAgentBuilder` converts `SessionBuildOptions` into `AgentBuildConfig` and delegates to `AgentFactory::build_agent()`. Runtime-backed surfaces thread `RuntimeBuildMode::SessionOwned(bindings)` through this seam; standalone/testing paths should opt into `RuntimeBuildMode::StandaloneEphemeral` explicitly.
  </Step>
...
The agent executes a state machine loop. The user-facing loop shape is `LoopState` in `meerkat-core/src/state.rs`, while canonical transition legality is enforced by `MeerkatMachine` (the unified Meerkat runtime authority in `meerkat-runtime/src/meerkat_machine.rs`):
...
| `WaitingForOps` | No LLM work, waiting for machine-owned barrier operations to complete (gated by `barrier_satisfied` in `MeerkatMachine`) |
...
**Agent integration**: `CommsToolDispatcher` wraps the base tool dispatcher via `wrap_with_comms()`, overlaying comms tools (`send_message`, `send_request`, `send_response`, `peers`) without modifying the `CompositeDispatcher`. Inbox delivery and runtime admission are separate concerns: transport queues work first, then runtime-backed ingress decides when those messages become session input.
...
|---------|--------|-------------|
| `MeerkatMachine` | Session-scoped runtime authority: turn execution, input ingress, ops lifecycle, tool visibility, comms drain, peer ingress, control plane (retire/reset/recover/destroy) | `meerkat-runtime` |
| `MobMachine` | Multi-agent orchestration: identity-first roster, flow execution, work dispatch, lifecycle (spawn/retire/respawn/reset/destroy), topology | `meerkat-mob` |
| `ScheduleLifecycleMachine` | Schedule triggers and cron management (perimeter) | `meerkat-schedule` |
| `OccurrenceLifecycleMachine` | Schedule occurrence dispatch and delivery (perimeter) | `meerkat-schedule` |
...
1. **Comms drain seam**: `MeerkatMachine` emits `SpawnDrainTask`/`AbortDrainTask` effects. The comms drain lifecycle must spawn or abort the actual drain task and feed back `TaskSpawned`/`TaskExited`. Generated protocol helpers (`protocol_comms_drain_spawn.rs`, `protocol_comms_drain_abort.rs`) constrain the obligation and feedback to exactly the allowed inputs.

2. **Ops barrier seam**: `MeerkatMachine` tracks operation completions internally. The barrier-satisfaction signal (all pending barrier ops complete) flows through `AsyncOpRef` with typed `WaitPolicy` (Barrier vs Detached). The `barrier_satisfied` guard on `ToolCallsResolved` ensures the turn cannot advance until the barrier is met.
...
| `meerkat-machine-schema` | Machine definitions (MeerkatMachine, MobMachine, perimeter), composition ontology, handoff protocols |
| `meerkat-machine-kernels` | Generated sealed mutator implementations |
| `meerkat-runtime` | Runtime control plane, `RuntimeDriver`, `MeerkatMachine` |
| `meerkat-store` | `SessionStore` implementations (SQLite, JSONL, in-memory) |
...
- [Machine simplification proposal](/docs/architecture/machine-simplification-proposal.md) - Phase 1 simplification plan, landed reductions, verdicts, and verification status
- [Deferred tool catalog proposal](/docs/architecture/deferred-tool-catalog-proposal.md) - portable search/load architecture for large external tool pools
