...
[dependencies]
meerkat = { version = "0.8.11", default-features = false, features = ["anthropic"] }

# Add persistence
meerkat = { version = "0.8.11", default-features = false, features = ["anthropic", "session-store"] }

# Add compaction
meerkat = { version = "0.8.11", default-features = false, features = ["anthropic", "session-store", "session-compaction"] }

# Add scheduling
meerkat = { version = "0.8.11", default-features = false, features = ["anthropic", "session-store", "schedule"] }

# Kitchen sink
meerkat = { version = "0.8.11", features = [
...
| Anthropic Messages | Leading System prefix as top-level system blocks | Later System rejected |
| Gemini `generateContent` | Leading System prefix as `systemInstruction.parts` | Later System rejected |
| Private ChatGPT Responses | Leading System prefix as one instruction string | Later System rejected |
| OpenAI Realtime | Leading System prefix as one session instruction string | Later System rejected |

Top-level-only adapters lift only a contiguous leading `System` prefix. Within
that prefix they preserve exact bytes, order, empty and whitespace-only
messages, and duplicates. Distinct provider blocks or parts remain distinct;
singular string channels use `"\n\n"` as their stable separator. If a
`System` occurs after any non-System transcript message, request construction
fails locally rather than silently changing chronology.
The runtime consults the selected client's wire capability before durable turn
mutation. It also preflights the full existing transcript before a
provider/model rebind or resume override; adapter rejection is the final
fail-closed backstop.

Realtime reconstruction has no second instruction authority:
`runtime_system_context` is retired, and reconnect/refresh derives its
instruction projection only from the canonical transcript's leading `System`
prefix. `SystemNotice` remains an explicit in-place history event.
