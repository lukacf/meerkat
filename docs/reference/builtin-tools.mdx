...
- a forker in the middle of a turn receives it at the turn's next checkpoint
  when its session takes runtime input mid-turn; a turn-driven forker instead
  runs exactly one follow-up turn that sees it (the running turn's own later
  model calls do not);
...
identity. `MobMcpState::relink_restored_fork_children` runs the pass
explicitly.
