...
| `realtime/open_info` | Realtime | Get bootstrap metadata for opening a realtime channel |
| `realtime/status` | Realtime | Read product-layer realtime channel status for a target |
| `realtime/capabilities` | Realtime | Read product-layer realtime capabilities for a target |
| `session/state` | Runtime | Read runtime-backed ingress state |
| `session/realtime_attachment_status` | Runtime | Read runtime-backed realtime attachment status for a session |
| `session/realtime_attachment_statuses` | Runtime | Batch read realtime attachment statuses for many sessions |
| `session/accept` | Runtime | Admit external work directly into runtime ingress |
| `session/retire` | Runtime | Retire runtime-backed ingress |
| `session/reset` | Runtime | Reset runtime-backed ingress |
| `session/input` | Runtime | Read runtime input lifecycle state |
| `session/inputs` | Runtime | List runtime input lifecycle records |
...
| `auth/profile/create` | Auth | Persist credentials for a binding (CLI-driven for now) |
| `auth/profile/delete` | Auth | Delete persisted credentials for a binding (CLI-driven for now) |
| `auth/profile/test` | Auth | Dry-run a binding's resolve path |
| `auth/login/start` | Auth | Begin interactive authorization-code OAuth (returns authorize_url + pkce_verifier) |
| `auth/login/complete` | Auth | Exchange authorization code for tokens; persists to TokenStore |
| `auth/login/device_start` | Auth | Begin device-code OAuth; returns user_code + verification URL |
| `auth/login/device_complete` | Auth | Single-poll completion for a device-code OAuth login |
| `auth/login/provision_api_key` | Auth | Console-OAuth → API key provisioning (Anthropic oauth_to_api_key) |
| `auth/status/get` | Auth | Get auth status for a binding |
| `auth/logout` | Auth | Revoke + remove persisted credentials for a binding |
| `realm/list` | Realm | List realm summaries |
| `realm/get` | Realm | Get full realm connection set |

<Note>
`session/state`, `session/accept`, `session/retire`, `session/reset`, `session/input`, `session/inputs` are advertised only when the server is running with the runtime-backed ingress/control-plane surface enabled.
</Note>
...
the runtime. Observe status via `session/realtime_attachment_status`
(single) or `session/realtime_attachment_statuses` (batch). There is no
...
```json Response
{
  ...
  "result": {
    ...
    "methods": [
      ...
      "session/state", "session/realtime_attachment_status", "session/realtime_attachment_statuses", "session/accept", "session/retire", "session/reset",
      "session/input", "session/inputs",
...
`connection_ref` is optional. When present, the server resolves the
session through the referenced `{realm_id, binding_id}` pair instead of
the legacy flat provider path.
...
The runtime inspects `ModelCapabilities.realtime` for the resolved
model and brings up the OpenAI Realtime transport automatically. Poll
`session/realtime_attachment_status` to confirm the binding reaches
...
Read the runtime-owned realtime attachment status projection for a session.
...
```json Request
{
  "jsonrpc": "2.0",
  "id": 22,
  "method": "session/realtime_attachment_status",
  "params": {
    "session_id": "01936f8a-7b2c-7000-8000-000000000001"
  }
}
...
Batch read of realtime attachment status for multiple sessions in a single round trip. Useful for observing all members of a mob at once.
...
```json Request
{
  "jsonrpc": "2.0",
  "id": 23,
  "method": "session/realtime_attachment_statuses",
  "params": {
    "session_ids": [
      "01936f8a-7b2c-7000-8000-000000000001",
      "01936f8a-7b2c-7000-8000-000000000002"
    ]
  }
}
```
...
```json Response
{
  ...
  "result": {
    "statuses": [
      {"session_id": "01936f8a-7b2c-7000-8000-000000000001", "status": "binding_ready"},
      {"session_id": "01936f8a-7b2c-7000-8000-000000000002", "status": "unattached"}
    ]
  }
}
```
...
### Realtime bootstrap methods (`realtime/*`)

In addition to attachment status observation, three product-layer methods handle opening and inspecting realtime WebSocket channels:

| Method | Purpose |
|--------|---------|
| `realtime/open_info` | Issue a single-use bootstrap token for opening a realtime channel |
...
These are emitted by `rkat-rpc`'s realtime WebSocket listener. They are the public product layer on top of the capability-driven attachment — the runtime manages attach/detach automatically based on `ModelCapabilities.realtime`, and these methods let callers discover the actual transport and open the audio socket.
