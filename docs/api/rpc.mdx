...
| `boundary_append_applied` | A durable steer's typed appends were written into the running turn at a model boundary (`run_id`, `input_id`, model-projected `content`, `append_count`); reconcile with the run terminal, since a discarded image redelivers the input once |
