...
#[test]
fn session_persistence_version_authority_matches_codegen_output() {
    use meerkat_machine_schema::catalog::dsl;

    let machine = dsl::dsl_session_persistence_version_authority_machine_production_schema();
    let rendered = xtask::protocol_codegen::render_session_persistence_version_authority(&machine)
    ...
    assert_eq!(
        normalize(&committed),
        normalize(&rendered),
        "session_persistence_version_authority.rs diverged from codegen output. If this is intentional, run `cargo xtask protocol-codegen` and commit the result."
    );
}
...
#[test]
fn session_durable_config_authority_matches_codegen_output() {
    use meerkat_machine_schema::catalog::dsl;

    let machine = dsl::dsl_session_durable_config_authority_machine_production_schema();
    let rendered = xtask::protocol_codegen::render_session_durable_config_authority(&machine)
        .expect("render session_durable_config_authority");
    let rendered = rustfmt(&rendered);

    let committed_path =
        repo_root().join("meerkat-core/src/generated/session_durable_config_authority.rs");
    let committed = std::fs::read_to_string(&committed_path)
        .unwrap_or_else(|_| panic!("read {}", committed_path.display()));
    ...
    assert_eq!(
        normalize(&committed),
        normalize(&rendered),
        "session_durable_config_authority.rs diverged from codegen output. If this is intentional, run `cargo xtask protocol-codegen` and commit the result."
    );
}
...
fn comms_trust_authority_minting_is_generated_only() {
    ...
    for path in files {
        ...
        if source.contains("generated_authority_bridge_token()")
            && !(generated_protocol_file
                || codegen_file
                || generated_bridge_validator_file
                || drift_test_file)
        {
