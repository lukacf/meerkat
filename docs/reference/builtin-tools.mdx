...
**Detached.** Hosts that outlive the call declare that they can: the JSON-RPC,
REST, and MCP servers, `rkat run --keep-alive`, and `rkat mob deploy --surface
rpc`. There `fork_off` returns without waiting for the child's turn:
...
When the child's turn ends, the outcome is recorded as
`{"tool": "fork_off", "job_id": ..., "outcome": {...}}`. The `outcome` names the
child (`agent_identity`, `member_ref`) and carries a `status`:
...
The record reaches the forker in two ways:

- **Durably, in the forker's own transcript.** It is appended once to the
  forker's session as an ordinary System message ("Background fork_off job
  `<job_id>` finished:" followed by the record). It stays there across later
  model calls, turns, and reloads, and it remains readable after the child is
  retired: in the transcript the model sees, and through the forker's session
  history (`session/history` over JSON-RPC, `GET /sessions/{id}/history` over
  REST).
- **As a wake-up.** The background job for `job_id` then completes (or fails)
  with the same record as its detail, which wakes an idle forker. That notice
  is transient; the durable System message is the copy to rely on. If the
  record cannot be appended, the notice is still delivered.
...
- a child still running is observed until its turn ends, with `max_run_secs`
  ...
  descendants);
- an idle child that already replied after its fork prefix has that result
  delivered;
- otherwise `restart_interrupted` is delivered and the child stays seated.

Delivery writes the same durable System message under the same idempotency key,
so a job whose outcome was recorded before the restart is never recorded
twice. A restarted host cannot wake an idle forker, because the job's
in-memory operation did not survive; the forker sees the re-linked outcome on
its next turn. `MobMcpState::relink_restored_fork_children` runs the pass
explicitly.
...
text carries the same outcome record.

Neither form has a default deadline: the child runs for as long as its task
needs. The agent loop's default tool deadline does not cut a blocking call; a
deadline the host itself imposes on tool calls still applies. `max_run_secs` is an opt-in autokill, honored in both forms, that
cancels the child's run and retires the child once it has run that long; omit
it for no limit. `fork_off` rejects arguments it does not define, so a
misspelled or invented field is an argument error instead of a silently
ignored intent.

`council` follows the same split. Detached, it returns `status: "running"`,
the `council_id`, and a `job_id`, and the sealed outcome (`result`, `cleanup`,
`replayed`) is recorded and delivered the same way, as
`{"tool": "council", "job_id": ..., "outcome": {...}}`; a council that fails
after the call returned records `{"error": ...}` as its outcome, and a council
whose exit reason is a failure (`participant_seating_failed`,
`wiring_incomplete`, `exchange_failed`, `coordinator_interrupted`) fails its
job while the record keeps the full sealed outcome. Blocking, it returns the
sealed outcome as its tool result. Its own `timeout_seconds` bounds
it in both forms, and the agent loop's default tool deadline does not. See
...
`DetachedCompletionDelivery::Available` or `Unavailable`); the default is
`Available`. Detached delivery also needs the session's operation registry,
which runtime-backed sessions bind; without one the tools block.
