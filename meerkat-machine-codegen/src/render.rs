...
pub fn render_machine_kernel_module(schema: &MachineSchema) -> String {
    ...
    let kernel_named_variant_import = if named_variants.is_empty() {
        ""
    } else {
        " KernelNamedVariant,"
    };
    ...
    pushln!(
        &mut out,
        "    GeneratedMachineKernel, KernelEffectVariant, KernelField, KernelFields, KernelHelperName, KernelInput, KernelInputVariant,{kernel_named_variant_import} KernelPhase, KernelSignal, KernelSignalVariant, KernelState, KernelTransitionName, KernelValue, TransitionOutcome, TransitionRefusal,"
    );
    pushln!(&mut out, "}};");
    pushln!(&mut out);
    pushln!(
    ...
    render_kernel_named_variant_modules(&mut out, &named_variants);
    pushln!(
        &mut out,
...
#[cfg(not(test))]
fn render_kernel_named_variant_modules(
    out: &mut String,
    ...
    pushln!(out, "pub mod named_variant {{");
    for (enum_name, values) in variants {
        let module_name = rust_ident(&to_snake_case(enum_name));
        let enum_literal = rust_string_literal(enum_name);
        pushln!(out, "    pub mod {module_name} {{");
        for value in values {
            let function_name = rust_ident(&to_snake_case(value));
            let value_literal = rust_string_literal(value);
            pushln!(out, "        #[must_use]");
            pushln!(
                out,
                "        pub fn {function_name}() -> super::super::KernelNamedVariant {{"
            );
            pushln!(
                out,
                "            super::super::KernelNamedVariant::new_static({enum_literal}, {value_literal})"
            );
            pushln!(out, "        }}");
        }
        pushln!(out, "    }}");
    ...
    pushln!(out, "pub mod named_value {{");
    for (enum_name, values) in variants {
        let module_name = rust_ident(&to_snake_case(enum_name));
        pushln!(out, "    pub mod {module_name} {{");
        for value in values {
            ...
            pushln!(
                out,
                "        pub fn {function_name}() -> super::super::KernelValue {{"
            );
            pushln!(
                out,
                "            let named = super::super::named_variant::{module_name}::{function_name}();"
            );
            pushln!(
                out,
                "            super::super::KernelValue::NamedVariant {{ enum_name: named.enum_name, variant: named.variant }}"
            );
            pushln!(out, "        }}");
        }
        pushln!(out, "    }}");
