...
fn generate_auth_lease_durable_lifecycle_marker_contract() -> Result<String> {
    ...
    writeln!(&mut out, "// Generated by `xtask protocol-codegen`")?;
    writeln!(&mut out)?;
    writeln!(&mut out, "use crate::handles::AuthLeasePhase;")?;
    writeln!(&mut out)?;
    writeln!(
        &mut out,
    ...
    writeln!(&mut out, "pub const SCHEMA_VERSION: u64 = 3;")?;
    ...
    writeln!(&mut out, "pub struct DurableAuthLifecycleMarker {{")?;
    ...
    writeln!(
        &mut out,
        "pub fn read_marker_from_metadata(metadata: &serde_json::Value) -> Option<DurableAuthLifecycleMarker> {{"
    )?;
    writeln!(
        &mut out,
    ...
    writeln!(
        &mut out,
        "pub fn metadata_with_marker(metadata: &serde_json::Value, marker: DurableAuthLifecycleMarker) -> serde_json::Value {{"
    )?;
