...
//!
//! The entry point supplies `fixture_mob`: integration roots bind the
//! external library; the lib-test root binds `crate`. Keep that choice
//! explicit so fixtures never pull the self-dev-dependency into lib tests.
//!
//! Contents:
...
use super::fixture_mob as meerkat_mob;
...
use self::meerkat_mob::runtime::HostBindRequest;
use meerkat_comms::{
...
use self::meerkat_mob::runtime::bridge_protocol::{
...
use self::meerkat_mob::runtime::host_actor::{
...
use self::meerkat_mob::runtime::host_materialize::{
    MaterializeLlmPreflightOutcome, MaterializePreflightProbe,
};
use tokio::sync::watch;

/// Pin the crate identity behind the imported host authority surfaces.
pub fn mob_authority_type_ids() -> [std::any::TypeId; 6] {
    fn function_type_id<F: 'static>(_: F) -> std::any::TypeId {
        std::any::TypeId::of::<F>()
    }
    [
        std::any::TypeId::of::<MobHostActorConfig>(),
        std::any::TypeId::of::<MobHostActorHandle>(),
        std::any::TypeId::of::<HostBindRequest>(),
        function_type_id(spawn_mob_host_actor),
...
impl meerkat_mob::MobSessionService for FailingOnceSessionService {
    ...
    async fn load_session_for_resume(
        ...
        use self::meerkat_mob::runtime::ResumeSessionLoad;
