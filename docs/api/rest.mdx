...
| `POST` | `/mob/temporary-councils/run` | Run one bounded temporary council of source-owned forked participants (mob feature) |
| `POST` | `/mob/temporary-councils/recover` | Converge every unfinished temporary council; owner/admin maintenance (mob feature) |
| `GET` | `/mob/temporary-councils/{council_id}` | Read one sealed temporary-council record projection (mob feature) |
