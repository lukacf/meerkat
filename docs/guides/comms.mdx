...
Each agent has an Ed25519 keypair managed by the `Keypair` type (`crates/meerkat-comms/src/identity.rs`).
...
Incoming connections are validated in `handle_connection()` (`crates/meerkat-comms/src/io_task.rs`):
...
**Inproc transport**: The `InprocRegistry` (`crates/meerkat-comms/src/inproc.rs`) is a process-global registry segmented by namespace. Meerkat uses realm-scoped namespaces so inproc peers from different realms are isolated by default. Messages are delivered directly in-memory without serialization.
...
The `MessageIntent` enum (`crates/meerkat-comms/src/agent/types.rs`) is the
...
The `Inbox` (`crates/meerkat-comms/src/inbox.rs`) is a bounded queue (default capacity: 1024) with a `Notify` mechanism for waking waiting tasks.
...
```bash
# Keep-alive with structured stdin events
...
- **agent-scoped**: use `MobHandle::subscribe_agent_events(...)` or `mob/stream_open` (RPC) / `Member.subscribe()` or `Mob.subscribeMemberEvents(agentIdentity)` (Web SDK) / `mob.subscribe_member_events(agent_identity)` (Python SDK) / `mob.subscribeMemberEvents(agentIdentity)` (TypeScript SDK)
- **mob-scoped**: use `MobHandle::subscribe_mob_events(...)` or `mob/stream_open` (RPC) / `Mob.subscribeEvents()` (Web SDK) / `mob.subscribe_events()` (Python SDK) / `mob.subscribeEvents()` (TypeScript SDK)
