...
| `mob/temporary_council_run` | Mob | `MobTemporaryCouncilRunParams` | `MobTemporaryCouncilRunResult` | Run one bounded temporary council of source-owned forked participants (mob feature) |
| `mob/temporary_council_get` | Mob | `MobTemporaryCouncilGetParams` | `MobTemporaryCouncilGetResult` | Read one sealed temporary-council record projection (mob feature) |
| `mob/temporary_council_recover` | Mob | - | `MobTemporaryCouncilRecoverResult` | Converge every unfinished temporary council; owner/admin maintenance (mob feature) |
...
```json Response
{
  ...
  "id": 8,
  "result": {
    "contract_version": {"major": 0, "minor": 8, "patch": 30},
    "capabilities": [
      {
        "id": "sessions",
