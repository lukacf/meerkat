...
//!
//! Provides `raik mcp add|remove|list|get` commands for managing MCP server configuration.

use raik_core::mcp_config::{
...
pub fn add_server(
    ...
    if McpConfig::server_exists(&name, scope)? {
        anyhow::bail!(
            "MCP server '{}' already exists in {} scope. Remove it first with: raik mcp remove {} --scope {}",
    ...
    let server = match (transport, url, command.is_empty()) {
        ...
        (Some(McpTransportKind::Stdio), _, true) => {
            anyhow::bail!("Stdio transport requires a command. Usage: raik mcp add <name> -t stdio -- <command> [args...]");
        }
        ...
        (Some(McpTransportKind::StreamableHttp | McpTransportKind::Sse), None, _) => {
            anyhow::bail!("HTTP/SSE transport requires --url. Usage: raik mcp add <name> -t http --url <url>");
        }
        ...
        (None, None, true) => {
            anyhow::bail!(
                "Either command or URL is required.\n\
                 Stdio: raik mcp add <name> -- <command> [args...]\n\
                 HTTP:  raik mcp add <name> --url <url>"
            );
        }
    };
...
pub fn list_servers(scope: Option<McpScope>, json_output: bool) -> anyhow::Result<()> {
    let servers = match scope {
        Some(s) => {
            ...
            config
                .servers
                .into_iter()
                .map(|server| raik_core::mcp_config::McpServerWithScope { server, scope: s })
                .collect()
        }
    ...
    if json_output {
        ...
        if servers.is_empty() {
            ...
            println!("\nAdd a server with: raik mcp add <name> -- <command> [args...]");
            return Ok(());
        }
