...
WitnessInit_peer_ingress_ready_begins_run ==
    ...
    /\ witness_remaining_script_inputs = <<[machine |-> "peer_comms", variant |-> "TrustPeer", payload |-> [peer_id |-> "peer_1"], source_kind |-> "entry", source_route |-> "witness:peer_ingress_ready_begins_run:2", source_machine |-> "external_entry", source_effect |-> "TrustPeer", effect_id |-> 0], [machine |-> "peer_comms", variant |-> "ReceivePeerEnvelope", payload |-> [content_shape |-> "TextOnly", peer_id |-> "peer_1", raw_item_id |-> "raw_1", raw_kind |-> "request", request_id |-> Some("req_1"), reservation_key |-> Some("resv_1"), text_projection |-> "run staging handoff"], source_kind |-> "entry", source_route |-> "witness:peer_ingress_ready_begins_run:3", source_machine |-> "external_entry", source_effect |-> "ReceivePeerEnvelope", effect_id |-> 0], [machine |-> "peer_comms", variant |-> "SubmitTypedPeerInput", payload |-> [raw_item_id |-> "raw_1"], source_kind |-> "entry", source_route |-> "witness:peer_ingress_ready_begins_run:4", source_machine |-> "external_entry", source_effect |-> "SubmitTypedPeerInput", effect_id |-> 0], [machine |-> "runtime_control", variant |-> "AdmissionAccepted", payload |-> [admission_effect |-> "SubmitAdmittedIngressEffect", content_shape |-> "TextOnly", handling_mode |-> "Steer", request_id |-> None, reservation_key |-> None, work_id |-> "raw_1"], source_kind |-> "entry", source_route |-> "witness:peer_ingress_ready_begins_run:5", source_machine |-> "external_entry", source_effect |-> "AdmissionAccepted", effect_id |-> 0], [machine |-> "runtime_ingress", variant |-> "StageDrainSnapshot", payload |-> [contributing_work_ids |-> <<"raw_1">>, run_id |-> "runid_1"], source_kind |-> "entry", source_route |-> "witness:peer_ingress_ready_begins_run:6", source_machine |-> "external_entry", source_effect |-> "StageDrainSnapshot", effect_id |-> 0]>>
...
WitnessStateConstraint_peer_ingress_ready_begins_run == /\ model_step_count <= 11 /\ Len(pending_inputs) <= 6 /\ Cardinality(observed_inputs) <= 12 /\ Len(pending_routes) <= 3 /\ Cardinality(delivered_routes) <= 4 /\ Cardinality(emitted_effects) <= 5 /\ Cardinality(observed_transitions) <= 11 /\ Cardinality(peer_comms_trusted_peers) <= 5 /\ Cardinality(DOMAIN peer_comms_raw_item_peer) <= 5 /\ Cardinality(DOMAIN peer_comms_raw_item_kind) <= 5 /\ Cardinality(DOMAIN peer_comms_classified_as) <= 5 /\ Cardinality(DOMAIN peer_comms_text_projection) <= 5 /\ Cardinality(DOMAIN peer_comms_content_shape) <= 5 /\ Cardinality(DOMAIN peer_comms_request_id) <= 5 /\ Cardinality(DOMAIN peer_comms_reservation_key) <= 5 /\ Cardinality(DOMAIN peer_comms_trusted_snapshot) <= 5 /\ Len(peer_comms_submission_queue) <= 5 /\ Cardinality(runtime_ingress_admitted_inputs) <= 5 /\ Len(runtime_ingress_admission_order) <= 5 /\ Cardinality(DOMAIN runtime_ingress_content_shape) <= 5 /\ Cardinality(DOMAIN runtime_ingress_request_id) <= 5 /\ Cardinality(DOMAIN runtime_ingress_reservation_key) <= 5 /\ Cardinality(DOMAIN runtime_ingress_policy_snapshot) <= 5 /\ Cardinality(DOMAIN runtime_ingress_handling_mode) <= 5 /\ Cardinality(DOMAIN runtime_ingress_lifecycle) <= 5 /\ Cardinality(DOMAIN runtime_ingress_terminal_outcome) <= 5 /\ Len(runtime_ingress_queue) <= 5 /\ Len(runtime_ingress_steer_queue) <= 5 /\ Len(runtime_ingress_current_run_contributors) <= 5 /\ Cardinality(DOMAIN runtime_ingress_last_run) <= 5 /\ Cardinality(DOMAIN runtime_ingress_last_boundary_sequence) <= 5 /\ Cardinality(runtime_ingress_silent_intent_overrides) <= 5
...
WitnessTransitionObserved_peer_ingress_ready_begins_run_peer_comms_SubmitTypedPeerInputDelivered == <> (\E packet \in observed_transitions : /\ packet.machine = "peer_comms" /\ packet.transition = "SubmitTypedPeerInputDelivered")
