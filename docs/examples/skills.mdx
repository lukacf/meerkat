...
`CLI --skill` is the preload-oriented local path. Raw APIs carry create-time
skill injection through `turn_metadata.skill_references`.
...
Use `turn_metadata.skill_references` when the skill should be injected for the
first turn rather than preloaded into the session system prompt. SDKs keep
`skill_refs` as a convenience option and serialize it into that carrier.
...
Inject skills into a specific turn on an existing session. The skill content is added to the context for that turn only. On the CLI, `--skill` remains the preload-style runtime-local path; raw RPC and REST carry per-turn skills inside `turn_metadata.skill_references`, while SDKs expose `skill_refs` convenience options.
