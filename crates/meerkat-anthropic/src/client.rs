...
mod tests {
    ...
    -> Result<(), Box<dyn std::error::Error>> {
        let client = AnthropicClient::new("test-key".to_string())?;
        for model in ["claude-sonnet-4-5", "claude-sonnet-4-6", "claude-opus-5-5"] {
            for later in 1..=2 {
                ...
                let wire = body["messages"]
                    .as_array()
                    .ok_or_else(|| format!("{model}: no {} in {body}", "messages"))?;
    ...
    async fn stream_dispatch_projects_canonical_replay_into_captured_request() {
        ...
        let observed = assistant["content"][1]["text"].as_str().unwrap();
