...
<ResponseField name="sequence" type="int">Monotonic sequence number.</ResponseField>
<ResponseField name="event" type="Optional[dict]">The agent event payload.</ResponseField>
<ResponseField name="contract_version" type="str">Contract version of the server.</ResponseField>
</Accordion>
...
Async iterator that yields `WireEvent` objects from the `rkat rpc` notification stream:
...
<Note>
`EventStream` implements `__aiter__` and `__anext__`. It yields `WireEvent` instances until the stream closes.
</Note>
