...
that supplies a durable store without a persistent root. A record whose
...
- **Detached (hosts that outlive the call).** A host with a runtime adapter
  ...
  system notice ("Background council job `<job_id>` finished (`completed` or
  `failed`):" followed by the outcome, with `persisted: true` on its typed
  block). It is delivered as a runtime input with the idempotency key
  `council:<job_id>`: an idle convener runs one turn that sees it, and a busy
  one sees it at its next checkpoint or in one follow-up turn. The status is
...
uses the same idempotency key, so the outcome is recorded once.
