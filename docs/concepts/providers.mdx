...
- Anthropic, Gemini, the private ChatGPT Responses backend, and OpenAI
  Realtime expose top-level-only instruction channels. Their adapters collect
  every `System` payload in authored System-message order, using distinct
  provider blocks or parts where supported and `"\n\n"` as the stable
  separator for singular strings. `SystemNotice` remains an explicit in-place
  event.

Empty, whitespace-only, and duplicate instructions are significant: adapters
do not trim, deduplicate, replace, or silently drop them. System placement is
never a session-admission or provider-rebind restriction. Realtime reconnect
and refresh use the same transcript-derived ordered projection; there is no
provider-only `runtime_system_context` sidecar.
