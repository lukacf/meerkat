...
<Steps>
  ...
  <Step title="Extraction turn fires">
    `perform_extraction_turn()` (in `meerkat-core/src/agent/extraction.rs`) sends an additional LLM call with no tools (extraction is pure text generation), temperature `0.0` for deterministic output, and a prompt asking for valid JSON matching the schema.
  </Step>
  ...
  <Step title="On failure">
    Retries up to `structured_output_retries` times with error feedback, then returns `AgentError::StructuredOutputValidationFailed`.
  </Step>
</Steps>
