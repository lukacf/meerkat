...
Scope: All `0.5`-style internal design and review work
Applies to: machine schema, generated authority, runtime/control plane, comms, mob, shell helpers, public surfaces

## Purpose

This document states the non-negotiable architectural rules for Meerkat `0.5`.
...
- partial machine ownership for only the convenient subset of a failure family
- surface-local timeout wrappers
- success classification for incomplete execution
- projections with no explicit rebuild / staleness contract
- handwritten terminal mapping branches that diverge from machine truth
- dynamic model/provider changes without dynamic policy recomputation

## Review Questions

When reviewing a proposal, ask these first:

1. What exact semantic facts are introduced or changed?
2. Who is the one canonical owner of each fact?
3. Does any shell/helper/cache still carry parallel truth?
4. Is every semantically important distinction typed?
5. If a condition can be observed in two places, does it still have one terminal path?
6. Are app-facing APIs exposing domain handles rather than infra identity?
7. If raw infra identity escapes, is it definitely canonical?
8. If policy depends on dynamic identity, where is recomputation defined?
9. Are projections genuinely projections, or are they shadow state?
10. Did the change reduce ambiguity, or merely relocate it?

## One-Line Version

Meerkat `0.5` dogma is simple:

> semantic truth must be singular, typed, canonical, and machine-owned; everything else is mechanics or a bug.
