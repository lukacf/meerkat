...
```rust
use async_trait::async_trait;
use meerkat::{AgentFactory, Config, SessionStore, SessionFilter, StoreError, Session, SessionId, build_ephemeral_service};
...
// Stage a stdio-based MCP server for addition
...
// Stage an HTTP/SSE-based MCP server for addition
...
// Apply staged operations at the boundary (non-blocking)
let result = router.apply_staged().await?;

// List all visible tools (from the projection snapshot)
