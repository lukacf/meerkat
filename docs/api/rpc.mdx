...
<Note>
The previous realtime RPC surface is retired while the new live adapter API is
rebuilt. Do not rely on model-capability-driven realtime attachment or
`realtime/*` methods.
</Note>
...
The previous realtime RPC surface has been retired. `realtime/open_info`,
`realtime/status`, `realtime/capabilities`, and
`session/realtime_attachment_status` are no longer public methods. A new live
API will be documented after the live adapter host is exposed.
