...
impl AgentToolDispatcher for McpRouter {
    ...
    async fn dispatch(&self, name: &str, args: &Value) -> Result<Value, ToolError> {
        let result_str = self.call_tool(name, args).await.map_err(|e| match e {
        ...
        // MCP results are strings, usually JSON
        let value: Value = serde_json::from_str(&result_str).unwrap_or(Value::String(result_str));
        Ok(value)
    }
}
