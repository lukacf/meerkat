...
BaseInit ==
    ...
    /\ meerkat_current_llm_identity = None
    /\ meerkat_current_capability_surface = None
    /\ meerkat_capability_surface_status = "Unresolved"
    /\ meerkat_capability_base_filter = "All"
...
RECURSIVE meerkat_StagePersistentFilterIdle_ForEach0_filter_witnesses(_, _, _)
meerkat_StagePersistentFilterIdle_ForEach0_filter_witnesses(acc, remaining, outer_witnesses) == IF remaining = {} THEN acc ELSE LET item == CHOOSE x \in remaining : TRUE IN LET name == item IN LET next_acc == MapSet(acc, name, (IF name \in DOMAIN outer_witnesses THEN outer_witnesses[name] ELSE "None")) IN meerkat_StagePersistentFilterIdle_ForEach0_filter_witnesses(next_acc, remaining \ {item}, outer_witnesses)

RECURSIVE meerkat_StagePersistentFilterAttached_ForEach1_filter_witnesses(_, _, _)
meerkat_StagePersistentFilterAttached_ForEach1_filter_witnesses(acc, remaining, outer_witnesses) == IF remaining = {} THEN acc ELSE LET item == CHOOSE x \in remaining : TRUE IN LET name == item IN LET next_acc == MapSet(acc, name, (IF name \in DOMAIN outer_witnesses THEN outer_witnesses[name] ELSE "None")) IN meerkat_StagePersistentFilterAttached_ForEach1_filter_witnesses(next_acc, remaining \ {item}, outer_witnesses)

RECURSIVE meerkat_StagePersistentFilterRunning_ForEach2_filter_witnesses(_, _, _)
meerkat_StagePersistentFilterRunning_ForEach2_filter_witnesses(acc, remaining, outer_witnesses) == IF remaining = {} THEN acc ELSE LET item == CHOOSE x \in remaining : TRUE IN LET name == item IN LET next_acc == MapSet(acc, name, (IF name \in DOMAIN outer_witnesses THEN outer_witnesses[name] ELSE "None")) IN meerkat_StagePersistentFilterRunning_ForEach2_filter_witnesses(next_acc, remaining \ {item}, outer_witnesses)

RECURSIVE meerkat_StagePersistentFilterRetired_ForEach3_filter_witnesses(_, _, _)
meerkat_StagePersistentFilterRetired_ForEach3_filter_witnesses(acc, remaining, outer_witnesses) == IF remaining = {} THEN acc ELSE LET item == CHOOSE x \in remaining : TRUE IN LET name == item IN LET next_acc == MapSet(acc, name, (IF name \in DOMAIN outer_witnesses THEN outer_witnesses[name] ELSE "None")) IN meerkat_StagePersistentFilterRetired_ForEach3_filter_witnesses(next_acc, remaining \ {item}, outer_witnesses)

RECURSIVE meerkat_StagePersistentFilterStopped_ForEach4_filter_witnesses(_, _, _)
meerkat_StagePersistentFilterStopped_ForEach4_filter_witnesses(acc, remaining, outer_witnesses) == IF remaining = {} THEN acc ELSE LET item == CHOOSE x \in remaining : TRUE IN LET name == item IN LET next_acc == MapSet(acc, name, (IF name \in DOMAIN outer_witnesses THEN outer_witnesses[name] ELSE "None")) IN meerkat_StagePersistentFilterStopped_ForEach4_filter_witnesses(next_acc, remaining \ {item}, outer_witnesses)

RECURSIVE meerkat_RequestDeferredToolsIdle_ForEach5_staged_requested_deferred_names(_, _)
meerkat_RequestDeferredToolsIdle_ForEach5_staged_requested_deferred_names(acc, remaining) == IF remaining = {} THEN acc ELSE LET item == CHOOSE x \in remaining : TRUE IN LET name == item IN LET next_acc == (acc \cup {name}) IN meerkat_RequestDeferredToolsIdle_ForEach5_staged_requested_deferred_names(next_acc, remaining \ {item})

RECURSIVE meerkat_RequestDeferredToolsIdle_ForEach6_requested_witnesses(_, _, _)
meerkat_RequestDeferredToolsIdle_ForEach6_requested_witnesses(acc, remaining, outer_witnesses) == IF remaining = {} THEN acc ELSE LET item == CHOOSE x \in remaining : TRUE IN LET name == item IN LET next_acc == MapSet(acc, name, (IF name \in DOMAIN outer_witnesses THEN outer_witnesses[name] ELSE "None")) IN meerkat_RequestDeferredToolsIdle_ForEach6_requested_witnesses(next_acc, remaining \ {item}, outer_witnesses)

RECURSIVE meerkat_RequestDeferredToolsAttached_ForEach7_staged_requested_deferred_names(_, _)
meerkat_RequestDeferredToolsAttached_ForEach7_staged_requested_deferred_names(acc, remaining) == IF remaining = {} THEN acc ELSE LET item == CHOOSE x \in remaining : TRUE IN LET name == item IN LET next_acc == (acc \cup {name}) IN meerkat_RequestDeferredToolsAttached_ForEach7_staged_requested_deferred_names(next_acc, remaining \ {item})

RECURSIVE meerkat_RequestDeferredToolsAttached_ForEach8_requested_witnesses(_, _, _)
meerkat_RequestDeferredToolsAttached_ForEach8_requested_witnesses(acc, remaining, outer_witnesses) == IF remaining = {} THEN acc ELSE LET item == CHOOSE x \in remaining : TRUE IN LET name == item IN LET next_acc == MapSet(acc, name, (IF name \in DOMAIN outer_witnesses THEN outer_witnesses[name] ELSE "None")) IN meerkat_RequestDeferredToolsAttached_ForEach8_requested_witnesses(next_acc, remaining \ {item}, outer_witnesses)

RECURSIVE meerkat_RequestDeferredToolsRunning_ForEach9_staged_requested_deferred_names(_, _)
meerkat_RequestDeferredToolsRunning_ForEach9_staged_requested_deferred_names(acc, remaining) == IF remaining = {} THEN acc ELSE LET item == CHOOSE x \in remaining : TRUE IN LET name == item IN LET next_acc == (acc \cup {name}) IN meerkat_RequestDeferredToolsRunning_ForEach9_staged_requested_deferred_names(next_acc, remaining \ {item})

RECURSIVE meerkat_RequestDeferredToolsRunning_ForEach10_requested_witnesses(_, _, _)
meerkat_RequestDeferredToolsRunning_ForEach10_requested_witnesses(acc, remaining, outer_witnesses) == IF remaining = {} THEN acc ELSE LET item == CHOOSE x \in remaining : TRUE IN LET name == item IN LET next_acc == MapSet(acc, name, (IF name \in DOMAIN outer_witnesses THEN outer_witnesses[name] ELSE "None")) IN meerkat_RequestDeferredToolsRunning_ForEach10_requested_witnesses(next_acc, remaining \ {item}, outer_witnesses)

RECURSIVE meerkat_RequestDeferredToolsRetired_ForEach11_staged_requested_deferred_names(_, _)
meerkat_RequestDeferredToolsRetired_ForEach11_staged_requested_deferred_names(acc, remaining) == IF remaining = {} THEN acc ELSE LET item == CHOOSE x \in remaining : TRUE IN LET name == item IN LET next_acc == (acc \cup {name}) IN meerkat_RequestDeferredToolsRetired_ForEach11_staged_requested_deferred_names(next_acc, remaining \ {item})

RECURSIVE meerkat_RequestDeferredToolsRetired_ForEach12_requested_witnesses(_, _, _)
meerkat_RequestDeferredToolsRetired_ForEach12_requested_witnesses(acc, remaining, outer_witnesses) == IF remaining = {} THEN acc ELSE LET item == CHOOSE x \in remaining : TRUE IN LET name == item IN LET next_acc == MapSet(acc, name, (IF name \in DOMAIN outer_witnesses THEN outer_witnesses[name] ELSE "None")) IN meerkat_RequestDeferredToolsRetired_ForEach12_requested_witnesses(next_acc, remaining \ {item}, outer_witnesses)

RECURSIVE meerkat_RequestDeferredToolsStopped_ForEach13_staged_requested_deferred_names(_, _)
meerkat_RequestDeferredToolsStopped_ForEach13_staged_requested_deferred_names(acc, remaining) == IF remaining = {} THEN acc ELSE LET item == CHOOSE x \in remaining : TRUE IN LET name == item IN LET next_acc == (acc \cup {name}) IN meerkat_RequestDeferredToolsStopped_ForEach13_staged_requested_deferred_names(next_acc, remaining \ {item})

RECURSIVE meerkat_RequestDeferredToolsStopped_ForEach14_requested_witnesses(_, _, _)
meerkat_RequestDeferredToolsStopped_ForEach14_requested_witnesses(acc, remaining, outer_witnesses) == IF remaining = {} THEN acc ELSE LET item == CHOOSE x \in remaining : TRUE IN LET name == item IN LET next_acc == MapSet(acc, name, (IF name \in DOMAIN outer_witnesses THEN outer_witnesses[name] ELSE "None")) IN meerkat_RequestDeferredToolsStopped_ForEach14_requested_witnesses(next_acc, remaining \ {item}, outer_witnesses)
...
meerkat_UnregisterSessionIdle(arg_session_id) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       ...
       /\ meerkat_staged_visibility_revision' = 0
       /\ meerkat_committed_visibility_revision' = 0
       /\ UNCHANGED << meerkat_current_llm_identity, meerkat_current_capability_surface, meerkat_capability_surface_status, meerkat_capability_base_filter, mob_phase, mob_active_identity, mob_active_runtime_id, mob_active_fence_token, mob_current_generation, mob_inflight_work_id, mob_active_member_count, mob_active_run_count, mob_pending_spawn_count, mob_retiring_member_count, mob_wiring_edge_count, mob_task_count, mob_event_subscription_count, mob_coordinator_bound, witness_current_script_input, witness_remaining_script_inputs >>
       /\ pending_inputs' = SeqRemove(pending_inputs, packet)
       /\ observed_inputs' = observed_inputs
       /\ pending_routes' = pending_routes
       /\ delivered_routes' = delivered_routes
...
meerkat_ReconfigureSessionLlmIdentityAttached(arg_previous_identity, arg_previous_visibility_state, arg_previous_capability_surface, arg_previous_capability_surface_status, arg_target_identity, arg_target_capability_surface, arg_next_visibility_state, arg_next_capability_base_filter, arg_next_active_visibility_revision, arg_tool_visibility_delta) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       ...
       /\ (meerkat_session_id # None)
       /\ (meerkat_active_runtime_id # None)
       /\ ((packet.payload.next_active_visibility_revision = meerkat_active_visibility_revision) \/ (packet.payload.next_active_visibility_revision = ((IF (meerkat_active_visibility_revision > meerkat_staged_visibility_revision) THEN meerkat_active_visibility_revision ELSE meerkat_staged_visibility_revision) + 1)))
       /\ meerkat_phase' = "Attached"
       /\ meerkat_current_llm_identity' = Some(packet.payload.target_identity)
       /\ meerkat_current_capability_surface' = Some(packet.payload.target_capability_surface)
       /\ meerkat_capability_surface_status' = "resolved"
       /\ meerkat_capability_base_filter' = packet.payload.next_capability_base_filter
       /\ meerkat_active_visibility_revision' = packet.payload.next_active_visibility_revision
       /\ UNCHANGED << meerkat_session_id, meerkat_active_runtime_id, meerkat_active_fence_token, meerkat_active_generation, meerkat_active_work_id, meerkat_attachment_live, meerkat_wake_pending, meerkat_process_pending, meerkat_pre_run_phase, meerkat_peer_ingress_configured, meerkat_drain_running, meerkat_inherited_base_filter, meerkat_active_filter, meerkat_staged_filter, meerkat_active_requested_deferred_names, meerkat_staged_requested_deferred_names, meerkat_requested_witnesses, meerkat_filter_witnesses, meerkat_staged_visibility_revision, meerkat_committed_visibility_revision, mob_phase, mob_active_identity, mob_active_runtime_id, mob_active_fence_token, mob_current_generation, mob_inflight_work_id, mob_active_member_count, mob_active_run_count, mob_pending_spawn_count, mob_retiring_member_count, mob_wiring_edge_count, mob_task_count, mob_event_subscription_count, mob_coordinator_bound, witness_current_script_input, witness_remaining_script_inputs >>
       /\ pending_inputs' = SeqRemove(pending_inputs, packet)
       /\ observed_inputs' = observed_inputs
       /\ pending_routes' = pending_routes
       /\ delivered_routes' = delivered_routes
...
meerkat_StagePersistentFilterIdle(arg_filter, arg_witnesses) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       ...
       /\ meerkat_filter_witnesses' = meerkat_StagePersistentFilterIdle_ForEach0_filter_witnesses(meerkat_filter_witnesses, DOMAIN packet.payload.witnesses, packet.payload.witnesses)
       /\ meerkat_staged_visibility_revision' = ((IF (meerkat_active_visibility_revision > meerkat_staged_visibility_revision) THEN meerkat_active_visibility_revision ELSE meerkat_staged_visibility_revision) + 1)
       /\ UNCHANGED << meerkat_session_id, meerkat_active_runtime_id, meerkat_active_fence_token, meerkat_active_generation, meerkat_active_work_id, meerkat_attachment_live, meerkat_wake_pending, meerkat_process_pending, meerkat_pre_run_phase, meerkat_peer_ingress_configured, meerkat_drain_running, meerkat_current_llm_identity, meerkat_current_capability_surface, meerkat_capability_surface_status, meerkat_capability_base_filter, meerkat_inherited_base_filter, meerkat_active_filter, meerkat_active_requested_deferred_names, meerkat_staged_requested_deferred_names, meerkat_requested_witnesses, meerkat_active_visibility_revision, meerkat_committed_visibility_revision, mob_phase, mob_active_identity, mob_active_runtime_id, mob_active_fence_token, mob_current_generation, mob_inflight_work_id, mob_active_member_count, mob_active_run_count, mob_pending_spawn_count, mob_retiring_member_count, mob_wiring_edge_count, mob_task_count, mob_event_subscription_count, mob_coordinator_bound, witness_current_script_input, witness_remaining_script_inputs >>
       /\ pending_inputs' = SeqRemove(pending_inputs, packet)
       /\ observed_inputs' = observed_inputs
       /\ pending_routes' = pending_routes
       /\ delivered_routes' = delivered_routes
...
meerkat_StagePersistentFilterAttached(arg_filter, arg_witnesses) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       ...
       /\ meerkat_phase' = "Attached"
       /\ meerkat_staged_filter' = packet.payload.filter
       /\ meerkat_filter_witnesses' = meerkat_StagePersistentFilterAttached_ForEach1_filter_witnesses(meerkat_filter_witnesses, DOMAIN packet.payload.witnesses, packet.payload.witnesses)
       /\ meerkat_staged_visibility_revision' = ((IF (meerkat_active_visibility_revision > meerkat_staged_visibility_revision) THEN meerkat_active_visibility_revision ELSE meerkat_staged_visibility_revision) + 1)
       /\ UNCHANGED << meerkat_session_id, meerkat_active_runtime_id, meerkat_active_fence_token, meerkat_active_generation, meerkat_active_work_id, meerkat_attachment_live, meerkat_wake_pending, meerkat_process_pending, meerkat_pre_run_phase, meerkat_peer_ingress_configured, meerkat_drain_running, meerkat_current_llm_identity, meerkat_current_capability_surface, meerkat_capability_surface_status, meerkat_capability_base_filter, meerkat_inherited_base_filter, meerkat_active_filter, meerkat_active_requested_deferred_names, meerkat_staged_requested_deferred_names, meerkat_requested_witnesses, meerkat_active_visibility_revision, meerkat_committed_visibility_revision, mob_phase, mob_active_identity, mob_active_runtime_id, mob_active_fence_token, mob_current_generation, mob_inflight_work_id, mob_active_member_count, mob_active_run_count, mob_pending_spawn_count, mob_retiring_member_count, mob_wiring_edge_count, mob_task_count, mob_event_subscription_count, mob_coordinator_bound, witness_current_script_input, witness_remaining_script_inputs >>
       /\ pending_inputs' = SeqRemove(pending_inputs, packet)
       /\ observed_inputs' = observed_inputs
...
meerkat_StagePersistentFilterRunning(arg_filter, arg_witnesses) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       ...
       /\ meerkat_phase' = "Running"
       /\ meerkat_staged_filter' = packet.payload.filter
       /\ meerkat_filter_witnesses' = meerkat_StagePersistentFilterRunning_ForEach2_filter_witnesses(meerkat_filter_witnesses, DOMAIN packet.payload.witnesses, packet.payload.witnesses)
       /\ meerkat_staged_visibility_revision' = ((IF (meerkat_active_visibility_revision > meerkat_staged_visibility_revision) THEN meerkat_active_visibility_revision ELSE meerkat_staged_visibility_revision) + 1)
       /\ UNCHANGED << meerkat_session_id, meerkat_active_runtime_id, meerkat_active_fence_token, meerkat_active_generation, meerkat_active_work_id, meerkat_attachment_live, meerkat_wake_pending, meerkat_process_pending, meerkat_pre_run_phase, meerkat_peer_ingress_configured, meerkat_drain_running, meerkat_current_llm_identity, meerkat_current_capability_surface, meerkat_capability_surface_status, meerkat_capability_base_filter, meerkat_inherited_base_filter, meerkat_active_filter, meerkat_active_requested_deferred_names, meerkat_staged_requested_deferred_names, meerkat_requested_witnesses, meerkat_active_visibility_revision, meerkat_committed_visibility_revision, mob_phase, mob_active_identity, mob_active_runtime_id, mob_active_fence_token, mob_current_generation, mob_inflight_work_id, mob_active_member_count, mob_active_run_count, mob_pending_spawn_count, mob_retiring_member_count, mob_wiring_edge_count, mob_task_count, mob_event_subscription_count, mob_coordinator_bound, witness_current_script_input, witness_remaining_script_inputs >>
       /\ pending_inputs' = SeqRemove(pending_inputs, packet)
       /\ observed_inputs' = observed_inputs
...
meerkat_StagePersistentFilterRetired(arg_filter, arg_witnesses) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       ...
       /\ meerkat_phase' = "Retired"
       /\ meerkat_staged_filter' = packet.payload.filter
       /\ meerkat_filter_witnesses' = meerkat_StagePersistentFilterRetired_ForEach3_filter_witnesses(meerkat_filter_witnesses, DOMAIN packet.payload.witnesses, packet.payload.witnesses)
       /\ meerkat_staged_visibility_revision' = ((IF (meerkat_active_visibility_revision > meerkat_staged_visibility_revision) THEN meerkat_active_visibility_revision ELSE meerkat_staged_visibility_revision) + 1)
       /\ UNCHANGED << meerkat_session_id, meerkat_active_runtime_id, meerkat_active_fence_token, meerkat_active_generation, meerkat_active_work_id, meerkat_attachment_live, meerkat_wake_pending, meerkat_process_pending, meerkat_pre_run_phase, meerkat_peer_ingress_configured, meerkat_drain_running, meerkat_current_llm_identity, meerkat_current_capability_surface, meerkat_capability_surface_status, meerkat_capability_base_filter, meerkat_inherited_base_filter, meerkat_active_filter, meerkat_active_requested_deferred_names, meerkat_staged_requested_deferred_names, meerkat_requested_witnesses, meerkat_active_visibility_revision, meerkat_committed_visibility_revision, mob_phase, mob_active_identity, mob_active_runtime_id, mob_active_fence_token, mob_current_generation, mob_inflight_work_id, mob_active_member_count, mob_active_run_count, mob_pending_spawn_count, mob_retiring_member_count, mob_wiring_edge_count, mob_task_count, mob_event_subscription_count, mob_coordinator_bound, witness_current_script_input, witness_remaining_script_inputs >>
       /\ pending_inputs' = SeqRemove(pending_inputs, packet)
       /\ observed_inputs' = observed_inputs
...
meerkat_StagePersistentFilterStopped(arg_filter, arg_witnesses) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       ...
       /\ meerkat_phase' = "Stopped"
       /\ meerkat_staged_filter' = packet.payload.filter
       /\ meerkat_filter_witnesses' = meerkat_StagePersistentFilterStopped_ForEach4_filter_witnesses(meerkat_filter_witnesses, DOMAIN packet.payload.witnesses, packet.payload.witnesses)
       /\ meerkat_staged_visibility_revision' = ((IF (meerkat_active_visibility_revision > meerkat_staged_visibility_revision) THEN meerkat_active_visibility_revision ELSE meerkat_staged_visibility_revision) + 1)
       /\ UNCHANGED << meerkat_session_id, meerkat_active_runtime_id, meerkat_active_fence_token, meerkat_active_generation, meerkat_active_work_id, meerkat_attachment_live, meerkat_wake_pending, meerkat_process_pending, meerkat_pre_run_phase, meerkat_peer_ingress_configured, meerkat_drain_running, meerkat_current_llm_identity, meerkat_current_capability_surface, meerkat_capability_surface_status, meerkat_capability_base_filter, meerkat_inherited_base_filter, meerkat_active_filter, meerkat_active_requested_deferred_names, meerkat_staged_requested_deferred_names, meerkat_requested_witnesses, meerkat_active_visibility_revision, meerkat_committed_visibility_revision, mob_phase, mob_active_identity, mob_active_runtime_id, mob_active_fence_token, mob_current_generation, mob_inflight_work_id, mob_active_member_count, mob_active_run_count, mob_pending_spawn_count, mob_retiring_member_count, mob_wiring_edge_count, mob_task_count, mob_event_subscription_count, mob_coordinator_bound, witness_current_script_input, witness_remaining_script_inputs >>
       /\ pending_inputs' = SeqRemove(pending_inputs, packet)
       /\ observed_inputs' = observed_inputs
...
meerkat_RequestDeferredToolsIdle(arg_names, arg_witnesses) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       ...
       /\ (meerkat_session_id # None)
       /\ meerkat_phase' = "Idle"
       /\ meerkat_staged_requested_deferred_names' = meerkat_RequestDeferredToolsIdle_ForEach5_staged_requested_deferred_names(meerkat_staged_requested_deferred_names, packet.payload.names)
       /\ meerkat_requested_witnesses' = meerkat_RequestDeferredToolsIdle_ForEach6_requested_witnesses(meerkat_requested_witnesses, DOMAIN packet.payload.witnesses, packet.payload.witnesses)
       /\ meerkat_staged_visibility_revision' = ((IF (meerkat_active_visibility_revision > meerkat_staged_visibility_revision) THEN meerkat_active_visibility_revision ELSE meerkat_staged_visibility_revision) + 1)
       /\ UNCHANGED << meerkat_session_id, meerkat_active_runtime_id, meerkat_active_fence_token, meerkat_active_generation, meerkat_active_work_id, meerkat_attachment_live, meerkat_wake_pending, meerkat_process_pending, meerkat_pre_run_phase, meerkat_peer_ingress_configured, meerkat_drain_running, meerkat_current_llm_identity, meerkat_current_capability_surface, meerkat_capability_surface_status, meerkat_capability_base_filter, meerkat_inherited_base_filter, meerkat_active_filter, meerkat_staged_filter, meerkat_active_requested_deferred_names, meerkat_filter_witnesses, meerkat_active_visibility_revision, meerkat_committed_visibility_revision, mob_phase, mob_active_identity, mob_active_runtime_id, mob_active_fence_token, mob_current_generation, mob_inflight_work_id, mob_active_member_count, mob_active_run_count, mob_pending_spawn_count, mob_retiring_member_count, mob_wiring_edge_count, mob_task_count, mob_event_subscription_count, mob_coordinator_bound, witness_current_script_input, witness_remaining_script_inputs >>
       /\ pending_inputs' = SeqRemove(pending_inputs, packet)
       /\ observed_inputs' = observed_inputs
       /\ pending_routes' = pending_routes
       /\ delivered_routes' = delivered_routes
...
meerkat_RequestDeferredToolsAttached(arg_names, arg_witnesses) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       ...
       /\ (meerkat_session_id # None)
       /\ meerkat_phase' = "Attached"
       /\ meerkat_staged_requested_deferred_names' = meerkat_RequestDeferredToolsAttached_ForEach7_staged_requested_deferred_names(meerkat_staged_requested_deferred_names, packet.payload.names)
       /\ meerkat_requested_witnesses' = meerkat_RequestDeferredToolsAttached_ForEach8_requested_witnesses(meerkat_requested_witnesses, DOMAIN packet.payload.witnesses, packet.payload.witnesses)
       /\ meerkat_staged_visibility_revision' = ((IF (meerkat_active_visibility_revision > meerkat_staged_visibility_revision) THEN meerkat_active_visibility_revision ELSE meerkat_staged_visibility_revision) + 1)
       /\ UNCHANGED << meerkat_session_id, meerkat_active_runtime_id, meerkat_active_fence_token, meerkat_active_generation, meerkat_active_work_id, meerkat_attachment_live, meerkat_wake_pending, meerkat_process_pending, meerkat_pre_run_phase, meerkat_peer_ingress_configured, meerkat_drain_running, meerkat_current_llm_identity, meerkat_current_capability_surface, meerkat_capability_surface_status, meerkat_capability_base_filter, meerkat_inherited_base_filter, meerkat_active_filter, meerkat_staged_filter, meerkat_active_requested_deferred_names, meerkat_filter_witnesses, meerkat_active_visibility_revision, meerkat_committed_visibility_revision, mob_phase, mob_active_identity, mob_active_runtime_id, mob_active_fence_token, mob_current_generation, mob_inflight_work_id, mob_active_member_count, mob_active_run_count, mob_pending_spawn_count, mob_retiring_member_count, mob_wiring_edge_count, mob_task_count, mob_event_subscription_count, mob_coordinator_bound, witness_current_script_input, witness_remaining_script_inputs >>
       /\ pending_inputs' = SeqRemove(pending_inputs, packet)
       /\ observed_inputs' = observed_inputs
...
meerkat_RequestDeferredToolsRunning(arg_names, arg_witnesses) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       ...
       /\ (meerkat_session_id # None)
       /\ meerkat_phase' = "Running"
       /\ meerkat_staged_requested_deferred_names' = meerkat_RequestDeferredToolsRunning_ForEach9_staged_requested_deferred_names(meerkat_staged_requested_deferred_names, packet.payload.names)
       /\ meerkat_requested_witnesses' = meerkat_RequestDeferredToolsRunning_ForEach10_requested_witnesses(meerkat_requested_witnesses, DOMAIN packet.payload.witnesses, packet.payload.witnesses)
       /\ meerkat_staged_visibility_revision' = ((IF (meerkat_active_visibility_revision > meerkat_staged_visibility_revision) THEN meerkat_active_visibility_revision ELSE meerkat_staged_visibility_revision) + 1)
       /\ UNCHANGED << meerkat_session_id, meerkat_active_runtime_id, meerkat_active_fence_token, meerkat_active_generation, meerkat_active_work_id, meerkat_attachment_live, meerkat_wake_pending, meerkat_process_pending, meerkat_pre_run_phase, meerkat_peer_ingress_configured, meerkat_drain_running, meerkat_current_llm_identity, meerkat_current_capability_surface, meerkat_capability_surface_status, meerkat_capability_base_filter, meerkat_inherited_base_filter, meerkat_active_filter, meerkat_staged_filter, meerkat_active_requested_deferred_names, meerkat_filter_witnesses, meerkat_active_visibility_revision, meerkat_committed_visibility_revision, mob_phase, mob_active_identity, mob_active_runtime_id, mob_active_fence_token, mob_current_generation, mob_inflight_work_id, mob_active_member_count, mob_active_run_count, mob_pending_spawn_count, mob_retiring_member_count, mob_wiring_edge_count, mob_task_count, mob_event_subscription_count, mob_coordinator_bound, witness_current_script_input, witness_remaining_script_inputs >>
       /\ pending_inputs' = SeqRemove(pending_inputs, packet)
       /\ observed_inputs' = observed_inputs
...
meerkat_RequestDeferredToolsRetired(arg_names, arg_witnesses) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       ...
       /\ (meerkat_session_id # None)
       /\ meerkat_phase' = "Retired"
       /\ meerkat_staged_requested_deferred_names' = meerkat_RequestDeferredToolsRetired_ForEach11_staged_requested_deferred_names(meerkat_staged_requested_deferred_names, packet.payload.names)
       /\ meerkat_requested_witnesses' = meerkat_RequestDeferredToolsRetired_ForEach12_requested_witnesses(meerkat_requested_witnesses, DOMAIN packet.payload.witnesses, packet.payload.witnesses)
       /\ meerkat_staged_visibility_revision' = ((IF (meerkat_active_visibility_revision > meerkat_staged_visibility_revision) THEN meerkat_active_visibility_revision ELSE meerkat_staged_visibility_revision) + 1)
       /\ UNCHANGED << meerkat_session_id, meerkat_active_runtime_id, meerkat_active_fence_token, meerkat_active_generation, meerkat_active_work_id, meerkat_attachment_live, meerkat_wake_pending, meerkat_process_pending, meerkat_pre_run_phase, meerkat_peer_ingress_configured, meerkat_drain_running, meerkat_current_llm_identity, meerkat_current_capability_surface, meerkat_capability_surface_status, meerkat_capability_base_filter, meerkat_inherited_base_filter, meerkat_active_filter, meerkat_staged_filter, meerkat_active_requested_deferred_names, meerkat_filter_witnesses, meerkat_active_visibility_revision, meerkat_committed_visibility_revision, mob_phase, mob_active_identity, mob_active_runtime_id, mob_active_fence_token, mob_current_generation, mob_inflight_work_id, mob_active_member_count, mob_active_run_count, mob_pending_spawn_count, mob_retiring_member_count, mob_wiring_edge_count, mob_task_count, mob_event_subscription_count, mob_coordinator_bound, witness_current_script_input, witness_remaining_script_inputs >>
       /\ pending_inputs' = SeqRemove(pending_inputs, packet)
       /\ observed_inputs' = observed_inputs
...
meerkat_RequestDeferredToolsStopped(arg_names, arg_witnesses) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       ...
       /\ (meerkat_session_id # None)
       /\ meerkat_phase' = "Stopped"
       /\ meerkat_staged_requested_deferred_names' = meerkat_RequestDeferredToolsStopped_ForEach13_staged_requested_deferred_names(meerkat_staged_requested_deferred_names, packet.payload.names)
       /\ meerkat_requested_witnesses' = meerkat_RequestDeferredToolsStopped_ForEach14_requested_witnesses(meerkat_requested_witnesses, DOMAIN packet.payload.witnesses, packet.payload.witnesses)
       /\ meerkat_staged_visibility_revision' = ((IF (meerkat_active_visibility_revision > meerkat_staged_visibility_revision) THEN meerkat_active_visibility_revision ELSE meerkat_staged_visibility_revision) + 1)
       /\ UNCHANGED << meerkat_session_id, meerkat_active_runtime_id, meerkat_active_fence_token, meerkat_active_generation, meerkat_active_work_id, meerkat_attachment_live, meerkat_wake_pending, meerkat_process_pending, meerkat_pre_run_phase, meerkat_peer_ingress_configured, meerkat_drain_running, meerkat_current_llm_identity, meerkat_current_capability_surface, meerkat_capability_surface_status, meerkat_capability_base_filter, meerkat_inherited_base_filter, meerkat_active_filter, meerkat_staged_filter, meerkat_active_requested_deferred_names, meerkat_filter_witnesses, meerkat_active_visibility_revision, meerkat_committed_visibility_revision, mob_phase, mob_active_identity, mob_active_runtime_id, mob_active_fence_token, mob_current_generation, mob_inflight_work_id, mob_active_member_count, mob_active_run_count, mob_pending_spawn_count, mob_retiring_member_count, mob_wiring_edge_count, mob_task_count, mob_event_subscription_count, mob_coordinator_bound, witness_current_script_input, witness_remaining_script_inputs >>
       /\ pending_inputs' = SeqRemove(pending_inputs, packet)
       /\ observed_inputs' = observed_inputs
...
meerkat_PrepareBindingsInitializing(arg_agent_runtime_id, arg_fence_token, arg_generation) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       ...
       /\ UNCHANGED << meerkat_session_id, meerkat_active_work_id, meerkat_attachment_live, meerkat_wake_pending, meerkat_process_pending, meerkat_pre_run_phase, meerkat_peer_ingress_configured, meerkat_drain_running, meerkat_current_llm_identity, meerkat_current_capability_surface, meerkat_capability_surface_status, meerkat_capability_base_filter, meerkat_inherited_base_filter, meerkat_active_filter, meerkat_staged_filter, meerkat_active_requested_deferred_names, meerkat_staged_requested_deferred_names, meerkat_requested_witnesses, meerkat_filter_witnesses, meerkat_active_visibility_revision, meerkat_staged_visibility_revision, meerkat_committed_visibility_revision, mob_phase, mob_active_identity, mob_active_runtime_id, mob_active_fence_token, mob_current_generation, mob_inflight_work_id, mob_active_member_count, mob_active_run_count, mob_pending_spawn_count, mob_retiring_member_count, mob_wiring_edge_count, mob_task_count, mob_event_subscription_count, mob_coordinator_bound, witness_current_script_input, witness_remaining_script_inputs >>
       ...
       /\ emitted_effects' = emitted_effects \cup { [machine |-> "meerkat", variant |-> "RuntimeBound", payload |-> [agent_runtime_id |-> (IF "value" \in DOMAIN Some(packet.payload.agent_runtime_id) THEN Some(packet.payload.agent_runtime_id)["value"] ELSE None), fence_token |-> (IF "value" \in DOMAIN Some(packet.payload.fence_token) THEN Some(packet.payload.fence_token)["value"] ELSE None), generation |-> (IF "value" \in DOMAIN Some(packet.payload.generation) THEN Some(packet.payload.generation)["value"] ELSE None)], effect_id |-> (model_step_count + 1), source_transition |-> "PrepareBindingsInitializing"] }
...
meerkat_PrepareBindingsIdle(arg_agent_runtime_id, arg_fence_token, arg_generation) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       ...
       /\ emitted_effects' = emitted_effects \cup { [machine |-> "meerkat", variant |-> "RuntimeBound", payload |-> [agent_runtime_id |-> (IF "value" \in DOMAIN Some(packet.payload.agent_runtime_id) THEN Some(packet.payload.agent_runtime_id)["value"] ELSE None), fence_token |-> (IF "value" \in DOMAIN Some(packet.payload.fence_token) THEN Some(packet.payload.fence_token)["value"] ELSE None), generation |-> (IF "value" \in DOMAIN Some(packet.payload.generation) THEN Some(packet.payload.generation)["value"] ELSE None)], effect_id |-> (model_step_count + 1), source_transition |-> "PrepareBindingsIdle"] }
...
meerkat_PrepareBindingsAttached(arg_agent_runtime_id, arg_fence_token, arg_generation) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       ...
       /\ emitted_effects' = emitted_effects \cup { [machine |-> "meerkat", variant |-> "RuntimeBound", payload |-> [agent_runtime_id |-> (IF "value" \in DOMAIN Some(packet.payload.agent_runtime_id) THEN Some(packet.payload.agent_runtime_id)["value"] ELSE None), fence_token |-> (IF "value" \in DOMAIN Some(packet.payload.fence_token) THEN Some(packet.payload.fence_token)["value"] ELSE None), generation |-> (IF "value" \in DOMAIN Some(packet.payload.generation) THEN Some(packet.payload.generation)["value"] ELSE None)], effect_id |-> (model_step_count + 1), source_transition |-> "PrepareBindingsAttached"] }
...
meerkat_PrepareBindingsRunning(arg_agent_runtime_id, arg_fence_token, arg_generation) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       ...
       /\ emitted_effects' = emitted_effects \cup { [machine |-> "meerkat", variant |-> "RuntimeBound", payload |-> [agent_runtime_id |-> (IF "value" \in DOMAIN Some(packet.payload.agent_runtime_id) THEN Some(packet.payload.agent_runtime_id)["value"] ELSE None), fence_token |-> (IF "value" \in DOMAIN Some(packet.payload.fence_token) THEN Some(packet.payload.fence_token)["value"] ELSE None), generation |-> (IF "value" \in DOMAIN Some(packet.payload.generation) THEN Some(packet.payload.generation)["value"] ELSE None)], effect_id |-> (model_step_count + 1), source_transition |-> "PrepareBindingsRunning"] }
...
meerkat_PrepareBindingsRetired(arg_agent_runtime_id, arg_fence_token, arg_generation) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       ...
       /\ emitted_effects' = emitted_effects \cup { [machine |-> "meerkat", variant |-> "RuntimeBound", payload |-> [agent_runtime_id |-> (IF "value" \in DOMAIN Some(packet.payload.agent_runtime_id) THEN Some(packet.payload.agent_runtime_id)["value"] ELSE None), fence_token |-> (IF "value" \in DOMAIN Some(packet.payload.fence_token) THEN Some(packet.payload.fence_token)["value"] ELSE None), generation |-> (IF "value" \in DOMAIN Some(packet.payload.generation) THEN Some(packet.payload.generation)["value"] ELSE None)], effect_id |-> (model_step_count + 1), source_transition |-> "PrepareBindingsRetired"] }
...
meerkat_PrepareBindingsStopped(arg_agent_runtime_id, arg_fence_token, arg_generation) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       ...
       /\ emitted_effects' = emitted_effects \cup { [machine |-> "meerkat", variant |-> "RuntimeBound", payload |-> [agent_runtime_id |-> (IF "value" \in DOMAIN Some(packet.payload.agent_runtime_id) THEN Some(packet.payload.agent_runtime_id)["value"] ELSE None), fence_token |-> (IF "value" \in DOMAIN Some(packet.payload.fence_token) THEN Some(packet.payload.fence_token)["value"] ELSE None), generation |-> (IF "value" \in DOMAIN Some(packet.payload.generation) THEN Some(packet.payload.generation)["value"] ELSE None)], effect_id |-> (model_step_count + 1), source_transition |-> "PrepareBindingsStopped"] }
...
meerkat_SetPeerIngressContextIdle(arg_keep_alive) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       ...
       /\ UNCHANGED << meerkat_session_id, meerkat_active_runtime_id, meerkat_active_fence_token, meerkat_active_generation, meerkat_active_work_id, meerkat_attachment_live, meerkat_wake_pending, meerkat_process_pending, meerkat_pre_run_phase, meerkat_current_llm_identity, meerkat_current_capability_surface, meerkat_capability_surface_status, meerkat_capability_base_filter, meerkat_inherited_base_filter, meerkat_active_filter, meerkat_staged_filter, meerkat_active_requested_deferred_names, meerkat_staged_requested_deferred_names, meerkat_requested_witnesses, meerkat_filter_witnesses, meerkat_active_visibility_revision, meerkat_staged_visibility_revision, meerkat_committed_visibility_revision, mob_phase, mob_active_identity, mob_active_runtime_id, mob_active_fence_token, mob_current_generation, mob_inflight_work_id, mob_active_member_count, mob_active_run_count, mob_pending_spawn_count, mob_retiring_member_count, mob_wiring_edge_count, mob_task_count, mob_event_subscription_count, mob_coordinator_bound, witness_current_script_input, witness_remaining_script_inputs >>
       /\ pending_inputs' = SeqRemove(pending_inputs, packet)
       /\ observed_inputs' = observed_inputs
       /\ pending_routes' = pending_routes
       /\ delivered_routes' = delivered_routes
...
meerkat_BoundaryAppliedPromote(arg_revision) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       ...
       /\ UNCHANGED << meerkat_session_id, meerkat_active_runtime_id, meerkat_active_fence_token, meerkat_active_generation, meerkat_active_work_id, meerkat_attachment_live, meerkat_wake_pending, meerkat_process_pending, meerkat_pre_run_phase, meerkat_peer_ingress_configured, meerkat_drain_running, meerkat_current_llm_identity, meerkat_current_capability_surface, meerkat_capability_surface_status, meerkat_capability_base_filter, meerkat_inherited_base_filter, meerkat_staged_filter, meerkat_staged_requested_deferred_names, meerkat_requested_witnesses, meerkat_filter_witnesses, meerkat_staged_visibility_revision, mob_phase, mob_active_identity, mob_active_runtime_id, mob_active_fence_token, mob_current_generation, mob_inflight_work_id, mob_active_member_count, mob_active_run_count, mob_pending_spawn_count, mob_retiring_member_count, mob_wiring_edge_count, mob_task_count, mob_event_subscription_count, mob_coordinator_bound, witness_current_script_input, witness_remaining_script_inputs >>
       /\ pending_inputs' = SeqRemove(pending_inputs, packet)
       /\ observed_inputs' = observed_inputs
       /\ pending_routes' = pending_routes
       /\ delivered_routes' = delivered_routes
...
meerkat_PublishCommittedVisibleSetIdle(arg_active_filter, arg_staged_filter, arg_active_requested_deferred_names, arg_staged_requested_deferred_names, arg_active_visibility_revision, arg_staged_visibility_revision) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       ...
       /\ (\A name \in packet.payload.active_requested_deferred_names : (name \in packet.payload.staged_requested_deferred_names))
       ...
       /\ meerkat_phase' = "Idle"
       /\ meerkat_active_filter' = packet.payload.active_filter
       /\ meerkat_staged_filter' = packet.payload.staged_filter
       /\ meerkat_active_requested_deferred_names' = packet.payload.active_requested_deferred_names
       /\ meerkat_staged_requested_deferred_names' = packet.payload.staged_requested_deferred_names
       /\ meerkat_active_visibility_revision' = packet.payload.active_visibility_revision
       /\ meerkat_staged_visibility_revision' = packet.payload.staged_visibility_revision
       /\ meerkat_committed_visibility_revision' = packet.payload.active_visibility_revision
       /\ UNCHANGED << meerkat_session_id, meerkat_active_runtime_id, meerkat_active_fence_token, meerkat_active_generation, meerkat_active_work_id, meerkat_attachment_live, meerkat_wake_pending, meerkat_process_pending, meerkat_pre_run_phase, meerkat_peer_ingress_configured, meerkat_drain_running, meerkat_current_llm_identity, meerkat_current_capability_surface, meerkat_capability_surface_status, meerkat_capability_base_filter, meerkat_inherited_base_filter, meerkat_requested_witnesses, meerkat_filter_witnesses, mob_phase, mob_active_identity, mob_active_runtime_id, mob_active_fence_token, mob_current_generation, mob_inflight_work_id, mob_active_member_count, mob_active_run_count, mob_pending_spawn_count, mob_retiring_member_count, mob_wiring_edge_count, mob_task_count, mob_event_subscription_count, mob_coordinator_bound, witness_current_script_input, witness_remaining_script_inputs >>
       /\ pending_inputs' = SeqRemove(pending_inputs, packet)
       /\ observed_inputs' = observed_inputs
       /\ pending_routes' = pending_routes
       /\ delivered_routes' = delivered_routes
...
meerkat_RunCompleted(arg_work_id) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       ...
       /\ meerkat_phase' = "Attached"
       /\ meerkat_active_work_id' = None
       /\ UNCHANGED << meerkat_session_id, meerkat_active_runtime_id, meerkat_active_fence_token, meerkat_active_generation, meerkat_attachment_live, meerkat_wake_pending, meerkat_process_pending, meerkat_pre_run_phase, meerkat_peer_ingress_configured, meerkat_drain_running, meerkat_current_llm_identity, meerkat_current_capability_surface, meerkat_capability_surface_status, meerkat_capability_base_filter, meerkat_inherited_base_filter, meerkat_active_filter, meerkat_staged_filter, meerkat_active_requested_deferred_names, meerkat_staged_requested_deferred_names, meerkat_requested_witnesses, meerkat_filter_witnesses, meerkat_active_visibility_revision, meerkat_staged_visibility_revision, meerkat_committed_visibility_revision, mob_phase, mob_active_identity, mob_active_runtime_id, mob_active_fence_token, mob_current_generation, mob_inflight_work_id, mob_active_member_count, mob_active_run_count, mob_pending_spawn_count, mob_retiring_member_count, mob_wiring_edge_count, mob_task_count, mob_event_subscription_count, mob_coordinator_bound, witness_current_script_input, witness_remaining_script_inputs >>
...
meerkat_RecoverFromRetired ==
    /\ \E packet \in SeqElements(pending_inputs) :
       ...
       /\ pending_routes' = pending_routes
       /\ delivered_routes' = delivered_routes
       /\ emitted_effects' = emitted_effects \cup { [machine |-> "meerkat", variant |-> "RuntimeNotice", payload |-> [detail |-> "runtime recovering", kind |-> "recover"], effect_id |-> (model_step_count + 1), source_transition |-> "RecoverFromRetired"] }
       /\ observed_transitions' = observed_transitions \cup {[machine |-> "meerkat", transition |-> "RecoverFromRetired", actor |-> "meerkat_kernel", step |-> (model_step_count + 1), from_phase |-> meerkat_phase, to_phase |-> "Retired"]}
       /\ model_step_count' = model_step_count + 1


meerkat_RecoverFromStopped ==
    /\ \E packet \in SeqElements(pending_inputs) :
       /\ packet.machine = "meerkat"
       /\ packet.variant = "Recover"
       /\ ~HigherPriorityReady("meerkat_kernel")
       ...
       /\ pending_routes' = pending_routes
       /\ delivered_routes' = delivered_routes
       /\ emitted_effects' = emitted_effects \cup { [machine |-> "meerkat", variant |-> "RuntimeNotice", payload |-> [detail |-> "runtime recovering", kind |-> "recover"], effect_id |-> (model_step_count + 1), source_transition |-> "RecoverFromStopped"] }
       /\ observed_transitions' = observed_transitions \cup {[machine |-> "meerkat", transition |-> "RecoverFromStopped", actor |-> "meerkat_kernel", step |-> (model_step_count + 1), from_phase |-> meerkat_phase, to_phase |-> "Stopped"]}
       /\ model_step_count' = model_step_count + 1


meerkat_RecoverFromInitializing ==
    /\ \E packet \in SeqElements(pending_inputs) :
       /\ packet.machine = "meerkat"
       /\ packet.variant = "Recover"
       /\ ~HigherPriorityReady("meerkat_kernel")
       ...
       /\ pending_routes' = pending_routes
       /\ delivered_routes' = delivered_routes
       /\ emitted_effects' = emitted_effects \cup { [machine |-> "meerkat", variant |-> "RuntimeNotice", payload |-> [detail |-> "runtime recovering", kind |-> "recover"], effect_id |-> (model_step_count + 1), source_transition |-> "RecoverFromInitializing"] }
       /\ observed_transitions' = observed_transitions \cup {[machine |-> "meerkat", transition |-> "RecoverFromInitializing", actor |-> "meerkat_kernel", step |-> (model_step_count + 1), from_phase |-> meerkat_phase, to_phase |-> "Initializing"]}
       /\ model_step_count' = model_step_count + 1
...
meerkat_Reset ==
    /\ \E packet \in SeqElements(pending_inputs) :
       ...
       /\ meerkat_pre_run_phase' = None
       /\ meerkat_drain_running' = FALSE
       /\ UNCHANGED << meerkat_session_id, meerkat_active_runtime_id, meerkat_attachment_live, meerkat_peer_ingress_configured, meerkat_current_llm_identity, meerkat_current_capability_surface, meerkat_capability_surface_status, meerkat_capability_base_filter, meerkat_inherited_base_filter, meerkat_active_filter, meerkat_staged_filter, meerkat_active_requested_deferred_names, meerkat_staged_requested_deferred_names, meerkat_requested_witnesses, meerkat_filter_witnesses, meerkat_active_visibility_revision, meerkat_staged_visibility_revision, meerkat_committed_visibility_revision, mob_phase, mob_active_identity, mob_active_runtime_id, mob_active_fence_token, mob_current_generation, mob_inflight_work_id, mob_active_member_count, mob_active_run_count, mob_pending_spawn_count, mob_retiring_member_count, mob_wiring_edge_count, mob_task_count, mob_event_subscription_count, mob_coordinator_bound, witness_current_script_input, witness_remaining_script_inputs >>
       /\ pending_inputs' = SeqRemove(pending_inputs, packet)
       /\ observed_inputs' = observed_inputs
       /\ pending_routes' = pending_routes
       /\ delivered_routes' = delivered_routes
...
meerkat_StopRuntimeExecutorDetached ==
    /\ \E packet \in SeqElements(pending_inputs) :
       /\ packet.machine = "meerkat"
       /\ packet.variant = "StopRuntimeExecutor"
       /\ ~HigherPriorityReady("meerkat_kernel")
       /\ meerkat_phase = "Initializing" \/ meerkat_phase = "Idle" \/ meerkat_phase = "Attached" \/ meerkat_phase = "Running" \/ meerkat_phase = "Retired"
       /\ ~(meerkat_attachment_live)
       /\ meerkat_phase' = "Stopped"
       /\ meerkat_active_work_id' = None
       /\ meerkat_pre_run_phase' = None
       /\ meerkat_drain_running' = FALSE
       /\ UNCHANGED << meerkat_session_id, meerkat_active_runtime_id, meerkat_active_fence_token, meerkat_active_generation, meerkat_attachment_live, meerkat_wake_pending, meerkat_process_pending, meerkat_peer_ingress_configured, meerkat_current_llm_identity, meerkat_current_capability_surface, meerkat_capability_surface_status, meerkat_capability_base_filter, meerkat_inherited_base_filter, meerkat_active_filter, meerkat_staged_filter, meerkat_active_requested_deferred_names, meerkat_staged_requested_deferred_names, meerkat_requested_witnesses, meerkat_filter_witnesses, meerkat_active_visibility_revision, meerkat_staged_visibility_revision, meerkat_committed_visibility_revision, mob_phase, mob_active_identity, mob_active_runtime_id, mob_active_fence_token, mob_current_generation, mob_inflight_work_id, mob_active_member_count, mob_active_run_count, mob_pending_spawn_count, mob_retiring_member_count, mob_wiring_edge_count, mob_task_count, mob_event_subscription_count, mob_coordinator_bound, witness_current_script_input, witness_remaining_script_inputs >>
       /\ pending_inputs' = SeqRemove(pending_inputs, packet)
       /\ observed_inputs' = observed_inputs
       /\ pending_routes' = pending_routes
       /\ delivered_routes' = delivered_routes
       /\ emitted_effects' = emitted_effects \cup { [machine |-> "meerkat", variant |-> "RuntimeNotice", payload |-> [detail |-> "runtime executor stopped", kind |-> "stop"], effect_id |-> (model_step_count + 1), source_transition |-> "StopRuntimeExecutorDetached"] }
       /\ observed_transitions' = observed_transitions \cup {[machine |-> "meerkat", transition |-> "StopRuntimeExecutorDetached", actor |-> "meerkat_kernel", step |-> (model_step_count + 1), from_phase |-> meerkat_phase, to_phase |-> "Stopped"]}
       /\ model_step_count' = model_step_count + 1


meerkat_StopRuntimeExecutorLiveAttached ==
    /\ \E packet \in SeqElements(pending_inputs) :
       /\ packet.machine = "meerkat"
       /\ packet.variant = "StopRuntimeExecutor"
       /\ ~HigherPriorityReady("meerkat_kernel")
       ...
       /\ pending_routes' = pending_routes
       /\ delivered_routes' = delivered_routes
       /\ emitted_effects' = emitted_effects \cup { [machine |-> "meerkat", variant |-> "RuntimeNotice", payload |-> [detail |-> "runtime executor stopped", kind |-> "stop"], effect_id |-> (model_step_count + 1), source_transition |-> "StopRuntimeExecutorLiveAttached"] }
       /\ observed_transitions' = observed_transitions \cup {[machine |-> "meerkat", transition |-> "StopRuntimeExecutorLiveAttached", actor |-> "meerkat_kernel", step |-> (model_step_count + 1), from_phase |-> meerkat_phase, to_phase |-> "Attached"]}
       /\ model_step_count' = model_step_count + 1


meerkat_StopRuntimeExecutorLiveRunning ==
    /\ \E packet \in SeqElements(pending_inputs) :
       /\ packet.machine = "meerkat"
       /\ packet.variant = "StopRuntimeExecutor"
       /\ ~HigherPriorityReady("meerkat_kernel")
       ...
       /\ pending_routes' = pending_routes
       /\ delivered_routes' = delivered_routes
       /\ emitted_effects' = emitted_effects \cup { [machine |-> "meerkat", variant |-> "RuntimeNotice", payload |-> [detail |-> "runtime executor stopped", kind |-> "stop"], effect_id |-> (model_step_count + 1), source_transition |-> "StopRuntimeExecutorLiveRunning"] }
       /\ observed_transitions' = observed_transitions \cup {[machine |-> "meerkat", transition |-> "StopRuntimeExecutorLiveRunning", actor |-> "meerkat_kernel", step |-> (model_step_count + 1), from_phase |-> meerkat_phase, to_phase |-> "Running"]}
       /\ model_step_count' = model_step_count + 1
...
meerkat_Destroy ==
    /\ \E packet \in SeqElements(pending_inputs) :
       ...
       /\ meerkat_pre_run_phase' = None
       /\ meerkat_drain_running' = FALSE
       /\ UNCHANGED << meerkat_session_id, meerkat_active_runtime_id, meerkat_active_fence_token, meerkat_active_generation, meerkat_attachment_live, meerkat_peer_ingress_configured, meerkat_current_llm_identity, meerkat_current_capability_surface, meerkat_capability_surface_status, meerkat_capability_base_filter, meerkat_inherited_base_filter, meerkat_active_filter, meerkat_staged_filter, meerkat_active_requested_deferred_names, meerkat_staged_requested_deferred_names, meerkat_requested_witnesses, meerkat_filter_witnesses, meerkat_active_visibility_revision, meerkat_staged_visibility_revision, meerkat_committed_visibility_revision, mob_phase, mob_active_identity, mob_active_runtime_id, mob_active_fence_token, mob_current_generation, mob_inflight_work_id, mob_active_member_count, mob_active_run_count, mob_pending_spawn_count, mob_retiring_member_count, mob_wiring_edge_count, mob_task_count, mob_event_subscription_count, mob_coordinator_bound, witness_current_script_input, witness_remaining_script_inputs >>
...
meerkat_EnsureSessionWithExecutorIdle(arg_session_id) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       ...
       /\ meerkat_phase = "Idle"
       /\ meerkat_phase' = "Attached"
       /\ meerkat_attachment_live' = TRUE
       /\ UNCHANGED << meerkat_session_id, meerkat_active_runtime_id, meerkat_active_fence_token, meerkat_active_generation, meerkat_active_work_id, meerkat_wake_pending, meerkat_process_pending, meerkat_pre_run_phase, meerkat_peer_ingress_configured, meerkat_drain_running, meerkat_current_llm_identity, meerkat_current_capability_surface, meerkat_capability_surface_status, meerkat_capability_base_filter, meerkat_inherited_base_filter, meerkat_active_filter, meerkat_staged_filter, meerkat_active_requested_deferred_names, meerkat_staged_requested_deferred_names, meerkat_requested_witnesses, meerkat_filter_witnesses, meerkat_active_visibility_revision, meerkat_staged_visibility_revision, meerkat_committed_visibility_revision, mob_phase, mob_active_identity, mob_active_runtime_id, mob_active_fence_token, mob_current_generation, mob_inflight_work_id, mob_active_member_count, mob_active_run_count, mob_pending_spawn_count, mob_retiring_member_count, mob_wiring_edge_count, mob_task_count, mob_event_subscription_count, mob_coordinator_bound, witness_current_script_input, witness_remaining_script_inputs >>
       /\ pending_inputs' = SeqRemove(pending_inputs, packet)
       /\ observed_inputs' = observed_inputs
       /\ pending_routes' = pending_routes
       /\ delivered_routes' = delivered_routes
...
meerkat_ContainsSessionAttached(arg_session_id) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       /\ packet.machine = "meerkat"
       /\ packet.variant = "ContainsSession"
       /\ packet.payload.session_id = arg_session_id
       ...
       /\ delivered_routes' = delivered_routes
       /\ emitted_effects' = emitted_effects
       /\ observed_transitions' = observed_transitions \cup {[machine |-> "meerkat", transition |-> "ContainsSessionAttached", actor |-> "meerkat_kernel", step |-> (model_step_count + 1), from_phase |-> meerkat_phase, to_phase |-> "Attached"]}
       /\ model_step_count' = model_step_count + 1


meerkat_SessionHasExecutorAttached(arg_session_id) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       /\ packet.machine = "meerkat"
       /\ packet.variant = "SessionHasExecutor"
       /\ packet.payload.session_id = arg_session_id
       ...
       /\ delivered_routes' = delivered_routes
       /\ emitted_effects' = emitted_effects
       /\ observed_transitions' = observed_transitions \cup {[machine |-> "meerkat", transition |-> "SessionHasExecutorAttached", actor |-> "meerkat_kernel", step |-> (model_step_count + 1), from_phase |-> meerkat_phase, to_phase |-> "Attached"]}
       /\ model_step_count' = model_step_count + 1


meerkat_SessionHasCommsAttached(arg_session_id) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       /\ packet.machine = "meerkat"
       /\ packet.variant = "SessionHasComms"
       /\ packet.payload.session_id = arg_session_id
       ...
       /\ delivered_routes' = delivered_routes
       /\ emitted_effects' = emitted_effects
       /\ observed_transitions' = observed_transitions \cup {[machine |-> "meerkat", transition |-> "SessionHasCommsAttached", actor |-> "meerkat_kernel", step |-> (model_step_count + 1), from_phase |-> meerkat_phase, to_phase |-> "Attached"]}
       /\ model_step_count' = model_step_count + 1


meerkat_OpsLifecycleRegistryAttached(arg_session_id) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       /\ packet.machine = "meerkat"
       /\ packet.variant = "OpsLifecycleRegistry"
       /\ packet.payload.session_id = arg_session_id
       ...
       /\ delivered_routes' = delivered_routes
       /\ emitted_effects' = emitted_effects
       /\ observed_transitions' = observed_transitions \cup {[machine |-> "meerkat", transition |-> "OpsLifecycleRegistryAttached", actor |-> "meerkat_kernel", step |-> (model_step_count + 1), from_phase |-> meerkat_phase, to_phase |-> "Attached"]}
       /\ model_step_count' = model_step_count + 1


meerkat_InputStateAttached(arg_session_id, arg_input_id) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       /\ packet.machine = "meerkat"
       /\ packet.variant = "InputState"
       /\ packet.payload.session_id = arg_session_id
       ...
       /\ delivered_routes' = delivered_routes
       /\ emitted_effects' = emitted_effects
       /\ observed_transitions' = observed_transitions \cup {[machine |-> "meerkat", transition |-> "InputStateAttached", actor |-> "meerkat_kernel", step |-> (model_step_count + 1), from_phase |-> meerkat_phase, to_phase |-> "Attached"]}
       /\ model_step_count' = model_step_count + 1


meerkat_ListActiveInputsAttached(arg_session_id) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       /\ packet.machine = "meerkat"
       /\ packet.variant = "ListActiveInputs"
       /\ packet.payload.session_id = arg_session_id
       ...
       /\ delivered_routes' = delivered_routes
       /\ emitted_effects' = emitted_effects
       /\ observed_transitions' = observed_transitions \cup {[machine |-> "meerkat", transition |-> "ListActiveInputsAttached", actor |-> "meerkat_kernel", step |-> (model_step_count + 1), from_phase |-> meerkat_phase, to_phase |-> "Attached"]}
       /\ model_step_count' = model_step_count + 1
...
meerkat_ContainsSessionRunning(arg_session_id) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       /\ packet.machine = "meerkat"
       /\ packet.variant = "ContainsSession"
       /\ packet.payload.session_id = arg_session_id
       ...
       /\ delivered_routes' = delivered_routes
       /\ emitted_effects' = emitted_effects
       /\ observed_transitions' = observed_transitions \cup {[machine |-> "meerkat", transition |-> "ContainsSessionRunning", actor |-> "meerkat_kernel", step |-> (model_step_count + 1), from_phase |-> meerkat_phase, to_phase |-> "Running"]}
       /\ model_step_count' = model_step_count + 1


meerkat_SessionHasExecutorRunning(arg_session_id) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       /\ packet.machine = "meerkat"
       /\ packet.variant = "SessionHasExecutor"
       /\ packet.payload.session_id = arg_session_id
       ...
       /\ delivered_routes' = delivered_routes
       /\ emitted_effects' = emitted_effects
       /\ observed_transitions' = observed_transitions \cup {[machine |-> "meerkat", transition |-> "SessionHasExecutorRunning", actor |-> "meerkat_kernel", step |-> (model_step_count + 1), from_phase |-> meerkat_phase, to_phase |-> "Running"]}
       /\ model_step_count' = model_step_count + 1


meerkat_SessionHasCommsRunning(arg_session_id) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       /\ packet.machine = "meerkat"
       /\ packet.variant = "SessionHasComms"
       /\ packet.payload.session_id = arg_session_id
       ...
       /\ delivered_routes' = delivered_routes
       /\ emitted_effects' = emitted_effects
       /\ observed_transitions' = observed_transitions \cup {[machine |-> "meerkat", transition |-> "SessionHasCommsRunning", actor |-> "meerkat_kernel", step |-> (model_step_count + 1), from_phase |-> meerkat_phase, to_phase |-> "Running"]}
       /\ model_step_count' = model_step_count + 1


meerkat_OpsLifecycleRegistryRunning(arg_session_id) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       /\ packet.machine = "meerkat"
       /\ packet.variant = "OpsLifecycleRegistry"
       /\ packet.payload.session_id = arg_session_id
       ...
       /\ delivered_routes' = delivered_routes
       /\ emitted_effects' = emitted_effects
       /\ observed_transitions' = observed_transitions \cup {[machine |-> "meerkat", transition |-> "OpsLifecycleRegistryRunning", actor |-> "meerkat_kernel", step |-> (model_step_count + 1), from_phase |-> meerkat_phase, to_phase |-> "Running"]}
       /\ model_step_count' = model_step_count + 1


meerkat_InputStateRunning(arg_session_id, arg_input_id) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       /\ packet.machine = "meerkat"
       /\ packet.variant = "InputState"
       /\ packet.payload.session_id = arg_session_id
       ...
       /\ delivered_routes' = delivered_routes
       /\ emitted_effects' = emitted_effects
       /\ observed_transitions' = observed_transitions \cup {[machine |-> "meerkat", transition |-> "InputStateRunning", actor |-> "meerkat_kernel", step |-> (model_step_count + 1), from_phase |-> meerkat_phase, to_phase |-> "Running"]}
       /\ model_step_count' = model_step_count + 1


meerkat_ListActiveInputsRunning(arg_session_id) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       /\ packet.machine = "meerkat"
       /\ packet.variant = "ListActiveInputs"
       /\ packet.payload.session_id = arg_session_id
       ...
       /\ delivered_routes' = delivered_routes
       /\ emitted_effects' = emitted_effects
       /\ observed_transitions' = observed_transitions \cup {[machine |-> "meerkat", transition |-> "ListActiveInputsRunning", actor |-> "meerkat_kernel", step |-> (model_step_count + 1), from_phase |-> meerkat_phase, to_phase |-> "Running"]}
       /\ model_step_count' = model_step_count + 1
...
meerkat_ContainsSessionRetired(arg_session_id) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       /\ packet.machine = "meerkat"
       /\ packet.variant = "ContainsSession"
       /\ packet.payload.session_id = arg_session_id
       ...
       /\ delivered_routes' = delivered_routes
       /\ emitted_effects' = emitted_effects
       /\ observed_transitions' = observed_transitions \cup {[machine |-> "meerkat", transition |-> "ContainsSessionRetired", actor |-> "meerkat_kernel", step |-> (model_step_count + 1), from_phase |-> meerkat_phase, to_phase |-> "Retired"]}
       /\ model_step_count' = model_step_count + 1


meerkat_SessionHasExecutorRetired(arg_session_id) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       /\ packet.machine = "meerkat"
       /\ packet.variant = "SessionHasExecutor"
       /\ packet.payload.session_id = arg_session_id
       ...
       /\ delivered_routes' = delivered_routes
       /\ emitted_effects' = emitted_effects
       /\ observed_transitions' = observed_transitions \cup {[machine |-> "meerkat", transition |-> "SessionHasExecutorRetired", actor |-> "meerkat_kernel", step |-> (model_step_count + 1), from_phase |-> meerkat_phase, to_phase |-> "Retired"]}
       /\ model_step_count' = model_step_count + 1


meerkat_SessionHasCommsRetired(arg_session_id) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       /\ packet.machine = "meerkat"
       /\ packet.variant = "SessionHasComms"
       /\ packet.payload.session_id = arg_session_id
       ...
       /\ delivered_routes' = delivered_routes
       /\ emitted_effects' = emitted_effects
       /\ observed_transitions' = observed_transitions \cup {[machine |-> "meerkat", transition |-> "SessionHasCommsRetired", actor |-> "meerkat_kernel", step |-> (model_step_count + 1), from_phase |-> meerkat_phase, to_phase |-> "Retired"]}
       /\ model_step_count' = model_step_count + 1


meerkat_OpsLifecycleRegistryRetired(arg_session_id) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       /\ packet.machine = "meerkat"
       /\ packet.variant = "OpsLifecycleRegistry"
       /\ packet.payload.session_id = arg_session_id
       ...
       /\ delivered_routes' = delivered_routes
       /\ emitted_effects' = emitted_effects
       /\ observed_transitions' = observed_transitions \cup {[machine |-> "meerkat", transition |-> "OpsLifecycleRegistryRetired", actor |-> "meerkat_kernel", step |-> (model_step_count + 1), from_phase |-> meerkat_phase, to_phase |-> "Retired"]}
       /\ model_step_count' = model_step_count + 1


meerkat_InputStateRetired(arg_session_id, arg_input_id) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       /\ packet.machine = "meerkat"
       /\ packet.variant = "InputState"
       /\ packet.payload.session_id = arg_session_id
       ...
       /\ delivered_routes' = delivered_routes
       /\ emitted_effects' = emitted_effects
       /\ observed_transitions' = observed_transitions \cup {[machine |-> "meerkat", transition |-> "InputStateRetired", actor |-> "meerkat_kernel", step |-> (model_step_count + 1), from_phase |-> meerkat_phase, to_phase |-> "Retired"]}
       /\ model_step_count' = model_step_count + 1


meerkat_ListActiveInputsRetired(arg_session_id) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       /\ packet.machine = "meerkat"
       /\ packet.variant = "ListActiveInputs"
       /\ packet.payload.session_id = arg_session_id
       ...
       /\ delivered_routes' = delivered_routes
       /\ emitted_effects' = emitted_effects
       /\ observed_transitions' = observed_transitions \cup {[machine |-> "meerkat", transition |-> "ListActiveInputsRetired", actor |-> "meerkat_kernel", step |-> (model_step_count + 1), from_phase |-> meerkat_phase, to_phase |-> "Retired"]}
       /\ model_step_count' = model_step_count + 1
...
meerkat_ContainsSessionStopped(arg_session_id) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       /\ packet.machine = "meerkat"
       /\ packet.variant = "ContainsSession"
       /\ packet.payload.session_id = arg_session_id
       ...
       /\ delivered_routes' = delivered_routes
       /\ emitted_effects' = emitted_effects
       /\ observed_transitions' = observed_transitions \cup {[machine |-> "meerkat", transition |-> "ContainsSessionStopped", actor |-> "meerkat_kernel", step |-> (model_step_count + 1), from_phase |-> meerkat_phase, to_phase |-> "Stopped"]}
       /\ model_step_count' = model_step_count + 1


meerkat_SessionHasExecutorStopped(arg_session_id) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       /\ packet.machine = "meerkat"
       /\ packet.variant = "SessionHasExecutor"
       /\ packet.payload.session_id = arg_session_id
       ...
       /\ delivered_routes' = delivered_routes
       /\ emitted_effects' = emitted_effects
       /\ observed_transitions' = observed_transitions \cup {[machine |-> "meerkat", transition |-> "SessionHasExecutorStopped", actor |-> "meerkat_kernel", step |-> (model_step_count + 1), from_phase |-> meerkat_phase, to_phase |-> "Stopped"]}
       /\ model_step_count' = model_step_count + 1


meerkat_SessionHasCommsStopped(arg_session_id) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       /\ packet.machine = "meerkat"
       /\ packet.variant = "SessionHasComms"
       /\ packet.payload.session_id = arg_session_id
       ...
       /\ delivered_routes' = delivered_routes
       /\ emitted_effects' = emitted_effects
       /\ observed_transitions' = observed_transitions \cup {[machine |-> "meerkat", transition |-> "SessionHasCommsStopped", actor |-> "meerkat_kernel", step |-> (model_step_count + 1), from_phase |-> meerkat_phase, to_phase |-> "Stopped"]}
       /\ model_step_count' = model_step_count + 1


meerkat_OpsLifecycleRegistryStopped(arg_session_id) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       /\ packet.machine = "meerkat"
       /\ packet.variant = "OpsLifecycleRegistry"
       /\ packet.payload.session_id = arg_session_id
       ...
       /\ delivered_routes' = delivered_routes
       /\ emitted_effects' = emitted_effects
       /\ observed_transitions' = observed_transitions \cup {[machine |-> "meerkat", transition |-> "OpsLifecycleRegistryStopped", actor |-> "meerkat_kernel", step |-> (model_step_count + 1), from_phase |-> meerkat_phase, to_phase |-> "Stopped"]}
       /\ model_step_count' = model_step_count + 1


meerkat_InputStateStopped(arg_session_id, arg_input_id) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       /\ packet.machine = "meerkat"
       /\ packet.variant = "InputState"
       /\ packet.payload.session_id = arg_session_id
       ...
       /\ delivered_routes' = delivered_routes
       /\ emitted_effects' = emitted_effects
       /\ observed_transitions' = observed_transitions \cup {[machine |-> "meerkat", transition |-> "InputStateStopped", actor |-> "meerkat_kernel", step |-> (model_step_count + 1), from_phase |-> meerkat_phase, to_phase |-> "Stopped"]}
       /\ model_step_count' = model_step_count + 1


meerkat_ListActiveInputsStopped(arg_session_id) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       /\ packet.machine = "meerkat"
       /\ packet.variant = "ListActiveInputs"
       /\ packet.payload.session_id = arg_session_id
       ...
       /\ delivered_routes' = delivered_routes
       /\ emitted_effects' = emitted_effects
       /\ observed_transitions' = observed_transitions \cup {[machine |-> "meerkat", transition |-> "ListActiveInputsStopped", actor |-> "meerkat_kernel", step |-> (model_step_count + 1), from_phase |-> meerkat_phase, to_phase |-> "Stopped"]}
       /\ model_step_count' = model_step_count + 1
...
meerkat_IngestIdle(arg_runtime_id) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       /\ packet.machine = "meerkat"
...
meerkat_AcceptWithCompletionIdle(arg_input_id) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       /\ packet.machine = "meerkat"
       ...
       /\ pending_routes' = pending_routes
       /\ delivered_routes' = delivered_routes
       /\ emitted_effects' = emitted_effects \cup { [machine |-> "meerkat", variant |-> "IngressAccepted", payload |-> [tag |-> "unit"], effect_id |-> (model_step_count + 1), source_transition |-> "AcceptWithCompletionIdle"] }
       /\ observed_transitions' = observed_transitions \cup {[machine |-> "meerkat", transition |-> "AcceptWithCompletionIdle", actor |-> "meerkat_kernel", step |-> (model_step_count + 1), from_phase |-> meerkat_phase, to_phase |-> "Idle"]}
       /\ model_step_count' = model_step_count + 1
...
meerkat_RuntimeStateIdle(arg_runtime_id) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       /\ packet.machine = "meerkat"
       ...
       /\ delivered_routes' = delivered_routes
       /\ emitted_effects' = emitted_effects
       /\ observed_transitions' = observed_transitions \cup {[machine |-> "meerkat", transition |-> "RuntimeStateIdle", actor |-> "meerkat_kernel", step |-> (model_step_count + 1), from_phase |-> meerkat_phase, to_phase |-> "Idle"]}
       /\ model_step_count' = model_step_count + 1
...
meerkat_RuntimeStateRetired(arg_runtime_id) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       /\ packet.machine = "meerkat"
       /\ packet.variant = "RuntimeState"
       /\ packet.payload.runtime_id = arg_runtime_id
       ...
       /\ delivered_routes' = delivered_routes
       /\ emitted_effects' = emitted_effects
       /\ observed_transitions' = observed_transitions \cup {[machine |-> "meerkat", transition |-> "RuntimeStateRetired", actor |-> "meerkat_kernel", step |-> (model_step_count + 1), from_phase |-> meerkat_phase, to_phase |-> "Retired"]}
       /\ model_step_count' = model_step_count + 1


meerkat_RuntimeStateStopped(arg_runtime_id) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       /\ packet.machine = "meerkat"
       /\ packet.variant = "RuntimeState"
       /\ packet.payload.runtime_id = arg_runtime_id
       ...
       /\ delivered_routes' = delivered_routes
       /\ emitted_effects' = emitted_effects
       /\ observed_transitions' = observed_transitions \cup {[machine |-> "meerkat", transition |-> "RuntimeStateStopped", actor |-> "meerkat_kernel", step |-> (model_step_count + 1), from_phase |-> meerkat_phase, to_phase |-> "Stopped"]}
       /\ model_step_count' = model_step_count + 1


meerkat_LoadBoundaryReceiptIdle(arg_runtime_id, arg_sequence) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       /\ packet.machine = "meerkat"
       ...
       /\ delivered_routes' = delivered_routes
       /\ emitted_effects' = emitted_effects
       /\ observed_transitions' = observed_transitions \cup {[machine |-> "meerkat", transition |-> "LoadBoundaryReceiptIdle", actor |-> "meerkat_kernel", step |-> (model_step_count + 1), from_phase |-> meerkat_phase, to_phase |-> "Idle"]}
       /\ model_step_count' = model_step_count + 1
...
meerkat_LoadBoundaryReceiptRetired(arg_runtime_id, arg_sequence) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       /\ packet.machine = "meerkat"
       /\ packet.variant = "LoadBoundaryReceipt"
       /\ packet.payload.runtime_id = arg_runtime_id
       ...
       /\ delivered_routes' = delivered_routes
       /\ emitted_effects' = emitted_effects
       /\ observed_transitions' = observed_transitions \cup {[machine |-> "meerkat", transition |-> "LoadBoundaryReceiptRetired", actor |-> "meerkat_kernel", step |-> (model_step_count + 1), from_phase |-> meerkat_phase, to_phase |-> "Retired"]}
       /\ model_step_count' = model_step_count + 1


meerkat_LoadBoundaryReceiptStopped(arg_runtime_id, arg_sequence) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       /\ packet.machine = "meerkat"
       /\ packet.variant = "LoadBoundaryReceipt"
       /\ packet.payload.runtime_id = arg_runtime_id
       ...
       /\ delivered_routes' = delivered_routes
       /\ emitted_effects' = emitted_effects
       /\ observed_transitions' = observed_transitions \cup {[machine |-> "meerkat", transition |-> "LoadBoundaryReceiptStopped", actor |-> "meerkat_kernel", step |-> (model_step_count + 1), from_phase |-> meerkat_phase, to_phase |-> "Stopped"]}
       /\ model_step_count' = model_step_count + 1


meerkat_PrepareIdle(arg_session_id) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       /\ packet.machine = "meerkat"
       ...
       /\ UNCHANGED << meerkat_session_id, meerkat_active_runtime_id, meerkat_active_fence_token, meerkat_active_generation, meerkat_active_work_id, meerkat_attachment_live, meerkat_wake_pending, meerkat_process_pending, meerkat_peer_ingress_configured, meerkat_drain_running, meerkat_current_llm_identity, meerkat_current_capability_surface, meerkat_capability_surface_status, meerkat_capability_base_filter, meerkat_inherited_base_filter, meerkat_active_filter, meerkat_staged_filter, meerkat_active_requested_deferred_names, meerkat_staged_requested_deferred_names, meerkat_requested_witnesses, meerkat_filter_witnesses, meerkat_active_visibility_revision, meerkat_staged_visibility_revision, meerkat_committed_visibility_revision, mob_phase, mob_active_identity, mob_active_runtime_id, mob_active_fence_token, mob_current_generation, mob_inflight_work_id, mob_active_member_count, mob_active_run_count, mob_pending_spawn_count, mob_retiring_member_count, mob_wiring_edge_count, mob_task_count, mob_event_subscription_count, mob_coordinator_bound, witness_current_script_input, witness_remaining_script_inputs >>
       /\ pending_inputs' = SeqRemove(pending_inputs, packet)
       /\ observed_inputs' = observed_inputs
       /\ pending_routes' = pending_routes
       /\ delivered_routes' = delivered_routes
...
meerkat_OwnerTerminatedRunning ==
    /\ \E packet \in SeqElements(pending_inputs) :
       /\ packet.machine = "meerkat"
       ...
       /\ pending_routes' = pending_routes
       /\ delivered_routes' = delivered_routes
       /\ emitted_effects' = emitted_effects \cup { [machine |-> "meerkat", variant |-> "CompletionResolved", payload |-> [tag |-> "unit"], effect_id |-> (model_step_count + 1), source_transition |-> "OwnerTerminatedRunning"] }
       /\ observed_transitions' = observed_transitions \cup {[machine |-> "meerkat", transition |-> "OwnerTerminatedRunning", actor |-> "meerkat_kernel", step |-> (model_step_count + 1), from_phase |-> meerkat_phase, to_phase |-> "Running"]}
       /\ model_step_count' = model_step_count + 1
...
meerkat_RecycleFromIdleOrRetired ==
    /\ \E packet \in SeqElements(pending_inputs) :
       ...
       /\ meerkat_peer_ingress_configured' = FALSE
       /\ meerkat_drain_running' = FALSE
       /\ UNCHANGED << meerkat_session_id, meerkat_active_runtime_id, meerkat_attachment_live, meerkat_pre_run_phase, meerkat_current_llm_identity, meerkat_current_capability_surface, meerkat_capability_surface_status, meerkat_capability_base_filter, meerkat_inherited_base_filter, meerkat_active_filter, meerkat_staged_filter, meerkat_active_requested_deferred_names, meerkat_staged_requested_deferred_names, meerkat_requested_witnesses, meerkat_filter_witnesses, meerkat_active_visibility_revision, meerkat_staged_visibility_revision, meerkat_committed_visibility_revision, mob_phase, mob_active_identity, mob_active_runtime_id, mob_active_fence_token, mob_current_generation, mob_inflight_work_id, mob_active_member_count, mob_active_run_count, mob_pending_spawn_count, mob_retiring_member_count, mob_wiring_edge_count, mob_task_count, mob_event_subscription_count, mob_coordinator_bound, witness_current_script_input, witness_remaining_script_inputs >>
       /\ pending_inputs' = SeqRemove(pending_inputs, packet)
       /\ observed_inputs' = observed_inputs
       /\ pending_routes' = pending_routes
       /\ delivered_routes' = delivered_routes
...
mob_SpawnRunning(arg_agent_identity, arg_agent_runtime_id, arg_fence_token, arg_generation) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       /\ packet.machine = "mob"
       ...
       /\ pending_routes' = pending_routes
       /\ delivered_routes' = delivered_routes \cup { [route |-> "binding_request_reaches_meerkat", source_machine |-> "mob", effect |-> "RequestRuntimeBinding", target_machine |-> "meerkat", target_input |-> "PrepareBindings", payload |-> [agent_runtime_id |-> (IF "value" \in DOMAIN Some(packet.payload.agent_runtime_id) THEN Some(packet.payload.agent_runtime_id)["value"] ELSE None), fence_token |-> (IF "value" \in DOMAIN Some(packet.payload.fence_token) THEN Some(packet.payload.fence_token)["value"] ELSE None), generation |-> (IF "value" \in DOMAIN Some(packet.payload.generation) THEN Some(packet.payload.generation)["value"] ELSE None)], actor |-> "meerkat_kernel", effect_id |-> (model_step_count + 1), source_transition |-> "SpawnRunning"] }
       /\ emitted_effects' = emitted_effects \cup { [machine |-> "mob", variant |-> "RequestRuntimeBinding", payload |-> [agent_identity |-> (IF "value" \in DOMAIN Some(packet.payload.agent_identity) THEN Some(packet.payload.agent_identity)["value"] ELSE None), agent_runtime_id |-> (IF "value" \in DOMAIN Some(packet.payload.agent_runtime_id) THEN Some(packet.payload.agent_runtime_id)["value"] ELSE None), fence_token |-> (IF "value" \in DOMAIN Some(packet.payload.fence_token) THEN Some(packet.payload.fence_token)["value"] ELSE None), generation |-> (IF "value" \in DOMAIN Some(packet.payload.generation) THEN Some(packet.payload.generation)["value"] ELSE None)], effect_id |-> (model_step_count + 1), source_transition |-> "SpawnRunning"], [machine |-> "mob", variant |-> "EmitMemberLifecycleNotice", payload |-> [agent_identity |-> (IF "value" \in DOMAIN Some(packet.payload.agent_identity) THEN Some(packet.payload.agent_identity)["value"] ELSE None), kind |-> "spawned"], effect_id |-> (model_step_count + 1), source_transition |-> "SpawnRunning"] }
...
mob_SubmitWorkRunning(arg_agent_runtime_id, arg_fence_token, arg_work_id) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       /\ packet.machine = "mob"
       ...
       /\ delivered_routes' = delivered_routes
       /\ emitted_effects' = emitted_effects
       /\ observed_transitions' = observed_transitions \cup {[machine |-> "mob", transition |-> "SubmitWorkRunning", actor |-> "mob_kernel", step |-> (model_step_count + 1), from_phase |-> mob_phase, to_phase |-> "Running"]}
       /\ model_step_count' = model_step_count + 1
...
mob_TaskCreateRunning ==
    /\ \E packet \in SeqElements(pending_inputs) :
       ...
       /\ UNCHANGED << meerkat_phase, meerkat_session_id, meerkat_active_runtime_id, meerkat_active_fence_token, meerkat_active_generation, meerkat_active_work_id, meerkat_attachment_live, meerkat_wake_pending, meerkat_process_pending, meerkat_pre_run_phase, meerkat_peer_ingress_configured, meerkat_drain_running, meerkat_current_llm_identity, meerkat_current_capability_surface, meerkat_capability_surface_status, meerkat_capability_base_filter, meerkat_inherited_base_filter, meerkat_active_filter, meerkat_staged_filter, meerkat_active_requested_deferred_names, meerkat_staged_requested_deferred_names, meerkat_requested_witnesses, meerkat_filter_witnesses, meerkat_active_visibility_revision, meerkat_staged_visibility_revision, meerkat_committed_visibility_revision, mob_active_identity, mob_active_runtime_id, mob_active_fence_token, mob_current_generation, mob_inflight_work_id, mob_active_member_count, mob_active_run_count, mob_pending_spawn_count, mob_retiring_member_count, mob_wiring_edge_count, mob_event_subscription_count, mob_coordinator_bound, witness_current_script_input, witness_remaining_script_inputs >>
       /\ pending_inputs' = SeqRemove(pending_inputs, packet)
       /\ observed_inputs' = observed_inputs
       /\ pending_routes' = pending_routes
       /\ delivered_routes' = delivered_routes
...
mob_SubscribeAgentEventsRunning ==
    /\ \E packet \in SeqElements(pending_inputs) :
       ...
       /\ UNCHANGED << meerkat_phase, meerkat_session_id, meerkat_active_runtime_id, meerkat_active_fence_token, meerkat_active_generation, meerkat_active_work_id, meerkat_attachment_live, meerkat_wake_pending, meerkat_process_pending, meerkat_pre_run_phase, meerkat_peer_ingress_configured, meerkat_drain_running, meerkat_current_llm_identity, meerkat_current_capability_surface, meerkat_capability_surface_status, meerkat_capability_base_filter, meerkat_inherited_base_filter, meerkat_active_filter, meerkat_staged_filter, meerkat_active_requested_deferred_names, meerkat_staged_requested_deferred_names, meerkat_requested_witnesses, meerkat_filter_witnesses, meerkat_active_visibility_revision, meerkat_staged_visibility_revision, meerkat_committed_visibility_revision, mob_active_identity, mob_active_runtime_id, mob_active_fence_token, mob_current_generation, mob_inflight_work_id, mob_active_member_count, mob_active_run_count, mob_pending_spawn_count, mob_retiring_member_count, mob_wiring_edge_count, mob_task_count, mob_coordinator_bound, witness_current_script_input, witness_remaining_script_inputs >>
       /\ pending_inputs' = SeqRemove(pending_inputs, packet)
       /\ observed_inputs' = observed_inputs
       /\ pending_routes' = pending_routes
       /\ delivered_routes' = delivered_routes
...
mob_CancelWorkStopped(arg_work_id) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       /\ packet.machine = "mob"
       /\ packet.variant = "CancelWork"
       /\ packet.payload.work_id = arg_work_id
       ...
       /\ pending_routes' = pending_routes
       /\ delivered_routes' = delivered_routes
       /\ emitted_effects' = emitted_effects \cup { [machine |-> "mob", variant |-> "FlowTerminalized", payload |-> [tag |-> "unit"], effect_id |-> (model_step_count + 1), source_transition |-> "CancelWorkStopped"] }
       /\ observed_transitions' = observed_transitions \cup {[machine |-> "mob", transition |-> "CancelWorkStopped", actor |-> "mob_kernel", step |-> (model_step_count + 1), from_phase |-> mob_phase, to_phase |-> "Stopped"]}
       /\ model_step_count' = model_step_count + 1


mob_CancelWorkCompleted(arg_work_id) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       /\ packet.machine = "mob"
       /\ packet.variant = "CancelWork"
       /\ packet.payload.work_id = arg_work_id
       ...
       /\ pending_routes' = pending_routes
       /\ delivered_routes' = delivered_routes
       /\ emitted_effects' = emitted_effects \cup { [machine |-> "mob", variant |-> "FlowTerminalized", payload |-> [tag |-> "unit"], effect_id |-> (model_step_count + 1), source_transition |-> "CancelWorkCompleted"] }
       /\ observed_transitions' = observed_transitions \cup {[machine |-> "mob", transition |-> "CancelWorkCompleted", actor |-> "mob_kernel", step |-> (model_step_count + 1), from_phase |-> mob_phase, to_phase |-> "Completed"]}
       /\ model_step_count' = model_step_count + 1


mob_CancelWorkDestroyed(arg_work_id) ==
    /\ \E packet \in SeqElements(pending_inputs) :
       /\ packet.machine = "mob"
       /\ packet.variant = "CancelWork"
       /\ packet.payload.work_id = arg_work_id
       ...
       /\ pending_routes' = pending_routes
       /\ delivered_routes' = delivered_routes
       /\ emitted_effects' = emitted_effects \cup { [machine |-> "mob", variant |-> "FlowTerminalized", payload |-> [tag |-> "unit"], effect_id |-> (model_step_count + 1), source_transition |-> "CancelWorkDestroyed"] }
       /\ observed_transitions' = observed_transitions \cup {[machine |-> "mob", transition |-> "CancelWorkDestroyed", actor |-> "mob_kernel", step |-> (model_step_count + 1), from_phase |-> mob_phase, to_phase |-> "Destroyed"]}
       /\ model_step_count' = model_step_count + 1
...
Inject_spawn_member(arg_agent_identity, arg_agent_runtime_id, arg_fence_token, arg_generation) ==
    ...
    /\ model_step_count' = model_step_count + 1
    /\ UNCHANGED << meerkat_phase, meerkat_session_id, meerkat_active_runtime_id, meerkat_active_fence_token, meerkat_active_generation, meerkat_active_work_id, meerkat_attachment_live, meerkat_wake_pending, meerkat_process_pending, meerkat_pre_run_phase, meerkat_peer_ingress_configured, meerkat_drain_running, meerkat_current_llm_identity, meerkat_current_capability_surface, meerkat_capability_surface_status, meerkat_capability_base_filter, meerkat_inherited_base_filter, meerkat_active_filter, meerkat_staged_filter, meerkat_active_requested_deferred_names, meerkat_staged_requested_deferred_names, meerkat_requested_witnesses, meerkat_filter_witnesses, meerkat_active_visibility_revision, meerkat_staged_visibility_revision, meerkat_committed_visibility_revision, mob_phase, mob_active_identity, mob_active_runtime_id, mob_active_fence_token, mob_current_generation, mob_inflight_work_id, mob_active_member_count, mob_active_run_count, mob_pending_spawn_count, mob_retiring_member_count, mob_wiring_edge_count, mob_task_count, mob_event_subscription_count, mob_coordinator_bound, pending_routes, delivered_routes, emitted_effects, observed_transitions, witness_current_script_input, witness_remaining_script_inputs >>
...
DeliverQueuedRoute ==
    ...
    /\ LET route == Head(pending_routes) IN
       ...
       /\ UNCHANGED << meerkat_phase, meerkat_session_id, meerkat_active_runtime_id, meerkat_active_fence_token, meerkat_active_generation, meerkat_active_work_id, meerkat_attachment_live, meerkat_wake_pending, meerkat_process_pending, meerkat_pre_run_phase, meerkat_peer_ingress_configured, meerkat_drain_running, meerkat_current_llm_identity, meerkat_current_capability_surface, meerkat_capability_surface_status, meerkat_capability_base_filter, meerkat_inherited_base_filter, meerkat_active_filter, meerkat_staged_filter, meerkat_active_requested_deferred_names, meerkat_staged_requested_deferred_names, meerkat_requested_witnesses, meerkat_filter_witnesses, meerkat_active_visibility_revision, meerkat_staged_visibility_revision, meerkat_committed_visibility_revision, mob_phase, mob_active_identity, mob_active_runtime_id, mob_active_fence_token, mob_current_generation, mob_inflight_work_id, mob_active_member_count, mob_active_run_count, mob_pending_spawn_count, mob_retiring_member_count, mob_wiring_edge_count, mob_task_count, mob_event_subscription_count, mob_coordinator_bound, emitted_effects, observed_transitions, witness_current_script_input, witness_remaining_script_inputs >>
...
CoreNext ==
    ...
    \/ \E arg_session_id \in SessionIdValues : meerkat_ContainsSessionAttached(arg_session_id)
    \/ \E arg_session_id \in SessionIdValues : meerkat_SessionHasExecutorAttached(arg_session_id)
    \/ \E arg_session_id \in SessionIdValues : meerkat_SessionHasCommsAttached(arg_session_id)
    \/ \E arg_session_id \in SessionIdValues : meerkat_OpsLifecycleRegistryAttached(arg_session_id)
    \/ \E arg_session_id \in SessionIdValues : \E arg_input_id \in InputIdValues : meerkat_InputStateAttached(arg_session_id, arg_input_id)
    \/ \E arg_session_id \in SessionIdValues : meerkat_ListActiveInputsAttached(arg_session_id)
    ...
    \/ \E arg_session_id \in SessionIdValues : meerkat_ContainsSessionRunning(arg_session_id)
    \/ \E arg_session_id \in SessionIdValues : meerkat_SessionHasExecutorRunning(arg_session_id)
    \/ \E arg_session_id \in SessionIdValues : meerkat_SessionHasCommsRunning(arg_session_id)
    \/ \E arg_session_id \in SessionIdValues : meerkat_OpsLifecycleRegistryRunning(arg_session_id)
    \/ \E arg_session_id \in SessionIdValues : \E arg_input_id \in InputIdValues : meerkat_InputStateRunning(arg_session_id, arg_input_id)
    \/ \E arg_session_id \in SessionIdValues : meerkat_ListActiveInputsRunning(arg_session_id)
    ...
    \/ \E arg_session_id \in SessionIdValues : meerkat_ContainsSessionRetired(arg_session_id)
    \/ \E arg_session_id \in SessionIdValues : meerkat_SessionHasExecutorRetired(arg_session_id)
    \/ \E arg_session_id \in SessionIdValues : meerkat_SessionHasCommsRetired(arg_session_id)
    \/ \E arg_session_id \in SessionIdValues : meerkat_OpsLifecycleRegistryRetired(arg_session_id)
    \/ \E arg_session_id \in SessionIdValues : \E arg_input_id \in InputIdValues : meerkat_InputStateRetired(arg_session_id, arg_input_id)
    \/ \E arg_session_id \in SessionIdValues : meerkat_ListActiveInputsRetired(arg_session_id)
    ...
    \/ \E arg_session_id \in SessionIdValues : meerkat_ContainsSessionStopped(arg_session_id)
    \/ \E arg_session_id \in SessionIdValues : meerkat_SessionHasExecutorStopped(arg_session_id)
    \/ \E arg_session_id \in SessionIdValues : meerkat_SessionHasCommsStopped(arg_session_id)
    \/ \E arg_session_id \in SessionIdValues : meerkat_OpsLifecycleRegistryStopped(arg_session_id)
    \/ \E arg_session_id \in SessionIdValues : \E arg_input_id \in InputIdValues : meerkat_InputStateStopped(arg_session_id, arg_input_id)
    \/ \E arg_session_id \in SessionIdValues : meerkat_ListActiveInputsStopped(arg_session_id)
    ...
    \/ meerkat_EnsureDrainRunningAttached
    \/ meerkat_EnsureDrainRunningRunning
    \/ \E arg_runtime_id \in StringValues : meerkat_IngestIdle(arg_runtime_id)
    ...
    \/ \E arg_input_id \in InputIdValues : meerkat_AcceptWithCompletionIdle(arg_input_id)
    ...
    \/ meerkat_ClassifyPlainEventAttached
    \/ meerkat_ClassifyPlainEventRunning
    \/ \E arg_runtime_id \in StringValues : meerkat_RuntimeStateIdle(arg_runtime_id)
    ...
    \/ \E arg_runtime_id \in StringValues : meerkat_RuntimeStateRetired(arg_runtime_id)
    \/ \E arg_runtime_id \in StringValues : meerkat_RuntimeStateStopped(arg_runtime_id)
    \/ \E arg_runtime_id \in StringValues : \E arg_sequence \in 0..2 : meerkat_LoadBoundaryReceiptIdle(arg_runtime_id, arg_sequence)
    ...
    \/ \E arg_runtime_id \in StringValues : \E arg_sequence \in 0..2 : meerkat_LoadBoundaryReceiptRetired(arg_runtime_id, arg_sequence)
    \/ \E arg_runtime_id \in StringValues : \E arg_sequence \in 0..2 : meerkat_LoadBoundaryReceiptStopped(arg_runtime_id, arg_sequence)
    \/ \E arg_session_id \in SessionIdValues : meerkat_PrepareIdle(arg_session_id)
    ...
    \/ meerkat_RecycleFromIdleOrRetired
    \/ meerkat_RecycleFromAttached
    \/ \E arg_agent_identity \in AgentIdentityValues : \E arg_agent_runtime_id \in AgentRuntimeIdValues : \E arg_fence_token \in FenceTokenValues : \E arg_generation \in GenerationValues : mob_SpawnRunning(arg_agent_identity, arg_agent_runtime_id, arg_fence_token, arg_generation)
    ...
    \/ \E arg_agent_runtime_id \in AgentRuntimeIdValues : \E arg_fence_token \in FenceTokenValues : \E arg_work_id \in WorkIdValues : mob_SubmitWorkRunning(arg_agent_runtime_id, arg_fence_token, arg_work_id)
    ...
    \/ \E arg_work_id \in WorkIdValues : mob_CancelWorkStopped(arg_work_id)
    \/ \E arg_work_id \in WorkIdValues : mob_CancelWorkCompleted(arg_work_id)
    \/ \E arg_work_id \in WorkIdValues : mob_CancelWorkDestroyed(arg_work_id)
    \/ mob_CancelAllWorkRunning
    \/ QuiescentStutter
...
CoverageInstrumentation == RouteCoverage_binding_request_reaches_meerkat /\ RouteCoverage_retire_request_reaches_meerkat /\ RouteCoverage_destroy_request_reaches_meerkat /\ RouteCoverage_runtime_bound_reaches_mob /\ RouteCoverage_runtime_retired_reaches_mob /\ RouteCoverage_runtime_destroyed_reaches_mob /\ RouteCoverage_work_completed_reaches_mob /\ RouteCoverage_work_failed_reaches_mob /\ RouteCoverage_work_cancelled_reaches_mob

CiStateConstraint == /\ model_step_count <= 8 /\ Len(pending_inputs) <= 8 /\ Cardinality(observed_inputs) <= 10 /\ Len(pending_routes) <= 8 /\ Cardinality(delivered_routes) <= 0 /\ Cardinality(emitted_effects) <= 0 /\ Cardinality(observed_transitions) <= 8 /\ Cardinality(meerkat_active_requested_deferred_names) <= 0 /\ Cardinality(meerkat_staged_requested_deferred_names) <= 0 /\ Cardinality(DOMAIN meerkat_requested_witnesses) <= 0 /\ Cardinality(DOMAIN meerkat_filter_witnesses) <= 0
DeepStateConstraint == /\ model_step_count <= 6 /\ Len(pending_inputs) <= 2 /\ Cardinality(observed_inputs) <= 6 /\ Len(pending_routes) <= 2 /\ Cardinality(delivered_routes) <= 2 /\ Cardinality(emitted_effects) <= 2 /\ Cardinality(observed_transitions) <= 6 /\ Cardinality(meerkat_active_requested_deferred_names) <= 2 /\ Cardinality(meerkat_staged_requested_deferred_names) <= 2 /\ Cardinality(DOMAIN meerkat_requested_witnesses) <= 2 /\ Cardinality(DOMAIN meerkat_filter_witnesses) <= 2
WitnessStateConstraint_basic_round_trip == /\ model_step_count <= 8 /\ Len(pending_inputs) <= 8 /\ Cardinality(observed_inputs) <= 13 /\ Len(pending_routes) <= 8 /\ Cardinality(delivered_routes) <= 3 /\ Cardinality(emitted_effects) <= 0 /\ Cardinality(observed_transitions) <= 8 /\ Cardinality(meerkat_active_requested_deferred_names) <= 0 /\ Cardinality(meerkat_staged_requested_deferred_names) <= 0 /\ Cardinality(DOMAIN meerkat_requested_witnesses) <= 0 /\ Cardinality(DOMAIN meerkat_filter_witnesses) <= 0
WitnessStateConstraint_retire_runtime_path == /\ model_step_count <= 8 /\ Len(pending_inputs) <= 8 /\ Cardinality(observed_inputs) <= 12 /\ Len(pending_routes) <= 8 /\ Cardinality(delivered_routes) <= 2 /\ Cardinality(emitted_effects) <= 0 /\ Cardinality(observed_transitions) <= 8 /\ Cardinality(meerkat_active_requested_deferred_names) <= 0 /\ Cardinality(meerkat_staged_requested_deferred_names) <= 0 /\ Cardinality(DOMAIN meerkat_requested_witnesses) <= 0 /\ Cardinality(DOMAIN meerkat_filter_witnesses) <= 0
