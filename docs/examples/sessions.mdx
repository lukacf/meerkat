...
Cancel an in-flight turn. Runtime-backed public interrupt handling returns a
successful no-op for an existing idle session. This does not make missing or
archived targets successful no-ops, and direct substrate calls differ.
...
<Tabs>
  ...
  <Tab title="Rust">
    A directly constructed `EphemeralSessionService` returns
    `SessionError::NotRunning` when idle. This example propagates that result
    with `?`; it does not normalize it to a successful no-op.
...
<Tabs>
  ...
  <Tab title="JSON-RPC">
    ...
    The wire `event` is an `EventEnvelope<AgentEvent>`; the agent event lives
    at `params.event.payload`. The SDK stream helpers below expose decoded
    agent events instead.
  </Tab>
  <Tab title="REST">
    ```bash
  ...
  <Tab title="MCP">
    ...
    When `stream: true`, this agent-event path sends MCP logging notifications
    (`notifications/message`), not `notifications/progress`. Each notification
    has `params.level: "info"` and a `params.data` object containing
    `source: "meerkat"`, `session_id`, and `event` as an
    `EventEnvelope<AgentEvent>`. The actual agent event is at
    `params.data.event.payload`.
...
<Tabs>
  <Tab title="CLI">
    ...
    distinct from the per-call output-token limit.
  </Tab>
  <Tab title="JSON-RPC">
    ```json
  ...
  <Tab title="TypeScript">
    ...
    Config-document keys use the schema's snake_case. SDK wrapper options
    such as `expectedGeneration` remain camelCase.
  </Tab>
  <Tab title="Rust">
    ```rust
