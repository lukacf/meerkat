...
Flags such as invalid UTF-8 appear only when set. Rust callers of
`ShellTool` still get the typed `ShellOutput` through `ToolOutput::into_json`.
...
like foreground output. Rust callers still get the typed `BackgroundJob`
through `ToolOutput::into_json`. A background job's completion notice carries
the same exit status and output.
